...
public class Interceptador
	...
	private static Logger log = Logger.getLogger(Interceptador.class);
	 ...
	 */
	private void salvarModificacao(ObjetoTransacao novo, OperacaoEfetuada operacaoEfetuada, Collection usuariosAcao,
					Collection<TabelaLinhaColunaAlteracao> tabelaLinhaColunaAlteracoes, int tipoAlteracao) throws SecurityException,
		...
		Collection coll = getControladorUtil().pesquisar(filtroTabela, Tabela.class.getName());
		Tabela tabela = null;
		...
		if(coll != null && !coll.isEmpty()){
			...
			tabela.setDescricaoTabela(nomeTabela);
		...
		while(it.hasNext()){
			...
			filtroTabelaColuna.adicionarParametro(new ParametroSimples(FiltroTabelaColuna.COLUNA, tabelaLinhaColunaAlteracao
							.getTabelaColuna().getColuna()));
			coll = this.getControladorUtil().pesquisar(filtroTabelaColuna, TabelaColuna.class.getName());
			...
			if(coll != null && !coll.isEmpty()){
				...
				this.getControladorUtil().inserir(tabelaColuna);
			}
			tabelaLinhaColunaAlteracao.setTabelaColuna(tabelaColuna);
		}

		this.getControladorTransacao().inserirOperacaoEfetuada(usuariosAcao, operacaoEfetuada, tabelaLinhaAlteracao,
						tabelaLinhaColunaAlteracoes, novo.getClass().getSimpleName());
		...
		if(arg0 instanceof ObjetoTransacao){
			...
			if(objetoTransacao.getIdAntigo() != null){
				...
				if(objetoTransacao.getIdAntigo() instanceof ObjetoGcom){
					filtro.adicionarParametro(new ParametroSimplesColecao(objetoTransacao.retornaCamposChavePrimaria()[0], objetoTransacao
									.getIdAntigo()));
				}else{
					filtro.adicionarParametro(new ParametroSimples(objetoTransacao.retornaCamposChavePrimaria()[0], objetoTransacao
									.getIdAntigo()));
				}
			}
			try{
				...
				if((antigo.getUltimaAlteracao() != null)
								&& (objetoTransacao.getUltimaAlteracao() == null || antigo.getUltimaAlteracao().getTime() != objetoTransacao
												.getUltimaAlteracao().getTime())){
					// throw new IllegalArgumentException(" Data diferente.");
				}
				...
				Collection<TabelaLinhaColunaAlteracao> tabelaLinhaColunaAlteracoes = compareObjetoTransacao(objetoTransacao, antigo,
								colecaoNomesAtributos);

				if(tabelaLinhaColunaAlteracoes != null && !tabelaLinhaColunaAlteracoes.isEmpty()){

					String idPrincipal = objetoTransacao.getOperacaoEfetuada().getIdObjetoPrincipal() + "";

					// Adicionando uma linha com a descricao do objeto, caso esta descricao não seja
					// vazia
					...
					for(int i = 0; keys != null && i < keys.length; i++){
						...
						if(!(descricaoVelho.equals("") && descricaoNovo.equals("")) && !descricaoVelho.equals(idPrincipal)){
							...
							TabelaLinhaColunaAlteracao tabelaLinhaColunaAlteracao = gerarTabelaLinhaColunaAlteracao(id, objetoTransacao
											.getTipoAtributo(id), descricaoNovo, descricaoVelho);
							if(tabelaLinhaColunaAlteracao != null) tabelaLinhaColunaAlteracoes.add(tabelaLinhaColunaAlteracao);
						}
						break;
					}

					salvarModificacao(objetoTransacao, objetoTransacao.getOperacaoEfetuada(), objetoTransacao.getUsuarioAcaoUsuarioHelp(),
	...
	public void verificarObjetoAlterado(Object velho, Object novo){
		...
		if(velho instanceof ObjetoTransacao && novo instanceof ObjetoTransacao){
			...
			RegistradorOperacao.get().registrarOperacao(objNovo);
			...
			Collection<TabelaLinhaColunaAlteracao> tabelaLinhaColunaAlteracoes = compareObjetoTransacao(objNovo, objVelho, objVelho
							.retornarAtributosSelecionadosRegistro());
			if(tabelaLinhaColunaAlteracoes != null && !tabelaLinhaColunaAlteracoes.isEmpty()){
				try{
	 ...
	 *            -
	 *            ObjetoTransacao
	 *            //TODO Foi incluído o catch em lazyInicialization para que não seja abortado a
	 *            auditoria nos atributos relacionados
	 *            à entidade principal. nova Solução tem que ser adotada.
	 */
	public Collection<TabelaLinhaColunaAlteracao> compareObjetoTransacao(ObjetoTransacao novo, ObjetoTransacao velho,
					String[] colecaoNomesAtributos){
		...
		for(int i = 0; i < colecaoNomesAtributos.length; i++){
			...
			try{
				...
				if((retorno instanceof ObjetoGcom) || (metodo.getReturnType().isPrimitive()) || (retorno instanceof Short)
								|| (retorno instanceof Byte) || (retorno instanceof Character) || (retorno instanceof Integer)
								|| (retorno instanceof Long) || (retorno instanceof Double) || (retorno instanceof Float)
								|| (retorno instanceof String) || (retorno instanceof Boolean) || (retorno instanceof BigDecimal)
								|| (retorno instanceof Date) || (retorno instanceof Collection)){

					Collection<TabelaLinhaColunaAlteracao> auxAlteracoes = new Vector<TabelaLinhaColunaAlteracao>();
					verificarDiferencaAtributo(auxAlteracoes, colecaoNomesAtributos[i], novo, retornoMetodoNovo, velho, retornoMetodoVelho,
	 ...
	 * @param nomeMetodo
	 * @param nomeAtributo
	 *            Nome do atributo que será verificado
	 * @param novo
	 *            Objeto novo que contém tal atributo
	 * @param retornoMetodoNovo
	 *            Valor novo deste atributo
	 * @param velho
	 *            Objeto velho que contém tal atributo
	 * @param retornoMetodoVelho
	 *            Valor velho deste atributo
	 * @throws SecurityException
	 * @throws NoSuchMethodException
	 * @throws IllegalArgumentException
	 * @throws IllegalAccessException
	 ...
	 * @throws ControladorException
	 */
	private void verificarDiferencaAtributo(Collection<TabelaLinhaColunaAlteracao> tabelaLinhaColunaAlteracoes, String nomeAtributo,
					ObjetoTransacao novo, Object retornoMetodoNovo, ObjetoTransacao velho, Object retornoMetodoVelho,
					OperacaoEfetuada operacaoEfetuada, Collection usuarios, boolean itemDeColecao) throws SecurityException,
					NoSuchMethodException, IllegalArgumentException, IllegalAccessException, InvocationTargetException,
					ControladorException{
		...
		// outro
		if(retornoMetodoVelho instanceof ObjetoTransacao || retornoMetodoNovo instanceof ObjetoTransacao){
			...
			if(gcomNovo != null && gcomVelho != null){
				...
				for(int i = 0; keys != null && i < keys.length; i++){
					String id = keys[i];
					// String nomeMetodoKey = "get" + id.substring(0, 1).toUpperCase() +
					// id.substring(1, id.length());

					// Method metodoNovo = gcomNovo.getClass().getMethod(nomeMetodoKey, (Class[])
					// null);
					// Object retornoKeyNovo = metodoNovo.invoke(gcomNovo, (Object[]) null);
					Object retornoKeyNovo = getValorAtributo(gcomNovo, id);

					// Method metodoVelho = gcomVelho.getClass().getMethod(nomeMetodoKey, (Class[])
					// null);
					// Object retornoKeyVelho = metodoVelho.invoke(gcomVelho, (Object[]) null);
					Object retornoKeyVelho = getValorAtributo(gcomVelho, id);
					...
					if(retornoKeyNovo != null && retornoKeyVelho != null){
						...
						// Se os ids forem diferentes,primeiro registra-se o ID com o valor da
						// descricao do objeto

						if(!retornoKeyVelho.equals(retornoKeyNovo)){

							TabelaLinhaColunaAlteracao tabelaLinhaColunaAlteracao = gerarTabelaLinhaColunaAlteracao(nomeAtributo, novo
											.getTipoAtributo(nomeAtributo), descricaoNovo, descricaoVelho);
							if(tabelaLinhaColunaAlteracao != null) tabelaLinhaColunaAlteracoes.add(tabelaLinhaColunaAlteracao);
						}

						// Verificando se os atributos deste atributo deverá ser anasalido
						// (recursivamente)
						// quando o getdescricaoParaRegistraTRansacao retorna "" é pq deve entrar no
						// objeto pra
						// percorrer os campos
						if(gcomVelho.temControleAlteracao() && (descricaoVelho.equals("") && descricaoNovo.equals(""))){

							// Entrando na classe deste atributo para analisar se houve alteração em
							// seus atributos
							...
							if(alteracoes != null && !alteracoes.isEmpty()){
								...
								TabelaLinhaColunaAlteracao tabelaLinhaColunaAlteracao = gerarTabelaLinhaColunaAlteracao(id,
												retornoMetodoNovo.getClass(), descricaoNovo, descricaoVelho);
								if(tabelaLinhaColunaAlteracao != null && tabelaLinhaColunaAlteracoes.isEmpty()){
				...
				for(int i = 0; keys != null && i < keys.length; i++){
					...
					TabelaLinhaColunaAlteracao tabelaLinhaColunaAlteracao = gerarTabelaLinhaColunaAlteracao(nomeCampo, classe,
									descricaoNovo, descricaoVelho);
					if(tabelaLinhaColunaAlteracao != null){
						tabelaLinhaColunaAlteracoes.add(tabelaLinhaColunaAlteracao);
						if((gcomVelho != null && gcomVelho.temControleAlteracao()) || (gcomNovo != null && gcomNovo.temControleAlteracao())){
			...
			ControleAlteracaoColecao controle = ControleAlteracaoColecao.gerarControle((Collection) retornoMetodoVelho,
							(Collection) retornoMetodoNovo);
			// System.out.println(nomeAtributo + ".adicionadas=" +
			// controle.getAdicionadas().size());
			// System.out.println(nomeAtributo + ".removidas=" + controle.getRemovidas().size());
			// System.out.println(nomeAtributo + ".alteradas=" + controle.getAlteradas().size());

			if(!controle.getAlteradas().isEmpty()){
				for(Iterator iter = controle.getAlteradas().keySet().iterator(); iter.hasNext();){
					...
					verificarDiferencaAtributo(alteracoes, nomeAtributo, novo, elementDepois, velho, elementAntes, operacaoEfetuada,
									usuarios, true);
					if(alteracoes != null && !alteracoes.isEmpty()){
						salvarModificacao((ObjetoTransacao) elementAntes, operacaoEfetuada, usuarios, alteracoes, AlteracaoTipo.ALTERACAO);
					}
				}
			}
			if(!controle.getAdicionadas().isEmpty()){
				for(Iterator iter = controle.getAdicionadas().iterator(); iter.hasNext();){
					Object elementDepois = iter.next();
					Collection<TabelaLinhaColunaAlteracao> alteracoes = new Vector();
					verificarDiferencaAtributo(alteracoes, nomeAtributo, novo, elementDepois, velho, null, operacaoEfetuada, usuarios, true);
					if(alteracoes != null && !alteracoes.isEmpty()){
						salvarModificacao((ObjetoTransacao) elementDepois, operacaoEfetuada, usuarios, alteracoes, AlteracaoTipo.INCLUSAO);
					}
				}
			}
			if(!controle.getRemovidas().isEmpty()){
				for(Iterator iter = controle.getRemovidas().iterator(); iter.hasNext();){
					Object elementAntes = iter.next();
					Collection<TabelaLinhaColunaAlteracao> alteracoes = new Vector();
					verificarDiferencaAtributo(alteracoes, nomeAtributo, novo, null, velho, elementAntes, operacaoEfetuada, usuarios, true);
					if(alteracoes != null && !alteracoes.isEmpty()){
						salvarModificacao((ObjetoTransacao) elementAntes, operacaoEfetuada, usuarios, alteracoes, AlteracaoTipo.EXCLUSAO);
					}
			...
			if(retornoMetodoNovo != null && retornoMetodoVelho != null){
				if(!retornoMetodoNovo.equals(retornoMetodoVelho)){
					...
					TabelaLinhaColunaAlteracao tabelaLinhaColunaAlteracao = gerarTabelaLinhaColunaAlteracao(nomeAtributo, novo.getClass(),
									retornoMetodoNovo, retornoMetodoVelho);
					if(tabelaLinhaColunaAlteracao != null) tabelaLinhaColunaAlteracoes.add(tabelaLinhaColunaAlteracao);
				...
				TabelaLinhaColunaAlteracao tabelaLinhaColunaAlteracao = gerarTabelaLinhaColunaAlteracao(nomeAtributo, classe,
								retornoMetodoNovo, retornoMetodoVelho);
				if(tabelaLinhaColunaAlteracao != null){
					tabelaLinhaColunaAlteracoes.add(tabelaLinhaColunaAlteracao);
				}
	 ...
	 * @throws IllegalArgumentException
	 */
	private TabelaLinhaColunaAlteracao gerarTabelaLinhaColunaAlteracao(String nomeAtributo, Class classe, Object valorNovo,
					Object valorVelho) throws SecurityException, NoSuchMethodException, IllegalArgumentException, IllegalAccessException,
	...
	public boolean onSave(Object arg0, Serializable arg1, Object[] arg2, String[] arg3, Type[] arg4) throws CallbackException{
		...
		if(RegistradorOperacao.get() == null){
			return true;
		}
		if(arg0 instanceof ObjetoTransacao){
			...
			RegistradorOperacao.get().registrarOperacao(objetoTransacao);
			...
			if(objetoTransacao.getOperacaoEfetuada() == null){
				return true;
				// throw new CallbackException("atencao.operacaoEfetuada.vazia");
			}
		...
		if(obj instanceof ObjetoTransacao){
			...
			if(objetoTransacao.getIdAntigo() == null){
				...
				try{
					...
					Collection<TabelaLinhaColunaAlteracao> collTabelaLinhaColunaAlteracao = new ArrayList<TabelaLinhaColunaAlteracao>();
					...
					for(int i = 0; keys != null && i < keys.length; i++){
						...
						// Method metodoNovo = objetoTransacao.getClass().getMethod(nomeMetodoKey,
						// (Class[]) null);
						// Object retornoKeyNovo = metodoNovo.invoke(objetoTransacao, (Object[])
						// null);
						...
						if(!descricaoNovo.equals("")){
							...
							TabelaLinhaColunaAlteracao tabelaLinhaColunaAlteracao = gerarTabelaLinhaColunaAlteracao(id, objetoTransacao
											.getTipoAtributo(id), descricaoNovo, "");
							if(tabelaLinhaColunaAlteracao != null) tabelaLinhaColunaAlteracao.setTabelaLinhaAlteracao(tabelaLinhaAlteracao);
					...
					collTabelaLinhaColunaAlteracao.addAll(gerarTabelaLinhaColunaAlteracaoInclusaoOuExclusao(true, false, objetoTransacao,
									tabelaLinhaAlteracao));
					...
					if(objetoTransacao.getOperacaoEfetuada().getArgumentoValor() == 0){
						...
						if(objId instanceof Number){
							objetoTransacao.getOperacaoEfetuada().setArgumentoValor(((Number) objId).intValue());
						}else{
							// TODO: resolver quando a chave primária não for do tipo Integer
						}

					}

					this.getControladorTransacao().inserirOperacaoEfetuada(objetoTransacao.getUsuarioAcaoUsuarioHelp(),
									objetoTransacao.getOperacaoEfetuada(), tabelaLinhaAlteracao, collTabelaLinhaColunaAlteracao,
									objetoTransacao.getClass().getSimpleName());
	...
	public boolean onFlushDirty(Object arg0, Serializable arg1, Object[] arg2, Object[] arg3, String[] arg4, Type[] arg5)
		...
		if(arg0 instanceof ObjetoTransacao){
			...
			if(!novo.temControleAlteracao()){
				return true;
			}

			if(RegistradorOperacao.get() != null) RegistradorOperacao.get().registrarOperacao(novo);
	...
	public void onDelete(Object arg0, Serializable arg1, Object[] arg2, String[] arg3, Type[] arg4) throws CallbackException{
		...
		if(arg0 instanceof ObjetoTransacao){
			...
			if(RegistradorOperacao.get() != null) RegistradorOperacao.get().registrarOperacao(objetoTransacao);
	...
	public void registrarExclusao(Object obj){
		...
		if(obj instanceof ObjetoTransacao){
			...
			try{
				...
				collTabelaLinhaColunaAlteracao.addAll(gerarTabelaLinhaColunaAlteracaoInclusaoOuExclusao(false, true, objetoTransacao,
								tabelaLinhaAlteracao));
				// fim: adicionando descricao como alteracao
	...
	private Integer[] getIds(ObjetoGcom objetoTransacao) throws SecurityException, NoSuchMethodException, IllegalArgumentException,
		...
		if(arg != null && arg.length > 0){
			String id = arg[0];
			// String nomeMetodoKey = "get" + id.substring(0, 1).toUpperCase() + id.substring(1,
			// id.length());
			// Method metodo = objetoTransacao.getClass().getMethod(nomeMetodoKey, (Class[]) null);
			// pega o retonro do primeiro id
			Object retornoKey = getValorAtributo(objetoTransacao, id);
			...
			if(retornoKey instanceof Integer){
				...
				if(retornoKey instanceof ObjetoGcom){
					...
					if(camposChavePrimaria != null && camposChavePrimaria.length > 0){
						...
						// String nomeCamposChavePrimaria = "get" +
						// idCamposChavePrimaria.substring(0, 1).toUpperCase()
						// + idCamposChavePrimaria.substring(1, idCamposChavePrimaria.length());
						// Method metodoCamposChavePrimaria =
						// objetoGcom.getClass().getMethod(nomeCamposChavePrimaria, (Class[]) null);
						// invoca o id do objeto que é id do objeto passado
						// Object retornoKeyCamposChavePrimaria =
						// metodoCamposChavePrimaria.invoke(objetoGcom, (Object[]) null);
						Object retornoKeyCamposChavePrimaria = getValorAtributo(objetoGcom, idCamposChavePrimaria);
						if(retornoKeyCamposChavePrimaria instanceof Integer){
							retorno[0] = ((Integer) retornoKeyCamposChavePrimaria);
						}else{
							retorno[0] = Integer.valueOf(0);
						}
					}
				}else{
					retorno[0] = Integer.valueOf(0);
	...
	private Collection gerarTabelaLinhaColunaAlteracaoInclusaoOuExclusao(boolean insersao, boolean exclusao,
					ObjetoTransacao objetoTransacao, TabelaLinhaAlteracao tabelaLinhaAlteracao) throws ControladorException,
		...
		for(int i = 0; i < metodos.length; i++){
			...
			if(nomeMetodo.startsWith("get") && !nomeMetodo.startsWith("getClass") && !nomeMetodo.startsWith("getUltimaAlteracao")){
				...
				try{
					...
					// verifica se o retorno eh do tipo conhecido
					if((retornoMetodoVelho instanceof ObjetoGcom) || (metodo.getReturnType().isPrimitive())
									|| (retornoMetodoVelho instanceof Short) || (retornoMetodoVelho instanceof Byte)
									|| (retornoMetodoVelho instanceof Character) || (retornoMetodoVelho instanceof Integer)
									|| (retornoMetodoVelho instanceof Long) || (retornoMetodoVelho instanceof Double)
									|| (retornoMetodoVelho instanceof Float) || (retornoMetodoVelho instanceof String)
									|| (retornoMetodoVelho instanceof Boolean) || (retornoMetodoVelho instanceof BigDecimal)
						...
						if(retornoMetodoNovo != null && tabelaColuna != null){
							...
							if(retornoMetodoNovo instanceof ObjetoGcom){
								...
								// tipo ObjetoTransacao, pois será usado um método definido nesta
								// classe
	...
	private TabelaColuna gerarTabelaColuna(Tabela tabela, String nomeAtributo, ObjetoTransacao objetoTransacao) throws ControladorException{
		...
		if(nomeColuna != null){
			...
			Collection coll = this.getControladorUtil().pesquisar(filtroTabelaColuna, TabelaColuna.class.getName());
	 ...
	 * @param idOperacao
	 */
	public void registrarTransacaoHQL(Collection colecaoObjetosAtualizados, ClassMetadata classMetadata, Map<String, Object> alteracoes,
					Integer idOperacao){
		...
		if(colecaoObjetosAtualizados != null){
			for(Object objetoAtualizado : colecaoObjetosAtualizados){
				if(objetoAtualizado instanceof ObjetoTransacao){
					...
					RegistradorOperacao registradorOperacao = new RegistradorOperacao(idOperacao, new UsuarioAcaoUsuarioHelper(
					...
					if(propriedades != null && propriedades.length > 0){
						...
						for(int i = 0; i < propriedades.length; i++){
							...
							if(alteracoes.containsKey(propriedades[i])){
								...
								String nomeMetodo;
								nomeMetodo = "set" + nomeAtributo.substring(0, 1).toUpperCase()
												+ nomeAtributo.substring(1, nomeAtributo.length());
								Method metodo;
	 ...
	 * @return
	 */
	private String consultarDescricao(Object obj){
		...
		try{
			...
			// isto acontece nas situacoes que foi instanciado um objeto apenas com seu
			// identificador
			...
			log.warn("Não conseguiu consultar descrição do objeto", ex);
		}
	...
	private void salvarTabelaLinhaAlteracaoPrincipal(OperacaoEfetuada operacaoEfetuada, ObjetoTransacao objetoTransacao){
		...
		if(operacaoEfetuada != null && operacaoEfetuada.getId() != null && operacaoEfetuada.getTabelaLinhaAlteracaoPrincipal() == null){
			TabelaLinhaAlteracao tabelaLinhaAlteracao = new TabelaLinhaAlteracao();
			tabelaLinhaAlteracao.setUltimaAlteracao(new Date(System.currentTimeMillis()));
			...
			try{
				coll = getControladorUtil().pesquisar(filtroTabela, Tabela.class.getName());
			}catch(ControladorException e){
				e.printStackTrace();
			}
			Tabela tabela = null;
