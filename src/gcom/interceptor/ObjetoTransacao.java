 ...
 * @author thiago
 */
public abstract class ObjetoTransacao
				extends ObjetoGcom {

	private static final long serialVersionUID = 1L;

	private static final Logger LOGGER = Logger.getLogger(ObjetoTransacao.class);
	...
	public void adicionarUsuario(Usuario usuario, UsuarioAcao usuarioAcao){
		...
		for(Iterator iter = this.usuarioAcaoUsuario.iterator(); iter.hasNext();){
			...
			if(UAUHExistente.getUsuario() != null && UAUHExistente.getUsuario().equals(usuario) && UAUHExistente.getUsuarioAcao() != null
							&& UAUHExistente.getUsuarioAcao().equals(usuarioAcao)){
				return;
			}

		}
		this.usuarioAcaoUsuario.add(new UsuarioAcaoUsuarioHelper(usuario, usuarioAcao));
	}
	...
	public Object get(String atributo){
		...
		LOGGER.debug("Buscando atributo: " + this.getClass().getSimpleName() + "." + atributo);
	...
	public Class getTipoAtributo(String atributo){
		...
		LOGGER.debug("Buscando atributo: " + this.getClass().getName() + "." + atributo);
		StringTokenizer st = new StringTokenizer(atributo, ".");
		String nomeMetodo = null;
		// este atual servirá para os casos conjugados
		Object atual = this;
	 ...
	 * utilizado foi a remoção(alguns casos nem remove) seguida de uma inclusão para operacionalizar
	 * uma alteração.
	 ...
	 * lazies. Caso esteja true este atributo no filtro, para cada objeto retornado da pesquisa, é
	 * chamado
	 * este método.
