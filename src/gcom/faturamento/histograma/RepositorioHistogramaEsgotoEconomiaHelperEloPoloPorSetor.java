 ...
 *
 */
public class RepositorioHistogramaEsgotoEconomiaHelperEloPoloPorSetor
				extends AbstractRepositorioHistogramaEsgotoEconomiaHelper {

	/**
	 * RepositorioHistogramaEsgotoEconomiaHelperEloPoloPorSetor
	 * <p>
	 ...
	 * @since 13/06/2013
	 */
	public RepositorioHistogramaEsgotoEconomiaHelperEloPoloPorSetor() {
	...
	protected HistogramaEsgotoEconomiaDTO preencherHistogramaHelper(Object[] objects){
		...
		opcaoTotalizacao.append(DIVISOR_TOTALIZADOR_ITEM);
		opcaoTotalizacao.append(objects[4]).append(DIVISOR_TOTALIZADOR_ID_DESC).append(String.valueOf(objects[5]).trim());
		...
		for(int i = 0; i < itemQuebra.length; i++){
			switch(i){
				case 0:
					...
					opcao = OpcaoTotalizacaoEnum.ELO_POLO.getOpcao();
					break;
				case 1:
					Localidade localidade = new Localidade();
					localidade.setId(itemQuebra[i]);
					filtroQuebra.setLocalidade(localidade);
					opcao = OpcaoTotalizacaoEnum.ELO_POLO_LOCALIDADE.getOpcao();
					break;
			}
		}
