...
public class RepositorioLeituraHBM
				implements IRepositorioLeitura {
	...
	public Integer verificarExistenciaLeituraAnormalidade(Integer idLeituraAnormalidade) throws ErroRepositorioException{
		...
		try{
			consulta = "select la.id " + "from LeituraAnormalidade la " + "where la.id = :idLeituraAnormalidade";

			retorno = (Integer) session.createQuery(consulta).setInteger("idLeituraAnormalidade", idLeituraAnormalidade).setMaxResults(1)
							.uniqueResult();
