...
public class RelatorioManterRota
	...
	public Object executar() throws TarefaException{
		...
		if(colecaoRotas != null && !colecaoRotas.isEmpty()){
			...
			Collections.sort((List) colecaoRotas, new Comparator() {
				...
				public int compare(Object a, Object b){
					...
					String setor1 = ""
									+ Util.adicionarZerosEsquedaNumero(3, ((Rota) a).getSetorComercial().getLocalidade().getId().toString())
									+ Util.adicionarZerosEsquedaNumero(3, Integer.toString(((Rota) a).getSetorComercial().getCodigo()));
					String setor2 = ""
									+ Util.adicionarZerosEsquedaNumero(3, ((Rota) b).getSetorComercial().getLocalidade().getId().toString())
									+ Util.adicionarZerosEsquedaNumero(3, Integer.toString(((Rota) b).getSetorComercial().getCodigo()));
			...
			while(colecaoRotasIterator.hasNext()){
				...
				if(rota.getSetorComercial() != null){
					...
					setorComercial = rota.getSetorComercial().getCodigo() + " - " + rota.getSetorComercial().getDescricao();

					if(rota.getSetorComercial().getLocalidade() != null){
						localidade = rota.getSetorComercial().getLocalidade().getId().toString() + " - "
										+ rota.getSetorComercial().getLocalidade().getDescricao();
					}
				}
				...
				if(rota.getLeiturista() != null){
					Leiturista leituristaRota = rota.getLeiturista();
					if(leituristaRota.getFuncionario() != null){
						Funcionario funcionario = leituristaRota.getFuncionario();
						if(funcionario.getNome() != null){
							leiturista = funcionario.getNome();
						}
					}else if(leituristaRota.getCliente() != null){
						Cliente cliente = leituristaRota.getCliente();
						if(cliente.getNome() != null){
							leiturista = cliente.getNome();
						}
					}
				}
		...
		if(rotaParametros.getSetorComercial().getLocalidade().getId() != null){
			...
			parametros.put("idLocalidade", rotaParametros.getSetorComercial().getLocalidade().getId().toString());

		}else{
			parametros.put("idLocalidade", "");
		}

		parametros.put("nomeLocalidade", rotaParametros.getSetorComercial().getLocalidade().getDescricao());

		if(rotaParametros.getSetorComercial().getId() != null){

			parametros.put("idSetorComercial", "" + rotaParametros.getSetorComercial().getCodigo());

		}else{
			parametros.put("idSetorComercial", "");
		}

		if(rotaParametros.getLeiturista() != null){
			parametros.put("codigoLeiturista", rotaParametros.getLeiturista().getId().toString());
		}else{
			parametros.put("codigoLeiturista", "");
		}

		parametros.put("nomeSetorComercial", rotaParametros.getSetorComercial().getDescricao());

		parametros.put("grupoFaturamento", rotaParametros.getFaturamentoGrupo().getDescricao());

		String indicadorUso = "";

		if(rotaParametros.getIndicadorUso() != null && !rotaParametros.getIndicadorUso().equals("")){
		...
		retorno = this.gerarRelatorio(ConstantesRelatorios.RELATORIO_ROTA_MANTER, parametros, ds, tipoFormatoRelatorio);

		// ------------------------------------
		// Grava o relatório no sistema
		try{
			persistirRelatorioConcluido(retorno, Relatorio.MANTER_ROTA, idFuncionalidadeIniciada, null);
	...
	public int calcularTotalRegistrosRelatorio(){
		...
		retorno = Fachada.getInstancia().totalRegistrosPesquisa((FiltroRota) getParametro("filtroRota"), Rota.class.getName());
