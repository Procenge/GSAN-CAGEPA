 ...
 */
public class RelatorioAcompanhamentoMovimentoArrecadadores
	...
	public Object executar() throws TarefaException{
		...
		Concessionaria concessionaria = (Concessionaria) getParametro("concessionaria");
		...
		// anoMesReferenciaAnterior digitada pelo usuário.
		Integer anoMesReferencia = Util.formatarMesAnoComBarraParaAnoMes(mesAnoReferencia);

		// anoMesReferenciaAnterior para pegar os pagamentos
		Integer anoMesReferenciaAnterior = Util.subtrairMesDoAnoMes(anoMesReferencia.intValue(), 0);
		Integer anoReferencia = Util.obterAno(anoMesReferenciaAnterior);
		Integer mesReferencia = Util.obterMes(anoMesReferenciaAnterior);

		String ultimoDia = Util.obterUltimoDiaMes(mesReferencia, anoReferencia);

		// Datas do anoMesReferenciaAnterior
		String dataPagamentoInicial = "01/" + Util.formatarAnoMesParaMesAno(anoMesReferenciaAnterior);
		String dataPagamentoFinal = ultimoDia + "/" + Util.formatarAnoMesParaMesAno(anoMesReferenciaAnterior);
		...
		Collection colecaoMovimentoArrecadadoresRelatorioHelper = fachada.pesquisarMovimentoArrecadadoresRelatorio(anoMesReferencia,
						arrecadador.getId(), arrecadacaoForma.getId(), dataPagamentoInicialFormatada, dataPagamentoFinalFormatada,
						idConcessionaria);
		...
		if(colecaoMovimentoArrecadadoresRelatorioHelper != null && !colecaoMovimentoArrecadadoresRelatorioHelper.isEmpty()){
			...
			while(colecaoMovimentoArrecadadoresRelatorioHelperIterator.hasNext()){
				...
				if(movimentoArrecadadoresRelatorioHelperAnterior != null
								&& (!movimentoArrecadadoresRelatorioHelperAnterior.getArrecadador().equals(
												movimentoArrecadadoresRelatorioHelper.getArrecadador()) || !movimentoArrecadadoresRelatorioHelperAnterior
												.getDescricaoArrecadacaoForma().equals(
																movimentoArrecadadoresRelatorioHelper.getDescricaoArrecadacaoForma()))){
		...
		if(arrecadador != null && arrecadador.getId() != null){
			...
			parametros.put("nomeArrecadador", arrecadador.getCliente().getNome());
		}

		if(arrecadacaoForma != null && arrecadacaoForma.getId() != null){
			parametros.put("formaArrecadacao", arrecadacaoForma.getDescricao());
		}
		Boolean indEmpresaTrabalhaConcessionaria = null;
		if(concessionaria != null){
			if(concessionaria.getNome() != null && !concessionaria.getNome().equals("")){
				parametros.put("concessionaria", concessionaria.getNome());
			}
			indEmpresaTrabalhaConcessionaria = new Boolean(true);
		}else{
			indEmpresaTrabalhaConcessionaria = new Boolean(false);
		}

		parametros.put("indEmpresaTrabalhaConcessionaria", indEmpresaTrabalhaConcessionaria);
		...
		try{
			persistirRelatorioConcluido(retorno, Relatorio.RELATORIO_ACOMP_MOV_ARRECADADORES, idFuncionalidadeIniciada, null);
