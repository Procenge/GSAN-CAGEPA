...
public class RelatorioCertidaoNegativaBean
	...
	public RelatorioCertidaoNegativaBean(RelatorioCertidaoNegativaHelper helper) {
		...
		if(helper.getItens() == null || helper.getItens().size() == 0){
			this.nomeClienteUsuario = "			Certifico, a vista de documentos comprobatórios e pesquisa de registros "
							+ "na nossa base de dados, que o imóvel abaixo descrito,  atualmente sob " + "responsabilidade do Sr. "
		...
		if(helper.getItens() != null && !helper.getItens().isEmpty()){
			...
			this.arrayRelatorioCertidaoNegativaBean.addAll(helper.getItens());
			this.arrayJrItens = new JRBeanCollectionDataSource(this.arrayRelatorioCertidaoNegativaBean);
