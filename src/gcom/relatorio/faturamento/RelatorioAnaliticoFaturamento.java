 ...
 * @date 22/01/2009
 *       Alteração no relatorio para retirada da informacao de codigo de barras no relatorio.
 */

public class RelatorioAnaliticoFaturamento
	...
	private Collection<RelatorioAnaliticoFaturamentoBean> inicializarBeanRelatorio(Collection colecaoAnaliticoRelatorio){
		...
		while(iter.hasNext()){
			...
			valorImposto = rel.getValorImposto();
			...
			filtroSetorComercial.adicionarParametro(new ParametroSimples(FiltroSetorComercial.CODIGO_SETOR_COMERCIAL, rel
							.getCodigoSetorComercial()));
			filtroSetorComercial.adicionarParametro(new ParametroSimples(FiltroSetorComercial.ID_LOCALIDADE, rel.getIdLocalidade()));
	...
	public Object executar() throws TarefaException{
		...
		Collection<RelatorioAnaliticoFaturamentoBean> colecaoBean = this.inicializarBeanRelatorio(dadosRelatorio);
		...
		BeanComparator comparador = new BeanComparator("inscricao");
		Collections.sort((List) colecaoBean, comparador);

		RelatorioDataSource ds = new RelatorioDataSource((java.util.List) colecaoBean);

		retorno = this.gerarRelatorio(ConstantesRelatorios.RELATORIO_ANALITICO_FATURAMENTO, parametros, ds, tipoFormatoRelatorio);

		// ------------------------------------
		// Grava o relatório no sistema
		try{
			persistirRelatorioConcluido(retorno, Relatorio.ANALITICO_FATURAMENTO, idFuncionalidadeIniciada, null);
	...
	public int calcularTotalRegistrosRelatorio(){
		...
		return colecaoAnaliticoFaturamento.size();
	}
