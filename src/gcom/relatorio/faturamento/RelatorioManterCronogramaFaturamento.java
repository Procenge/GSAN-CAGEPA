...
public class RelatorioManterCronogramaFaturamento
	...
	public RelatorioManterCronogramaFaturamento(Usuario usuario) {
		...
		super(usuario, ConstantesRelatorios.RELATORIO_CRONOGRAMA_FATURAMENTO_MANTER);
	}
	...
	public Object executar() throws TarefaException{
		...
		filtroFaturamentoAtividadeCronograma.setCampoOrderBy(
		...
		Collection colecaoFaturamentoAtividadeCronograma = fachada.pesquisar(filtroFaturamentoAtividadeCronograma,
		...
		if(colecaoFaturamentoAtividadeCronograma != null && !colecaoFaturamentoAtividadeCronograma.isEmpty()){
			...
			while(colecaoFaturamentoAtividadeCronogramaIterator.hasNext()){
				...
				if(faturamentoAtividadeCronograma.getFaturamentoAtividade().getFaturamentoAtividadePrecedente() != null){
					predecessora = faturamentoAtividadeCronograma.getFaturamentoAtividade().getFaturamentoAtividadePrecedente()
									.getDescricao();
				}
				...
				if(faturamentoAtividadeCronograma.getFaturamentoAtividade().getIndicadorObrigatoriedadeAtividade() != null
								&& faturamentoAtividadeCronograma.getFaturamentoAtividade().getIndicadorObrigatoriedadeAtividade().equals(
												new Short(ConstantesSistema.INDICADOR_USO_ATIVO))){
				...
				if(faturamentoAtividadeCronograma.getDataRealizacao() != null){
					dataRealizacao = Util.formatarDataComHora(faturamentoAtividadeCronograma.getDataRealizacao());
				}
				...
				if(faturamentoAtividadeCronograma.getComando() != null){
					dataComando = Util.formatarDataComHora(faturamentoAtividadeCronograma.getComando());
				}

				relatorioBean = new RelatorioManterCronogramaFaturamentoBean(

				// Grupo
								faturamentoAtividadeCronograma.getFaturamentoGrupoCronogramaMensal().getFaturamentoGrupo().getDescricao(),

								// Ano/Mês
								faturamentoAtividadeCronograma.getFaturamentoGrupoCronogramaMensal().getMesAno(),

								// Atividade
								faturamentoAtividadeCronograma.getFaturamentoAtividade().getDescricao(),
		...
		if(faturamentoGrupoCronogramaMensalParametros.getFaturamentoGrupo().getDescricao() != null){
			parametros.put("grupo", faturamentoGrupoCronogramaMensalParametros.getFaturamentoGrupo().getDescricao());
		}else{
			parametros.put("grupo", "");
		}

		if(faturamentoGrupoCronogramaMensalParametros.getAnoMesReferencia() != null
						&& faturamentoGrupoCronogramaMensalParametros.getAnoMesReferencia().intValue() != 0){
			parametros.put("mesAno", faturamentoGrupoCronogramaMensalParametros.getMesAno());
		}else{
		...
		retorno = this.gerarRelatorio(ConstantesRelatorios.RELATORIO_CRONOGRAMA_FATURAMENTO_MANTER, parametros, ds, tipoFormatoRelatorio);

		// ------------------------------------
		// Grava o relatório no sistema
		try{
			persistirRelatorioConcluido(retorno, Relatorio.MANTER_CRONOGRAMA_FATURAMENTO, idFuncionalidadeIniciada, null);
	...
	public void agendarTarefaBatch(){
		...
		AgendadorTarefas.agendarTarefa("RelatorioManterCronogramaFaturamento", this);
	}
