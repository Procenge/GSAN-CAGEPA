 ...
 */
public class RelatorioManterCategoria
	...
	public Object executar() throws TarefaException{
		...
		if(colecaoCategorias != null && !colecaoCategorias.isEmpty()){
			...
			while(colecaoCategoriasIterator.hasNext()){
				...
				if(categoria.getVezesMediaAltoConsumo() != null){
					fatorMultiplicativoAlto = Util.formatarMoedaReal(categoria.getVezesMediaAltoConsumo());
				}

				String fatorMultiplicativoEstouro = "";

				if(categoria.getVezesMediaEstouro() != null){
					fatorMultiplicativoEstouro = Util.formatarMoedaReal(categoria.getVezesMediaEstouro());
				}

				String percentualBaixoConsumo = "";

				if(categoria.getPorcentagemMediaBaixoConsumo() != null){
					fatorMultiplicativoEstouro = Util.formatarMoedaReal(categoria.getPorcentagemMediaBaixoConsumo());
				}
		...
		if(categoriaParametros.getId() != null){
			parametros.put("codigo", categoriaParametros.getId().toString());
		}else{
		...
		if(categoriaParametros.getIndicadorUso() != null && !categoriaParametros.getIndicadorUso().equals("")){
			if(categoriaParametros.getIndicadorUso().equals(new Short("1"))){
				indicadorUso = "Ativo";
			}else if(categoriaParametros.getIndicadorUso().equals(new Short("2"))){
		...
		retorno = gerarRelatorio(ConstantesRelatorios.RELATORIO_CATEGORIA_MANTER, parametros, ds, tipoFormatoRelatorio);

		// ------------------------------------
		// Grava o relatório no sistema
		try{
			persistirRelatorioConcluido(retorno, Relatorio.MANTER_CATEGORIA, idFuncionalidadeIniciada, null);
	...
	public int calcularTotalRegistrosRelatorio(){
		...
		retorno = Fachada.getInstancia().totalRegistrosPesquisa((FiltroCategoria) getParametro("filtroCategoria"),
