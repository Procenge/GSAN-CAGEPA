...
public class RelatorioManterLogradouro
	...
	public Object executar() throws TarefaException{
		...
		Short idBairro = null;
		if(this.getParametro("idBairro") != null){
			idBairro = (Short) getParametro("idBairro");
		}
		...
		if(logradouros != null && !logradouros.isEmpty()){
			...
			while(logradouroIterator.hasNext()){
				...
				Logradouro logradouro = (Logradouro) logradouroIterator.next();
				...
				if(idBairro != null){
					filtroLogradouroBairro.adicionarParametro(new ParametroSimples(FiltroLogradouroBairro.CD_BAIRRO, idBairro));
				}
		...
		if(logradouroParametros.getIndicadorUso() != null && !logradouroParametros.getIndicadorUso().equals("")){
		...
		retorno = this.gerarRelatorio(ConstantesRelatorios.RELATORIO_LOGRADOURO_MANTER, parametros, ds, tipoFormatoRelatorio);

		// ------------------------------------
		// Grava o relatório no sistema
		try{
			persistirRelatorioConcluido(retorno, Relatorio.MANTER_LOGRADOURO, idFuncionalidadeIniciada, null);
	...
	public int calcularTotalRegistrosRelatorio(){
		...
		if(getParametro("logradouros") != null && getParametro("logradouros") instanceof Collection){
