...
public class RelatorioManterQuadra
	...
	public Object executar() throws TarefaException{
		...
		Collection quadrasNovas = fachada.pesquisar(filtroQuadra, Quadra.class.getName());

		Collections.sort((List) quadrasNovas, new Comparator() {

			public int compare(Object a, Object b){

				String idLocal1 = "" + ((Quadra) a).getSetorComercial().getLocalidade().getId();
				String idLocal2 = "" + ((Quadra) b).getSetorComercial().getLocalidade().getId();
				if(idLocal1.compareTo(idLocal2) == 0){

					String idSetor1 = "" + ((Quadra) a).getSetorComercial().getId();
					String idSetor2 = "" + ((Quadra) b).getSetorComercial().getId();
		...
		if(quadrasNovas != null && !quadrasNovas.isEmpty()){
			...
			while(quadraNovaIterator.hasNext()){
				...
				if(quadraNova.getIndicadorRedeAgua() != null && quadraNova.getIndicadorRedeAgua().equals(new Short("1"))){

					indicadorRedeAgua = "SIM";

				}else if(quadraNova.getIndicadorRedeAgua() != null && quadraNova.getIndicadorRedeAgua().equals(new Short("2"))){
				...
				if(quadraNova.getIndicadorRedeEsgoto() != null && quadraNova.getIndicadorRedeEsgoto().equals(new Short("1"))){

					indicadorRedeEsgoto = "SIM";

				}else if(quadraNova.getIndicadorRedeEsgoto() != null && quadraNova.getIndicadorRedeEsgoto().equals(new Short("2"))){
				...
				String descricaoPerfilQuadra = "";
				if(quadraNova.getQuadraPerfil() != null){

					descricaoPerfilQuadra = quadraNova.getQuadraPerfil().getDescricao();
				}
				...
				// Número da Quadra
								...
								quadraNova.getSetorComercial().getLocalidade().getDescricao(),
								...
								// Rota
								quadraNova.getRota().getId().toString(),
				...
				if(quadraNova.getBacia() != null){
					if(quadraNova.getBacia().getSistemaEsgoto() != null){

					relatorioBean.setSistemaEsgoto("");
					}else{
						relatorioBean.setSistemaEsgoto("");
					}
					relatorioBean.setBacia(quadraNova.getBacia().getDescricao());
				}

				// Distrito Operacional
				if(quadraNova.getDistritoOperacional() != null){
					relatorioBean.setDistritoOperacional(quadraNova.getDistritoOperacional().getDescricaoAbreviada());
				}

				// Setor Censitário
				if(quadraNova.getIbgeSetorCensitario() != null){
					relatorioBean.setSetorCensitario(quadraNova.getIbgeSetorCensitario().getDescricao());
				}

				// Zeis
				if(quadraNova.getZeis() != null){
					relatorioBean.setZeis(quadraNova.getZeis().getDescricaoAbreviada());
				}
		...
		parametros.put("quadra", quadraParametros.getNumeroQuadra() == -1 ? "" : "" + quadraParametros.getNumeroQuadra());

		parametros.put("idLocalidade", quadraParametros.getSetorComercial().getLocalidade().getId() == null ? "" : ""
						+ quadraParametros.getSetorComercial().getLocalidade().getId());

		parametros.put("nomeLocalidade", quadraParametros.getSetorComercial().getLocalidade().getDescricao());

		parametros.put("idSetorComercial", quadraParametros.getSetorComercial().getCodigo() == 0 ? "" : ""
						+ quadraParametros.getSetorComercial().getCodigo());

		parametros.put("nomeSetorComercial", quadraParametros.getSetorComercial().getDescricao());

		String indicadorUso = "";

		if(quadraParametros.getIndicadorUso() != null && !quadraParametros.getIndicadorUso().equals("")){
			if(quadraParametros.getIndicadorUso().equals(new Short("1"))){
				indicadorUso = "Ativo";
			}else if(quadraParametros.getIndicadorUso().equals(new Short("2"))){
		...
		retorno = this.gerarRelatorio(ConstantesRelatorios.RELATORIO_QUADRA_MANTER, parametros, ds, tipoFormatoRelatorio);

		// ------------------------------------
		// Grava o relatório no sistema
		try{
			persistirRelatorioConcluido(retorno, Relatorio.MANTER_QUADRA, idFuncionalidadeIniciada, null);
	...
	public int calcularTotalRegistrosRelatorio(){
		...
		retorno = Fachada.getInstancia().totalRegistrosPesquisa((FiltroQuadra) getParametro("filtroQuadra"), Quadra.class.getName());
