 ...
 * @date 14/02/2007
 */
public class RelatorioConsultarDebitosResumido
	...
	private Collection<RelatorioConsultarDebitosResumidoBean> inicializarBeanRelatorio(Collection<ContaValoresHelper> colecaoContas,
		...
		Collections.sort((List) colecaoContasTemp, new Comparator() {
			...
			public int compare(Object a, Object b){
				...
				Integer imovel1 = new Integer(((ContaValoresHelper) a).getConta().getImovel().getId());
				Integer imovel2 = new Integer(((ContaValoresHelper) b).getConta().getImovel().getId());
		...
		while(iterarColecao){
			...
			if(colecaoContasTemp != null && !colecaoContasTemp.isEmpty()){
				...
				while(colecaoContasIterator.hasNext()){
					...
					if(idImovel == null){
						idImovel = contaValoresHelper.getConta().getImovel().getId();
					}
					...
					if(contaValoresHelper.getConta().getValorTotal() != null){
						valorContas = valorContas.add(contaValoresHelper.getConta().getValorTotal());
					...
					if(contaValoresHelper.getValorTotalContaValores() != null){
						valorAcrescimos = valorAcrescimos.add(contaValoresHelper.getValorTotalContaValores());
					}
				}
			...
			// Débito a Cobrar
			if(colecaoDebitosACobrarTemp != null && !colecaoDebitosACobrarTemp.isEmpty()){

				Iterator colecaoDebitosACobrarIterator = colecaoDebitosACobrarTemp.iterator();
			...
			// Crédito a Realizar
			if(colecaoCreditosARealizarTemp != null && !colecaoCreditosARealizarTemp.isEmpty()){

				Iterator colecaoCreditosARealizarIterator = colecaoCreditosARealizarTemp.iterator();
			...
			// Guia Pagamento
			if(colecaoGuiasPagamentoTemp != null && !colecaoGuiasPagamentoTemp.isEmpty()){

				Iterator colecaoGuiasPagamentoIterator = colecaoGuiasPagamentoTemp.iterator();
				...
				while(colecaoGuiasPagamentoIterator.hasNext()){
					...
					if(guiaPagamento.getImovel() != null){
						if(idImovel == null || idImovel.equals(guiaPagamento.getImovel().getId())){
							valorGuias = valorGuias.add(guiaPagamentoValoresHelper.getValorTotalPrestacao());
							valorAcrescimos = valorAcrescimos.add(guiaPagamentoValoresHelper.getValorAcrescimosImpontualidade());
			...
			if((colecaoContasTemp == null || colecaoContasTemp.isEmpty())
							&& (colecaoDebitosACobrarTemp == null || colecaoDebitosACobrarTemp.isEmpty())
							&& (colecaoCreditosARealizarTemp == null || colecaoCreditosARealizarTemp.isEmpty())
							&& (colecaoGuiasPagamentoTemp == null || colecaoGuiasPagamentoTemp.isEmpty())){
				iterarColecao = false;
			}

			String inscricaoImovel = fachada.pesquisarInscricaoImovel(idImovel, true);
			String nomeUsuario = fachada.consultarClienteUsuarioImovel(idImovel);
			...
			// Imóvel
							...
							ligacaoEsgotoSituacao.getDescricaoAbreviado(),
		...
		Collections.sort((List) retorno, new Comparator() {
			...
			public int compare(Object a, Object b){
				...
				Integer imovel1 = new Integer(((RelatorioConsultarDebitosResumidoBean) a).getIdImovel());
				Integer imovel2 = new Integer(((RelatorioConsultarDebitosResumidoBean) b).getIdImovel());
	...
	public Object executar() throws TarefaException{
		...
		int qtdeImoveis = (Integer) dados[0];
		int qtdeEconomias = (Integer) dados[1];
		...
		Collection<RelatorioConsultarDebitosResumidoBean> colecaoBean = this.inicializarBeanRelatorio(colecaoContaValores,
						colecaoDebitoACobrar, colecaoCreditoARealizar, colecaoGuiaPagamento);
		...
		retorno = this.gerarRelatorio(ConstantesRelatorios.RELATORIO_CONSULTAR_DEBITOS_RESUMIDO, parametros, ds, tipoFormatoRelatorio);
