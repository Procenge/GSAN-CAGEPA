...
public class RelatorioManterCronogramaCobranca
	...
	public RelatorioManterCronogramaCobranca(Usuario usuario) {
		...
		super(usuario, ConstantesRelatorios.RELATORIO_CRONOGRAMA_COBRANCA_MANTER);
	}
	...
	public Object executar() throws TarefaException{
		...
		filtroCobrancaAcaoAtividadeCronograma.adicionarCaminhoParaCarregamentoEntidade("cobrancaAtividade.cobrancaAtividadePredecessora");
		...
		if(colecaoCobrancaAcaoAtividadeCronograma != null && !colecaoCobrancaAcaoAtividadeCronograma.isEmpty()){
			...
			Iterator colecaoCobrancaAcaoAtividadeCronogramaIterator = colecaoCobrancaAcaoAtividadeCronograma.iterator();
			...
			while(colecaoCobrancaAcaoAtividadeCronogramaIterator.hasNext()){
				...
				if(cobrancaAcaoAtividadeCronograma.getCobrancaAtividade().getCobrancaAtividadePredecessora() != null){
					predecessora = cobrancaAcaoAtividadeCronograma.getCobrancaAtividade().getCobrancaAtividadePredecessora()
				...
				if(cobrancaAcaoAtividadeCronograma.getCobrancaAcaoCronograma().getCobrancaAcao().getCobrancaAcaoPredecessora() != null){
					predecessoraAcao = cobrancaAcaoAtividadeCronograma.getCobrancaAcaoCronograma().getCobrancaAcao()
									.getCobrancaAcaoPredecessora().getDescricaoCobrancaAcao();
				}

				// Obrigatória
				String obrigatoria = "";

				if(cobrancaAcaoAtividadeCronograma.getCobrancaAcaoCronograma().getCobrancaAcao().getIndicadorObrigatoriedade() != null
								&& cobrancaAcaoAtividadeCronograma.getCobrancaAcaoCronograma().getCobrancaAcao()
												.getIndicadorObrigatoriedade().equals(new Short(ConstantesSistema.INDICADOR_USO_ATIVO))){
					obrigatoria = "SIM";
				}else{
					obrigatoria = "NÃO";
				}
				...
				if(cobrancaAcaoAtividadeCronograma.getDataPrevista() != null){
					dataPrevista = Util.formatarData(cobrancaAcaoAtividadeCronograma.getDataPrevista());
				}
				...
				if(cobrancaAcaoAtividadeCronograma.getRealizacao() != null){
					dataRealizacao = Util.formatarDataComHora(cobrancaAcaoAtividadeCronograma.getRealizacao());
				}
				...
				if(cobrancaAcaoAtividadeCronograma.getComando() != null){
					dataComando = Util.formatarDataComHora(cobrancaAcaoAtividadeCronograma.getComando());
				}
				...
				// Grupo
								...
								// Ano/Mês
								cobrancaAcaoAtividadeCronograma.getCobrancaAcaoCronograma().getCobrancaGrupoCronogramaMes().getMesAno(),

								// Ação Cobrança
								cobrancaAcaoAtividadeCronograma.getCobrancaAcaoCronograma().getCobrancaAcao().getDescricaoCobrancaAcao(),
		...
		if(cobrancaGrupoCronogramaMesParametros.getCobrancaGrupo().getDescricao() != null){
			parametros.put("grupo", cobrancaGrupoCronogramaMesParametros.getCobrancaGrupo().getDescricao());
		}else{
			parametros.put("grupo", "");
		}

		if(cobrancaGrupoCronogramaMesParametros.getAnoMesReferencia() != 0){
			parametros.put("mesAno", cobrancaGrupoCronogramaMesParametros.getMesAno());
		}else{
		...
		retorno = this.gerarRelatorio(ConstantesRelatorios.RELATORIO_CRONOGRAMA_COBRANCA_MANTER, parametros, ds, tipoFormatoRelatorio);

		// ------------------------------------
		// Grava o relatório no sistema
		try{
			persistirRelatorioConcluido(retorno, Relatorio.MANTER_CRONOGRAMA_COBRANCA, idFuncionalidadeIniciada, null);
	...
	public void agendarTarefaBatch(){
		...
		AgendadorTarefas.agendarTarefa("RelatorioManterCronogramaCobranca", this);
	}
