...
public class ExtratoDebitoRelatorioHelper implements Serializable {
     ...
     * @created 12/09/2006
     * 
     * Alteração v0.06 - 22/10/2008 - vsm 
     */
...
//		}
     ...
     * (DBAC_VLDEBITO - ((DBAC_VLDEBITO/DBAC_NNPRESTAODEBITO) * DBAC_NNPRESTACAOCOBRADA))
     * 
     * @author Vivianne Sousa
     * @created 12/09/2006
     * 
     * @author Saulo Lima
     * @date 10/09/2009
     * Alterado o tipo de arredondamento + Alteração na lógica
     */
	public BigDecimal[] getValorTotalRestanteDebitosACobrar(){

		BigDecimal[] retorno = new BigDecimal[2];
		BigDecimal retornoValorTotal = BigDecimal.ZERO;
		BigDecimal retornoValorDesconto = BigDecimal.ZERO;

		Map<Integer, DebitoACobrar> colecaoDebitoACobrarMap = new HashMap<Integer, DebitoACobrar>();

		Iterator<CobrancaDocumentoItem> iterator = this.getColecaoCobrancaDocumentoItemDebitosACobrar().iterator();

		while(iterator.hasNext()){

			DebitoACobrar debitoACobrar = (iterator.next()).getDebitoACobrarGeral().getDebitoACobrar();

			if(!colecaoDebitoACobrarMap.containsKey(debitoACobrar.getId())){

				colecaoDebitoACobrarMap.put(debitoACobrar.getId(), debitoACobrar);

				BigDecimal valorDebito = debitoACobrar.getValorDebito();
				BigDecimal numPrestacaoDebito = new BigDecimal(debitoACobrar.getNumeroPrestacaoDebito());

				BigDecimal retornoDivisao = BigDecimal.ZERO;
				BigDecimal retornoMultiplicacao = BigDecimal.ZERO;
				BigDecimal valorCalculado = BigDecimal.ZERO;

				// Caso QuantidadeParcelasAntecipadas seja diferente de NULO e ZERO, calcular o
				// valor Antecipado. Caso contrário, calcular o valor total.
				Integer quantidadeAPagar = debitoACobrar.getNumeroPrestacaoDebito() - debitoACobrar.getNumeroPrestacaoCobradas();
				if(debitoACobrar.getQuantidadeParcelasAntecipadas() != null
								&& debitoACobrar.getQuantidadeParcelasAntecipadas().intValue() != 0
								&& !(quantidadeAPagar.equals(debitoACobrar.getQuantidadeParcelasAntecipadas()))){

					retornoDivisao = valorDebito.divide(numPrestacaoDebito, Parcelamento.CASAS_DECIMAIS, Parcelamento.TIPO_ARREDONDAMENTO);
					retornoDivisao = retornoDivisao.setScale(Parcelamento.CASAS_DECIMAIS, Parcelamento.TIPO_ARREDONDAMENTO);
					valorCalculado = retornoDivisao.multiply(new BigDecimal(debitoACobrar.getQuantidadeParcelasAntecipadas()));
					valorCalculado = valorCalculado.setScale(Parcelamento.CASAS_DECIMAIS, Parcelamento.TIPO_ARREDONDAMENTO);

				}else{

					BigDecimal numPrestacaoCobrada = new BigDecimal(debitoACobrar.getNumeroPrestacaoCobradas());
					retornoDivisao = valorDebito.divide(numPrestacaoDebito, Parcelamento.CASAS_DECIMAIS, Parcelamento.TIPO_ARREDONDAMENTO);

					retornoDivisao = retornoDivisao.setScale(Parcelamento.CASAS_DECIMAIS, Parcelamento.TIPO_ARREDONDAMENTO);

					retornoMultiplicacao = retornoDivisao.multiply(numPrestacaoCobrada);
					retornoMultiplicacao = retornoMultiplicacao.setScale(Parcelamento.CASAS_DECIMAIS, Parcelamento.TIPO_ARREDONDAMENTO);

					valorCalculado = valorDebito.subtract(retornoMultiplicacao);
					valorCalculado = valorCalculado.setScale(Parcelamento.CASAS_DECIMAIS, Parcelamento.TIPO_ARREDONDAMENTO);
				}

				retornoValorTotal = retornoValorTotal.add(valorCalculado);

				if(debitoACobrar.getDebitoTipo().getId().equals(DebitoTipo.JUROS_SOBRE_PARCELAMENTO)){
					retornoValorDesconto = retornoValorDesconto.add(valorCalculado);
				}
			}

		}

		retornoValorTotal = retornoValorTotal.setScale(Parcelamento.CASAS_DECIMAIS, Parcelamento.TIPO_ARREDONDAMENTO);
		retornoValorDesconto = retornoValorDesconto.setScale(Parcelamento.CASAS_DECIMAIS, Parcelamento.TIPO_ARREDONDAMENTO);

		retorno[0] = retornoValorTotal;
		retorno[1] = retornoValorDesconto;
