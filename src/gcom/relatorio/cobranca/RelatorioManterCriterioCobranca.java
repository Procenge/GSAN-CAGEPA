...
public class RelatorioManterCriterioCobranca
	...
	public Object executar() throws TarefaException{
		...
		if(colecaoCobrancaCriterio != null && !colecaoCobrancaCriterio.isEmpty()){
			...
			Iterator colecaoCobrancaCriterioIterator = colecaoCobrancaCriterio.iterator();

			// laço para criar a coleção de parâmetros da analise
			while(colecaoCobrancaCriterioIterator.hasNext()){

				CobrancaCriterio cobrancaCriterio = (CobrancaCriterio) colecaoCobrancaCriterioIterator.next();
				...
				if(colecaoCobrancaCriterioLinha != null && !colecaoCobrancaCriterioLinha.isEmpty()){
					...
					while(colecaoCobrancaCriterioLinhaIterator.hasNext()){
						...
						CobrancaCriterioLinha cobrancaCriterioLinha = (CobrancaCriterioLinha) colecaoCobrancaCriterioLinhaIterator.next();
						...
						// Descricao
										...
										Util.formatarData(cobrancaCriterio.getDataInicioVigencia()),

										// Número Anos
										cobrancaCriterio.getNumeroContaAntiga().toString(),

										// Imóvel com Situação Especial de Cobrança
										cobrancaCriterio.getIndicadorEmissaoImovelParalisacao().equals(
														ConstantesSistema.INDICADOR_USO_ATIVO) ? "SIM" : "NÃO",

										// Imóvel com Situação de Cobrança
										cobrancaCriterio.getIndicadorEmissaoImovelSituacaoCobranca().equals(
														ConstantesSistema.INDICADOR_USO_ATIVO) ? "SIM" : "NÃO",

										// Considerar Contas em Revisão
										cobrancaCriterio.getIndicadorEmissaoContaRevisao().equals(ConstantesSistema.INDICADOR_USO_ATIVO) ? "SIM"
														: "NÃO",

										// Imóvel com Débito só da Conta do Mês
										cobrancaCriterio.getIndicadorEmissaoDebitoContaMes().equals(ConstantesSistema.INDICADOR_USO_ATIVO) ? "SIM"
														: "NÃO",

										// Inquilino com Débito só da Conta do Mês
										cobrancaCriterio.getIndicadorEmissaoInquilinoDebitoContaMes().equals(
														ConstantesSistema.INDICADOR_USO_ATIVO) ? "SIM" : "NÃO",

										// Imóvel com Débito só de Contas Antigas
										cobrancaCriterio.getIndicadorEmissaoDebitoContaAntiga().equals(
														ConstantesSistema.INDICADOR_USO_ATIVO) ? "SIM" : "NÃO",
										...
										// Imóvel Perfil
										cobrancaCriterioLinha.getImovelPerfil().getDescricao(),

										// Categoria
										cobrancaCriterioLinha.getCategoria().getDescricao(),

										// Intervalo de Valor do Débito
										Util.formatarMoedaReal(cobrancaCriterioLinha.getValorMinimoDebito()) + " a "
														+ Util.formatarMoedaReal(cobrancaCriterioLinha.getValorMaximoDebito()),

										// Intervalo de Quantidade de Contas
										cobrancaCriterioLinha.getQuantidadeMinimaContas() + " a "
														+ cobrancaCriterioLinha.getQuantidadeMaximaContas(),

										// Valor Mínimo da Conta do Mês
										Util.formatarMoedaReal(cobrancaCriterioLinha.getValorMinimoContaMes()),

										// Valor Mínimo do Débito para Cliente com
										// Débito Automático
										Util.formatarMoedaReal(cobrancaCriterioLinha.getValorMinimoDebitoDebitoAutomatico()),

										// Quantidade Mínima de Contas para Cliente com
										// Débito Automático
										cobrancaCriterioLinha.getQuantidadeMinimaContasDebitoAutomatico().toString());
		...
		// Descrição
		parametros.put("descricao", cobrancaCriterioParametros.getDescricaoCobrancaCriterio());

		// Data de Início da Vigência
		if(cobrancaCriterioParametros.getDataInicioVigencia() != null){
			parametros.put("dataInicio", Util.formatarData(cobrancaCriterioParametros.getDataInicioVigencia()));
		}else{
		...
		if(cobrancaCriterioParametros.getNumeroContaAntiga() != null){
			parametros.put("numeroAnos", cobrancaCriterioParametros.getNumeroContaAntiga().toString());
		}else{
		...
		if(cobrancaCriterioParametros.getIndicadorEmissaoImovelParalisacao() != null){
			if(cobrancaCriterioParametros.getIndicadorEmissaoImovelParalisacao().equals(ConstantesSistema.INDICADOR_USO_ATIVO)){
		...
		if(cobrancaCriterioParametros.getIndicadorEmissaoImovelSituacaoCobranca() != null){
			if(cobrancaCriterioParametros.getIndicadorEmissaoImovelSituacaoCobranca().equals(ConstantesSistema.INDICADOR_USO_ATIVO)){
		...
		if(cobrancaCriterioParametros.getIndicadorEmissaoContaRevisao() != null){
			if(cobrancaCriterioParametros.getIndicadorEmissaoContaRevisao().equals(ConstantesSistema.INDICADOR_USO_ATIVO)){
		...
		if(cobrancaCriterioParametros.getIndicadorEmissaoDebitoContaMes() != null){
			if(cobrancaCriterioParametros.getIndicadorEmissaoDebitoContaMes().equals(ConstantesSistema.INDICADOR_USO_ATIVO)){
		...
		if(cobrancaCriterioParametros.getIndicadorEmissaoInquilinoDebitoContaMes() != null){
			if(cobrancaCriterioParametros.getIndicadorEmissaoInquilinoDebitoContaMes().equals(ConstantesSistema.INDICADOR_USO_ATIVO)){
		...
		if(cobrancaCriterioParametros.getIndicadorEmissaoDebitoContaAntiga() != null){
			if(cobrancaCriterioParametros.getIndicadorEmissaoDebitoContaAntiga().equals(ConstantesSistema.INDICADOR_USO_ATIVO)){
		...
		if(cobrancaCriterioParametros.getIndicadorUso() != null){
			if(cobrancaCriterioParametros.getIndicadorUso().equals(ConstantesSistema.INDICADOR_USO_ATIVO)){
		...
		retorno = this.gerarRelatorio(ConstantesRelatorios.RELATORIO_CRITERIO_COBRANCA_MANTER, parametros, ds, tipoFormatoRelatorio);

		// ------------------------------------
		// Grava o relatório no sistema
		try{
			persistirRelatorioConcluido(retorno, Relatorio.MANTER_CRITERIO_COBRANCA, idFuncionalidadeIniciada, null);
	...
	public int calcularTotalRegistrosRelatorio(){
		...
		retorno = Fachada.getInstancia().totalRegistrosPesquisa((FiltroCobrancaCriterio) getParametro("filtroCobrancaCriterio"),
	...
	public void agendarTarefaBatch(){
		...
		AgendadorTarefas.agendarTarefa("RelatorioManterCriterioCobranca", this);
	}
