 ...
 * @date 14/02/2007
 */
public class RelatorioConsultarDebitos
	...
	private Collection<RelatorioConsultarDebitosBean> inicializarBeanRelatorio(Collection<ContaValoresHelper> colecaoContas,
		...
		if(colecaoContas != null && !colecaoContas.isEmpty()){
			...
			while(colecaoContasIterator.hasNext()){
				...
				if(contaValoresHelper.getConta().getConsumoEsgoto() != null){
					...
					valorTotalEsgotoConsumoContas = valorTotalAguaConsumoContas + contaValoresHelper.getConta().getConsumoEsgoto();
				...
				if(contaValoresHelper.getValorTotalContaValores() != null){
					...
					valorTotalAcrescimosImpontualidadeContas = valorTotalAcrescimosImpontualidadeContas.add(contaValoresHelper
									.getValorTotalContaValores());

					if(idImovelAnterior.equals(contaValoresHelper.getConta().getImovel().getId())){
						valorSubtotalAcrescimosImpontualidadeContas = valorSubtotalAcrescimosImpontualidadeContas.add(contaValoresHelper
										.getValorTotalContaValores());
					}
				}
				...
				if(pesquisaCliente != null && !pesquisaCliente.trim().equals("")){
					...
					if(relatorioEndereco != null && !relatorioEndereco.trim().equals("")){
						...
						String nomeUsuario = fachada.consultarClienteUsuarioImovel(contaValoresHelper.getConta().getImovel().getId());
		...
		if(colecaoDebitosACobrar != null && !colecaoDebitosACobrar.isEmpty()){
			...
			while(colecaoDebitosACobrarIterator.hasNext()){
				...
				if(debitoACobrar.getParcelasRestante() != 0){
					parcelasCobrar = "" + debitoACobrar.getParcelasRestante();
				}

				String valorCobrar = "";
				if(debitoACobrar.getValorTotal() != null){
					valorCobrar = Util.formatarMoedaReal(debitoACobrar.getValorTotal());
					...
					if(idImovelAnterior.equals(debitoACobrar.getImovel().getId())){
						valorSubtotalDebitoACobrar = valorSubtotalDebitoACobrar.add(debitoACobrar.getValorTotal());
					}
				}
				...
				if(pesquisaCliente != null && !pesquisaCliente.trim().equals("")){
					...
					if(!idImovelAnterior.equals(debitoACobrar.getImovel().getId())){
						...
						if(count > 1){
							...
							idImovelAnterior = debitoACobrar.getImovel().getId();
							if(debitoACobrar.getValorTotal() != null){
								valorSubtotalDebitoACobrar = debitoACobrar.getValorTotal();
							}else{
					...
					if(relatorioEndereco != null && !relatorioEndereco.trim().equals("")){
						...
						String nomeUsuario = fachada.consultarClienteUsuarioImovel(debitoACobrar.getImovel().getId());
		...
		if(colecaoCreditosARealizar != null && !colecaoCreditosARealizar.isEmpty()){
			...
			while(colecaoCreditosARealizarIterator.hasNext()){
				...
				if(creditoARealizar.getParcelasRestante() != 0){
					parcelasCreditar = "" + creditoARealizar.getParcelasRestante();
				}

				String valorCreditar = "";
				if(creditoARealizar.getValorTotal() != null){
					valorCreditar = Util.formatarMoedaReal(creditoARealizar.getValorTotal());
					valorTotalCreditos = valorTotalCreditos.add(creditoARealizar.getValorTotal());
				}
				...
				if(pesquisaCliente != null && !pesquisaCliente.trim().equals("")){
					...
					if(relatorioEndereco != null && !relatorioEndereco.trim().equals("")){
						...
						String nomeUsuario = fachada.consultarClienteUsuarioImovel(creditoARealizar.getImovel().getId());
		...
		if(colecaoGuiasPagamento != null && !colecaoGuiasPagamento.isEmpty()){
			...
			while(colecaoGuiasPagamentoIterator.hasNext()){
				...
				GuiaPagamentoPrestacao prestacao = (GuiaPagamentoPrestacao) Util.retonarObjetoDeColecao(guiaPagamentoValoresHelper
								.getGuiaPagamentoPrestacoes());
				if(prestacao != null){

					if(prestacao.getDataEmissao() != null){
						dataEmissaoGuia = Util.formatarData(prestacao.getDataEmissao());
					}

					if(prestacao.getDataVencimento() != null){
						dataVencimentoGuia = Util.formatarData(prestacao.getDataVencimento());
					}
				}

				String numeroPrestacaoGuia = "";
				if(guiaPagamentoValoresHelper.getNumeroPrestacao() != null){
					numeroPrestacaoGuia = guiaPagamentoValoresHelper.getNumeroPrestacao().toString();
				}

				String valorGuia = "";
				if(guiaPagamentoValoresHelper.getValorTotalPrestacao() != null){
					valorGuia = Util.formatarMoedaReal(guiaPagamentoValoresHelper.getValorTotalPrestacao());
					valorTotalGuias = valorTotalGuias.add(guiaPagamentoValoresHelper.getValorTotalPrestacao());
				}
				...
				if(pesquisaCliente != null && !pesquisaCliente.trim().equals("")){
					...
					if(guiaPagamento.getImovel() != null && guiaPagamento.getImovel().getId() != null){
						...
						nomeUsuario = fachada.consultarClienteUsuarioImovel(new Integer(idImovel));
					...
					if(relatorioEndereco != null && !relatorioEndereco.trim().equals("")){
						...
						// Conta Id
										...
										null,
										// Guia Pagamento Id
										guiaPagamentoValoresHelper.getIdGuiaPagamento().toString(),
										// Tipo Débito
										tipoDebito,
										// Data Emissão
										dataEmissaoGuia,
	...
	public Object executar() throws TarefaException{
		...
		Collection<RelatorioConsultarDebitosBean> colecaoBean = this.inicializarBeanRelatorio(colecaoContaValores, colecaoDebitoACobrar,
						colecaoCreditoARealizar, colecaoGuiaPagamento);
		...
		if(gerarDebitoACobrarTaxa != null && gerarDebitoACobrarTaxa && debitoTipo != null){
			...
			if(idImovel != null){
				fachada.gerarDebitoACobrar(Integer.valueOf(idImovel), sistema.getAnoMesFaturamento(), debitoTipo, null);
			}
		...
		if(relatorioEndereco != null && !relatorioEndereco.trim().equals("")){
			retorno = this.gerarRelatorio(ConstantesRelatorios.RELATORIO_CONSULTAR_DEBITOS_CLIENTE_ENDERECO, parametros, ds,
