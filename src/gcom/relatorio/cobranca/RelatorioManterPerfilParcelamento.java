...
public class RelatorioManterPerfilParcelamento
	...
	public Object executar() throws TarefaException{
		...
		if(colecaoParcelamentoPerfil != null && !colecaoParcelamentoPerfil.isEmpty()){
			...
			Iterator colecaoParcelamentoPerfilIterator = colecaoParcelamentoPerfil.iterator();

			// laço para criar a coleção de parâmetros da analise
			while(colecaoParcelamentoPerfilIterator.hasNext()){

				ParcelamentoPerfil parcelamentoPerfil = (ParcelamentoPerfil) colecaoParcelamentoPerfilIterator.next();
				...
				// prestação
				Collection colecaoReparcelamentosConsecutivos = fachada.pesquisarReparcelamentoConsecutivo(parcelamentoPerfil.getId());

				if(colecaoReparcelamentosConsecutivos != null && !colecaoReparcelamentosConsecutivos.isEmpty()){

					Iterator colecaoReparcelamentosConsecutivosIterator = colecaoReparcelamentosConsecutivos.iterator();
					...
					while(colecaoReparcelamentosConsecutivosIterator.hasNext()){
						...
						// Dados do Perfil de Parcelamento:
										...
										parcelamentoPerfil.getImovelSituacaoTipo() == null ? "" : parcelamentoPerfil
														.getImovelSituacaoTipo().getDescricaoImovelSituacaoTipo(),

										// Perfil do Imóvel
										parcelamentoPerfil.getImovelPerfil() == null ? "" : parcelamentoPerfil.getImovelPerfil()
														.getDescricao(),

										// Subcategoria
										parcelamentoPerfil.getSubcategoria() == null ? "" : parcelamentoPerfil.getSubcategoria()
														.getDescricao(),
										...
										// Id Reparcelamento
										parcelamentoQuantidadeReparcelamentoRelatorioHelper.getIdReparcelamento().toString(),

										// Quantidade Máxima de Reparcelamentos
										// Consecutivos
										parcelamentoQuantidadeReparcelamentoRelatorioHelper.getQuantidadeMaximaReparcelamento().toString(),

										// Id Prestação
										parcelamentoQuantidadeReparcelamentoRelatorioHelper.getIdPrestacao().toString(),

										// Quantidade Máxima de Prestações do
										// Parcelamento
										parcelamentoQuantidadeReparcelamentoRelatorioHelper.getQuantidadeMaximaPrestacoes().toString(),

										// Taxa de Juros
										Util.formatarMoedaReal(parcelamentoQuantidadeReparcelamentoRelatorioHelper.getTaxaJuros()),

										// Percentual Mínimo de Entrada
										Util.formatarMoedaReal(parcelamentoQuantidadeReparcelamentoRelatorioHelper
				...
				Collection colecaoDescontosAntiguidade = fachada.pesquisarParcelamentoDescontoAntiguidade(parcelamentoPerfil.getId());

				if(colecaoDescontosAntiguidade != null && !colecaoDescontosAntiguidade.isEmpty()){

					Iterator colecaoDescontosAntiguidadeIterator = colecaoDescontosAntiguidade.iterator();
				...
				// Pesquisa a coleção de parcelamento desconto antiguidade referente
				// ao perfil de parcelamento
				Collection colecaoDescontosInatividade = fachada.pesquisarParcelamentoDescontoInatividade(parcelamentoPerfil.getId());

				if(colecaoDescontosInatividade != null && !colecaoDescontosInatividade.isEmpty()){

					Iterator colecaoDescontosInatividadeIterator = colecaoDescontosInatividade.iterator();
		...
		// RD
		if(parcelamentoPerfilParametros.getResolucaoDiretoria().getNumeroResolucaoDiretoria() != null){
			parametros.put("rd", parcelamentoPerfilParametros.getResolucaoDiretoria().getNumeroResolucaoDiretoria());
		}else{
			parametros.put("rd", "");
		}

		// Tipo da Situação do Imóvel
		if(parcelamentoPerfilParametros.getImovelSituacaoTipo().getDescricaoImovelSituacaoTipo() != null){
			parametros.put("imovelSituacaoTipo", parcelamentoPerfilParametros.getImovelSituacaoTipo().getDescricaoImovelSituacaoTipo());
		}else{
		...
		if(parcelamentoPerfilParametros.getImovelPerfil().getDescricao() != null){
			parametros.put("imovelPerfil", parcelamentoPerfilParametros.getImovelPerfil().getDescricao());
		}else{
		...
		if(parcelamentoPerfilParametros.getSubcategoria().getDescricao() != null){
			parametros.put("subcategoria", parcelamentoPerfilParametros.getSubcategoria().getDescricao());
		}else{
		...
		retorno = this.gerarRelatorio(ConstantesRelatorios.RELATORIO_PERFIL_PARCELAMENTO_MANTER, parametros, ds, tipoFormatoRelatorio);

		// ------------------------------------
		// Grava o relatório no sistema
		try{
			persistirRelatorioConcluido(retorno, Relatorio.MANTER_PERFIL_PARCELAMENTO, idFuncionalidadeIniciada, null);
	...
	public int calcularTotalRegistrosRelatorio(){
		...
		retorno = Fachada.getInstancia().totalRegistrosPesquisa((FiltroParcelamentoPerfil) getParametro("filtroParcelamentoPerfil"),
	...
	public void agendarTarefaBatch(){
		...
		AgendadorTarefas.agendarTarefa("RelatorioManterPerfilParcelamento", this);
	}
