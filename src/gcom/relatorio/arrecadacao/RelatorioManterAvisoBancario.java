	...
	public Object executar() throws TarefaException{
		...
		if(colecaoAvisosBancarios != null && !colecaoAvisosBancarios.isEmpty()){
			...
			while(colecaoAvisosBancariosIterator.hasNext()){
				...
				// Data de pagamento do aviso bancário
				...
				if(avisoBancarioRelatorioHelper.getTipo().shortValue() == AvisoBancario.INDICADOR_CREDITO.shortValue()){
					...
					// 1.8.4.1. Se Aviso de Credito (AVBC_ICCREDITODEBITO=1) Então VL.Realizado =
					// VL.Inf.Arrecadação + VL.Acertos Arrecadação - VL.Deduções.
					valorLiquido = avisoBancarioRelatorioHelper.getTotalArrecadacao().add(valorAcerto).subtract(totalDeducoes);

					// VL.Diferença=(VL.Calculado – (VL.Informado + Vl.Acertos) );
					...
					// 1.8.4.2 Se Aviso de Debito (AVBC_ICCREDITODEBITO=2) Então VL.Realizado =
					// VL.Inf.Devolução + VL.Acertos Devolução - VL.Deduções
				...
				// 2.2.7. Situação (Aberto/Fechado) (Se Diferença=0, FECHADO senão ABERTO);
