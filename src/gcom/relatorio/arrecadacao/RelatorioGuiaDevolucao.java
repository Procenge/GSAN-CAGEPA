 ...
 * @date 05/10/06
 */
public class RelatorioGuiaDevolucao
	...
	private Collection<RelatorioGuiaDevolucaoBean> inicializarBeanRelatorio(Collection<GuiaPagamentoRelatorioHelper> dadosRelatorio){
		...
		while(iterator.hasNext()){
			...
			GuiaDevolucaoRelatorioHelper guiaDevolucaoRelatorioHelper = (GuiaDevolucaoRelatorioHelper) iterator.next();
			...
			if(guiaDevolucaoRelatorioHelper.getConta() != null && guiaDevolucaoRelatorioHelper.getAgencia() != null
				...
				contaDebito = "CONTA DÉBITO: " + guiaDevolucaoRelatorioHelper.getConta() + " - AG: "
								+ guiaDevolucaoRelatorioHelper.getAgencia() + " - NÃO PAGAR APÓS "
								+ Util.formatarData(guiaDevolucaoRelatorioHelper.getDataValidade());
			}

			RelatorioGuiaDevolucaoBean beanCliente = new RelatorioGuiaDevolucaoBean(sequencialAno, valor, registroServicoAtendimento,
							matriculaImovel, cliente, nomeCliente, cpfCnpj, endereco, identidade, valorExtenso, observacao, dataDigitacao,
							dataAnalise, dataAutorizacao, nomeUsuario, nomeAnalista, nomeAutorizador, "VIA CLIENTE", contaDebito);

			retorno.add(beanCliente);

			RelatorioGuiaDevolucaoBean beanArrecadador = new RelatorioGuiaDevolucaoBean(sequencialAno, valor, registroServicoAtendimento,
							matriculaImovel, cliente, nomeCliente, cpfCnpj, endereco, identidade, valorExtenso, observacao, dataDigitacao,
							dataAnalise, dataAutorizacao, nomeUsuario, nomeAnalista, nomeAutorizador, "VIA AG. ARRECADADOR", contaDebito);

			retorno.add(beanArrecadador);

			RelatorioGuiaDevolucaoBean beanCompesa = new RelatorioGuiaDevolucaoBean(sequencialAno, valor, registroServicoAtendimento,
							matriculaImovel, cliente, nomeCliente, cpfCnpj, endereco, identidade, valorExtenso, observacao, dataDigitacao,
							dataAnalise, dataAutorizacao, nomeUsuario, nomeAnalista, nomeAutorizador, "VIA "
											+ sistemaParametro.getNomeAbreviadoEmpresa(), contaDebito);
	...
	public Object executar() throws TarefaException{
		...
		Collection<RelatorioGuiaDevolucaoBean> colecaoBean = this.inicializarBeanRelatorio(dadosRelatorio);
		...
		retorno = this.gerarRelatorio(ConstantesRelatorios.RELATORIO_GUIA_DEVOLUCAO, parametros, ds, tipoFormatoRelatorio);

		// ------------------------------------
		// Grava o relatório no sistema
		try{
			persistirRelatorioConcluido(retorno, Relatorio.GUIA_DEVOLUCAO, idFuncionalidadeIniciada, null);
