 ...
 * @date 04/07/2007
 */
public class RelatorioAcompanhamentoMovimentoArrecadadoresPorNSA
	...
	private Collection<RelatorioAcompanhamentoMovimentoArrecadadoresPorNSABean> inicializarBeanRelatorio(
		...
		while(iterator.hasNext()){
			...
			if(helper.getValor() != null && !helper.getValor().equals(0)){
				// BigDecimal valor = new BigDecimal("0.00");
				Integer tamanho = helper.getValor().toString().length();
				int inteiro = tamanho - 2;
				valor = new BigDecimal(helper.getValor().toString().substring(0, inteiro) + "."
								+ helper.getValor().toString().substring(inteiro, inteiro + 2));
			}
			...
			if(helper.getConcessionaria() != null){
				concessionaria = helper.getConcessionaria();
			}

			RelatorioAcompanhamentoMovimentoArrecadadoresPorNSABean bean = new RelatorioAcompanhamentoMovimentoArrecadadoresPorNSABean(
							banco, formaArrecadacao, nsa, dataGeracao, qtdeRegistros, valor, tarifa, valorAPagar, mesAnoArrecadacao,
							concessionaria);
	...
	public Object executar() throws TarefaException{
		...
		Boolean indConcessionaria = (Boolean) getParametro("indConcessionaria");
		...
		parametros.put("indConcessionaria", indConcessionaria);

		Collection<RelatorioAcompanhamentoMovimentoArrecadadoresPorNSABean> colecaoBean = this.inicializarBeanRelatorio(dadosRelatorio);
		...
		retorno = this.gerarRelatorio(ConstantesRelatorios.RELATORIO_ACOMPANHAMENTO_MOVIMENTO_ARRECADADORES_POR_NSA, parametros, ds,
						tipoFormatoRelatorio);

		// ------------------------------------
		// Grava o relatório no sistema
		try{
			persistirRelatorioConcluido(retorno, Relatorio.ACOMPANHAMENTO_MOVIMENTO_ARRECADADORES_POR_NSA, idFuncionalidadeIniciada, null);
