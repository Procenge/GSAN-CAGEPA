...
public class RelatorioEvolucaoContasAReceberContabil
	...
	private Collection<RelatorioEvolucaoContasAReceberContabilBean> inicializarBeanRelatorio(Collection listaCamposConsultaRelatorio){

		Iterator iterator = listaCamposConsultaRelatorio.iterator();

		Collection<RelatorioEvolucaoContasAReceberContabilBean> retorno = new ArrayList();

		while(iterator.hasNext()){
			// ResumoeFaturamentoRelatorioHelper resumoFaturamentoRelatorioHelper =
			// (ResumoFaturamentoRelatorioHelper) iterator.next();
			//			
	...
	public Object executar() throws TarefaException{
		...
		Collection<RelatorioEvolucaoContasAReceberContabilBean> colecaoBean = fachada
						.consultarDadosEvolucaoContasAReceberContabilRelatorio(opcaoTotalizacao, mesAno, codigoGerencia, codigoLocalidade,
										unidadeNegocio);
		...
		}else if(opcaoTotalizacao.equalsIgnoreCase("gerenciaRegionalUnidadeNegocio")){
			GerenciaRegional gerencia = pesquisarGerenciaRegional(codigoGerencia);
			if(gerencia != null){
		...
		retorno = this.gerarRelatorio(ConstantesRelatorios.RELATORIO_EVOLUCAO_CONTAS_A_RECEBER_CONTABIL, parametros, ds,
						tipoFormatoRelatorio);

		// ------------------------------------
		// Grava o relatório no sistema
		/*
		 * try {//TODO Variável abaixo não foi encontrada na Classe no 1o. Build
	...
	private GerenciaRegional pesquisarGerenciaRegional(Integer idGerencia){
		...
		Collection<GerenciaRegional> gerenciaPesquisada = fachada.pesquisar(filtroGerencia, GerenciaRegional.class.getName());
	...
	private UnidadeNegocio pesquisarUnidadeNegocio(Integer idUnidadeNegocio){
		...
		filtroUN.adicionarParametro(new ParametroSimples(FiltroUnidadeNegocio.ID, idUnidadeNegocio));

		Collection<UnidadeNegocio> UNPesquisada = fachada.pesquisar(filtroUN, UnidadeNegocio.class.getName());
