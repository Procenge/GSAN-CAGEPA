...
/**
 * Classe a ser utilizada para retornar dados detalhado de uma OS's
 * 
 * @author Andre.Lopes
 */
public class GetDetalhesOSWebservice
	...
	protected void preencherJSONBody(ActionForm form, HttpServletRequest request) throws NegocioException{
		...
		Integer numeroOs = recuperarParametroInteiro("numeroOS", "Número da OS", true, false, request);
		Integer codigoServicoOs = recuperarParametroInteiro("codigoServicoOS", "Código do serviço", false, false, request);

		try{
			FiltrarOrdemServicoHelper filtro = new FiltrarOrdemServicoHelper();

			filtro.setNumeroOS(numeroOs);
			filtro.setCodigoServicoOs(codigoServicoOs);
			// filtro.setPesquisarMaxTramite(false); teste para trazer registro!
			// ver:
			// select * from tramite where orse_id is not null;

			OrdemServicoDetalhesJSONHelper resultadoConsulta = ServiceLocator.getInstancia().getControladorOrdemServico()
							.pesquisarOrdemServicoDetalhesWebService(filtro);

			if(resultadoConsulta == null || resultadoConsulta.getNumeroOs() == null){
				informarStatus(STATUS_TIPO_ALERTA, MensagemUtil.obterMensagem(Constantes.RESOURCE_BUNDLE, "atencao.colecao_vazia"));
			}else{

				adicionarObjetoAoBody("OS", resultadoConsulta);
			}
