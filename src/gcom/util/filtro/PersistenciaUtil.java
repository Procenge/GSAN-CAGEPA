...
public class PersistenciaUtil {
	 ...
	 */
	public static Collection processaColecoesParaCarregamento(Collection nomeColecoes, Collection colecaoDados)
		...
		if(!nomeColecoes.isEmpty()){
			...
			while(iteratorNomes.hasNext()){
				...
				while(iteratorDados.hasNext()){
					...
					try{
						nomeColecao = nomeColecao.substring(0, 1).toUpperCase() + nomeColecao.substring(1, nomeColecao.length());

						Collection colecao = ((Collection) objetoDado.getClass().getMethod("get" + nomeColecao, (Class[]) null).invoke(
						...
						try{
							objetoDado.getClass().getMethod("set" + nomeColecao, new Class[] {Set.class}).invoke(objetoDado,
											(Object[]) null);
	 ...
	 *                Descrição da exceção
	 */
	public static void processaObjetosParaCarregamento(Collection nomeObjetos, Collection colecaoDados) throws ErroRepositorioException{
			...
			while(iteratorNomes.hasNext()){
				...
				while(iteratorDados.hasNext()){
					...
					while(separador.hasMoreTokens()){
						...
						StringBuffer token = new StringBuffer(separador.nextToken());

						token.insert(0, "get");
						token.replace(3, 4, token.substring(3, 4).toUpperCase());
						...
						try{
							...
							if(retorno == null && contadorIteracao != 0){
								...
								objetoDado = ((retorno == null) ? objetoDado : retorno);
								contadorIteracao++;
							}

							retorno = objetoDado.getClass().getMethod(token.toString(), (Class[]) null).invoke(objetoDado, (Object[]) null);
	...
	public static String processaObjetosParaCarregamentoJoinFetch(String aliasTabela, Collection nomeObjetos)
					throws ErroRepositorioException{

		StringBuilder resultadoJoinsMontados = new StringBuilder();

		// Verifica se o usuario informou algum objeto para ser carregado
		if(!nomeObjetos.isEmpty()){

			Iterator iteratorNomes = nomeObjetos.iterator();
			StringBuilder elementoJoin = new StringBuilder();
			...
			while(iteratorNomes.hasNext()){
				...
				elementoJoin.delete(0, elementoJoin.length());

				while(separador.hasMoreTokens()){

					if(elementoJoin.length() == 0){
						elementoJoin.append(aliasTabela).append(".").append(separador.nextToken());
					}else{
						elementoJoin.append(".").append(separador.nextToken());
					}

					// O join com o comp_id não funciona
					// Elimina o join do comp_id
					if(!elementoJoin.toString().endsWith(".comp_id")){

						// int idxDot = elementoJoin.lastIndexOf(".");
						// StringBuilder aliased = new
						// StringBuilder(elementoJoin).append(" ").append(elementoJoin.substring(idxDot
						// + 1));

						resultadoJoinsMontados.append(" left join fetch ").append(elementoJoin);
					}
		...
		return resultadoJoinsMontados.toString();
	}
