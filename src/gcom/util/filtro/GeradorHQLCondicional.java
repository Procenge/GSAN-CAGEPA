...
public class GeradorHQLCondicional {
	...
	private static Query efetuarQuery(CondicionalQuery condicionalQuery, Session session) throws HibernateException{
		...
		while(iterator.hasNext()){
			...
			if(parametro instanceof String){
				...
				if(parametroString != null && !parametroString.trim().equals("")
					...
					// String
					parametro = parametroString.trim().replace('Ã', 'A').replace('Õ', 'O').replace('Á', 'A').replace('É', 'E').replace('Í',
									'I').replace('Ú', 'U').replace('À', 'A').replace('Â', 'A').replace('Ê', 'E').replace('Ô', 'O').replace(
									'Ü', 'U').replace('Ç', 'C').replace('ã', 'A').replace('õ', 'O').replace('á', 'A').replace('é', 'E')
									.replace('í', 'I').replace('ú', 'U').replace('à', 'A').replace('â', 'A').replace('ê', 'E').replace('ô',
													'O').replace('ü', 'U').replace('ç', 'C'); // ÃÕÁÉÍÓÚÀÂÊÔÜÇ
		...
		// Seta o número máximo de resultados retornados pela pesquisa
		// query
