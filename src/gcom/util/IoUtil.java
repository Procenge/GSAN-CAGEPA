...
public class IoUtil {
	...
	public static boolean validaData(int dia, int mes, int ano){
		...
		if(!((0 < mes) && (mes <= 12) && (dia > 0) && (dia <= numeroDeDiasDoMes(mes, ano)))){
			retorno = false;
		}
		return retorno;
	}
	 ...
	 * @return Uma instância da classe Date
	 */
	public static Date criarData(int dia, int mes, int ano, int hora, int minutos, int segundos){
	...
	public static long diferencaEntreDatas(Date dataInicial, Date dataFinal){
		...
		BigDecimal resultado = numerador.divide(denominador, 2, BigDecimal.ROUND_HALF_UP);
	...
	/**
	 * Método para le um conteúdo de um campo blob de um arquivo do tipo txt.
	 * 
	 * @author Ricardo Rodrigues
	 * @date 12/03/2012
	 * @param blob
	 * @return StringBuilder com todo o conteúdo do arquivo txt.
	 */
	public static StringBuilder lerConteudoCampoBlobTipoTxt(Blob blob){
		...
		try{
			...
			// thiagosantos: tem que finalizar os recursos.
			if(blobStream != null){
				try{
					blobStream.close();
				}catch(IOException e){
	...
	public static byte[] transformarObjetoParaBytes(Object objeto) throws IOException{
		...
		try{
			...
			e.printStackTrace();
			throw new IOException("Falha ao serializar a classe [" + objeto.getClass().getName() + "]");
		}
		return retorno;
	}
	...
	public static String converterClobParaString(Clob clob){

		StringBuffer resultadoConversao = new StringBuffer();
		String retorno = "";
		...
		try{
			...
			BufferedReader bufferLeitura = new BufferedReader(clob.getCharacterStream());

			while((retorno = bufferLeitura.readLine()) != null){
				resultadoConversao.append(retorno);
			}
		}catch(IOException e){
			throw new RuntimeException(e);
		}catch(SQLException e){
		...
		return resultadoConversao.toString();
	 ...
	 */
	public static byte[] getImagem(String caminhoCompleto) throws FileNotFoundException, IOException{
	...
	/**
	 * Método para obtem o conteúdo de um campo blob de um arquivo do tipo txt.
	 * 
	 ...
	 * @return StringBuilder com todo o conteúdo do arquivo txt.
	 */
	public static String obterConteudoCampoBlobTipoTxtFormatando(Blob blob){
