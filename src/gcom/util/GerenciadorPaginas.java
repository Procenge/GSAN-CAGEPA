...
public class GerenciadorPaginas {
	 ...
	 */
	public Pagina adicionaPagina(String descricao, String uriInicial, String uriFinal, String imagemAtiva, String imagemInativa){

		paginas.add(new Pagina(descricao, uriInicial, uriFinal, imagemAtiva, imagemInativa));
	...
	public String generateHint(HttpServletRequest request, Pagina pagina){
		...
		while(iterator.hasNext()){
			...
			if(mapRequest.get(atributoPagina.getId()) != null){
				descricaoAtributoRequest = ((String[]) mapRequest.get(atributoPagina.getId()))[0];
				atributoPagina.setTamanho(new Integer(descricaoAtributoRequest.length()));
				hint += "<b>" + descricaoAtributoPagina + ":</b> " + descricaoAtributoRequest + "<br>";
			}
		}
