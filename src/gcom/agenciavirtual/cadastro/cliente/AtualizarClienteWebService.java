...
public class AtualizarClienteWebService
	...
	protected void preencherJSONBody(ActionForm form, HttpServletRequest request) throws Exception{
		...
		try{
			...
			Fachada.getInstancia().atualizarCliente(clienteJSONHelper);
			adicionarValorPrimitivoAoBody("id", clienteJSONHelper.getId());
	...
	private ClienteJSONHelper obterClienteJSONhelper(HttpServletRequest request) throws Exception{
		...
		helper.setId(this.recuperarParametroInteiro("id", "ID", Boolean.TRUE, Boolean.TRUE, request));
		...
		helper.setIdOrgaoExpdidorRg(this.recuperarParametroInteiro("idOrgaoExpdidorRg", "ORGÃO EXPEDIDOR", Boolean.TRUE, Boolean.TRUE, request));
		helper.setSiglaEstado(this.recuperarParametroString("siglaEstado", "SIGLA ESTADO", Boolean.TRUE, Boolean.TRUE, request));
		helper.setIdUnidadeFederacao(this.recuperarParametroInteiro("idUnidadeFederacao", "UNIDADE FEDERAÇÃO", Boolean.TRUE, Boolean.TRUE, request));
		...
		helper.setIdTipoClienteEspecial(this.recuperarParametroInteiro("idTipoClienteEspecial", "TIPO CLIENTE ESPECIAL", Boolean.FALSE,	Boolean.TRUE, request));
		...
		helper.setIdEnderecoReferencia(this.recuperarParametroInteiro("idEnderecoReferencia", "ENDEREÇO REFERENCIA", Boolean.FALSE,	Boolean.TRUE, request));
		helper.setComplementoEndereco(this.recuperarParametroString("complementoEndereco", "COMPLEMENTO ENDEREÇO", Boolean.FALSE, Boolean.TRUE, request));
		...
		helper.setIcCobrancaCorrecao(this.recuperarParametroShort("icCobrancaCorrecao", "INDICADOR COBRANÇA CORREÇÃO", Boolean.FALSE, Boolean.TRUE, request));
		helper.setIcCobrancaImpostoFederal(this.recuperarParametroShort("icCobrancaImpostoFederal", "INDICADOR COBRANÇA IMPOSTO FEDERAL",	Boolean.FALSE, Boolean.TRUE, request));
