 ...
 * @version 1.0
 */
public class RepositorioGerenciaRegionalHBM
				implements IRepositorioGerenciaRegional {
	 ...
	 *                Erro no hibernate
	 */
	public Collection<GerenciaRegional> pesquisarGerenciaRegional() throws ErroRepositorioException{
		...
		try{
			...
			String consulta = "select gerenciaRegional " + "from GerenciaRegional gerenciaRegional ";
	 ...
	 *                Erro no hibernate
	 */
	public Object[] pesquisarObjetoGerenciaRegionalRelatorio(Integer idGerenciaRegional) throws ErroRepositorioException{
		...
		try{
			// cria o HQL para consulta
			String consulta = "select gr.greg_id as id, " + "gr.greg_nmabreviado as nomeAbreviado " + "from gerencia_regional gr "
							+ "where gr.greg_id = " + idGerenciaRegional.toString();

			// pesquisa a coleção de acordo com o parâmetro informado
			Collection colecaoGerenciasRegionais = session.createSQLQuery(consulta).addScalar("id", Hibernate.INTEGER).addScalar(
							"nomeAbreviado", Hibernate.STRING).list();
