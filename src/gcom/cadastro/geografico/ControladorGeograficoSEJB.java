 ...
 */
public class ControladorGeograficoSEJB
	...
	public void atualizarBairro(Bairro bairro, Collection colecaoBairroArea, Collection colecaoBairroAreaRemover, Usuario usuarioLogado)
		...
		// ------------ REGISTRAR TRANSAÇÃO ----------------------------
		RegistradorOperacao registradorOperacao = new RegistradorOperacao(Operacao.OPERACAO_BAIRRO_ATUALIZAR, new UsuarioAcaoUsuarioHelper(
		...
		bairro.adicionarUsuario(usuarioLogado, UsuarioAcao.USUARIO_ACAO_EFETUOU_OPERACAO);
		...
		Collection bairroNaBaseColecao = (Collection) getControladorUtil().pesquisar(filtroBairro, Bairro.class.getName());
		...
		// durante
		// esta atualização
		if(bairroNaBase.getUltimaAlteracao().after(bairro.getUltimaAlteracao())){
			sessionContext.setRollbackOnly();
			throw new ControladorException("atencao.atualizacao.timestamp");
		}
		...
		filtroBairro.adicionarParametro(new ParametroSimples(FiltroBairro.CODIGO, "" + bairro.getCodigo()));

		filtroBairro.adicionarParametro(new ParametroSimples(FiltroBairro.MUNICIPIO_ID, bairro.getMunicipio().getId()));

		Collection bairrosPesquisados = getControladorUtil().pesquisar(filtroBairro, Bairro.class.getName());

		if(bairrosPesquisados != null && !bairrosPesquisados.isEmpty()){

			// Procura o bairro na base
			Bairro bairroPesquisado = (Bairro) ((List) (bairrosPesquisados)).get(0);

			if(!bairroPesquisado.getId().equals(bairro.getId())){
				sessionContext.setRollbackOnly();
				throw new ControladorException("atencao.pesquisa_bairro_ja_cadastrada");
			}
		}
		...
		if(colecaoBairroArea != null && !colecaoBairroArea.isEmpty()){
			...
			while(iterator.hasNext()){
				...
				bairroArea.adicionarUsuario(usuarioLogado, UsuarioAcao.USUARIO_ACAO_EFETUOU_OPERACAO);
		...
		if(colecaoBairroAreaRemover != null && !colecaoBairroAreaRemover.isEmpty()){
			Iterator iteratorRemover = colecaoBairroAreaRemover.iterator();

			while(iteratorRemover.hasNext()){
				BairroArea bairroAreaRemover = (BairroArea) iteratorRemover.next();
	 ...
	 * @throws ControladorException
	 */
	public Collection pesquisarMunicipoPeloSetorComercial(String codigoSetorComercial, String idMunicipio) throws ControladorException{

		try{
			return repositorioGeografico.pesquisarMunicipoPeloSetorComercial(codigoSetorComercial, idMunicipio);
	...
	public boolean verificarMunicipioComCepPorLogradouro(Municipio municipio) throws ControladorException{
		...
		if(municipio.getCepInicio() != null && municipio.getCepFim() != null && municipio.getCepInicio().equals(municipio.getCepFim())){
	...
	public int pesquisarMaximoCodigoBairro(Integer idMunicipio) throws ControladorException{

		try{
			return repositorioGeografico.pesquisarMaximoCodigoBairro(idMunicipio);
		}catch(ErroRepositorioException ex){
			sessionContext.setRollbackOnly();
			throw new ControladorException("erro.sistema", ex);
		}
	 ...
	 *                Erro no hibernate
	 */
	public Municipio pesquisarObjetoMunicipioRelatorio(Integer idMunicipio) throws ControladorException{

		try{
			// pesquisa as gerencias regionais existentes no sistema
			Object[] objetoMunicipio = repositorioGeografico.pesquisarObjetoMunicipioRelatorio(idMunicipio);
	 ...
	 *                Erro no hibernate
	 */
	public Bairro pesquisarObjetoBairroRelatorio(Integer codigoBairro, Integer idMunicipio) throws ControladorException{

		try{
			// pesquisa as gerencias regionais existentes no sisitema
			Object[] objetoBairro = repositorioGeografico.pesquisarObjetoBairroRelatorio(codigoBairro, idMunicipio);
	...
	public Integer inserirMunicipio(Municipio municipio, Usuario usuarioLogado) throws ControladorException{
		...
		// [FS0005] - Validar CEP - somente para ADA
		if(municipio.getCepInicio() != null && municipio.getCepFim() != null){
			...
			if(cepInicial.intValue() > cepFinal.intValue()){
				throw new ControladorException("atencao.cep_inicio.anterior.cep_fim");
			}
		}
		...
		// ------------ REGISTRAR TRANSAÇÃO----------------------------
		RegistradorOperacao registradorOperacao = new RegistradorOperacao(Operacao.OPERACAO_MUNICIPIO_INSERIR, municipio.getId(), municipio
	 ...
	 * @throws ControladorException
	 */
	public Integer inserirBairro(Bairro bairro, Collection colecaoBairroArea, Usuario usuarioLogado) throws ControladorException{

		// ------------ REGISTRAR TRANSAÇÃO ----------------------------
		RegistradorOperacao registradorOperacao = new RegistradorOperacao(Operacao.OPERACAO_BAIRRO_INSERIR, new UsuarioAcaoUsuarioHelper(
						usuarioLogado, UsuarioAcao.USUARIO_ACAO_EFETUOU_OPERACAO));
	 ...
	 *                Erro no hibernate
	 */
	public Collection pesquisarBairroArea(Integer idBairro) throws ControladorException{
	...
	public void removerBairro(String[] ids, Usuario usuarioLogado) throws ControladorException{
		...
		while(i < ids.length){
			...
			try{
				...
				repositorioGeografico.removerTodosBairroAreaPorBairro(new Integer(idBairro));
				...
				sessionContext.setRollbackOnly();
				if(ex.getCause().initCause(ex) instanceof ConstraintViolationException){
					throw new ControladorException("atencao.dependencias.existentes", ex);
				}
				throw new ControladorException("erro.sistema", ex);
			}
		...
		// remover bairro(s)
		this.getControladorUtil().remover(ids, Bairro.class.getName(), operacaoEfetuada, colecaoUsuarios);
	...
	public void removerMunicipio(String[] ids, Usuario usuarioLogado) throws ControladorException{
		...
		for(int i = 0; i < ids.length; i++){
			...
			filtro.adicionarParametro(new ParametroSimples(FiltroMunicipio.ID, new Integer(ids[i])));

			Collection colecaoRetorno = getControladorUtil().pesquisar(filtro, Municipio.class.getName());

			Municipio municipio = (Municipio) Util.retonarObjetoDeColecao(colecaoRetorno);

			RegistradorOperacao registradorMunicipio = new RegistradorOperacao(Operacao.OPERACAO_MUNICIPIO_REMOVER, municipio.getId(),
							municipio.getId(), new UsuarioAcaoUsuarioHelper(usuarioLogado, UsuarioAcao.USUARIO_ACAO_EFETUOU_OPERACAO));
			...
			registradorMunicipio.registrarOperacao(municipio);
			// ------------ REGISTRAR TRANSAÇÃO ----------------

			this.getControladorUtil().remover(municipio);
	...
	public void atualizarMunicipio(Municipio municipio, Usuario usuarioLogado) throws ControladorException{
		...
		// ------------ REGISTRAR TRANSAÇÃO----------------------------
		RegistradorOperacao registradorOperacao = new RegistradorOperacao(Operacao.OPERACAO_MUNICIPIO_ATUALIZAR, municipio.getId(),
						municipio.getId(), new UsuarioAcaoUsuarioHelper(usuarioLogado, UsuarioAcao.USUARIO_ACAO_EFETUOU_OPERACAO));
	 ...
	 * @throws ControladorException
	 */
	public String pesquisarNomeMunicipioPorLocalidade(Integer idLocalidade) throws ControladorException{

		try{
			return repositorioGeografico.pesquisarNomeMunicipioPorLocalidade(idLocalidade);
		}catch(ErroRepositorioException ex){
			sessionContext.setRollbackOnly();
			throw new ControladorException("erro.sistema", ex);
		}
	 ...
	 * @throws ControladorException
	 */
	public Collection pesquisarMunicipioPeloNome(String nomeMunicipio) throws ControladorException{

		try{
			return repositorioGeografico.pesquisarMunicipioPeloNome(nomeMunicipio);
		}catch(ErroRepositorioException ex){
			sessionContext.setRollbackOnly();
			throw new ControladorException("erro.sistema", ex);
		}
	...
	public Collection<Integer> pesquisarTodosIdsMunicipios() throws ControladorException{

		try{
			return repositorioGeografico.pesquisarIdsMunicipio();
		}catch(ErroRepositorioException ex){
			sessionContext.setRollbackOnly();
			throw new ControladorException("erro.sistema", ex);
		}
