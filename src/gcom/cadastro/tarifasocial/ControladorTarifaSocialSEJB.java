...
public class ControladorTarifaSocialSEJB
	...
	public String[] verificarPreRequisitosCadastramentoTarifaSocial(Integer idImovel, Integer qtdEconomiasImovel)
		...
		// LIGAÇÃO DIFERENTE DE LIGADO, SUPRIMIDO OU CORTADO - T

		// Caso a situação da ligação de água do imóvel não seja ligada ou cortada
		// (LAST_ID com o valor diferente de ligado, ligado em revisão, suprimido ou cortado)
		if(imovel.getLigacaoAguaSituacao().getId().intValue() != LigacaoAguaSituacao.LIGADO.intValue()
						...
						// Alteração referente a OC0857313
						// .......................................................
		...
		// Caso o perfil da ligação de água do imóvel seja clandestina
		...
		// EXISTÊNCIA DE DÉBITOS DO CLIENTE - T

		// O sistema verifica se existem débitos para o Cliente do imóvel ou para cada um dos
		// clientes usuários das economias, caso o imóvel possua mais de uma economia.
		...
		// Realiza uma pesquisa pelos parametros fornecidos
		// caso o imóvel possua uma economia
		...
		// caso o imóvel possua mais de uma economia
		...
		/*
		 * Caso exista algum débito (retorno não nulo do UC0067), exibir a mensagem:
		 * “Tarifa Social só é permitida para Clientes adimplentes”
		 * Esta situação exige a tramitação do Registro de Atendimento. Confirma Tramitação?
		 */
		if(existeDebito){
	...
	public String[] verificarPreenchimentoInserirDadosTarifaSocial(Long numeroCelpe, BigDecimal areaConstruida, BigDecimal numeroIPTU,
		...
		if((numeroCartaoSocial != null && !numeroCartaoSocial.trim().equals(""))
			...
			if(numeroCartaoSocial == null || numeroCartaoSocial.trim().equals("")){
				...
				// [FS0018 – Validar número do Cartão do Programa Social]
				// Válida o dígito verificador do cartão social
		...
		// salário mínimo
		// Caso o valor da renda tenha sido informado e seja maior que um o salário mínimo
	 ...
	 * [FS0018] – Validar Número Cartão do Programa Social
	 * 
	 * @param numeroCartao
	 * @throws ControladorException
	 */
	...
	public void atualizarImovelPerfilTarifaSocial(Imovel imovel, boolean recadastramento) throws ControladorException{
		...
		// Inclui consumo tarifa para o imóvel com o valor "CONSUMO SOCIAL"
		...
		// Recupera o parâmetro Tarifa Consumo
		String parametroTarifaTemp = ParametroCadastro.P_TARIFA_CONSUMO_TARIFA_SOCIAL.executar();

		// Inclui consumo tarifa temporária para o imóvel com o valor do parâmetro
		...
		// Recupera o parâmetro Tarifa Percentual de Esgoto
		String parametroPercentualEsgoto = ParametroCadastro.P_PERCENTUAL_ESGOTO_TARIFA_SOCIAL.executar();

		if(recadastramento){
			if(imovel.getLigacaoEsgotoSituacao() != null){
				// Inclui faturamento situação tipo para o imóvel com o valor "PERCENTUAL ESGOTO"
			...
			// Caso o imóvel esteja ligado de esgoto e exista percentual diferenciado para o
			// benefício
			// da tarifa social
	...
	public void incluirFaturamentoSituacaoHistorico(Integer idImovel, Usuario usuarioLogado) throws ControladorException{
		...
		if(!Util.isVazioOrNulo(colecaoImovel)){
			...
			// 10.4.Caso o imóvel esteja ligado de esgoto (LEST_ID = “ligado” da tabela IMOVEL)
			// e exista percentual diferenciado para o benefício da tarifa social
			...
			if(imovel.getLigacaoEsgotoSituacao() != null && imovel.getLigacaoEsgotoSituacao().getId().equals(LigacaoEsgotoSituacao.LIGADO)
				...
				// esgoto”
				// Inclui faturamento situação tipo com o valor "PERCENTUAL ESGOTO"
				FaturamentoSituacaoTipo faturamentoSituacaoTipo = new FaturamentoSituacaoTipo();
				faturamentoSituacaoTipo.setId(FaturamentoSituacaoTipo.PERCENTUAL_DE_ESGOTO);

				faturamentoSituacaoHistorico.setFaturamentoSituacaoTipo(faturamentoSituacaoTipo);

				// FTSM_ID da tabela FATURAMENTO_SITUACAO_MOTIVO com valor igual a “benefício tarifa
				// social”
				...
				// USUR_ID Usuário logado no sistema
	...
	public void verificarPreenchimentoManterDadosTarifaSocial(Long numeroCelpe, BigDecimal areaConstruida, BigDecimal numeroIPTU,
		...
		if((numeroCartaoSocial != null && !numeroCartaoSocial.trim().equals(""))
			...
			if(numeroCartaoSocial == null || numeroCartaoSocial.trim().equals("")){
				...
				// [FS0027 – Validar número do Cartão do Programa Social]
				// Válida o dígito verificador do cartão social
				if(!Util.isVazioOuBranco(numeroCartaoSocial)){
					this.validarNumeroCartaoProgramaSocial(numeroCartaoSocial);
				}
	...
	public void manterTarifaSocial(Imovel imovelSessao, Collection colecaoTarifaSocialHelperAtualizar,
		...
		if(colecaoTarifasSociaisRecadastradas != null && !colecaoTarifasSociaisRecadastradas.isEmpty()){
			...
			while(colecaoTarifasSociaisRecadastradasIterator.hasNext()){
				...
				// Verifica se o usuário não excluiu o imóvel anterior da tarifa
				// social
				...
				if(colecaoTarifaSocialDadoEconomiaOutroImovel != null && !colecaoTarifaSocialDadoEconomiaOutroImovel.isEmpty()){
					...
					while(colecaoImoveisExcluidosTarifaSocialIterator.hasNext()){
						...
						if(tarifaSocialHelperImovelAnterior.getClienteImovel().getCliente().getId().equals(idClienteUsuario)){
							...
							// Inclusão Tarifa Social Dado Economia Historico
							...
							if(colecaoTarifaSocialNaoExcluidasImovel == null || colecaoTarifaSocialNaoExcluidasImovel.isEmpty()){
								// 4.1.2.1.1.2. Caso após as exclusões, o imóvel não apresente mais
								// nenhuma economia
								// na tarifa social:
								...
								// 4.1.2.1.1.2.2. Encerra a situação especial de faturamento para o
								// imóvel
		...
		if(colecaoTarifaSocialExcluida != null && !colecaoTarifaSocialExcluida.isEmpty()){
			...
			if(colecaoTarifaSocialNaoExcluidasImovel == null || colecaoTarifaSocialNaoExcluidasImovel.isEmpty()){
				// 4.1.2.1.1.2. Caso após as exclusões, o imóvel não apresente mais nenhuma economia
				// na tarifa social:
				// 4.1.2.1.1.2.1. Atualizar a tabela IMOVEL:
				getControladorImovel().atualizarImovelPerfilNormal(imovelSessao.getId(), true);

				// 4.1.2.1.1.2.2. Encerra a situação especial de faturamento para o imóvel
		...
		if(colecaoTarifaSocialDadoEconomia != null && !colecaoTarifaSocialDadoEconomia.isEmpty()){
			...
			if(colecaoTarifaSocialRecadastrar != null){
				...
				if(tarifaSocialDadoEconomiaRecadastrar.getDataExclusao() != null){
					...
					// 10.1. Transferir a tarifa social atual para histórico
					...
					// 10.2. Incluir os novos dados da tarifa social do imóvel, por economia, na
	 ...
	 * Verificar Permanência Tarifa Social
	...
	public void verificarPermanenciaTarifaSocial(Integer idImovel, Usuario usuarioLogado) throws ControladorException{
		...
		Collection colecaoImovel = this.getControladorUtil().pesquisar(filtroImovel, Imovel.class.getName());
		// 1. O sistema verifica se o imóvel está cadastrado na tarifa social.
		Imovel imovel = null;
		if(!Util.isVazioOrNulo(colecaoImovel)){
			...
			if(!Util.isVazioOuBranco(imovel)){
				...
				if(!Util.isVazioOrNulo(colecaoTarifaSocial)){
					...
					// 1.1. Caso o Imóvel esteja na tarifa social
					// ou com tarifa social suspensa, verifica permanência ou suspensão.
					if((!Util.isVazioOuBranco(imovel.getImovelPerfil()) && imovel.getImovelPerfil().getId().equals(
									ImovelPerfil.TARIFA_SOCIAL))
									|| tarifaVerificada){

						// 2. O sistema verifica se o imóvel recebido atende aos critérios de perda
						// da
						// tarifa social:
						FiltroImovelSubCategoria filtroImovelSubcategoria = new FiltroImovelSubCategoria();
						...
						// 2.1. Categoria: existe na tabela IMOVEL_SUBCATEGORIA IMOV_ID = matrícula
						// do
						// imóvel
						// e SCAT_ID com o valor correspondente a categoria não residencial.
						...
						// Inicializando os parâmetros que serão passados para a consulta do
						// [UC0067]
						...
						try{
							...
							throw new ActionServletException("erro.parametro.sistema.inexistente" + " - P_QUANTIDADE_MÍNIMA_DE_DEBITOS.");
						}
						...
						// 3. Caso o imóvel seja do perfil tarifa social (IPER_ID com o valor
						// correspondente
						// a “tarifa social” na tabela IMOVEL) e atenda a um dos critérios do item
						// 2,
						// então o sistema suspende o uso da tarifa social.
						if(imovel.getImovelPerfil() != null && imovel.getImovelPerfil().getId().equals(ImovelPerfil.TARIFA_SOCIAL)
							...
							// Inclui consumo tarifa temporária para o imóvel com valor "NULO"
							...
							// Caso o imóvel esteja ligado de esgoto e exista percentual
							// diferenciado
							// para o benefício
							// da tarifa social
							if(imovel.getLigacaoEsgotoSituacao() != null
											...
											&& !Util.isVazioOuBrancoOuZero(parametroPercentualEsgoto)){
								// Inclui faturamento situação tipo para o imóvel com o valor
								// "NULO"
								imovel.setFaturamentoSituacaoTipo(null);

								// Encerra Situação Especial de Faturamento.
							...
							if(Util.isNaoNuloBrancoZero(reativacaoAutomaticaTarifaSocial)
								...
								if(imovel.getLigacaoEsgotoSituacao() != null
									...
									if(sistemaParametros == null || sistemaParametros.getAnoMesFaturamento() == null){
										throw new ControladorException("atencao.naocadastrado", null,
														"Ano/Mês Faturamento Parametros Sistema");
									}
									...
									// de
									// esgoto”
									// Inclui faturamento situação tipo com o valor
									// "PERCENTUAL ESGOTO"
									FaturamentoSituacaoTipo faturamentoSituacaoTipo = new FaturamentoSituacaoTipo();
									faturamentoSituacaoTipo.setId(FaturamentoSituacaoTipo.PERCENTUAL_DE_ESGOTO);
								...
								// Inclui consumo tarifa temporária para o imóvel com o valor do
								// parâmetro
								ConsumoTarifa consumoTarifaTemporaria = new ConsumoTarifa();
