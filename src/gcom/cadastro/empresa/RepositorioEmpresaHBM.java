 ...
 * @author Sávio Luiz
 */
public class RepositorioEmpresaHBM
				implements IRepositorioEmpresa {
	 ...
	 * @date 09/01/2007
	 */
	public Collection pesquisarIdsEmpresa() throws ErroRepositorioException{
	...
	public int pesquisarProximoIdEmpresa() throws ErroRepositorioException{
		...
		try{
			consulta = "SELECT max(e.id) " + "FROM Empresa e ";

			maxIdEmpresa = session.createQuery(consulta).setMaxResults(1).uniqueResult();

			if(maxIdEmpresa != null){
				retorno = ((Number) maxIdEmpresa).intValue() + 1;
			}
