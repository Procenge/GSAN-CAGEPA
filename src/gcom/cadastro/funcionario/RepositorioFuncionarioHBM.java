...
public class RepositorioFuncionarioHBM
	...
	public Integer verificarExistenciaFuncionario(Integer idFuncionario) throws ErroRepositorioException{
		...
		try{
			consulta = "select count(f.id) " + "from Funcionario f " + "where f.id = :idFuncionario";

			retorno = ((Number) session.createQuery(consulta).setInteger("idFuncionario", idFuncionario.intValue()).setMaxResults(1)
