 ...
 * @date 25/06/2007
 */
public class InformarDadosGeracaoResumoAcaoConsultaEventualAction
				extends GcomAction {

	public ActionForward execute(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest httpServletRequest,
					HttpServletResponse httpServletResponse){

		ActionForward retorno = actionMapping.findForward("consultarResumoAcaoCobrancaEventualParametros");
		...
		StatusWizard statusWizard = new StatusWizard("consultarResumoAcaoCobrancaEventualWizardAction",
						"exibirInformarDadosGeracaoResumoAcaoConsultaEventualAction", "cancelarConsultarResumoAcaoCobrancaAction",
						"exibirInformarDadosGeracaoResumoAcaoConsultaEventualAction",
						"informarDadosGeracaoResumoAcaoConsultaEventualAction.do");

		statusWizard.inserirNumeroPaginaCaminho(statusWizard.new StatusWizardItem(1, "ParametrosPrimeiraAbaA.gif",
						"ParametrosPrimeiraAbaD.gif", "exibirDadosGeracaoConsultaEventualAction", ""));
		statusWizard.inserirNumeroPaginaCaminho(statusWizard.new StatusWizardItem(2, "ResultadoA.gif", "ResultadoD.gif",
		...
		String dataInicialEmissao = informarDadosGeracaoResumoAcaoConsultaEventualActionForm.getDataInicialEmissao();
		String dataFinalEmissao = informarDadosGeracaoResumoAcaoConsultaEventualActionForm.getDataFinalEmissao();
		...
		if(informarDadosGeracaoResumoAcaoConsultaEventualActionForm.getEloPolo() != null
						&& !informarDadosGeracaoResumoAcaoConsultaEventualActionForm.getEloPolo().equals("")){

			idEloPolo = new Integer(informarDadosGeracaoResumoAcaoConsultaEventualActionForm.getEloPolo());
		}

		Integer idUnidadeNegocio = null;
		if(informarDadosGeracaoResumoAcaoConsultaEventualActionForm.getUnidadeNegocio() != null
						&& !informarDadosGeracaoResumoAcaoConsultaEventualActionForm.getUnidadeNegocio().equals("")){

			idUnidadeNegocio = new Integer(informarDadosGeracaoResumoAcaoConsultaEventualActionForm.getUnidadeNegocio());
		}

		Integer idLocalidade = null;
		if(informarDadosGeracaoResumoAcaoConsultaEventualActionForm.getLocalidade() != null
						&& !informarDadosGeracaoResumoAcaoConsultaEventualActionForm.getLocalidade().equals("")){

			idLocalidade = new Integer(informarDadosGeracaoResumoAcaoConsultaEventualActionForm.getLocalidade());
		}

		Integer idSetorComercial = null;
		if(informarDadosGeracaoResumoAcaoConsultaEventualActionForm.getSetorComercial() != null
						&& !informarDadosGeracaoResumoAcaoConsultaEventualActionForm.getSetorComercial().equals("")){

			if(informarDadosGeracaoResumoAcaoConsultaEventualActionForm.getIdSetorComercial() != null
							&& !informarDadosGeracaoResumoAcaoConsultaEventualActionForm.getIdSetorComercial().equals("")){

				idSetorComercial = new Integer(informarDadosGeracaoResumoAcaoConsultaEventualActionForm.getIdSetorComercial());
			}else{
		...
		if(informarDadosGeracaoResumoAcaoConsultaEventualActionForm.getQuadra() != null
			...
			nmQuadra = new Integer(informarDadosGeracaoResumoAcaoConsultaEventualActionForm.getQuadra());
		}

		String[] cobrancaGrupo = informarDadosGeracaoResumoAcaoConsultaEventualActionForm.getGrupoCobranca();
		String[] cobrancaAcao = informarDadosGeracaoResumoAcaoConsultaEventualActionForm.getCobrancaAcao();
		String[] gerenciaRegional = informarDadosGeracaoResumoAcaoConsultaEventualActionForm.getGerencialRegional();

		String[] imovelPerfil = informarDadosGeracaoResumoAcaoConsultaEventualActionForm.getPerfilImovel();
		String[] ligacaoAguaSituacao = informarDadosGeracaoResumoAcaoConsultaEventualActionForm.getSituacaoLigacaoAgua();
		String[] ligacaoEsgotoSituacao = informarDadosGeracaoResumoAcaoConsultaEventualActionForm.getSituacaoLigacaoEsgoto();
		String[] categoria = informarDadosGeracaoResumoAcaoConsultaEventualActionForm.getCategoria();
		String[] esferaPoder = informarDadosGeracaoResumoAcaoConsultaEventualActionForm.getEsferaPoder();
		String[] empresa = informarDadosGeracaoResumoAcaoConsultaEventualActionForm.getEmpresa();
		...
		InformarDadosGeracaoResumoAcaoConsultaEventualHelper informarDadosGeracaoResumoAcaoConsultaEventualHelper = fachada
						.informarDadosGeracaoResumoAcaoConsultaEventual(dataInicialEmissao, dataFinalEmissao,
										idCobrancaAcaoAtividadeComando, tituloCobrancaAcaoAtividadeComando, cobrancaGrupo,
										gerenciaRegional, idEloPolo, idLocalidade, idSetorComercial, nmQuadra, imovelPerfil,
										ligacaoAguaSituacao, ligacaoEsgotoSituacao, categoria, esferaPoder, empresa, idUnidadeNegocio,
										cobrancaAcao);

		sessao.setAttribute("informarDadosGeracaoResumoAcaoConsultaEventualHelper", informarDadosGeracaoResumoAcaoConsultaEventualHelper);
		...
		if(gerenciaRegional != null){
			...
			Boolean primeiroParametroNulo = false;
			Integer numeroParametrosIsoladosConector = gerenciaRegional.length;
			...
			for(i = 0; i < gerenciaRegional.length; i++){
				...
				if(!gerenciaRegional[i].equals("") && !gerenciaRegional[i].equals("" + ConstantesSistema.NUMERO_NAO_INFORMADO)){
					...
					if(i + 1 < gerenciaRegional.length){
						if(primeiroParametroNulo){
							numeroParametrosIsoladosConector--;
							primeiroParametroNulo = false;
						}
						filtroGerenciaRegional.adicionarParametro(new ParametroSimples(FiltroGerenciaRegional.ID, gerenciaRegional[i],
										ConectorOr.CONECTOR_OR, numeroParametrosIsoladosConector));

					}else{
						filtroGerenciaRegional.adicionarParametro(new ParametroSimples(FiltroGerenciaRegional.ID, gerenciaRegional[i]));
					}
				}else{
					primeiroParametroNulo = true;
				}
			}
		...
		if(cobrancaGrupo != null){
			...
			Boolean primeiroParametroNulo = false;
			Integer numeroParametrosIsoladosConector = cobrancaGrupo.length;

			for(i = 0; i < cobrancaGrupo.length; i++){

				if(!cobrancaGrupo[i].equals("") && !cobrancaGrupo[i].equals("" + ConstantesSistema.NUMERO_NAO_INFORMADO)){
					...
					if(i + 1 < cobrancaGrupo.length){
						...
						filtroCobrancaGrupo.adicionarParametro(new ParametroSimples(FiltroCobrancaGrupo.ID, cobrancaGrupo[i],
										ConectorOr.CONECTOR_OR, numeroParametrosIsoladosConector));
		...
		sessao.setAttribute("colecaoCobrancaGrupoResultado", colecaoCobrancaGrupo);
		// ----
		CobrancaAcao cobrancaAcaoColecao = new CobrancaAcao();
		cobrancaAcaoColecao.setId(-1);
		...
		if(cobrancaAcao != null){
			cobrancaAcaoColecao.setDescricaoCobrancaAcao("OPÇÕES SELECIONADAS");
			colecaoCobrancaAcao.add(cobrancaAcaoColecao);
			FiltroCobrancaAcao filtroCobrancaAcao = new FiltroCobrancaAcao();

			// variáveis para verificar se algum parametro do filtro está vindo nulo. Caso
			// esteja, subtrai 1 ao numeroParametrosIsoladosConector para que o
			// geradorHQLCondicional
			// escreva a consulta corretamente
			Boolean primeiroParametroNulo = false;
			Integer numeroParametrosIsoladosConector = cobrancaAcao.length;

			for(i = 0; i < cobrancaAcao.length; i++){

				if(!cobrancaAcao[i].equals("") && !cobrancaAcao[i].equals("" + ConstantesSistema.NUMERO_NAO_INFORMADO)){

					if(i + 1 < cobrancaAcao.length){
						if(primeiroParametroNulo){
							numeroParametrosIsoladosConector--;
							primeiroParametroNulo = false;
						}
						filtroCobrancaAcao.adicionarParametro(new ParametroSimples(FiltroCobrancaAcao.ID, cobrancaAcao[i],
										ConectorOr.CONECTOR_OR, numeroParametrosIsoladosConector));
			...
			Collection colecaoCobrancaGrupoPesquisa = fachada.pesquisar(filtroCobrancaAcao, CobrancaAcao.class.getName());

			if(colecaoCobrancaGrupoPesquisa != null && !colecaoCobrancaGrupoPesquisa.isEmpty()){
				colecaoCobrancaAcao.addAll(colecaoCobrancaGrupoPesquisa);
			}
		}else{
			cobrancaAcaoColecao.setDescricaoCobrancaAcao("TODOS");
			colecaoCobrancaAcao.add(cobrancaAcaoColecao);
		}

		sessao.setAttribute("colecaoCobrancaAcaoResultado", colecaoCobrancaAcao);
		// ----
		...
		if(imovelPerfil != null){
			...
			Boolean primeiroParametroNulo = false;
			Integer numeroParametrosIsoladosConector = imovelPerfil.length;
		...
		if(ligacaoAguaSituacao != null){
			...
			Boolean primeiroParametroNulo = false;
			Integer numeroParametrosIsoladosConector = ligacaoAguaSituacao.length;
				...
				if(!ligacaoAguaSituacao[i].equals("") && !ligacaoAguaSituacao[i].equals("" + ConstantesSistema.NUMERO_NAO_INFORMADO)){
					...
					if(i + 1 < ligacaoAguaSituacao.length){
						...
						filtroLigacaoAguaSituacao.adicionarParametro(new ParametroSimples(FiltroLigacaoAguaSituacao.ID,
										ligacaoAguaSituacao[i], ConectorOr.CONECTOR_OR, numeroParametrosIsoladosConector));
		...
		if(ligacaoEsgotoSituacao != null){
			...
			Boolean primeiroParametroNulo = false;
			Integer numeroParametrosIsoladosConector = ligacaoEsgotoSituacao.length;
				...
				if(!ligacaoEsgotoSituacao[i].equals("") && !ligacaoEsgotoSituacao[i].equals("" + ConstantesSistema.NUMERO_NAO_INFORMADO)){
					...
					if(i + 1 < ligacaoEsgotoSituacao.length){
						...
						filtroLigacaoEsgotoSituacao.adicionarParametro(new ParametroSimples(FiltroLigacaoEsgotoSituacao.ID,
										ligacaoEsgotoSituacao[i], ConectorOr.CONECTOR_OR, numeroParametrosIsoladosConector));
		...
		if(categoria != null){
			...
			Boolean primeiroParametroNulo = false;
			Integer numeroParametrosIsoladosConector = categoria.length;
		...
		if(empresa != null){
			...
			for(i = 0; i < empresa.length; i++){
				if(!empresa[i].equals("") && !empresa[i].equals("" + ConstantesSistema.NUMERO_NAO_INFORMADO)){
					...
					if(i + 1 < empresa.length){
						filtroEmpresa.adicionarParametro(new ParametroSimples(FiltroEmpresa.ID, empresa[i], ConectorOr.CONECTOR_OR,
										empresa.length));
			...
			Collection colecaoEmpresaPesquisa = fachada.pesquisar(filtroEmpresa, Empresa.class.getName());

			if(colecaoEmpresaPesquisa != null && !colecaoEmpresaPesquisa.isEmpty()){
