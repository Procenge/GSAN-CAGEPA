...
import gcom.cobranca.bean.ResumoCobrancaAcaoRemuneracaoHelper;
 ...
 * @date 26/06/2007
 */
public class ExibirConsultarResumoAcaoCobrancaEventualPopupAction
				extends GcomAction {

	public final static Boolean EXIBE_PERCENTUAL_REMUNERACAO = true;
	...
	public ActionForward execute(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest httpServletRequest,
		...
		ActionForward retorno = actionMapping.findForward("consultarResumoAcaoCobrancaEventualPopup");
		...
		boolean exibePercentualRemuneracao = false;
		...
		if(colecaoCobrancaAcao != null && !colecaoCobrancaAcao.isEmpty()){
			...
			if(cobrancaAcao.getCobrancaCriterio() != null && !cobrancaAcao.getCobrancaCriterio().equals("")){
				if(cobrancaAcao.getCobrancaCriterio().getValorLimitePrioridade() != null){
					httpServletRequest.setAttribute("valorLimitePrioridade", Util.formatarMoedaReal(cobrancaAcao.getCobrancaCriterio()
		...
		if(!Util.isVazioOuBranco(httpServletRequest.getParameter("exibePercentualRemuneracao"))){
			exibePercentualRemuneracao = new Boolean(httpServletRequest.getParameter("exibePercentualRemuneracao").trim());
		}

		// 4.2.1.5.6. Para cada percentual de remuneração (o percentual de remuneração
		sessao.removeAttribute("colecaoResumoCobrancaAcaoRemuneracao");

		if(EXIBE_PERCENTUAL_REMUNERACAO.equals(exibePercentualRemuneracao)){

			List<ResumoCobrancaAcaoRemuneracaoHelper> colecaoResumoCobrancaAcaoRemuneracao = fachada.consultarCobrancaAcaoRemuneracao(
							idCobrancaAcao, idCobrancaAcaoSituacao, informarDadosGeracaoResumoAcaoConsultaEventualHelper);

			Map<String, BigDecimal> mapValorTotalRemuneracao = new HashMap<String, BigDecimal>();
			String descricaoDocumentoTipoCorrente = null;

			if(!Util.isVazioOrNulo(colecaoResumoCobrancaAcaoRemuneracao)){
				for(Object object : colecaoResumoCobrancaAcaoRemuneracao){
					ResumoCobrancaAcaoRemuneracaoHelper resumoCobrancaAcaoRemuneracaoHelper = (ResumoCobrancaAcaoRemuneracaoHelper) object;
					descricaoDocumentoTipoCorrente = resumoCobrancaAcaoRemuneracaoHelper.getDescricaoDocumentoTipo();
					if(!mapValorTotalRemuneracao.containsKey(descricaoDocumentoTipoCorrente)){
						mapValorTotalRemuneracao.put(descricaoDocumentoTipoCorrente, BigDecimal.ZERO);
					}
					BigDecimal valorCorrent = mapValorTotalRemuneracao.get(descricaoDocumentoTipoCorrente);
					mapValorTotalRemuneracao.put(descricaoDocumentoTipoCorrente,
									valorCorrent.add(resumoCobrancaAcaoRemuneracaoHelper.getValorRemuneracao()));
				}

				sessao.setAttribute("colecaoResumoCobrancaAcaoRemuneracao", colecaoResumoCobrancaAcaoRemuneracao);

				httpServletRequest.setAttribute("mapValorTotalRemuneracao", mapValorTotalRemuneracao);
