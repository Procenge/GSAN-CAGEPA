 ...
 * @since 31/03/2006
 */
public class ExibirManterResolucaoDiretoriaAction
				extends GcomAction {
	...
	public ActionForward execute(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest httpServletRequest,
		...
		ActionForward retorno = actionMapping.findForward("exibirManterResolucaoDiretoria");
		...
		FiltroResolucaoDiretoria filtroResolucaoDiretoria = (FiltroResolucaoDiretoria) sessao.getAttribute("filtroResolucaoDiretoria");
		ResolucaoDiretoriaGrupoHelper resolucaoDiretoriaGrupoHelper = (ResolucaoDiretoriaGrupoHelper) sessao
						.getAttribute("resolucaoDiretoriaGrupoHelper");

		// Aciona o controle de paginação para que sejam pesquisados apenas
		// os registros que aparecem na página
		Collection<ResolucaoDiretoria> colecaoResolucaoDiretoria = new ArrayList<ResolucaoDiretoria>();

		if(resolucaoDiretoriaGrupoHelper != null && !resolucaoDiretoriaGrupoHelper.equals("")){

			// Total de registros
			Integer totalRegistros = fachada.consultarResolucaoDiretoriaGrupo(resolucaoDiretoriaGrupoHelper, -1).size();

			if(totalRegistros == 0){
				// Nenhuma registro encontrado
				throw new ActionServletException("atencao.pesquisa.nenhumresultado", null, "Resolucao Diretoria");
			}

			// Paginação
			retorno = this.controlarPaginacao(httpServletRequest, retorno, totalRegistros);

			colecaoResolucaoDiretoria = fachada.consultarResolucaoDiretoriaGrupo(resolucaoDiretoriaGrupoHelper,
		...
		if(colecaoResolucaoDiretoria != null && !colecaoResolucaoDiretoria.isEmpty()){
			...
			if(colecaoResolucaoDiretoria.size() == 1
				...
				if(httpServletRequest.getParameter("atualizar") != null){
					...
					retorno = actionMapping.findForward("exibirAtualizarResolucaoDiretoria");
					...
					filtroResolucaoDiretoria.adicionarParametro(new ParametroSimples(FiltroResolucaoDiretoria.CODIGO, resolucaoDiretoria
									.getId()));
					...
					sessao.setAttribute("resolucaoDiretoria", colecaoResolucaoDiretoria.iterator().next());
				}else{
