 ...
 * @date 13/09/2007
 */
public class RemoverAtividadeCobrancaAction
				extends GcomAction {
	...
	public ActionForward execute(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest httpServletRequest,
		...
		ManterAtividadeCobrancaActionForm manterAtividadeCobrancaActionForm = (ManterAtividadeCobrancaActionForm) actionForm;
		HttpSession sessao = httpServletRequest.getSession(false);
		Collection colecaoCobrancaAtividade = (Collection) sessao.getAttribute("colecaoCobrancaAtividade");
		String[] selecaoCobrancaAtividade = manterAtividadeCobrancaActionForm.getCobrancaAtividadeSelectID();

		if(selecaoCobrancaAtividade == null || selecaoCobrancaAtividade.length < 1){
			throw new ActionServletException("atencao.registros.nao_selecionados");
		}

		Collection<CobrancaAtividade> colecaoCobrancaAtividadeRemocao = this.carregarColecaoCobrancaAtividade(colecaoCobrancaAtividade,
						selecaoCobrancaAtividade);
		...
		while(colecaoIter.hasNext()){
			...
			filtroCobrancaAtividadeNoBanco.adicionarParametro(new ParametroSimples(FiltroCobrancaAtividade.ID, CobAt.getId()));
			filtroCobrancaAtividadeNoBanco.adicionarCaminhoParaCarregamentoEntidade("processo");
			CobrancaAtividade cobrancaAtividadeNaBase = (CobrancaAtividade) Fachada.getInstancia().pesquisar(
							filtroCobrancaAtividadeNoBanco, CobrancaAtividade.class.getName()).iterator().next();
			if(cobrancaAtividadeNaBase == null || cobrancaAtividadeNaBase.getUltimaAlteracao().after(CobAt.getUltimaAlteracao())){
		...
		montarPaginaSucesso(httpServletRequest, selecaoCobrancaAtividade.length + " Atividade(s) de Cobrança removida(s) com sucesso.",
						"Realizar outra manutenção de Atividade de Cobrança", "exibirManterAtividadeCobrancaAction.do?menu=sim");
		return retorno;
	}
	...
	private Collection<CobrancaAtividade> carregarColecaoCobrancaAtividade(Collection colecaoCobrancaAtividade,
		...
		Collection<CobrancaAtividade> colecaoRetorno = new ArrayList<CobrancaAtividade>();
