...
/**
 * Relatorio Relaçao de Imoveis por Açao de Cobranca
 * 
 * @author Andre Nishimura
 * @date 22/07/2010
 */
public class ExibirFiltrarRelacaoImovelPorAcaoCobrancaAction
				extends GcomAction {

	public ActionForward execute(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest httpServletRequest,
					HttpServletResponse httpServletResponse){

		ActionForward retorno = actionMapping.findForward("exibirFiltrarRelacaoImovelPorAcaoCobrancaAction");
		...
		if(form.getLocalidade() != null && !form.getLocalidade().equals("") && !form.getLocalidade().equals("-1")){
		...
		if(form.getSetorComercial() == null || form.getSetorComercial()[0].equals("") || form.getSetorComercial()[0].equals("-1")){
			...
			colecaoBairro = new ArrayList();
			for(String setor : form.getSetorComercial()){
				Collection temp = fachada.pesquisarBairroPorSetorComercial(Integer.valueOf(setor));
				if(temp != null){
					for(Bairro bairro : (Collection<Bairro>) temp){
						if(!colecaoBairro.contains(bairro)){
