 ...
 */
public class InserirComandoAcaoCobrancaCronogramaAction
				extends GcomAction {
	...
	public ActionForward execute(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest httpServletRequest,
		...
		// InserirComandoAcaoCobrancaCronogramaActionForm form =
		// (InserirComandoAcaoCobrancaCronogramaActionForm) actionForm;

		// -- Validar se todas as rotas do grupo informado possui um criterio definido para a açao
		// de cobrança
		// Collection<Integer> idsAcoesCobranca = new ArrayList<Integer>();
		// idsAcoesCobranca.add(Integer.valueOf(form.getCobrancaAcao()));
		//	    
		// PesquisarQtdeRotasSemCriteriosParaAcoesCobranca filtro = new
		// PesquisarQtdeRotasSemCriteriosParaAcoesCobranca();
		// filtro.setIdGrupoCobranca(Integer.valueOf(form.getCobrancaGrupo()));
		// filtro.setIdsAcoesCobranca(idsAcoesCobranca);
		//	    
		// Integer qtdeRotasSemCriterios =
		// fachada.pesquisarQtdeRotasSemCriteriosParaAcoesCobranca(filtro);
		...
		if(sessao.getAttribute("cobrancaAcaoAtividadeCronograma") != null){
			...
			CobrancaAcaoAtividadeCronograma cobrancaAcaoAtividadeCronograma = (CobrancaAcaoAtividadeCronograma) sessao
							.getAttribute("cobrancaAcaoAtividadeCronograma");
			...
			RegistradorOperacao registradorOperacao = new RegistradorOperacao(Operacao.OPERACAO_INSERIR_COMANDO_ACAO_COBRANCA_CRONOGRAMA,
							new UsuarioAcaoUsuarioHelper(getUsuarioLogado(httpServletRequest), UsuarioAcao.USUARIO_ACAO_EFETUOU_OPERACAO));
			...
			cobrancaAcaoAtividadeCronograma.adicionarUsuario(getUsuarioLogado(httpServletRequest), UsuarioAcao.USUARIO_ACAO_EFETUOU_OPERACAO);
			registradorOperacao.registrarOperacao(cobrancaAcaoAtividadeCronograma);
			// ------------ REGISTRAR TRANSAÇÃO ----------------

			if(!GenericValidator.isBlankOrNull(form.getEmpresa())){
				...
				for(Iterator iteratorEmpresas = empresas.iterator(); iteratorEmpresas.hasNext();){
					empresa = (Empresa) iteratorEmpresas.next();
				}
				cobrancaAcaoAtividadeCronograma.setEmpresa(empresa);
			}

			// atualizar a data e hora do comando
		...
		montarPaginaSucesso(httpServletRequest, "A Ação " + cobrancaAcao.getDescricaoCobrancaAcao() + " do grupo "
						+ cobrancaGrupo.getDescricao() + ", para a atividade " + cobrancaAtividade.getDescricaoCobrancaAtividade()
		...
		if(sessao.getAttribute("cobrancaGrupoCronogramaMensal") != null){
			sessao.removeAttribute("cobrancaGrupoCronogramaMensal");
		}
