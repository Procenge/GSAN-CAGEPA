 ...
 * @date 29/01/2008
 */
public class ExibirConsultarDadosRegistroSERASAAction
				extends GcomAction {
	...
	private ActionForward exibirDadosRegistroTipoHeader(NegativadorMovimentoReg negativadorMovimentoReg,
		...
		if(conteudoRegistro != null && !conteudoRegistro.equals("")){
			...
			// H.03
			Date dataMovimento = Util.converteStringParaDate(Util.formatarData(conteudoRegistro.substring(10, 18).trim()));
			...
			// H.11
			Date diferencialRemessa = Util.converteStringParaDate(Util.formatarData(conteudoRegistro.substring(126, 130).trim()));
	...
	private ActionForward exibirDadosRegistroTipoDetalhe(NegativadorMovimentoReg negativadorMovimentoReg,
					ConsultarDadosRegistroActionForm form, Fachada fachada, HttpSession sessao, HttpServletRequest httpServletRequest,
					ActionMapping actionMapping){
		...
		if(conteudoRegistro != null && !conteudoRegistro.equals("")){
			...
			// D.04
			Date dataOcorrencia = Util.converteStringParaDate(Util.formatarData(conteudoRegistro.substring(8, 16).trim()));
			...
			// D.05
			Date dataTerminoContrato = Util.converteStringParaDate(Util.formatarData(conteudoRegistro.substring(16, 24).trim()));
			...
			// D.23
			Date dataNascimento = Util.converteStringParaDate(Util.formatarData(conteudoRegistro.substring(175, 183).trim()));
			...
			// D.37
			Date dataVencimentoDebitoMaisAntigo = Util.converteStringParaDate(Util
							.formatarData(conteudoRegistro.substring(501, 509).trim()));
