 ...
 * @date 18/12/2007
 */
public class InserirContratoNegativadorAction
				extends GcomAction {
	...
	public ActionForward execute(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest httpServletRequest,
		...
		// ------------ REGISTRAR TRANSAÇÃO ----------------
		RegistradorOperacao registradorOperacao = new RegistradorOperacao(Operacao.OPERACAO_INSERIR_CONTRATO_NEGATIVADOR,
		...
		if(inserirContratoNegativadorActionForm.getIdNegativador() != null
						&& !inserirContratoNegativadorActionForm.getIdNegativador().equals("")){
		...
		if(inserirContratoNegativadorActionForm.getValorContrato() != null
						&& !inserirContratoNegativadorActionForm.getValorContrato().equals("")){

			BigDecimal valorContrato = new BigDecimal(inserirContratoNegativadorActionForm.getValorContrato().replace(".", "").replace(",",
							"."));
			negativadorContrato.setValorContrato(valorContrato);
		}else{
			throw new ActionServletException("atencao.required", null, "Valor do Contrato");
		}

		if(inserirContratoNegativadorActionForm.getValorTarifaInclusao() != null
						&& !inserirContratoNegativadorActionForm.getValorTarifaInclusao().equals("")){

			BigDecimal valorTarifaInclusao = new BigDecimal(inserirContratoNegativadorActionForm.getValorTarifaInclusao().replace(".", "")
							.replace(",", "."));
			negativadorContrato.setValorTarifaInclusao(valorTarifaInclusao);
		}else{
			throw new ActionServletException("atencao.required", null, "Valor do Tarifa para Inclusão");
		}

		if(inserirContratoNegativadorActionForm.getNumeroPrazoInclusao() != null
						&& !inserirContratoNegativadorActionForm.getNumeroPrazoInclusao().equals("")){
			...
			throw new ActionServletException("atencao.required", null, "Prazo Para Negativação");
		}
		...
		if(inserirContratoNegativadorActionForm.getDataContratoInicio() != null
			...
			if(Util.validarDiaMesAno(dataContrato)){
				throw new ActionServletException("atencao.data.inicio.Contrato.invalida");
			}
			...
			throw new ActionServletException("atencao.required", null, "Data de Início do Contrato");
		}
		...
		if(inserirContratoNegativadorActionForm.getDataContratoFim() != null
			...
			if(Util.validarDiaMesAno(dataContrato)){
				throw new ActionServletException("atencao.data.fim.Contrato.invalida");
			}
			...
			throw new ActionServletException("atencao.required", null, "Data do Fim do Contrato");
		}
		...
		if(inserirContratoNegativadorActionForm.getNumeroContrato() != null
			...
			throw new ActionServletException("atencao.required", null, "numero  do contrato");
		}
		...
		if(negativadorContratoBanco != null){
			...
			if((Util.compararData(dataFimContrato, dataCorrente) == 0 || Util.compararData(dataFimContrato, dataCorrente) == 1)
		...
		negativadorContrato.adicionarUsuario(usuario, UsuarioAcao.USUARIO_ACAO_EFETUOU_OPERACAO);
		...
		montarPaginaSucesso(httpServletRequest, "Contrato do Negativador " + idNegativadorContrato + " inserido com sucesso.",
						"Inserir outro Contrato do Negativador", "exibirInserirContratoNegativadorAction.do?menu=sim",
						"exibirAtualizarContratoNegativadorAction.do?idRegistroAtualizacao=" + idNegativadorContrato,
						"Atualizar Contrato do Negativador Inserido");
