 ...
 *         date: 24/12/07
 */
public class AtualizarNegativadorAction
				extends GcomAction {
	...
	public ActionForward execute(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest httpServletRequest,
		...
		Integer clienteId = Integer.valueOf(form.getCodigoCliente());

		Integer imovelId = null;
		if(form.getCodigoImovel() != null && !"".equalsIgnoreCase(form.getCodigoImovel())){
			imovelId = Integer.valueOf(form.getCodigoImovel());
		}

		String inscricaoEstadual = form.getInscricaoEstadual();
		Integer idNegativador = Integer.valueOf(form.getIdNegativador());
		...
		Collection<Negativador> collNegativador = fachada.pesquisar(filtroNegativador, Negativador.class.getName());
		...
		if(imovelId != null){
			...
			filtroNegativador.adicionarParametro(new ParametroSimplesDiferenteDe(FiltroNegativador.ID, idNegativador));
			Collection<Negativador> collNegativadorImovel = fachada.pesquisar(filtroNegativador, Negativador.class.getName());
		...
		if(indicadorUso == ConstantesSistema.INDICADOR_USO_DESATIVO){
			...
			filtroNegativacaoImoveis.adicionarParametro(new ParametroSimplesDiferenteDe(FiltroNegativacaoImoveis.NEGATIVADOR_ID,
							idNegativador));
			...
			Collection<NegativacaoImovei> collNegativacaoImoveis = fachada.pesquisar(filtroNegativacaoImoveis, NegativacaoImovei.class
							.getName());
		...
		Collection<Negativador> negativadores = Fachada.getInstancia().pesquisar(filtroNegativador, Negativador.class.getName());
		...
		negativador.adicionarUsuario(usuario, UsuarioAcao.USUARIO_ACAO_EFETUOU_OPERACAO);
		registradorOperacao.registrarOperacao(negativador);
		// ------------ REGISTRAR TRANSAÇÃO ----------------

		Fachada.getInstancia().atualizar(negativador);

		montarPaginaSucesso(httpServletRequest, "Negativador " + form.getCodigoAgente() + " atualizado com sucesso.",
						"Realizar outra manutenção de Negativador", "exibirFiltrarNegativadorAction.do?menu=sim", "", "");
