 ...
 * @date: 27/12/2007
 */
public class ExibirFiltrarComandoNegativacaoPorCriterioAction
				extends GcomAction {
	...
	public ActionForward execute(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest httpServletRequest,
		...
		form.setTipoBuscaTituloComando(ConstantesSistema.TIPO_PESQUISA_INICIAL.toString());
		...
		String idDigitadoEnterUsuarioResponsavel = (String) form.getUsuarioResponsavelId();

		// verifica se o codigo do usuário responsável foi digitado foi digitado
		if(idDigitadoEnterUsuarioResponsavel != null && !idDigitadoEnterUsuarioResponsavel.trim().equals("")
						&& Integer.parseInt(idDigitadoEnterUsuarioResponsavel) > 0){

			FiltroUsuario filtroUsuario = new FiltroUsuario();

			filtroUsuario.adicionarParametro(new ParametroSimples(FiltroUsuario.ID, idDigitadoEnterUsuarioResponsavel));

			Collection usuarioEncontrado = fachada.pesquisar(filtroUsuario, Usuario.class.getName());
			...
			if(usuarioEncontrado != null && !usuarioEncontrado.isEmpty()){
				...
				form.setUsuarioResponsavelId(((Usuario) ((List) usuarioEncontrado).get(0)).getId().toString());
				form.setUsuarioResponsavelNome(((Usuario) ((List) usuarioEncontrado).get(0)).getNomeUsuario());
