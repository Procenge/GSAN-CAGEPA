 ...
 * @date 23/01/2008
 */
public class ExibirConsultarDadosRegistroSPCAction
				extends GcomAction {
	...
	public ActionForward execute(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest httpServletRequest,
		...
		if(negativadorMovimentoReg.getIndicadorCorrecao() != null){
			if(negativadorMovimentoReg.getIndicadorCorrecao().equals(ConstantesSistema.CORRIGIDO)
							|| negativadorMovimentoReg.getIndicadorCorrecao().equals(ConstantesSistema.NAO_CORRIGIDO)){
		...
		if(negativadorRegistroTipo != null){
			...
			if(negativadorRegistroTipo.getId().equals(NegativadorRegistroTipo.ID_SPC_HEADER_SP)){
				retorno = exibirDadosRegistroTipoHeader(negativadorMovimentoReg, form, fachada, sessao, httpServletRequest, actionMapping);
			}else if(negativadorRegistroTipo.getId().equals(NegativadorRegistroTipo.ID_SPC_DETALHE_CONSUMIDOR_SP)){
				retorno = exibirDadosRegistroTipoDetalheConsumidor(negativadorMovimentoReg, form, fachada, sessao, httpServletRequest,
								actionMapping);
			}else if(negativadorRegistroTipo.getId().equals(NegativadorRegistroTipo.ID_SPC_DETALHE_SPC_SP)){
				retorno = exibirDadosRegistroTipoDetalheSPC(negativadorMovimentoReg, form, fachada, sessao, httpServletRequest,
								actionMapping);
			}else if(negativadorRegistroTipo.getId().equals(NegativadorRegistroTipo.ID_SPC_TRAILLER_SP)){
		...
		// filtroNegativadorMovimentoRegRetMot, NegativadorMovimentoRegRetMot.class.getName());
		//			
		// Collection<NegativadorMovimentoRegRetMot> collNegativadorMovimentoRegRetMot =
	...
	private ActionForward exibirDadosRegistroTipoDetalheConsumidor(NegativadorMovimentoReg negativadorMovimentoReg,
					ConsultarDadosRegistroActionForm form, Fachada fachada, HttpSession sessao, HttpServletRequest httpServletRequest,
					ActionMapping actionMapping){
	...
	private ActionForward exibirDadosRegistroTipoDetalheSPC(NegativadorMovimentoReg negativadorMovimentoReg,
					ConsultarDadosRegistroActionForm form, Fachada fachada, HttpSession sessao, HttpServletRequest httpServletRequest,
					ActionMapping actionMapping){
