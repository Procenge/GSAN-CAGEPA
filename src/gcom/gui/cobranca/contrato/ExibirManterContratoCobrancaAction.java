...
public class ExibirManterContratoCobrancaAction
				extends GcomAction {

	/**
	 * Manter Contrato Cobranca
	 * 
	...
	public ActionForward execute(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest httpServletRequest,
		...
		FiltroCobrancaContrato filtroCobrancaContrato = (FiltroCobrancaContrato) sessao.getAttribute("filtroContratoCobranca");

		filtroCobrancaContrato.adicionarCaminhoParaCarregamentoEntidade("empresa");

		// Aciona o controle de paginação para que sejam pesquisados apenas os registros que
		// aparecem na página
		Map resultado = controlarPaginacao(httpServletRequest, retorno, filtroCobrancaContrato, CobrancaContrato.class.getName());
		Collection colecaoContratoCobranca = (Collection) resultado.get("colecaoRetorno");
		...
		if(colecaoContratoCobranca != null && !colecaoContratoCobranca.isEmpty()){
			...
			if(colecaoContratoCobranca.size() == 1
				 ...
				 * ExibirAtualizarFuncionalidadeAction e em caso negativo manda
				 * a coleção pelo request.
				 */

				if(httpServletRequest.getParameter("checkAtualizar") != null){
					retorno = actionMapping.findForward("atualizarContratoCobranca");
					CobrancaContrato cobrancaContrato = (CobrancaContrato) colecaoContratoCobranca.iterator().next();
					sessao.setAttribute("objetoContratoCobranca", cobrancaContrato);

				}else{
					httpServletRequest.setAttribute("colecaoContratoCobranca", colecaoContratoCobranca);
				}
			}else{
				httpServletRequest.setAttribute("colecaoContratoCobranca", colecaoContratoCobranca);
			}
