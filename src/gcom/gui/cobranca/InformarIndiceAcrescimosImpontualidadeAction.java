 ...
 * @date 18/09/2007
 */
public class InformarIndiceAcrescimosImpontualidadeAction
				extends GcomAction {
	...
	public ActionForward execute(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest httpServletRequest,
		...
		if(indiceAcrescimosImpontualidadeForm.getPercentualMulta() != null){
			BigDecimal percentualMulta = Util.formatarMoedaRealparaBigDecimal(indiceAcrescimosImpontualidadeForm.getPercentualMulta());
			if(percentualMulta.compareTo(new BigDecimal("0.00")) <= 0 || percentualMulta.compareTo(new BigDecimal("100.00")) > 0){
				throw new ActionServletException("atencao.percentual_invalido", null, "Multa");
			}
		}

		if(indiceAcrescimosImpontualidadeForm.getPercentualJurosMora() != null){
			BigDecimal percentualJurosMora = Util.formatarMoedaRealparaBigDecimal(indiceAcrescimosImpontualidadeForm
							.getPercentualJurosMora());
			if(percentualJurosMora.compareTo(new BigDecimal("0.00")) <= 0 || percentualJurosMora.compareTo(new BigDecimal("100.00")) > 0){
				throw new ActionServletException("atencao.percentual_invalido", null, "Juros de Mora");
			}
		}
		...
		if(colecaoIndicesAcrescimosImpontualidade != null && !colecaoIndicesAcrescimosImpontualidade.isEmpty()){
			...
			indicesAcrescimosImpontualidade.setPercentualMulta(Util.formatarMoedaRealparaBigDecimal(indiceAcrescimosImpontualidadeForm
							.getPercentualMulta()));
			indicesAcrescimosImpontualidade.setPercentualJurosMora(Util.formatarMoedaRealparaBigDecimal(indiceAcrescimosImpontualidadeForm
							.getPercentualJurosMora()));

			String valorComPonto = indiceAcrescimosImpontualidadeForm.getFatorCorrecao().replace(",", ".");
			indicesAcrescimosImpontualidade.setFatorAtualizacaoMonetaria(new BigDecimal(valorComPonto));
			...
			indicesAcrescimosImpontualidade.setAnoMesReferencia(Util.formatarMesAnoComBarraParaAnoMes(indiceAcrescimosImpontualidadeForm
							.getMesAnoReferencia()));
			indicesAcrescimosImpontualidade.setPercentualMulta(Util.formatarMoedaRealparaBigDecimal(indiceAcrescimosImpontualidadeForm
							.getPercentualMulta()));
			indicesAcrescimosImpontualidade.setPercentualJurosMora(Util.formatarMoedaRealparaBigDecimal(indiceAcrescimosImpontualidadeForm
		...
		// ------------ REGISTRAR TRANSAÇÃO----------------------------
		RegistradorOperacao registradorOperacao = new RegistradorOperacao(Operacao.OPERACAO_INFORMAR_INDICES_ACRESCIMOS_IMPONTUALIDADE,
		...
		indicesAcrescimosImpontualidade.adicionarUsuario(usuarioLogado, UsuarioAcao.USUARIO_ACAO_EFETUOU_OPERACAO);
		registradorOperacao.registrarOperacao(indicesAcrescimosImpontualidade);
		// ------------ REGISTRAR TRANSAÇÃO----------------------------

		montarPaginaSucesso(httpServletRequest, "Índices dos acréscimos de impontualidade de "
						+ indiceAcrescimosImpontualidadeForm.getMesAnoReferencia() + " " + mensagem + " com sucesso.", "", "");
		return retorno;
	}
