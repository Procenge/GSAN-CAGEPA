 ...
 */
public class ConsultarDebitoAction
				extends GcomAction {
	...
	public ActionForward execute(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest httpServletRequest,
		...
		sessao.removeAttribute("ConsultarDebitoClienteActionForm");
		...
		if(!Util.isVazioOuBranco(httpServletRequest.getParameter("codigoImovel"))){

			codigoImovel = (String) httpServletRequest.getParameter("codigoImovel");

		}else{

			codigoImovel = (String) consultarDebitoActionForm.getCodigoImovel();
		...
		if(!Util.isVazioOuBranco(httpServletRequest.getParameter("codigoCliente"))){

			codigoCliente = (String) httpServletRequest.getParameter("codigoCliente");

		}else{

			codigoCliente = (String) consultarDebitoActionForm.getCodigoCliente();
		...
		if(!Util.isVazioOuBranco(codigoCliente)){
			...
			if(consultarDebitoActionForm.getTipoRelacao() != null && !consultarDebitoActionForm.getTipoRelacao().trim().equals("-1")){
				tipoRelacao = Integer.valueOf(consultarDebitoActionForm.getTipoRelacao());
			}
		}

		// Verifica se o usuário não digitou código do cliente nem a matricula
		// do imovel
		if((Util.isVazioOuBranco(codigoImovel)) && (Util.isVazioOuBranco(codigoCliente)) && (Util.isVazioOuBranco(codigoClienteSuperior))){
			throw new ActionServletException("atencao.verificar.informado.imovel_cliente");
		}

		boolean peloMenosUmParametroInformado = false;

		if(!Util.isVazioOuBranco(tipoPesquisa)){
			if(tipoPesquisa.equalsIgnoreCase("imovel")){
			...
			}else if(tipoPesquisa.equalsIgnoreCase("cliente")){
				...
				peloMenosUmParametroInformado = true;
			}else if(tipoPesquisa.equalsIgnoreCase("clienteSuperior")){
			...
			if(!Util.isVazioOuBranco(codigoImovel)){
				...
				peloMenosUmParametroInformado = true;
			}else if((!Util.isVazioOuBranco(codigoCliente))){
				codigoCliente = codigoCliente.trim();
			...
			}else if((!Util.isVazioOuBranco(codigoClienteSuperior))){
		...
		if(!Util.isVazioOuBranco(tipoRelacao) && tipoRelacao != ConstantesSistema.NUMERO_NAO_INFORMADO){
			FiltroClienteRelacaoTipo filtroClienteRelacaoTipo = new FiltroClienteRelacaoTipo();

			filtroClienteRelacaoTipo
							.adicionarParametro(new ParametroSimples(FiltroClienteRelacaoTipo.CLIENTE_RELACAO_TIPO_ID, tipoRelacao));
			...
			/* tipoRelacaoSelecionada = */sessao.setAttribute("tipoRelacao", collectionClienteRelacaoTipo.iterator().next());
		}else{
			tipoRelacao = null;
		}
		...
		if(!peloMenosUmParametroInformado){
			throw new ActionServletException("atencao.selecionar.nenhum_parametro_informado");
		}

		if(!Util.isVazioOuBranco(httpServletRequest.getParameter("ehPopup"))){
			sessao.setAttribute("ehPopup", "true");
		}

		if(!Util.isVazioOuBranco(httpServletRequest.getParameter("caminhoRetornoTelaConsultaDebito"))){
