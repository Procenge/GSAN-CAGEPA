 ...
 */
public class ExibirInserirComandoAcaoCobrancaCronogramaAction
				extends GcomAction {
	...
	public ActionForward execute(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest httpServletRequest,
		...
		ActionForward retorno = actionMapping.findForward("exibirInserirComandoAcaoCobrancaCronograma");
		...
		if(sessao.getAttribute("colecaoCobrancaGrupo") == null){
			...
			if(colecaoCobrancaGrupo != null && !colecaoCobrancaGrupo.isEmpty()){
				...
				while(iteratorColecaoCobrancaGrupo.hasNext()){
					...
					filtroCobrancaGrupoCronogramaMensal.adicionarParametro(new ParametroSimples(
									FiltroCobrancaGrupoCronogramaMes.ID_COBRANCA_GRUPO, cobrancaGrupo.getId()));
					filtroCobrancaGrupoCronogramaMensal.adicionarParametro(new ParametroSimples(
									FiltroCobrancaGrupoCronogramaMes.ANO_MES_REFERENCIA, cobrancaGrupo.getAnoMesReferencia()));

					Collection colecaoCobrancaGrupoCronogramaMensal = (Collection) fachada.pesquisar(filtroCobrancaGrupoCronogramaMensal,
		...
		if(validarCobrancaGrupo != null && !validarCobrancaGrupo.equals("")){
			...
			if(idGrupoCobranca != null && !idGrupoCobranca.equals("")){
				...
				if(colecaoCobrancaGrupoCronogramaMensal != null && colecaoCobrancaGrupoCronogramaMensal.isEmpty()){
					throw new ActionServletException("atencao.cronograma_cobranca.inexistente", null, idGrupoCobranca + " no Mês Corrente "
									+ cobrancaGrupo.getAnoMesReferencia());
				}

				inserirComandoAcaoCobrancaCronogramaActionForm.setReferenciaCobranca(cobrancaGrupo.getAnoMesReferencia().toString());

				// cobrancao de grupo selecionado
				sessao.setAttribute("cobrancaGrupo", cobrancaGrupo);

				CobrancaGrupoCronogramaMes cobrancaGrupoCronogramaMensal = (CobrancaGrupoCronogramaMes) colecaoCobrancaGrupoCronogramaMensal
								.iterator().next();

				sessao.setAttribute("cobrancaGrupoCronogramaMensal", cobrancaGrupoCronogramaMensal);
				...
				if(colecaoCobrancaAcao != null && !colecaoCobrancaAcao.isEmpty()){
					...
					while(iColecaoCobrancaAcao.hasNext()){
						...
						filtroCobrancaAcaoCronograma.adicionarParametro(new ParametroSimples(
										FiltroCobrancaAcaoCronograma.ID_COBRANCA_GRUPO_CRONOGRAMA_MES, cobrancaGrupoCronogramaMensal
														.getId()));
						colecaoCobrancaAcaoCronograma = fachada.pesquisar(filtroCobrancaAcaoCronograma, CobrancaAcaoCronograma.class
										.getName());
						...
						if(colecaoCobrancaAcaoCronograma != null && !colecaoCobrancaAcaoCronograma.isEmpty()){
							...
							filtroCobrancaAcaoAtividadeCronograma.adicionarParametro(new ParametroSimples(
											FiltroCobrancaAcaoAtividadeCronograma.ID_COBRANCA_ACAO_CRONOGRAMA, cobrancaAcaoCronograma
															.getId()));
							colecaoCobrancaAcaoAtividadeCronograma = fachada.pesquisar(filtroCobrancaAcaoAtividadeCronograma,
						...
						if(cobrancaAcao.getIndicadorRepeticao().shortValue() == (short) 1){
							...
							if(colecaoCobrancaAcaoAtividadeCronograma != null && !colecaoCobrancaAcaoAtividadeCronograma.isEmpty()){
								...
								while(iteratorColecaoCobrancaAcaoAtividadeCronograma.hasNext() & achou){
									CobrancaAcaoAtividadeCronograma cobrancaAcaoAtividadeCronograma = (CobrancaAcaoAtividadeCronograma) iteratorColecaoCobrancaAcaoAtividadeCronograma
													.next();
									if(cobrancaAcaoAtividadeCronograma.getRealizacao() == null
													& cobrancaAcaoAtividadeCronograma.getCobrancaAtividade().getIndicadorComando()
																	.shortValue() == (short) 1){
										primeiraCondicao = true;
										achou = false;
									}
						...
						if(cobrancaAcao.getCobrancaAcaoPredecessora() == null){
							...
							filtroCobrancaAcaoCronograma.adicionarParametro(new ParametroSimples(
											FiltroCobrancaAcaoCronograma.ID_COBRANCA_ACAO, cobrancaAcao.getCobrancaAcaoPredecessora()
															.getId()));
							filtroCobrancaAcaoCronograma.adicionarParametro(new ParametroSimples(
											FiltroCobrancaAcaoCronograma.ID_COBRANCA_GRUPO_CRONOGRAMA_MES, cobrancaGrupoCronogramaMensal
															.getId()));
							...
							if(colecaoCobrancaAcaoAtividadeCronograma != null && !colecaoCobrancaAcaoAtividadeCronograma.isEmpty()){
								...
								while(iteratorColecaoCobrancaAcaoAtividadeCronograma.hasNext() & achou){
									...
									if(cobrancaAcaoAtividadeCronograma.getCobrancaAtividade().getIndicadorComando().shortValue() == (short) 1){
										if((cobrancaAcaoAtividadeCronograma.getRealizacao() != null)
														|| (cobrancaAcaoAtividadeCronograma.getRealizacao() != null && (cobrancaAcaoAtividadeCronograma
																		.getRealizacao().compareTo(
																						cobrancaAcaoAtividadeCronograma.getComando()) < 1))){
					...
					if(colecaoCobranAcaoAptas != null && !colecaoCobranAcaoAptas.isEmpty()){
						...
						throw new ActionServletException("atencao.nenhuma.acao_cobranca", null, cobrancaGrupo.getId() + " no mês corrente "
										+ cobrancaGrupo.getAnoMesReferencia() + " já foram realizadas");
					}
				...
				inserirComandoAcaoCobrancaCronogramaActionForm.setReferenciaCobranca("");
				if(sessao.getAttribute("colecaoCobrancaAcao") != null){
					sessao.removeAttribute("colecaoCobrancaAcao");
				}
				if(sessao.getAttribute("colecaoCobrancaAtividade") != null){
		...
		if(validarCobrancaAcao != null && !validarCobrancaAcao.equals("")){
			...
			if(idAcaoCobranca != null && !idAcaoCobranca.equals("")){
				if(sessao.getAttribute("cobrancaGrupo") != null && sessao.getAttribute("cobrancaGrupoCronogramaMensal") != null){
					CobrancaGrupo cobrancaGrupo = (CobrancaGrupo) sessao.getAttribute("cobrancaGrupo");
					CobrancaGrupoCronogramaMes cobrancaGrupoCronogramaMensal = (CobrancaGrupoCronogramaMes) sessao
									.getAttribute("cobrancaGrupoCronogramaMensal");

					// validando a cobrança ação selecionada
					// essa validação é necessária pq a ação pode ser carrega sem ter validado pelo
					// [SF004]
					FiltroCobrancaAcao filtroCobrancaAcao = new FiltroCobrancaAcao();
					filtroCobrancaAcao.adicionarParametro(new ParametroSimples("id", idAcaoCobranca));
					...
					filtroCobrancaAcaoCronograma.adicionarParametro(new ParametroSimples(FiltroCobrancaAcaoCronograma.ID_COBRANCA_ACAO,
									idAcaoCobranca));
					filtroCobrancaAcaoCronograma.adicionarParametro(new ParametroSimples(
									FiltroCobrancaAcaoCronograma.ID_COBRANCA_GRUPO_CRONOGRAMA_MES, cobrancaGrupoCronogramaMensal.getId()));
					Collection colecaoCobrancaAcaoCronograma = fachada.pesquisar(filtroCobrancaAcaoCronograma, CobrancaAcaoCronograma.class
									.getName());
					...
					// null,cobrancaGrupo.getId() + " no mês corrente " +
					// cobrancaGrupo.getAnoMesReferencia() + ", não existe a Ação " +
					// cobrancaAcao.getDescricaoCobrancaAcao());
					//			
					// }
					// fim da validação cobrança ação

					// FiltroCobrancaAcaoCronogama filtroCobrancaAcaoCronogama = new
					// FiltroCobrancaAcaoCronogama();
					// filtroCobrancaAcaoCronogama.adicionarParametro(
					// new ParametroSimples("id", idAcaoConbranca));
					// filtroCobrancaAcaoCronogama.adicionarParametro(
					// new ParametroSimples("cobrancaGrupoCronogramaMensal.id",
					// cobrancaGrupoCronogramaMensal.getId()));

					// Collection colecaoCobrancaAcaoCronogama = (Collection)
					// fachada.pesquisar(filtroCobrancaAcaoCronogama,CobrancaAcaoCronogama.class.getName());

					// if(!(colecaoCobrancaAcaoCronogama != null &&
					// colecaoCobrancaAcaoCronogama.isEmpty())){
					// throw new ActionServletException("atencao.cronograma_acao.inexistente",
					// null, cobrancaGrupo.getId().toSting() + " no Mês Corrente " +
					// cobrancaGrupo.getAnoMesReferencia());
					// }

					CobrancaAcaoCronograma cobrancaAcaoCronograma = (CobrancaAcaoCronograma) colecaoCobrancaAcaoCronograma.iterator()
									.next();
					...
					filtroCobrancaAtividade.adicionarParametro(new ParametroSimples("indicadorComando", "1"));// valor
					// 1

					Collection colecaoCobrancaAtividade = (Collection) fachada.pesquisar(filtroCobrancaAtividade, CobrancaAtividade.class
									.getName());
					...
					if(colecaoCobrancaAtividade != null && !colecaoCobrancaAtividade.isEmpty()){
						...
						while(iColecaoCobrancaAtividade.hasNext()){
							boolean semRealizacao = false;
							boolean semAtividadePredecessora = false;
							boolean temCobrancaAtividadeCronograma = false;
							...
							filtroCobrancaAcaoAtividadeCronograma
											.adicionarCaminhoParaCarregamentoEntidade(FiltroCobrancaAcaoAtividadeCronograma.COBRANCA_ATIVIDADE);
							filtroCobrancaAcaoAtividadeCronograma.adicionarParametro(new ParametroSimples(
											FiltroCobrancaAcaoAtividadeCronograma.ID_COBRANCA_ATIVIDADE, cobrancaAtividade.getId()));

							filtroCobrancaAcaoAtividadeCronograma.adicionarParametro(new ParametroSimples(
											FiltroCobrancaAcaoAtividadeCronograma.ID_COBRANCA_ACAO_CRONOGRAMA, cobrancaAcaoCronograma
															.getId()));

							Collection colecaoCobrancaAcaoAtividadeCronograma = (Collection) fachada.pesquisar(
											filtroCobrancaAcaoAtividadeCronograma, CobrancaAcaoAtividadeCronograma.class.getName());

							if(colecaoCobrancaAcaoAtividadeCronograma != null && !colecaoCobrancaAcaoAtividadeCronograma.isEmpty()){
								temCobrancaAtividadeCronograma = true;
							}
							...
							if(colecaoCobrancaAcaoAtividadeCronograma != null && !colecaoCobrancaAcaoAtividadeCronograma.isEmpty()){
								...
								while(iteratorCobrancaAcaoAtividadeCronograma.hasNext() && achou){
									CobrancaAcaoAtividadeCronograma cobrancaAcaoAtividadeCronograma = (CobrancaAcaoAtividadeCronograma) iteratorCobrancaAcaoAtividadeCronograma
													.next();
							...
							if(cobrancaAtividade.getCobrancaAtividadePredecessora() == null){
								...
								filtroCobrancaAcaoAtividadeCronograma.adicionarParametro(new ParametroSimples(
												FiltroCobrancaAcaoAtividadeCronograma.ID_COBRANCA_ACAO_CRONOGRAMA, cobrancaAcaoCronograma
																.getId()));
								filtroCobrancaAcaoAtividadeCronograma.adicionarParametro(new ParametroSimples(
												FiltroCobrancaAcaoAtividadeCronograma.ID_COBRANCA_ATIVIDADE, cobrancaAtividade
																.getCobrancaAtividadePredecessora().getId()));
								...
								if(colecaoCobrancaAcaoAtividadeCronograma != null && !colecaoCobrancaAcaoAtividadeCronograma.isEmpty()){
									...
									while(iteratorCobrancaAcaoAtividadeCronograma.hasNext() && achou){
										...
										if(cobrancaAcaoAtividadeCronograma.getRealizacao() != null){
											semAtividadePredecessora = false;
											achou = false;
										}
									}
								}
							...
							if(semRealizacao & semAtividadePredecessora & temCobrancaAtividadeCronograma){
						...
						if(colecaoCobrancaAtividade != null && colecaoCobrancaAtividade.isEmpty()){
							throw new ActionServletException("atencao.nenhuma.atividade_cobranca", null, idAcaoCobranca + " para o Grupo "
											+ cobrancaGrupo.getId().toString() + ", no mês corrente " + cobrancaGrupo.getAnoMesReferencia()
											+ ", já foram realizadas");
						}else{
						...
						throw new ActionServletException("atencao.pesquisa_inexistente", null, "Tabela Cobrança Atividade");
					}
				}
			}else{
				if(sessao.getAttribute("colecaoCobrancaAtividade") != null){
		...
		if(validarCobrancaAtividade != null && !validarCobrancaAtividade.equals("")){
			...
			if(idAtividade != null && !idAtividade.equals("")){
				...
				if(sessao.getAttribute("cobrancaGrupoCronogramaMensal") != null && sessao.getAttribute("cobrancaAcaoCronogama") != null){
					CobrancaAcaoCronograma cobrancaAcaoCronogama = (CobrancaAcaoCronograma) sessao.getAttribute("cobrancaAcaoCronogama");

					FiltroCobrancaAtividade filtroCobrancaAtividade = new FiltroCobrancaAtividade();
					filtroCobrancaAtividade.adicionarParametro(new ParametroSimples("id", idAtividade));
					...
					filtroCobrancaAcaoAtividadeCronograma.adicionarCaminhoParaCarregamentoEntidade("cobrancaAcaoCronograma");
					filtroCobrancaAcaoAtividadeCronograma.adicionarParametro(new ParametroSimples(
									FiltroCobrancaAcaoAtividadeCronograma.ID_COBRANCA_ATIVIDADE, idAtividade));

					filtroCobrancaAcaoAtividadeCronograma.adicionarParametro(new ParametroSimples(
									FiltroCobrancaAcaoAtividadeCronograma.ID_COBRANCA_ACAO_CRONOGRAMA, cobrancaAcaoCronogama.getId()));

					Collection colecaoCobrancaAcaoAtividadeCronograma = (Collection) fachada.pesquisar(
									filtroCobrancaAcaoAtividadeCronograma, CobrancaAcaoAtividadeCronograma.class.getName());

					// [FS006] - Verificar existência da atividade no cronograma do grupo do mês
					// corrente
					// if(!(colecaoCobrancaAcaoAtividadeCronograma != null &&
					// !colecaoCobrancaAcaoAtividadeCronograma.isEmpty())){
					// throw new ActionServletException("atencao.nenhuma.atividade_grupo_acao",
					// null, cobrancaGrupo.getId().toString() + ", do Mês Corrente " +
					// cobrancaGrupo.getAnoMesReferencia() + ", não existe a Atividade " +
					// cobrancaAtividade.getDescricaoCobrancaAtividade() + " para a Ação " +
					// cobrancaAcao.getDescricaoCobrancaAcao());
					// }

					CobrancaAcaoAtividadeCronograma cobrancaAcaoAtividadeCronograma = (CobrancaAcaoAtividadeCronograma) colecaoCobrancaAcaoAtividadeCronograma
									.iterator().next();
