 ...
 * @date 31/10/2008
 *       Desfazer alterações no [UC0643] para a v0.04]
 */
public class ExibirFiltrarAcaoCobrancaAction
				extends GcomAction {
	...
	public ActionForward execute(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest httpServletRequest,
		...
		// Seta o retorno
		ActionForward retorno = actionMapping.findForward("filtrarAcaoCobranca");

		HttpSession sessao = httpServletRequest.getSession(false);

		Fachada fachada = Fachada.getInstancia();

		// caso venha da tela atualizar sem ter passado pelo filtro (no caso vim da tela de sucesso
		// do inserir)
		...
		if(httpServletRequest.getParameter("menu") != null && !httpServletRequest.getParameter("menu").equals("")){
			...
			acaoCobrancaFiltrarActionForm.setQtdDiasRealizacao("");
		...
		pesquisarEnter(acaoCobrancaFiltrarActionForm, httpServletRequest, fachada);

		return retorno;

	}

	private void pesquisarEnter(AcaoCobrancaFiltrarActionForm acaoCobrancaFiltrarActionForm, HttpServletRequest httpServletRequest,
					Fachada fachada){

		// pesquisa enter de critério de cobrança
		if(acaoCobrancaFiltrarActionForm.getIdCobrancaCriterio() != null
						&& !acaoCobrancaFiltrarActionForm.getIdCobrancaCriterio().equals("")
						&& (acaoCobrancaFiltrarActionForm.getDescricaoCobrancaCriterio() == null || acaoCobrancaFiltrarActionForm
			...
			try{
				filtroCobrancaCriterio.adicionarParametro(new ParametroSimples(FiltroCobrancaCriterio.ID, Integer
								.valueOf(acaoCobrancaFiltrarActionForm.getIdCobrancaCriterio())));
			...
			if(colecaoCobrancaCriterio != null && !colecaoCobrancaCriterio.isEmpty()){
				...
				acaoCobrancaFiltrarActionForm.setDescricaoCobrancaCriterio(cobrancaCriterio.getDescricaoCobrancaCriterio());
			}else{
				acaoCobrancaFiltrarActionForm.setIdCobrancaCriterio("");
				acaoCobrancaFiltrarActionForm.setDescricaoCobrancaCriterio("COBRANÇA CRITÉRIO INEXISTENTE");
			}
		...
		if(acaoCobrancaFiltrarActionForm.getIdServicoTipo() != null
			...
			try{
				filtroServicoTipo.adicionarParametro(new ParametroSimples(FiltroServicoTipo.ID, Integer
								.valueOf(acaoCobrancaFiltrarActionForm.getIdServicoTipo())));
			}catch(NumberFormatException ex){
			...
			if(colecaoServicoTipo != null && !colecaoServicoTipo.isEmpty()){
				...
				acaoCobrancaFiltrarActionForm.setDescricaoServicoTipo(servicoTipo.getDescricao());
			}else{
				acaoCobrancaFiltrarActionForm.setIdServicoTipo("");
				acaoCobrancaFiltrarActionForm.setDescricaoServicoTipo("TIPO DE SERVIÇO INEXISTENTE");
			}
	...
	private void pesquisasObrigatorias(Fachada fachada, HttpSession sessao){
		...
		Collection<ResolucaoDiretoria> colecaoResolucaoDiretoria = fachada.pesquisarResolucaoDiretoriaMaiorDataVigenciaInicio(null);
