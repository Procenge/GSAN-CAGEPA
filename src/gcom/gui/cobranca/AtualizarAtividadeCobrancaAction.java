 ...
 * @author Tiago Moreno
 */
public class AtualizarAtividadeCobrancaAction
				extends GcomAction {
	...
	public ActionForward execute(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest httpServletRequest,
		...
		String descricaoAtividadeCobranca = atualizarAtividadeCobrancaActionForm.getDescricaoAtividadeCobranca();
		String atividadePredecessora = atualizarAtividadeCobrancaActionForm.getIdAtividadePredecessora();
		String ordemCronograma = atualizarAtividadeCobrancaActionForm.getOrdemCronograma();
		String processoAssociado = atualizarAtividadeCobrancaActionForm.getIdProcessoAssociado();
		String opcaoCompoeCronograma = atualizarAtividadeCobrancaActionForm.getOpcaoCompoeCronograda();
		String opcaoAtividadeObrigatoria = atualizarAtividadeCobrancaActionForm.getOpcaoAtividadeObrigatoria();
		String opcaoComandada = atualizarAtividadeCobrancaActionForm.getOpcaoPodeSerComandada();
		String opcaoExecutada = atualizarAtividadeCobrancaActionForm.getOpcaoPodeSerExecutada();
		Date ultimaAlteracaoAtividade = atualizarAtividadeCobrancaActionForm.getUltimaAlteracao();

		FiltroCobrancaAtividade filtroCobrancaAtividade = new FiltroCobrancaAtividade();

		// FS0002 - Numero de ordem já utilizado
		filtroCobrancaAtividade.adicionarParametro(new ParametroSimplesDiferenteDe(FiltroCobrancaAtividade.ID, id));
		filtroCobrancaAtividade.adicionarParametro(new ParametroSimples(FiltroCobrancaAtividade.ORDEM_REALIZACAO, ordemCronograma));
		...
		if(atividadePredecessora != null && !atividadePredecessora.equalsIgnoreCase("")){
			filtroCobrancaAtividade.adicionarParametro(new ParametroSimples(FiltroCobrancaAtividade.ID, atividadePredecessora));
			cobrancaAtividadePredecessora = (CobrancaAtividade) fachada.pesquisar(filtroCobrancaAtividade,
							CobrancaAtividade.class.getName()).iterator().next();

			int ordemRealizacao = Integer.valueOf(ordemCronograma);
		...
		if(atividadePredecessora != null){
			if(id != null && id.equals(atividadePredecessora)){
				throw new ActionServletException("atencao.atividade_predecessora_nao_igual_atividade");
			}
		}
		// FS0006 - Fim do Bloco
		...
		filtroCobrancaAtividadeNoBanco.adicionarParametro(new ParametroSimples(FiltroCobrancaAtividade.ID, id));
		filtroCobrancaAtividadeNoBanco.adicionarCaminhoParaCarregamentoEntidade("processo");
		cobrancaAtividade = (CobrancaAtividade) fachada.pesquisar(filtroCobrancaAtividadeNoBanco, CobrancaAtividade.class.getName())
						.iterator().next();
		...
		if(ordemCronograma != null && !ordemCronograma.equalsIgnoreCase("")){
			cobrancaAtividade.setOrdemRealizacao(Short.valueOf(ordemCronograma));
		}
		...
		if(processoAssociado != null && !processoAssociado.equalsIgnoreCase("")){
			...
			if(processoNaBase != null && !processoNaBase.isEmpty()){
				...
				processo.setId(Integer.valueOf(processoAssociado));
		...
		if(opcaoCompoeCronograma != null && !opcaoCompoeCronograma.equalsIgnoreCase("")){
			cobrancaAtividade.setIndicadorCronograma(Short.valueOf(opcaoCompoeCronograma));
		}

		if(opcaoAtividadeObrigatoria != null && !opcaoAtividadeObrigatoria.equalsIgnoreCase("")){
			cobrancaAtividade.setIndicadorObrigatoriedade(Short.valueOf(opcaoAtividadeObrigatoria));
		}

		if(opcaoComandada != null && !opcaoComandada.equalsIgnoreCase("")){
			cobrancaAtividade.setIndicadorComando(Short.valueOf(opcaoComandada));
		}

		if(opcaoExecutada != null && !opcaoExecutada.equalsIgnoreCase("")){
			cobrancaAtividade.setIndicadorExecucao(Short.valueOf(opcaoExecutada));
		}

		cobrancaAtividade.setIndicadorUso(Short.valueOf("1"));

		// Verificar se o cobrança atividade já foi atualizado por outro usuário durante esta
		// atualização
		if(cobrancaAtividade == null || cobrancaAtividade.getUltimaAlteracao().after(ultimaAlteracaoAtividade)){
		...
		montarPaginaSucesso(httpServletRequest, "Atividade de Cobranca " + id + " atualizada com sucesso.",
						"Atualizar outra Atividade de Cobranca", "exibirManterAtividadeCobrancaAction.do?menu=sim");
