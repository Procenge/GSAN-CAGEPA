 ...
 * @author Tiago Moreno
 */
public class ExibirAtualizarAtividadeCobrancaAction
				extends GcomAction {
	...
	public ActionForward execute(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest httpServletRequest,
		...
		ActionForward retorno = actionMapping.findForward("atualizarAtividadeCobranca");
		...
		if(idAtividadeCobranca != null && !idAtividadeCobranca.equalsIgnoreCase("")){
			...
			CobrancaAtividade cobrancaAtividade = (CobrancaAtividade) fachada.pesquisar(filtroCobrancaAtividadeNaBase,
							CobrancaAtividade.class.getName()).iterator().next();

			if(cobrancaAtividade != null){
				atualizarAtividadeCobrancaActionForm.setDescricaoAtividadeCobranca(cobrancaAtividade.getDescricaoCobrancaAtividade());

				if(cobrancaAtividade.getCobrancaAtividadePredecessora() != null){
					atualizarAtividadeCobrancaActionForm.setIdAtividadePredecessora(cobrancaAtividade.getCobrancaAtividadePredecessora()
									.getId().toString());
				}else{
					atualizarAtividadeCobrancaActionForm.setIdAtividadePredecessora("-1");
				}
				atualizarAtividadeCobrancaActionForm.setDescricaoProcessoAssociado(cobrancaAtividade.getProcesso().getDescricaoProcesso());
				atualizarAtividadeCobrancaActionForm.setIdProcessoAssociado(cobrancaAtividade.getProcesso().getId().toString());
				atualizarAtividadeCobrancaActionForm.setOrdemCronograma(cobrancaAtividade.getOrdemRealizacao().toString());
				atualizarAtividadeCobrancaActionForm.setOpcaoAtividadeObrigatoria(cobrancaAtividade.getIndicadorObrigatoriedade()
								.toString());
				atualizarAtividadeCobrancaActionForm.setOpcaoCompoeCronograda(cobrancaAtividade.getIndicadorCronograma().toString());
				atualizarAtividadeCobrancaActionForm.setOpcaoPodeSerComandada(cobrancaAtividade.getIndicadorComando().toString());
				atualizarAtividadeCobrancaActionForm.setOpcaoPodeSerExecutada(cobrancaAtividade.getIndicadorExecucao().toString());
				atualizarAtividadeCobrancaActionForm.setUltimaAlteracao(cobrancaAtividade.getUltimaAlteracao());
			}
		}
