 ...
 * @date 12/07/2006
 */
public class ExibirControlarPermissoesEspeciaisUsuarioAction
				extends GcomAction {
	...
	public ActionForward execute(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest httpServletRequest,
		...
		ActionForward retorno = actionMapping.findForward("controlarPermissoesEspeciaisUsuario");
		...
		String[] permissaoEspecial = controlarAcessoUsuarioActionForm.getPermissoesEspeciais();
		...
		if(permissaoEspecial == null && permissoesCheckBoxVazias == null){
			...
			if(colecaoUsuarioGrupo != null && !colecaoUsuarioGrupo.isEmpty()){
				...
				filtroPemissaoEspecial.setCampoOrderBy(FiltroPemissaoEspecial.DESCRICAO);

				colecaoPermissaoEspecial = Fachada.getInstancia().pesquisar(filtroPemissaoEspecial, PermissaoEspecial.class.getName());

				FiltroUsuarioPemissaoEspecial filtroUsuarioPermissaoEspecialAtualizar = new FiltroUsuarioPemissaoEspecial();
				filtroUsuarioPermissaoEspecialAtualizar.adicionarCaminhoParaCarregamentoEntidade("permissaoEspecial");
				filtroUsuarioPermissaoEspecialAtualizar.adicionarParametro(new ParametroSimples(
								FiltroUsuarioPemissaoEspecial.USUARIO_COMP_ID, usuarioParaAtualizar.getId()));
				filtroUsuarioPermissaoEspecialAtualizar.setCampoOrderBy(FiltroUsuarioPemissaoEspecial.PERMISSAO_ESPECIAL_DESCRICAO);

				// colecao de usuário permissão especial
				Collection colecaoUsuarioPermissaoEspecialParaAtualizar = Fachada.getInstancia().pesquisar(
								filtroUsuarioPermissaoEspecialAtualizar, UsuarioPermissaoEspecial.class.getName());
				if(colecaoUsuarioPermissaoEspecialParaAtualizar != null && !colecaoUsuarioPermissaoEspecialParaAtualizar.isEmpty()){

					// seta os campos de permissão especial no form para
					// aparecer no
					// jsp como checado
					permissaoEspecial = Fachada.getInstancia().retornarPermissoesMarcadas(colecaoUsuarioPermissaoEspecialParaAtualizar);

				}
			}else{
				Object[] permissoesEspeciais = Fachada.getInstancia().pesquisarPermissoesEspeciaisUsuarioEUsuarioLogado(
			...
			controlarAcessoUsuarioActionForm.setPermissoesEspeciais(permissaoEspecial);
