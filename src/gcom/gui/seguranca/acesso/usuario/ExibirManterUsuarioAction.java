...
public class ExibirManterUsuarioAction
				extends GcomAction {
	...
	public ActionForward execute(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest httpServletRequest,
		...
		if(sessao.getAttribute("filtroUsuarioGrupo") != null){
			filtroUsuarioGrupo = (FiltroUsuarioGrupo) sessao.getAttribute("filtroUsuarioGrupo");
			sessao.setAttribute("filtroUsuario", filtroUsuarioGrupo);
		}

		if((retorno != null) && (retorno.getName().equalsIgnoreCase("manterUsuario"))){

			String identificadorAtualizar = (String) sessao.getAttribute("indicadorAtualizar");
			// 1º Passo - Pegar o total de registros através de um count da
			// consulta que aparecerá na tela
			Integer totalRegistros = Fachada.getInstancia().totalRegistrosPesquisaUsuarioGrupo(filtroUsuarioGrupo);
			sessao.setAttribute("totalRegistros", totalRegistros);
			// filtroUsuarioGrupo.setCampoOrderBy(FiltroUsuarioGrupo.USUARIO_NOME);
			...
			collectionUsuariosGrupos = Fachada.getInstancia().pesquisarUsuariosDosGruposUsuarios(filtroUsuarioGrupo,
							((Integer) httpServletRequest.getAttribute("numeroPaginasPesquisa")));
			// [FS0003] Nenhum registro encontrado
			if(collectionUsuariosGrupos == null || collectionUsuariosGrupos.isEmpty()){
			...
			if(totalRegistros == 1 && identificadorAtualizar != null){
				...
				Usuario usuarioParaAtualizar = (Usuario) Util.retonarObjetoDeColecao(collectionUsuariosGrupos);
				httpServletRequest.setAttribute("idRegistroAtualizacao", new Integer(usuarioParaAtualizar.getId()).toString());
				httpServletRequest.setAttribute("atualizar", "1");
			}else{
				sessao.setAttribute("collectionUsuariosGrupos", collectionUsuariosGrupos);
			}
