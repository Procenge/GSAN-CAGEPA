 ...
 * @date 08/06/2006
 */
public class BloquearDesbloquearAcessoUsuarioAction
				extends GcomAction {
	...
	public ActionForward execute(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest httpServletRequest,
		...
		RegistradorOperacao registradorOperacao = new RegistradorOperacao(Operacao.OPERACAO_ACESSO_USUARIO_ATUALIZAR,
		...
		usuario.adicionarUsuario(getUsuarioLogado(httpServletRequest), UsuarioAcao.USUARIO_ACAO_EFETUOU_OPERACAO);
		...
		if(login != null && !login.equals("")){
			...
			if(colecaoUsuario == null || colecaoUsuario.isEmpty()){
				throw new ActionServletException("atencao.login_nao_existente", null, "" + login + "");
			}else{
		...
		if(idSituacaoUsuario != null && !idSituacaoUsuario.trim().equals("" + ConstantesSistema.NUMERO_NAO_INFORMADO)){

			FiltroUsuarioSituacao filtroUsuarioSituacao = new FiltroUsuarioSituacao();
			filtroUsuarioSituacao.adicionarParametro(new ParametroSimples(FiltroUsuarioSituacao.ID, idSituacaoUsuario));

			Collection colecaoUsarioSituacao = fachada.pesquisar(filtroUsuarioSituacao, UsuarioSituacao.class.getName());

			UsuarioSituacao usuarioSituacao = (UsuarioSituacao) colecaoUsarioSituacao.iterator().next();
		...
		montarPaginaSucesso(httpServletRequest, usuario.getUsuarioSituacao().getDescricaoUsuarioSituacao() + " " + "efetuada com sucesso.",
						"Bloquear/Desbloquear acesso de outro usuário", "exibirBloquearDesbloquearAcessoUsuarioAction.do?menu=sim");
