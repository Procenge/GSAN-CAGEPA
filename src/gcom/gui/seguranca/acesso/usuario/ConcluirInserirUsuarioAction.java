 ...
 * @date 09/05/2006
 */
public class ConcluirInserirUsuarioAction
				extends GcomAction {
	...
	public ActionForward execute(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest httpServletRequest,
		...
		// throw new ActionServletException("atencao.usuario.abrangencia.nula");
		// }
		// if (usuario.getEmpresa() == null ){
		// throw new ActionServletException("atencao.usuario.empresa.nula");
		// }
		// if (usuario.getNomeUsuario() == null ){
		// throw new ActionServletException("atencao.usuario.nome.nula");
		// }
		// if (usuario.getUnidadeOrganizacional() == null ){
		// throw new
		// ActionServletException("atencao.usuario.unidadeEmpresa.nula");
		// }
		// if (usuario.getLogin() == null ){
		// throw new ActionServletException("atencao.usuario.login.nula");
		// }
					...
					if(usuarioTipo.getIndicadorFuncionario() == UsuarioTipo.INDICADOR_FUNCIONARIO){
						...
						if(form.getIdFuncionario() == null || form.getIdFuncionario().equals("")){
							...
							if(form.getIdFuncionario() != null
											&& (form.getNomeFuncionario() == null || form.getNomeFuncionario().equals(""))){
		...
		if(form.getAbrangencia() != null && !form.getAbrangencia().equals("")){
			...
			if(form.getAbrangencia().equals(UsuarioAbrangencia.ELO_POLO)){
				if(form.getIdElo() == null || form.getIdElo().equals("")){
					throw new ActionServletException("atencao.required", null, "Elo Pólo");
				}
			}
			...
			throw new ActionServletException("atencao.required", null, "Abrangência do Acesso");
		}
		...
		Fachada.getInstancia().inserirUsuario(usuarioCadastrar, idGrupos, colecaoUsuarioAcesso);

		if(usuario.getId() != null && usuario.getId().equals(usuarioCadastrar.getId())){
			sessao.setAttribute("usuarioLogado", usuarioCadastrar);
		}

		montarPaginaSucesso(httpServletRequest, "Usuário de Login " + usuarioCadastrar.getLogin() + " inserido com sucesso!",
						"Inserir outro Usuário", "exibirInserirUsuarioAction.do?menu=sim");
