 ...
 * @date 07/08/2006
 */
public class AtualizarOperacaoAction
				extends GcomAction {
	...
	public ActionForward execute(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest httpServletRequest,
		...
		String indicadorAuditoriaString = atualizarOperacaoActionForm.getIndicadorAuditoria();
		...
		if(idTipoOperacao != null && !idTipoOperacao.trim().equals("")){
			...
			operacaoTipo.setId(Util.obterInteger(idTipoOperacao));
		}
		...
		if(idFuncionalidade != null && !idFuncionalidade.trim().equals("")){
			...
			funcionalidade.setId(Util.obterInteger(idFuncionalidade));
		}
		...
		if(idArgumentoPesquisa != null && !idArgumentoPesquisa.trim().equals("")){
			...
			argumentoPesquisa.setId(Util.obterInteger(idArgumentoPesquisa));
		}
		...
		if(idOperacaoPesquisa != null && !idOperacaoPesquisa.trim().equals("")){
			...
			operacaoPesquisa.setId(Util.obterInteger(idOperacaoPesquisa));
		}

		int indicadorAuditoria;
		if(Util.isInteger(indicadorAuditoriaString)){
			indicadorAuditoria = Integer.valueOf(indicadorAuditoriaString).intValue();
		}else{
			throw new ActionServletException("atencao.campo.informado", null, "Indicador de Auditoria");
		}
		...
		operacao.setOperacaoPesquisa(operacaoPesquisa);
		operacao.setIndicadorAuditoria(indicadorAuditoria);
		...
		montarPaginaSucesso(httpServletRequest, "Operação " + operacao.getId() + " atualizado com sucesso.",
						"Realizar outra Manutenção Operação", "exibirFiltrarOperacaoAction.do?menu=sim");
