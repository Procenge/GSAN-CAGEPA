 ...
 * @date 01/08/2006
 */
public class ExibirAtualizarOperacaoAction
				extends GcomAction {
	...
	public ActionForward execute(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest httpServletRequest,
		 ...
		 */
		if((objetoConsulta == null || objetoConsulta.equalsIgnoreCase("")) && (removerTabela == null || removerTabela.equalsIgnoreCase(""))
			...
			if(idOperacaoPesquisaDigitado != null && !idOperacaoPesquisaDigitado.trim().equalsIgnoreCase("")){
				...
				if(operacaoPesquisa != null){
					...
					if(operacaoPesquisa.getOperacaoTipo() != null){
						if(operacaoPesquisa.getOperacaoTipo().getId().intValue() != OperacaoTipo.PESQUISAR.intValue()){
		...
		/*
		 * Caso o usuário tenha apertado o botão de desfazer, retira as coleções de tipo de operação
		 * e
	...
	private TabelaColuna pesquisarArgumentoPesquisa(String idArgumentoPesquisa){
		...
		if(colecaoTabelaColuna != null && !colecaoTabelaColuna.isEmpty()){
			...
			if(argumentoPesquisa.getIndicadorPrimaryKey().intValue() == ConstantesSistema.NAO.intValue()){
		...
		Collection colecaoVerificacaoArgumentoPesquisa = Fachada.getInstancia().pesquisar(filtroOperacao, Operacao.class.getName());
	 ...
	 * @param httpServletRequest
	 */
	private void exibirOperacao(String idOperacao, AtualizarOperacaoActionForm atualizarOperacaoActionForm, Fachada fachada,
		...
		if(colecaoPesquisa == null || colecaoPesquisa.isEmpty()){
			...
			if(idOperacaoTipoForm != null && !idOperacaoTipoForm.trim().equalsIgnoreCase("-1")){
				idOperacaoTipo = Util.obterInteger(idOperacaoTipoForm);
				...
				operacaoTipoSelecionada = (OperacaoTipo) Util.retonarObjetoDeColecao(Fachada.getInstancia().pesquisar(
								filtroOperacaoTipoForm, OperacaoTipo.class.getName()));
			}
			...
			atualizarOperacaoActionForm.setIndicadorAuditoria("" + operacao.getIndicadorAuditoria());
			 ...
			 */
			if(idOperacaoTipo.intValue() == OperacaoTipo.PESQUISAR.intValue()){
				...
				if(operacaoTipoSelecionada.getIndicadorAtualiza() == ConstantesSistema.SIM){
					...
					if(operacao.getOperacaoPesquisa() != null){
						atualizarOperacaoActionForm.setIdOperacaoPesquisa(operacao.getOperacaoPesquisa().getId().toString());

						atualizarOperacaoActionForm.setDescricaoOperacaoPesquisa(operacao.getOperacaoPesquisa().getDescricao());
					}
