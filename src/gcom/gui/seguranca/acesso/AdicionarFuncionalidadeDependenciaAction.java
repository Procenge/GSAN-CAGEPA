 ...
 * @date 03/05/2006
 */
public class AdicionarFuncionalidadeDependenciaAction
				extends GcomAction {
	...
	public ActionForward execute(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest httpServletRequest,
		...
		String idFuncionalidade = adicionarFuncionalidadeDependenciaActionForm.getComp_id();

		Collection colecaoFuncionalidade = null;
		Collection colecaoFuncionalidadeDependencia = null;

		String funcionalidadeID = adicionarFuncionalidadeDependenciaActionForm.getFuncionalidadeID();

		if(sessao.getAttribute("funcionalidade").equals("inserir")){

			retorno = actionMapping.findForward("inserirFuncionalidadeDependenciaAction");
			...
			if(sessao.getAttribute("colecaoFuncionalidadeTela") != null){
				colecaoFuncionalidade = (Collection) sessao.getAttribute("colecaoFuncionalidadeTela");
			}else{
			...
			if(idFuncionalidade != null && !idFuncionalidade.equalsIgnoreCase("")){
				...
				if(colecaoFuncionalidade != null && !colecaoFuncionalidade.isEmpty()){
					...
					while(iterator.hasNext()){
						...
						if(funcionalidade.getId().equals(new Integer(idFuncionalidade))){
							// Esta funcionalidade ja foi informada
							throw new ActionServletException("atencao.funcionalidade.ja_informada");
						}
				...
				colecaoFuncionalidade = fachada.pesquisar(filtroFuncionalidade, Funcionalidade.class.getName());
			}
			...
			retorno = actionMapping.findForward("atualizarFuncionalidadeDependenciaAction");

			httpServletRequest.setAttribute("idFuncionalidade", funcionalidadeID);

			if(sessao.getAttribute("colecaoFuncionalidadeTela") != null){
				colecaoFuncionalidadeDependencia = (Collection) sessao.getAttribute("colecaoFuncionalidadeTela");
			}else{
				colecaoFuncionalidadeDependencia = new ArrayList();
			}

			Funcionalidade funcionalidadePrincipal = (Funcionalidade) sessao.getAttribute("funcionalidadeAtualizar");

			// testa se a funcionalidade ja foi adicionada
			if(idFuncionalidade != null && !idFuncionalidade.equalsIgnoreCase("")){

				if(funcionalidadePrincipal.getId().equals(new Integer(idFuncionalidade))){
					// Esta funcionalidade é igual a principal
					throw new ActionServletException("atencao.funcionalidade.igual_principal");
				}
				...
				if(colecaoFuncionalidadeDependencia != null && !colecaoFuncionalidadeDependencia.isEmpty()){
					...
					while(iterator.hasNext()){
						...
						if(funcionalidade.getComp_id() != null){
							idFuncionalidadeDependencia = funcionalidade.getComp_id().getFuncionalidadeDependenciaId();
						}else{
							idFuncionalidadeDependencia = funcionalidade.getFuncionalidadeDependencia().getId();
						}

						if(idFuncionalidadeDependencia.equals(new Integer(idFuncionalidade))){
							// Esta funcionalidade ja foi informada
							throw new ActionServletException("atencao.funcionalidade.ja_informada");
						}
