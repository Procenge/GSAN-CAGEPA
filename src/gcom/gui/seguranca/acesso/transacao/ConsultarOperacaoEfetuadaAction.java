...
public class ConsultarOperacaoEfetuadaAction
				extends GcomAction {
	...
	public ActionForward execute(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest httpServletRequest,
		...
		// coloca o id da operação efetuada na sessão para que seja recuperada na tela de exibição
		// da operação.
		sessao.setAttribute("idOperacaoEfetuada", form.getIdOperacaoEfetuada());
		...
		filtroOperacaoEfetuada.adicionarParametro(new ParametroSimples(FiltroOperacaoEfetuada.ID, form.getIdOperacaoEfetuada()));

		Collection coll = Fachada.getInstancia().pesquisar(filtroOperacaoEfetuada, OperacaoEfetuada.class.getName());
		...
		if(coll != null && !coll.isEmpty()){
			...
			Collection collOperacao = Fachada.getInstancia().pesquisar(filtroOperacao, Operacao.class.getName());
			...
			filtroUsuarioAlteracao.adicionarParametro(new ParametroSimples(FiltroUsuarioAlteracao.OPERACAO_EFETUADA_ID, form
							.getIdOperacaoEfetuada()));
			...
			filtroTabelaLinhaColunaAlteracao.adicionarParametro(new ParametroSimples(FiltroTabelaLinhaColunaAlteracao.OPERACAO_EFETUADA_ID,
							form.getIdOperacaoEfetuada()));
			filtroTabelaLinhaColunaAlteracao.adicionarParametro(new ParametroSimples(FiltroTabelaLinhaColunaAlteracao.ALTERACAO_TIPO_ID,
	...
	/**
	 * No sistema, quando se tem alterações em coleções, o que está sendo feito é exclusão de todos
	 * e inclusão dos novos,
	 * então para ocultar tal ação, esse método faz um join das remoções e inclusoes, afim de
	 * aparentar ter
	 * sido uma alteração.
	 * 
	 * @param inclusoes
	...
	private Collection juntarInclusaoExclusaoMesmoTipo(Collection inclusoes, Collection remocoes){
		...
		// para cada junção montada no laço anterior, vamos setar o valor anterior na linha de
		// inclusão
			...
			for(Iterator iter2 = inclusoes.iterator(); iter2.hasNext();){
				...
				for(Iterator iter3 = remocoes.iterator(); iter3.hasNext();){
					...
					if(tlcaRem.getTabelaLinhaAlteracao().getId().equals(idTLARem)
						...
						// para cada idem juntado, removemos os respectivos da coleção de inclusão e
						// da de remoção
						iter2.remove();
						iter3.remove();

						// como foi efetuada uma remoção e uma inclusão, não tinha como saber se os
						// valores foram modificados
						// ou seja, pode acontecer de os conteúdos permanecerem o mesmo, neste caso
						// não vamos exibir
						// esta flag 'houveAlgumaAlteracao' servirá para identificar se houve algum
						// campo que foi modificado
						// e neste caso o conjunto de linhas (linha de identificação e linhas
						// alteradas) serão
		...
		// estas adições serão as linhas que efetivamente foram de inclusão, ou seja, não havia
		// exclusão associada a eles
		// da mesma forma, serao adicionadas as linhas de remoção que não tinha inclusões
		// associadas.
		linhasJuntadas.addAll(inclusoes);
		linhasJuntadas.addAll(remocoes);

		int[] idsTabelaColuna = new int[linhasJuntadas.size()];
	...
	// if (obj1 instanceof TabelaLinhaColunaAlteracao && obj2 instanceof
