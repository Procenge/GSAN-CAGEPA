...
public class ConsultarAuditoriaRetificacaoImplantacaoContasAction
				extends GcomAction {

	public ActionForward execute(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest httpServletRequest,
					HttpServletResponse httpServletResponse){

		ActionForward retorno = actionMapping.findForward("consultarAuditoriaRetificacaoImplantacaoContas");

		ConsultarAuditoriaRetificacaoImplantacaoContasActionForm form = (ConsultarAuditoriaRetificacaoImplantacaoContasActionForm) actionForm;
		...
		if(dataInicial != null && !dataInicial.equals("") && dataFinal != null && !dataFinal.equals("")){
			Date dateInicial = Util.converterStringParaData(dataInicial);
			Date dateFinal = Util.converterStringParaData(dataFinal);
		...
		// FS0006 - Verificar período de retificação/implantação de contas
		if(dataInicial != null && !dataInicial.equals("")){
			Date dateInicial = Util.converterStringParaData(dataInicial);
			Date date = new Date();
		...
		if(idUsuario != null && !idUsuario.equals("")){
			colecao = (Collection) fachada
							.pesquisarRetificacaoImplantacaoContaUsuario(new Integer(idUsuario), dataInicialInt,
							dataFinalInt);
		}

		// Parte do código que trata se foi escolhido o funcionário
		if(idFuncionario != null && !idFuncionario.equals("")){
			colecao = (Collection) fachada.pesquisarRetificacaoImplantacaoContaFuncionario(new Integer(idFuncionario),
							dataInicialInt, dataFinalInt);
		}
