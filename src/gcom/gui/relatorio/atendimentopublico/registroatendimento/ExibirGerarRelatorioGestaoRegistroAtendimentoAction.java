 ...
 * @date 25/03/2011
 */
public class ExibirGerarRelatorioGestaoRegistroAtendimentoAction
				extends GcomAction {

	public ActionForward execute(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest httpServletRequest,
					HttpServletResponse httpServletResponse){

		ActionForward retorno = actionMapping.findForward("exibirGerarRelatorioGestaoRegistroAtendimento");
		Fachada fachada = Fachada.getInstancia();
		HttpSession sessao = httpServletRequest.getSession(false);

		GerarRelatorioGestaoRegistroAtendimentoActionForm formulario = (GerarRelatorioGestaoRegistroAtendimentoActionForm) actionForm;
		...
		if(formulario.getTipoSolicitacao() != null && formulario.getTipoSolicitacao().length > 0){
			...
			getSolicitacaoTipoEspecificacao(sessao, fachada, formulario);
		}else{

			formulario.setSelectedTipoSolicitacaoSize("0");
		}

		// Unidade de Atendimento/Atual
		getUnidade(formulario, httpServletRequest, 1);

		// Unidade Superior
		if(formulario.getUnidadeSuperiorId() != null && !formulario.getUnidadeSuperiorId().equals("")){

			getUnidade(formulario, httpServletRequest, 2);
		}
	...
	private void getSolicitacaoTipoEspecificacao(HttpSession sessao, Fachada fachada,
					GerarRelatorioGestaoRegistroAtendimentoActionForm formulario){
	 ...
	 * @param tipo
	 */
	private void getUnidade(GerarRelatorioGestaoRegistroAtendimentoActionForm formulario, HttpServletRequest httpServletRequest, int tipo){
		...
		if(tipo == 1){
			...
			unidadeOrganizacional.setId(Integer.valueOf(formulario.getUnidadeSuperiorId()));
