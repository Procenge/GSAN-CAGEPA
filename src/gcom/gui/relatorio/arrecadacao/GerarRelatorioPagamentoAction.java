 ...
 */
public class GerarRelatorioPagamentoAction
	...
	public ActionForward execute(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest httpServletRequest,
		...
		if(sessao.getAttribute("idImovelPrincipalAba") != null && !sessao.getAttribute("idImovelPrincipalAba").equals("")){
			...
			idsDebitoTipo = consultarPagamentoActionForm.getDebitoTipo();
			idsCategoria = consultarPagamentoActionForm.getCategoria();
		...
		if(periodoPagamentoInicial != null && !periodoPagamentoInicial.equals("")){
			pagamentoParametrosInicial.setAnoMesReferenciaPagamento(Util.formatarMesAnoComBarraParaAnoMes(periodoPagamentoInicial));
			pagamentoParametrosFinal.setAnoMesReferenciaPagamento(Util.formatarMesAnoComBarraParaAnoMes(periodoPagamentoFinal));
		}
		if(dataPagamentoInicial != null && !dataPagamentoInicial.equals("")){
			pagamentoParametrosInicial.setDataPagamento(Util.converteStringParaDate(dataPagamentoInicial));
			pagamentoParametrosFinal.setDataPagamento(Util.converteStringParaDate(dataPagamentoFinal));
		}
		...
		RelatorioPagamento relatorioPagamento = null;

		if(indicadorTotalizarPorDataPagamento != null && indicadorTotalizarPorDataPagamento.equals(ConstantesSistema.SIM.toString())){
			relatorioPagamento = new RelatorioPagamento(usuarioLogado, ConstantesRelatorios.RELATORIO_PAGAMENTO_TOTALIZADO_POR_DATA);
		}else{
			relatorioPagamento = new RelatorioPagamento(usuarioLogado, ConstantesRelatorios.RELATORIO_PAGAMENTO);
		}
		...
		relatorioPagamento.addParametro("tipoFormatoRelatorio", Integer.parseInt(tipoRelatorio));

		String relatorioTipo = httpServletRequest.getParameter("relatorioTipo");

		if(Util.isVazioOuBranco(relatorioTipo)){
			// Opção Padrão: Ambos
			relatorioTipo = "3";
		}

		relatorioPagamento.addParametro("relatorioTipo", Integer.parseInt(relatorioTipo));

		try{
			retorno = processarExibicaoRelatorio(relatorioPagamento, tipoRelatorio, httpServletRequest, httpServletResponse, actionMapping);
