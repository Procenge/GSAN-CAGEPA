...
public class GerarRelatorioAvisoBancarioManterAction
	...
	public ActionForward execute(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest httpServletRequest,
		...
		String codigoBancoArrecadador = filtrarAvisoBancarioActionForm.getCodigoBanco();
		String codigoRemessa = filtrarAvisoBancarioActionForm.getCodigoRemessa();
		String identificacaoServico = filtrarAvisoBancarioActionForm.getIdentificacaoServico();
		String numeroSequencial = filtrarAvisoBancarioActionForm.getNumeroSequencial();
		...
		if((arrecadadorCodAgente != null) && (!arrecadadorCodAgente.trim().equals(""))){
			...
			filtroArrecadador.adicionarParametro(new ParametroSimples(FiltroArrecadador.CODIGO_AGENTE, arrecadadorCodAgente));
		...
		if((dataLancamentoInicioString != null) && (!dataLancamentoInicioString.equals(""))){
			...
			dataLancamentoInicio = Util.converteStringParaDate(dataLancamentoInicioString);
			dataLancamentoFim = Util.converteStringParaDate(dataLancamentoFimString);
			...
			avisoBancarioHelperParametros.setDataLancamentoInicial(dataLancamentoInicio);
			avisoBancarioHelperParametros.setDataLancamentoFinal(dataLancamentoFim);
		}
		...
		if((idMovimento != null) && (!idMovimento.trim().equals(""))){
			...
			arrecadadorMovimentoParametros.setCodigoBanco(new Short(codigoBancoArrecadador));
			arrecadadorMovimentoParametros.setCodigoRemessa(new Short(codigoRemessa));
			arrecadadorMovimentoParametros.setDescricaoIdentificacaoServico(identificacaoServico);
			arrecadadorMovimentoParametros.setNumeroSequencialArquivo(new Integer(numeroSequencial));
		...
		if((dataPrevisaoCreditoDebitoInicioString != null) && (!dataPrevisaoCreditoDebitoInicioString.trim().equals(""))){
			...
			avisoBancarioHelperParametros.setDataPrevistaInicial(dataPrevisaoCreditoDebitoInicio);
			avisoBancarioHelperParametros.setDataPrevistaFinal(dataPrevisaoCreditoDebitoFim);
		}
		...
		if((dataRealizacaoCreditoDebitoInicioString != null) && (!dataRealizacaoCreditoDebitoInicioString.trim().equals(""))){
			...
			dataRealizacaoCreditoDebitoInicio = Util.converteStringParaDate(dataRealizacaoCreditoDebitoInicioString);
			dataRealizacaoCreditoDebitoFim = Util.converteStringParaDate(dataRealizacaoCreditoDebitoFimString);
			...
			avisoBancarioHelperParametros.setDataRealizadaInicial(dataRealizacaoCreditoDebitoInicio);
			avisoBancarioHelperParametros.setDataRealizadaFinal(dataRealizacaoCreditoDebitoFim);
		}
		...
		if((periodoArrecadacaoInicio != null) && (!periodoArrecadacaoInicio.trim().equals(""))){
			...
			avisoBancarioHelperParametros.setAnoMesReferenciaArrecadacaoInicial(periodoArrecadacaoInicioTratado);
			avisoBancarioHelperParametros.setAnoMesReferenciaArrecadacaoFinal(periodoArrecadacaoFimTratado);
		}
		...
		if((intervaloValorRealizadoInicio != null) && (!intervaloValorRealizadoInicio.trim().equals(""))){
			...
			avisoBancarioHelperParametros.setValorRealizadoInicial(Util.formatarMoedaRealparaBigDecimal(intervaloValorRealizadoInicio));
			avisoBancarioHelperParametros.setValorRealizadoFinal(Util.formatarMoedaRealparaBigDecimal(intervaloValorRealizadoFim));
		}

		// Aviso Abertos/Fechados

		String avisosAbertosFechadosParametros = "";
		if(avisoAbertoFechado != null && (!avisoAbertoFechado.equals("-1"))){
			if(avisoAbertoFechado.equals("2") || avisoAbertoFechado.equals("3")){
				avisoBancarioHelper.setIndicadorAbertoFechado(Short.valueOf(avisoAbertoFechado));
				avisosAbertosFechadosParametros = avisoAbertoFechado;
			}
		}

		Collection colecaoAvisosBancarios = fachada.pesquisarAvisoBancarioRelatorio(avisoBancarioHelper);

		// cria uma instância da classe do relatório
		RelatorioManterAvisoBancario relatorioManterAvisoBancario = new RelatorioManterAvisoBancario((Usuario) (httpServletRequest
						.getSession(false)).getAttribute("usuarioLogado"));

		if(!Util.isVazioOrNulo(colecaoAvisosBancarios)){

			relatorioManterAvisoBancario.addParametro("totalRegistrosRelatorio", colecaoAvisosBancarios.size());
		}else{

			relatorioManterAvisoBancario.addParametro("totalRegistrosRelatorio", 0);
		}

		relatorioManterAvisoBancario.addParametro("avisoBancarioHelperParametros", avisoBancarioHelperParametros);
		relatorioManterAvisoBancario.addParametro("arrecadadorParametros", arrecadadorParametros);
		relatorioManterAvisoBancario.addParametro("contaBancariaParametros", contaBancariaParametros);
		relatorioManterAvisoBancario.addParametro("arrecadadorMovimentoParametros", arrecadadorMovimentoParametros);
		relatorioManterAvisoBancario.addParametro("avisosAbertosFechadosParametros", avisosAbertosFechadosParametros);
		relatorioManterAvisoBancario.addParametro("colecaoAvisosBancarios", colecaoAvisosBancarios);
		...
		relatorioManterAvisoBancario.addParametro("tipoFormatoRelatorio", Integer.parseInt(tipoRelatorio));
		try{
			retorno = processarExibicaoRelatorio(relatorioManterAvisoBancario, tipoRelatorio, httpServletRequest, httpServletResponse,
							actionMapping);
