...
public class GerarRelatorioResumoArrecadacaoAction
	...
	public ActionForward execute(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest httpServletRequest,
		...
		if(Util.isVazioOuBranco(mesAno)){
			throw new ActionServletException("atencao.required", null, "Mês/Ano da Arrecadação");
		}else if(!Util.validarMesAno(mesAno)){
			throw new ActionServletException("atencao.mes_ano_invalido");
		}else if(Util.compararAnoMesReferencia(Util.obterInteger(Util.formatarMesAnoParaAnoMes(mesAno)), getFachada()
						.pesquisarParametrosDoSistema().getAnoMesArrecadacao(), ">")){
			throw new ActionServletException("atencao.mes_ano_inferior", null, "Mês/Ano da Arrecadação deve ser inferior a "
							+ Util.formatarAnoMesSemBarraParaMesAnoComBarra(getFachada().pesquisarParametrosDoSistema()
		...
		if(opcaoTotalizacao.trim().equals("localidade")){
			...
			if(!Util.isVazioOuBranco(codigoSetorComercial)){
				SetorComercial setorComercial = pesquisarSetorComercial(codigoSetorComercial, codigoLocalidade, httpServletRequest);
		...
		RelatorioResumoArrecadacao relatorioResumoArrecadacao = new RelatorioResumoArrecadacao((Usuario) (httpServletRequest
		...
		relatorioResumoArrecadacao.addParametro("idSetorComercial", idSetorComercial);
		...
		relatorioResumoArrecadacao.addParametro("tipoFormatoRelatorio", Integer.parseInt(tipoRelatorio));

		retorno = processarExibicaoRelatorio(relatorioResumoArrecadacao, tipoRelatorio, httpServletRequest, httpServletResponse,
						actionMapping);
	...
	private SetorComercial pesquisarSetorComercial(String codigoSetorComercial, String idLocalidade, HttpServletRequest httpServletRequest){
		...
		if(Util.isVazioOrNulo(setorComercialPesquisado)){
			...
			retorno = (SetorComercial) Util.retonarObjetoDeColecao(setorComercialPesquisado);
		}
