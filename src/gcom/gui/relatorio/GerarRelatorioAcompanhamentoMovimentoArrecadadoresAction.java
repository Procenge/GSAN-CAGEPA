 ...
 */
public class GerarRelatorioAcompanhamentoMovimentoArrecadadoresAction
	...
	public ActionForward execute(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest httpServletRequest,
		...
		if(idArrecadador != null && !idArrecadador.equals("")){
			...
			if(colecaoArrecadadores != null && !colecaoArrecadadores.isEmpty()){
				...
				throw new ActionServletException("atencao.pesquisa_inexistente", null, "Arrecadador");
			}
		...
		if(idArrecadacaoForma != null && !idArrecadacaoForma.equals("")
						&& !idArrecadacaoForma.equals("" + ConstantesSistema.NUMERO_NAO_INFORMADO)){
			FiltroArrecadacaoForma filtroArrecadacaoForma = new FiltroArrecadacaoForma();

			filtroArrecadacaoForma.adicionarParametro(new ParametroSimples(FiltroArrecadacaoForma.CODIGO, idArrecadacaoForma));
			...
			if(colecaoArrecadacaoForma != null && !colecaoArrecadacaoForma.isEmpty()){
				...
				throw new ActionServletException("atencao.pesquisa_inexistente", null, "Forma de Arrecadação");
			}
		...
		String idConcessionaria = acompanharMovimentoArrecadadoresActionForm.getIdConcessionaria();

		Concessionaria concessionaria = null;
		if(idConcessionaria != null && !idConcessionaria.equals("")){
			if(!idConcessionaria.equals("-1")){
				...
				Collection<Concessionaria> colecaoConcessionarias = fachada.pesquisar(filtroConcessionaria, Concessionaria.class.getName());

				if(colecaoConcessionarias != null && !colecaoConcessionarias.isEmpty()){
					concessionaria = (Concessionaria) Util.retonarObjetoDeColecao(colecaoConcessionarias);
				}else{
					throw new ActionServletException("atencao.pesquisa_inexistente", null, "Concessionária");
				}
			}else{
		...
		fachada.gerarResumoAcompanhamentoMovimentoArrecadadores(
						(Usuario) (httpServletRequest.getSession(false)).getAttribute("usuarioLogado"), mesAnoReferencia, arrecadador,
						arrecadacaoForma, concessionaria);

		EnvioEmail email = fachada.pesquisarEnvioEmail(EnvioEmail.GERAR_RELATORIO_ACOMPANHAMENTO_MOVIMENTO_ARRECADADORES);
		...
		retorno = processarExibicaoRelatorio(relatorioAcompanhamentoMovimentoArrecadadores, tipoRelatorio, httpServletRequest,
		...
		// montarPaginaSucesso(
		// httpServletRequest,
		// "Movimento dos Arrecadadores enviado para processamento. O resultado será enviado para o email "
		// + email.getEmailReceptor()
		// + ".", "Voltar",
