...
public class GerarDadosParaLeituraAction
	...
	public ActionForward execute(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest httpServletRequest,
		...
		Collection colecaoGerarDadosParaLeituraHelper = this.gerarColecaoDadosParaLeituraHelper(items);

		String tipoRelatorio = httpServletRequest.getParameter("tipoRelatorio");

		RelatorioGerarDadosParaleitura relatorio = new RelatorioGerarDadosParaleitura(this.getUsuarioLogado(httpServletRequest));

		relatorio.addParametro("colecaoGerarDadosParaLeituraHelper", colecaoGerarDadosParaLeituraHelper);
		...
		try{
			...
			while(iter.hasNext()){
				...
				if(!item.isFormField()){
					 ...
					 */
					identificacaoArquivo = nomeItem.substring(nomeItem.length() - 6, nomeItem.length() - 4);

					// compara o final do nome do arquivo é .txt
					if(nomeItem.endsWith(".TXT") || identificacaoArquivo.equals("RT")){
						...
						while(!eof){
							...
							if(linhaLida == null || linhaLida.trim().length() == 0){
								...
								if(primeiraLinha){
									...
									// MATRICULA IMOVEL
									matriculaImovel = Util.converterStringParaInteger(linhaLida2.substring(0, 7));
									...
									gerarDadosParaLeituraHelper
													.setAnoMesReferncia(Util.formatarAnoMesParaMesAno(linhaLida.substring(0, 6)));
									if(grupo != null){
										gerarDadosParaLeituraHelper.setGrupo(grupo);
									}

									try{
										Date dataPrevista = fachada.pesquisarFaturamentoAtividadeCronogramaDataPrevista(Integer
														.parseInt(grupo), FaturamentoAtividade.EFETUAR_LEITURA, Integer.parseInt(linhaLida
														.substring(0, 6)));

										gerarDadosParaLeituraHelper.setDataPrevistaFaturamento(Util.formatarData(dataPrevista));
								...
								// MATRICULA IMOVEL
								matriculaImovel = Util.converterStringParaInteger(linhaLida.substring(0, 7));
								...
								if(retorno.size() == 1){
									...
									if(matriculaImovel != null){
										Object[] dadosLocalidadeSetor = fachada.pesquisarLocalidadeSetorImovel(matriculaImovel);
										if(dadosLocalidadeSetor != null){
											// descrição da localidade
											if(dadosLocalidadeSetor[0] != null){
												gerarDadosParaLeituraHelperCadastrado.setDescricaoLocalidade("" + dadosLocalidadeSetor[0]);
											}
											// codigo do setor
											if(dadosLocalidadeSetor[1] != null){
												gerarDadosParaLeituraHelperCadastrado.setCodigoSetor("" + dadosLocalidadeSetor[1]);
											}
											Integer codigoRota = Util.converterStringParaInteger(linhaLida.substring(7, 11));
											gerarDadosParaLeituraHelperCadastrado.setCodigoRota("" + codigoRota);
										}
									}
								...
								// matricula do imovel
								if(matriculaImovel != null){
									gerarDadosParaLeituraHelper.setMatriculaImovel(Util.retornaMatriculaImovelFormatada(matriculaImovel));
								}

								Integer sequencialRota = fachada.pesquisarSequencialRota(matriculaImovel);

								// sequencial da rota
								if(sequencialRota != null){
									gerarDadosParaLeituraHelper.setSequencialRota("" + sequencialRota);
								}

								// nome do usuário
								String nomeClienteUsuario = fachada.pesquisarNomeClientePorImovel(matriculaImovel);
								...
								try{
									enderecoImovel = fachada.pesquisarEnderecoAbreviadoCAER(matriculaImovel);
								}catch(ControladorException e){
									e.printStackTrace();
								}
								if(enderecoImovel != null){
									gerarDadosParaLeituraHelper.setEnderecoImovel(enderecoImovel);
								}

								// numero do hidrometro
								gerarDadosParaLeituraHelper.setNumeroHidrometro(linhaLida.substring(58, 68));

								// descrição do local de instalação e proteção
								Object[] dadosLocalProtecaoAgua = fachada.pesquisarDadosLocalProtecaoTipoLigacaoAgua(matriculaImovel);
								if(dadosLocalProtecaoAgua != null){
									...
									Object[] dadosLocalProtecaPoco = fachada.pesquisarDadosLocalProtecaoTipoTipoPoco(matriculaImovel);
								...
								if(inscricaoImovel != null){
									gerarDadosParaLeituraHelper.setInscricao(inscricaoImovel);
								}
