...
import gcom.relatorio.micromedicao.RelatorioAnaliseConsumoExcel;
...
public class GerarRelatorioAnaliseConsumoExcelAction
	...
	public ActionForward execute(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest httpServletRequest,
		...
		try{
			...
			String relativeWebPath = "xls/relatorioAnaliseConsumoTemplate.xls";
			String absoluteDiskPath = servletContext.getRealPath(relativeWebPath);

			RelatorioAnaliseConsumoExcel relatorio = new RelatorioAnaliseConsumoExcel((Usuario) (httpServletRequest.getSession(false))
							.getAttribute("usuarioLogado"));
			File template = new File(absoluteDiskPath);

			relatorio.addParametro("tipoFormatoRelatorio", Integer.valueOf(TarefaRelatorio.TIPO_XLS));
			relatorio.addParametro("mesAnoFaturamento", Util.formatarAnoMesParaMesAno(sistemaParametro.getAnoMesFaturamento().intValue()));
			relatorio.addParametro("mesAnoArrecadacao", Util.formatarAnoMesParaMesAno(sistemaParametro.getAnoMesArrecadacao().intValue()));
			relatorio.addParametro("grupo", faturamentoGrupo.getDescricao());
			relatorio.addParametro("template", template);

			relatorio.addParametro("idsImovel", valoresIdsImovel);

			if(relatorio.retornarTotalRegistrosRelatorio() <= 0){
