...
public class GerarRelatorioRotaManterAction
	...
	public ActionForward execute(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest httpServletRequest,
		...
		FiltroRota filtroRota = (FiltroRota) sessao.getAttribute("filtroRota");
		...
		if(idSetorComercial != null && !idSetorComercial.equals("")){
			...
			if(colecaoSetoresComerciais != null && !colecaoSetoresComerciais.isEmpty()){
				...
				Iterator colecaoSetoresComerciaisIterator = colecaoSetoresComerciais.iterator();

				setorComercial = (SetorComercial) colecaoSetoresComerciaisIterator.next();
		...
		if(idGrupoFaturamento != null && !idGrupoFaturamento.equals("")){
			...
			Collection colecaoFaturamentosGrupos = fachada.pesquisar(filtroFaturamentoGrupo, FaturamentoGrupo.class.getName());

			if(colecaoFaturamentosGrupos != null && !colecaoFaturamentosGrupos.isEmpty()){
				// Titulo do Logradouro Foi Encontrado
				Iterator colecaoFaturamentosGruposIterator = colecaoFaturamentosGrupos.iterator();

				faturamentoGrupo = (FaturamentoGrupo) colecaoFaturamentosGruposIterator.next();
		...
		if(rotaActionForm.getIndicadorUso() != null && !rotaActionForm.getIndicadorUso().equals("")){

			indicadorDeUso = new Short("" + rotaActionForm.getIndicadorUso());
		}

		Leiturista leiturista = null;
		if(rotaActionForm.getCodigoLeiturista() != null && !rotaActionForm.getCodigoLeiturista().trim().equalsIgnoreCase("")){
			leiturista = new Leiturista();
			leiturista.setId(new Integer(rotaActionForm.getCodigoLeiturista()));
		}
		...
		rotaParametros.setLeiturista(leiturista);

		// Fim da parte que vai mandar os parametros para o relatório

		// cria uma instância da classe do relatório
		RelatorioManterRota relatorioManterRota = new RelatorioManterRota((Usuario) (httpServletRequest.getSession(false))
						.getAttribute("usuarioLogado"));
		relatorioManterRota.addParametro("filtroRota", filtroRota);
		relatorioManterRota.addParametro("rotaParametros", rotaParametros);
		...
		relatorioManterRota.addParametro("tipoFormatoRelatorio", Integer.parseInt(tipoRelatorio));
		try{
			retorno = processarExibicaoRelatorio(relatorioManterRota, tipoRelatorio, httpServletRequest, httpServletResponse, actionMapping);
