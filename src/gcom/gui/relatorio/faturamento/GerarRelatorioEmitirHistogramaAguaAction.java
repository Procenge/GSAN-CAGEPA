...
public class GerarRelatorioEmitirHistogramaAguaAction
	...
	public ActionForward execute(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest httpServletRequest,
		...
		if(form.getQuadra() != null && !form.getQuadra().equals("")){
			...
			if(informouSetor){
				...
				filtroQuadra.adicionarParametro(new ParametroSimples(FiltroQuadra.ID_SETORCOMERCIAL, filtro.getSetorComercial().getId()));
		...
		// Indicador Medicao
		if(form.getMedicao() != null && !form.getMedicao().equals("")){
		...
		// Indicador Volume
		if(form.getVolumoFixoAgua() != null && !form.getVolumoFixoAgua().equals("")){
		...
		if(filtro.getConsumo() != null){
			...
			if(filtro.getConsumo().shortValue() == ConstantesSistema.INDICADOR_USO_ATIVO.shortValue()){
				...
				if(form.getColecaoConsumoFaixaLigacaoMedido() != null && !form.getColecaoConsumoFaixaLigacaoMedido().isEmpty()){
			...
			if(filtro.getConsumo().shortValue() == ConstantesSistema.INDICADOR_USO_DESATIVO.shortValue()){
				...
				if(form.getColecaoConsumoFaixaLigacaoNaoMedido() != null && !form.getColecaoConsumoFaixaLigacaoNaoMedido().isEmpty()){
		...
		// valida se existem dados para geracao do relatorio
		FiltrarEmitirHistogramaAguaHelper filtroClone = null;
		try{
			filtroClone = (FiltrarEmitirHistogramaAguaHelper) BeanUtils.cloneBean(filtro);
		}catch(Exception e){
			throw new ActionServletException("atencao.relatorio.erro");
		}
		Collection<EmitirHistogramaAguaHelper> colecao = Fachada.getInstancia().pesquisarEmitirHistogramaAgua(filtro);
		...
		RelatorioHistogramaAguaLigacao relatorio = new RelatorioHistogramaAguaLigacao(this.getUsuarioLogado(httpServletRequest));

		relatorio.addParametro("filtrarEmitirHistogramaAguaHelper", filtroClone);
