...
public class GerarRelatorioEmitirHistogramaEsgotoAction
	...
	public ActionForward execute(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest httpServletRequest,
		...
		if(form.getPerfilLigacaoEsgoto() != null && form.getPerfilLigacaoEsgoto().length > 0){
			...
			for(int i = 0; i < array.length; i++){
				...
				if(new Short(array[i]).intValue() != ConstantesSistema.NUMERO_NAO_INFORMADO){

					colecao.add(new Short(array[i]));
				}
			}
		...
		Collection<EmitirHistogramaEsgotoHelper> colecao = Fachada.getInstancia().pesquisarEmitirHistogramaEsgoto(filtro);
		...
		RelatorioHistogramaEsgotoLigacao relatorio = new RelatorioHistogramaEsgotoLigacao(this.getUsuarioLogado(httpServletRequest));

		relatorio.addParametro("filtrarEmitirHistogramaEsgotoHelper", filtro);

		relatorio.addParametro("colecao", colecao);
