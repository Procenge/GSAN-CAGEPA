 ...
 */
public class ExibirFiltrarRelatorioAnaliticoFaturamentoAction
				extends GcomAction {
	...
	public ActionForward execute(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest httpServletRequest,
		...
		ActionForward retorno = actionMapping.findForward("exibirFiltrarRelatorioAnaliticoFaturamento");
		...
		Integer anoMesTeste = (Integer) sessao.getAttribute("anoMes");
		...
		if(form != null && form.getIdGrupoFaturamento() != null && !form.getIdGrupoFaturamento().equalsIgnoreCase("-1")){
			...
			filtroFaturamentoGrupo.adicionarParametro(new ParametroSimples(FiltroFaturamentoGrupo.ID, form.getIdGrupoFaturamento()));
			...
			if(sessao.getAttribute("faturamentoGrupos") == null){
				...
				form.setIndicadorLocalidadeInformatizada("3");
			}
		}
		...
		if(adicionar != null && !adicionar.trim().equals("")){
			if(adicionar.trim().equalsIgnoreCase("localidade") && form.getLocalidadeFiltro() != null
							&& !form.getLocalidadeFiltro().equals("")){
			...
			}else if(adicionar.trim().equalsIgnoreCase("setor") && form.getSetorComercialFiltro() != null
				...
				if(form.getSetorComercialID() != null && !form.getSetorComercialID().trim().equals("")){
					setorComercial.setId(new Integer(form.getSetorComercialID()));
				}
				...
				if(form.getQuadraID() != null && !form.getQuadraID().trim().equals("") && form.getQuadraFiltro() != null
								&& !form.getQuadraFiltro().trim().equals("")){
	...
	private void pesquisarLocalidade(RelatorioAnaliticoFaturamentoActionForm form, Fachada fachada, HttpServletRequest httpServletRequest){
		...
		if(localidadeID != null && !localidadeID.equals("") && (nomeLocalidade == null || nomeLocalidade.equals(""))){
			...
			if(colecaoPesquisa == null || colecaoPesquisa.isEmpty()){
				...
				form.setLocalidadeFiltro(String.valueOf(objetoLocalidade.getId()));
	...
	private void pesquisarSetorComercial(RelatorioAnaliticoFaturamentoActionForm form, Fachada fachada,
					HttpServletRequest httpServletRequest){

		if(Util.isVazioOrNulo((Collection) httpServletRequest.getSession().getAttribute("colecaoLocalidades"))){
		...
		setorComercialCD = (String) form.getSetorComercialFiltro();
		String nomeSetorComercial = form.getSetorComercialNome();
		...
		if(setorComercialCD != null && !setorComercialCD.trim().equalsIgnoreCase("")
			...
			if(colecaoPesquisa == null || colecaoPesquisa.isEmpty()){
				...
				form.setSetorComercialFiltro(String.valueOf(objetoSetorComercial.getCodigo()));
	...
	// PESQUISA DE QUADRA
	private void pesquisarQuadra(RelatorioAnaliticoFaturamentoActionForm form, Fachada fachada, HttpServletRequest httpServletRequest){
		...
		if(colecaoSetor != null && !colecaoSetor.isEmpty()){
			setorID = "" + ((SetorComercial) colecaoSetor.iterator().next()).getId();
		}
		...
		// obrigatórios
		if(quadraNM != null && !quadraNM.trim().equalsIgnoreCase("") && (nomeQuadra == null || nomeQuadra.equals(""))){

			if(setorID != null && !setorID.trim().equalsIgnoreCase("")){
				// Adiciona o id do setor comercial que está no formulário
				// para compor a pesquisa.
				filtroQuadra.adicionarParametro(new ParametroSimples(FiltroQuadra.ID_SETORCOMERCIAL, setorID));
			}else{
			...
			if(colecaoPesquisa == null || colecaoPesquisa.isEmpty()){
				...
				form.setQuadraFiltro(String.valueOf(objetoQuadra.getNumeroQuadra()));
				form.setQuadraID(String.valueOf(objetoQuadra.getId()));
				// Mensagem de tela
