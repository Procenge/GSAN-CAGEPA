...
public class GerarRelatorioClienteManterAction
	...
	public ActionForward execute(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest httpServletRequest,
		...
		FiltroCliente filtroCliente = (FiltroCliente) sessao.getAttribute("filtroCliente");
		...
		if(sessao.getAttribute("clienteOutrosCriterios") == null || sessao.getAttribute("clienteOutrosCriterios").equals("")){
			...
			String cpf = null;
			String cpfPesquisado = (String) filtrarClienteActionForm.getCpfClienteFiltro();
			...
			String cnpj = null;
			String cnpjPesquisado = (String) filtrarClienteActionForm.getCnpjClienteFiltro();
			...
			String codigo = null;
			String codigoPesquisado = (String) filtrarClienteActionForm.getCodigoClienteFiltro();
			...
			String nome = null;
			String nomePesquisado = (String) filtrarClienteActionForm.getNomeClienteFiltro();
			...
			String cepPesquisado = (String) filtrarClienteActionForm.getCepClienteFiltro();
			...
			String idMunicipio = (String) filtrarClienteActionForm.getMunicipioClienteFiltro();
			...
			String idBairro = (String) filtrarClienteActionForm.getBairroClienteFiltro();
			...
			if(filtrarClienteActionForm.getIndicadorUsoClienteFiltro() != null
							&& !filtrarClienteActionForm.getIndicadorUsoClienteFiltro().equals("")){

				indicadorDeUso = new Short("" + filtrarClienteActionForm.getIndicadorUsoClienteFiltro());
				// filtroCliente.adicionarParametro(new ParametroSimples(
			...
			clienteParametros.setId(codigo == null ? null : new Integer(codigo));
			...
			if(bairro.getId() != null && logradouro.getId() != null){
				...
				logradouroBairro = fachada.pesquisarAssociacaoLogradouroBairro(bairro.getId(), logradouro.getId());
			...
			String codigo = null;
			String codigoPesquisado = (String) pesquisarActionForm.get("idCliente");
			...
			String nome = null;
			String nomePesquisado = (String) pesquisarActionForm.get("nomeCliente");
			...
			String nomeAbreviadoPesquisado = (String) pesquisarActionForm.get("nomeAbreviadoCliente");

			if(nomeAbreviadoPesquisado != null && !nomeAbreviadoPesquisado.equals("")){
			...
			String idClienteTipo = (String) pesquisarActionForm.get("tipoCliente");
			...
			if(idClienteTipo != null && !idClienteTipo.equals("" + ConstantesSistema.NUMERO_NAO_INFORMADO) && !idClienteTipo.equals("")){
				...
				Collection clientesTipos = fachada.pesquisar(filtroClienteTipo, ClienteTipo.class.getName());
			...
			String idRamoAtividade = (String) pesquisarActionForm.get("ramoAtividade");

			RamoAtividade ramoAtividade = null;

			if(idRamoAtividade != null && !idRamoAtividade.equals("" + ConstantesSistema.NUMERO_NAO_INFORMADO)
							&& !idRamoAtividade.equals("")){
				FiltroRamoAtividade filtroRamoAtividade = new FiltroRamoAtividade();

				filtroRamoAtividade.adicionarParametro(new ParametroSimples(FiltroRamoAtividade.ID, idRamoAtividade));
				filtroRamoAtividade.adicionarParametro(new ParametroSimples(FiltroRamoAtividade.INDICADOR_USO,
								ConstantesSistema.INDICADOR_USO_ATIVO));

				Collection ramosAtividades = fachada.pesquisar(filtroRamoAtividade, RamoAtividade.class.getName());
				...
				if(ramosAtividades != null && !ramosAtividades.isEmpty()){
					...
					ramoAtividade = (RamoAtividade) ramoAtividadeIterator.next();
			...
			String idOrgaoExpedidorRg = (String) pesquisarActionForm.get("orgaoEmissor");

			OrgaoExpedidorRg orgaoExpedidorRg = null;

			if(idOrgaoExpedidorRg != null && !idOrgaoExpedidorRg.equals("" + ConstantesSistema.NUMERO_NAO_INFORMADO)
							&& !idOrgaoExpedidorRg.equals("")){
				FiltroOrgaoExpedidorRg filtroOrgaoExpedidorRg = new FiltroOrgaoExpedidorRg();

				filtroOrgaoExpedidorRg.adicionarParametro(new ParametroSimples(FiltroOrgaoExpedidorRg.ID, idOrgaoExpedidorRg));
				filtroOrgaoExpedidorRg.adicionarParametro(new ParametroSimples(FiltroOrgaoExpedidorRg.INDICADOR_USO,
								ConstantesSistema.INDICADOR_USO_ATIVO));

				Collection orgaosExpedidoresRg = fachada.pesquisar(filtroOrgaoExpedidorRg, OrgaoExpedidorRg.class.getName());

				if(orgaosExpedidoresRg != null && !orgaosExpedidoresRg.isEmpty()){
					// O órgão expedidor foi encontrado
					Iterator orgaoExpedidorRgIterator = orgaosExpedidoresRg.iterator();

					orgaoExpedidorRg = (OrgaoExpedidorRg) orgaoExpedidorRgIterator.next();

				}else{
					throw new ActionServletException("atencao.pesquisa_inexistente", null, "Órgão Expedidor do Rg");
				}
			...
			if(idProfissao != null && !idProfissao.equals("" + ConstantesSistema.NUMERO_NAO_INFORMADO) && !idProfissao.equals("")){
				...
				filtroProfissao.adicionarParametro(new ParametroSimples(FiltroProfissao.INDICADOR_USO,
								ConstantesSistema.INDICADOR_USO_ATIVO));
			...
			if(idPessoaSexo != null && !idPessoaSexo.equals("" + ConstantesSistema.NUMERO_NAO_INFORMADO) && !idPessoaSexo.equals("")){
				...
				filtroPessoaSexo.adicionarParametro(new ParametroSimples(FiltroPessoaSexo.INDICADOR_USO,
								ConstantesSistema.INDICADOR_USO_ATIVO));

				Collection sexos = fachada.pesquisar(filtroPessoaSexo, PessoaSexo.class.getName());
			...
			String idClienteResponsavel = (String) pesquisarActionForm.get("idClienteResponsavel");

			if(idClienteResponsavel != null && !idClienteResponsavel.equals("")){
				FiltroCliente filtroClienteTipo = new FiltroCliente();

				filtroClienteTipo.adicionarParametro(new ParametroSimples(FiltroCliente.ID, idClienteResponsavel));

				Collection clientesResponsaveis = fachada.pesquisar(filtroClienteTipo, Cliente.class.getName());

				if(clientesResponsaveis != null && !clientesResponsaveis.isEmpty()){
					// O Cliente foi encontrado
					Iterator clienteResponsavelIterator = clientesResponsaveis.iterator();

					clienteResponsavel = (Cliente) clienteResponsavelIterator.next();
			...
			String dataEmissaoPesquisada = (String) pesquisarActionForm.get("dataEmissao");

			if(dataEmissaoPesquisada != null && !dataEmissaoPesquisada.equals("")){
			...
			String dataNascimentoPesquisada = (String) pesquisarActionForm.get("dataNascimento");

			if(dataNascimentoPesquisada != null && !dataNascimentoPesquisada.equals("")){
				SimpleDateFormat format = new SimpleDateFormat("dd/MM/yyyy");
			...
			if(pesquisarActionForm.get("indicadorUso") != null && !pesquisarActionForm.get("indicadorUso").equals("")){
		...
		RelatorioManterCliente relatorioManterCliente = new RelatorioManterCliente((Usuario) (httpServletRequest.getSession(false))
		...
		relatorioManterCliente.addParametro("clienteParametros", clienteParametros);
		relatorioManterCliente.addParametro("clienteEnderecoParametros", clienteEnderecoParametros);
		relatorioManterCliente.addParametro("municipio", municipio);
		relatorioManterCliente.addParametro("indicadorTela", indicadorTela);
		relatorioManterCliente.addParametro("clienteImovelParametros", clienteImovelParametros);
		...
		relatorioManterCliente.addParametro("tipoFormatoRelatorio", Integer.parseInt(tipoRelatorio));
		try{
			retorno = processarExibicaoRelatorio(relatorioManterCliente, tipoRelatorio, httpServletRequest, httpServletResponse,
							actionMapping);
