...
public class GerarRelatorioCronogramaCobrancaManterAction
	...
	public ActionForward execute(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest httpServletRequest,
		...
		if(idGrupoCobranca != null && !idGrupoCobranca.equals("") && !idGrupoCobranca.equals("" + ConstantesSistema.NUMERO_NAO_INFORMADO)){
			FiltroCobrancaGrupo filtroCobrancaGrupo = new FiltroCobrancaGrupo();

			filtroCobrancaGrupo.adicionarParametro(new ParametroSimples(FiltroCobrancaGrupo.ID, idGrupoCobranca));

			Collection colecaoGruposCobrancas = fachada.pesquisar(filtroCobrancaGrupo, CobrancaGrupo.class.getName());

			if(colecaoGruposCobrancas != null && !colecaoGruposCobrancas.isEmpty()){

				filtroCobrancaAcaoAtividadeCronograma.adicionarParametro(new ParametroSimples(
								FiltroCobrancaAcaoAtividadeCronograma.COBRANCA_GRUPO_CRONOGRAMA_MES_COBRANCA_GRUPO_ID, idGrupoCobranca));

				Iterator colecaoGruposCobrancasIterator = colecaoGruposCobrancas.iterator();

				cobrancaGrupo = (CobrancaGrupo) colecaoGruposCobrancasIterator.next();
			}
		}
		...
		if(mesAnoPesquisar != null && !mesAnoPesquisar.equals("")){
			...
			filtroCobrancaAcaoAtividadeCronograma.adicionarParametro(new ParametroSimples(
							FiltroCobrancaAcaoAtividadeCronograma.COBRANCA_GRUPO_CRONOGRAMA_MES_MES_ANO, anoMes));
		}

		// seta os parametros que serão mostrados no relatório
		cobrancaGrupoCronogramaMesParametros.setCobrancaGrupo(cobrancaGrupo);
		...
		relatorioManterCronogramaCobranca.addParametro("filtroCobrancaAcaoAtividadeCronograma", filtroCobrancaAcaoAtividadeCronograma);
		relatorioManterCronogramaCobranca.addParametro("cobrancaGrupoCronogramaMesParametros", cobrancaGrupoCronogramaMesParametros);
		...
		relatorioManterCronogramaCobranca.addParametro("tipoFormatoRelatorio", Integer.parseInt(tipoRelatorio));
		try{
			retorno = processarExibicaoRelatorio(relatorioManterCronogramaCobranca, tipoRelatorio, httpServletRequest, httpServletResponse,
							actionMapping);
