...
public class GerarRelatorioPerfilParcelamentoManterAction
	...
	public ActionForward execute(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest httpServletRequest,
		...
		if(idResolucaoDiretoria != null && !idResolucaoDiretoria.equals("")
						&& !idResolucaoDiretoria.equals("" + ConstantesSistema.NUMERO_NAO_INFORMADO)){
			...
			Collection colecaoResolucoesDiretorias = fachada.pesquisar(filtroResolucaoDiretoria, ResolucaoDiretoria.class.getName());

			if(colecaoResolucoesDiretorias != null && !colecaoResolucoesDiretorias.isEmpty()){

				Iterator colecaoResolucoesDiretoriasIterator = colecaoResolucoesDiretorias.iterator();

				resolucaoDiretoria = (ResolucaoDiretoria) colecaoResolucoesDiretoriasIterator.next();
		...
		if(idImovelSituacaoTipo != null && !idImovelSituacaoTipo.equals("")
			...
			filtroImovelSituacaoTipo.adicionarParametro(new ParametroSimples(FiltroImovelSituacaoTipo.ID, idImovelSituacaoTipo));

			Collection colecaoImoveisSituacoesTipos = fachada.pesquisar(filtroImovelSituacaoTipo, ImovelSituacaoTipo.class.getName());

			if(colecaoImoveisSituacoesTipos != null && !colecaoImoveisSituacoesTipos.isEmpty()){

				Iterator colecaoImoveisSituacoesTiposIterator = colecaoImoveisSituacoesTipos.iterator();

				imovelSituacaoTipo = (ImovelSituacaoTipo) colecaoImoveisSituacoesTiposIterator.next();
		...
		if(idImovelPerfil != null && !idImovelPerfil.equals("") && !idImovelPerfil.equals("" + ConstantesSistema.NUMERO_NAO_INFORMADO)){
			...
			if(colecaoImoveisPerfis != null && !colecaoImoveisPerfis.isEmpty()){
				...
				Iterator colecaoImoveisPerfisIterator = colecaoImoveisPerfis.iterator();

				imovelPerfil = (ImovelPerfil) colecaoImoveisPerfisIterator.next();
		...
		if(idSubcategoria != null && !idSubcategoria.equals("") && !idSubcategoria.equals("" + ConstantesSistema.NUMERO_NAO_INFORMADO)){
			...
			if(colecaoSubcategorias != null && !colecaoSubcategorias.isEmpty()){
				...
				subcategoria = (Subcategoria) colecaoSubcategoriasIterator.next();
		...
		relatorioManterPerfilParcelamento.addParametro("filtroParcelamentoPerfil", filtroParcelamentoPerfil);
		relatorioManterPerfilParcelamento.addParametro("parcelamentoPerfilParametros", parcelamentoPerfilParametros);
		...
		relatorioManterPerfilParcelamento.addParametro("tipoFormatoRelatorio", Integer.parseInt(tipoRelatorio));
		try{
			retorno = processarExibicaoRelatorio(relatorioManterPerfilParcelamento, tipoRelatorio, httpServletRequest, httpServletResponse,
							actionMapping);
