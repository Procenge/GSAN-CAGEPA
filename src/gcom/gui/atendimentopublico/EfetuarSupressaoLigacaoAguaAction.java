 ...
 * @date 14/07/2006
 */
public class EfetuarSupressaoLigacaoAguaAction
				extends GcomAction {
	...
	public ActionForward execute(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest httpServletRequest,
		...
		operacao.setId(Operacao.OPERACAO_SUPRESSAO_LIGACAO_AGUA_EFETUAR);
		...
		String numeroLeituraSupressao = efetuarSupressaoLigacaoAguaActionForm.getNumeroLeituraSupressao();
		...
		String idFuncionario = efetuarSupressaoLigacaoAguaActionForm.getIdFuncionario();
		String indicadorTipoSupressao = efetuarSupressaoLigacaoAguaActionForm.getIndicadorTipoSupressao();
		String dataEncerramento = efetuarSupressaoLigacaoAguaActionForm.getDataSupressao();
		String idServicoMotivoNaoCobranca = efetuarSupressaoLigacaoAguaActionForm.getMotivoNaoCobranca();
		String valorPercentual = efetuarSupressaoLigacaoAguaActionForm.getPercentualCobranca();
		String indicadorRetirardaHidrometro = efetuarSupressaoLigacaoAguaActionForm.getIndicadorRetirardaHidrometro();

		boolean retirarHidrometro = true;
		if(indicadorRetirardaHidrometro != null && indicadorRetirardaHidrometro.equals("2")){
			retirarHidrometro = false;
		}
		...
		if(indicadorTipoSupressao.equals("1")){
			...
			if(LigacaoAguaSituacao.SUPR_PARC_PEDIDO != -1){
				ligacaoAguaSituacao.setId(LigacaoAguaSituacao.SUPR_PARC_PEDIDO);
			}else{
				ligacaoAguaSituacao.setId(LigacaoAguaSituacao.SUPR_PARC);
			}
		...
		filtroSupressaoMotivo.adicionarParametro(new ParametroSimples(FiltroSupressaoMotivo.ID, new Integer(motivoSupressao)));

		SupressaoMotivo supressaoMotivo = (SupressaoMotivo) getFachada().pesquisar(filtroSupressaoMotivo, SupressaoMotivo.class.getName())
						.iterator().next();
		...
		if(ligacaoAgua.getHidrometroInstalacaoHistorico() != null){
			...
			filtroHidrometroInstalacaoHistorico.adicionarParametro(new ParametroSimples(FiltroHidrometroInstalacaoHistorico.ID, ligacaoAgua
			...
			Collection colecaoHidrometroInstalacaHistorico = fachada.pesquisar(filtroHidrometroInstalacaoHistorico,
			...
			if(hidrometroInstalacaoHistorico != null && numeroLeituraSupressao != null){
				...
				if(numeroLeituraSupressao != null && !numeroLeituraSupressao.equals("")){
					hidrometroInstalacaoHistorico.setNumeroLeituraSupressao(new Integer(numeroLeituraSupressao));
				}
		...
		if(idFuncionario != null && !idFuncionario.equals("")){
			...
			ligacaoAgua.setFuncionarioSupressaoAgua(func);
		}
		...
		if(ligacaoAgua.getNumeroSupressaoAgua() == null){
			ligacaoAgua.setNumeroSupressaoAgua(1);
		}else{
			ligacaoAgua.setNumeroSupressaoAgua(ligacaoAgua.getNumeroSupressaoAgua() + 1);
		}

		if(ligacaoAgua.getSupressaoTipo().getIndicadorParcial() != null
						&& ligacaoAgua.getSupressaoTipo().getIndicadorParcial().equals(new Short("1"))){
			if(ligacaoAgua.getNumeroSupressaoParcial() == null){
				ligacaoAgua.setNumeroSupressaoParcial(1);
			}else{
				ligacaoAgua.setNumeroSupressaoParcial(ligacaoAgua.getNumeroSupressaoParcial() + 1);
			}
		}

		if(ligacaoAgua.getSupressaoTipo().getIndicadorTotal() != null
						&& ligacaoAgua.getSupressaoTipo().getIndicadorTotal().equals(new Short("1"))){
			if(ligacaoAgua.getNumeroSupressaoTotal() == null){
				ligacaoAgua.setNumeroSupressaoTotal(1);
			}else{
				ligacaoAgua.setNumeroSupressaoTotal(ligacaoAgua.getNumeroSupressaoTotal() + 1);
			}
		}

		if(ligacaoAgua.getSupressaoMotivo().getIndicadorFaltaPagamento().equals(new Short("1"))){
			if(ligacaoAgua.getNumeroSupressaoFaltaPagamento() == null){
				ligacaoAgua.setNumeroSupressaoFaltaPagamento(1);
			}else{
				ligacaoAgua.setNumeroSupressaoFaltaPagamento(ligacaoAgua.getNumeroSupressaoFaltaPagamento() + 1);
			}
		}else if(ligacaoAgua.getSupressaoMotivo().getIndicadorInfracao().equals(new Short("1"))){
			if(ligacaoAgua.getNumeroSupressaoInfracao() == null){
				ligacaoAgua.setNumeroSupressaoInfracao(1);
			}else{
				ligacaoAgua.setNumeroSupressaoInfracao(ligacaoAgua.getNumeroSupressaoInfracao() + 1);
			}
		}else if(ligacaoAgua.getSupressaoMotivo().getIndicadorPedido().equals(new Short("1"))){
			if(ligacaoAgua.getNumeroSupressaoPedido() == null){
				ligacaoAgua.setNumeroSupressaoPedido(1);
			}else{
				ligacaoAgua.setNumeroSupressaoPedido(ligacaoAgua.getNumeroSupressaoPedido() + 1);
			}
		}
		...
		if(efetuarSupressaoLigacaoAguaActionForm.getIdTipoDebito() != null){

			ServicoNaoCobrancaMotivo servicoNaoCobrancaMotivo = null;
			if(efetuarSupressaoLigacaoAguaActionForm.getValorDebito() != null){
				String valorDebito = efetuarSupressaoLigacaoAguaActionForm.getValorDebito().toString().replace(".", "");
			...
			if(valorPercentual != null){
				ordemServico.setPercentualCobranca(new BigDecimal(efetuarSupressaoLigacaoAguaActionForm.getPercentualCobranca()));
			}
		...
		if(ordemServico.getServicoTipo().getDebitoTipo() != null && ordemServico.getServicoNaoCobrancaMotivo() == null){
			...
			try{
				...
				ex.setUrlBotaoVoltar("/gsan/exibirEfetuarSupressaoLigacaoAguaAction.do");
				throw ex;
			}
		}
		...
		if(efetuarSupressaoLigacaoAguaActionForm.getVeioEncerrarOS().equalsIgnoreCase("FALSE")){
			...
			try{
				fachada.efetuarSupressaoLigacaoAgua(integracaoComercialHelper, retirarHidrometro);
			}catch(FachadaException e){
		...
		if(retorno.getName().equalsIgnoreCase("telaSucesso")){
			montarPaginaSucesso(httpServletRequest, "Supressão da Ligação de Água para o imóvel " + imovel.getId()
							+ " efetuada com sucesso.", "Efetuar outra Supressão da Ligação de Água",
