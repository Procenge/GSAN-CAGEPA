 ...
 * @date 28/06/2006
 */
public class ExibirEfetuarRemanejamentoHidrometroAction
				extends GcomAction {
		...
		if(idOrdemServico != null && !idOrdemServico.trim().equals("")){
			...
			if(ordemServico != null){
				...
				if(ordemServico.getServicoTipo() != null && ordemServico.getServicoTipo().getDebitoTipo() != null){
					efetuarRemanejamentoHidrometroActionForm.setIdTipoDebito(ordemServico.getServicoTipo().getDebitoTipo().getId()
									.toString());
					efetuarRemanejamentoHidrometroActionForm.setDescricaoTipoDebito(ordemServico.getServicoTipo().getDebitoTipo()
									.getDescricao());

					BigDecimal valorDebito = fachada.obterValorDebito(ordemServico.getServicoTipo().getId(), imovel.getId(), new Short(
									LigacaoTipo.LIGACAO_AGUA + ""));
					efetuarRemanejamentoHidrometroActionForm.setValorDebito(Util.formataBigDecimal(valorDebito, 2, true));

					if(ordemServico.getServicoNaoCobrancaMotivo() != null){
						efetuarRemanejamentoHidrometroActionForm.setMotivoNaoCobranca(ordemServico.getServicoNaoCobrancaMotivo().getId()
										.toString());
					}
				...
				// Situação da Ligação de Esgoto
				String situacaoLigacaoEsgoto = ordemServico.getRegistroAtendimento().getImovel().getLigacaoEsgotoSituacao().getDescricao();
				...
				if(hidrometroInstalacaoHistorico.getMedicaoTipo() != null){
					...
					String medicaoTipo = hidrometroInstalacaoHistorico.getMedicaoTipo().getId().toString();
					efetuarRemanejamentoHidrometroActionForm.setTipoMedicao(medicaoTipo);
				...
				if(ordemServico.getRegistroAtendimento().getSolicitacaoTipoEspecificacao().getIndicadorLigacaoAgua().equals(
					...
					efetuarRemanejamentoHidrometroActionForm.setTipoMedicao("1");
				}else{
					efetuarRemanejamentoHidrometroActionForm.setTipoMedicao("2");
				}

				// Dados da Geração de Débito
				// this.pesquisarDadosGeracaoDebito(efetuarRemanejamentoHidrometroActionForm,
				// ordemServico);
	 ...
	 * data: 04/05/2007
	 */
	private BigDecimal calcularValores(HttpServletRequest httpServletRequest, OrdemServico ordemServico, ServicoTipo servicoTipo,
					EfetuarRemanejamentoHidrometroActionForm form){
		...
		if(calculaValores != null && calculaValores.equals("S")){
			...
			valorDebito = Fachada.getInstancia().obterValorDebito(servicoTipo.getId(),
							ordemServico.getRegistroAtendimento().getImovel().getId(), tipoMedicao);
	...
	private void pesquisarSelectObrigatorio(HttpServletRequest httpServletRequest){
		...
		Collection<HidrometroLocalInstalacao> colecaoHidrometroLocalInstalacao = Fachada.getInstancia().pesquisar(
		...
		Collection<HidrometroProtecao> colecaoHidrometroProtecao = Fachada.getInstancia().pesquisar(filtroHidrometroProtecao,
	 ...
	 * @date 22/08/2006
	 */
	private void pesquisarCliente(EfetuarRemanejamentoHidrometroActionForm efetuarRemanejamentoHidrometroActionForm, Integer matriculaImovel){
