 ...
 * @date 31/07/2006
 */
public class PesquisarEquipeAction
				extends GcomAction {
	...
	public ActionForward execute(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest httpServletRequest,
		...
		if(pesquisarEquipeActionForm.getEquipeId() != null && !pesquisarEquipeActionForm.getEquipeId().trim().equals("")){
		...
		if(pesquisarEquipeActionForm.getNomeEquipe() != null && !pesquisarEquipeActionForm.getNomeEquipe().trim().equalsIgnoreCase("")){
			peloMenosUmParametroInformado = true;
		...
		if(pesquisarEquipeActionForm.getPlacaVeiculo() != null && !pesquisarEquipeActionForm.getPlacaVeiculo().trim().equalsIgnoreCase("")){
			peloMenosUmParametroInformado = true;
		...
		if(pesquisarEquipeActionForm.getCargaTrabalhoDia() != null && !pesquisarEquipeActionForm.getCargaTrabalhoDia().trim().equals("")){
		...
		if(pesquisarEquipeActionForm.getUnidadeOrganizacionalId() != null
			...
			peloMenosUmParametroInformado = true;
			filtroEquipe.adicionarParametro(new ParametroSimples(FiltroEquipe.ID_UNIDADE_ORGANIZACIONAL, pesquisarEquipeActionForm
		...
		if(pesquisarEquipeActionForm.getTipoPerfilServicoId() != null
			...
			peloMenosUmParametroInformado = true;
			filtroEquipe.adicionarParametro(new ParametroSimples(FiltroEquipe.ID_SERVICO_PERFIL_TIPO, pesquisarEquipeActionForm
		...
		if(colecaoEquipe == null || colecaoEquipe.isEmpty()){
			...
			Map resultado = controlarPaginacao(httpServletRequest, retorno, filtroEquipe, Equipe.class.getName());
