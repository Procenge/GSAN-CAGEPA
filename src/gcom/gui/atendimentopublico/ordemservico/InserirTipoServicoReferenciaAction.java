 ...
 * @date 04/08/2006
 */
public class InserirTipoServicoReferenciaAction
				extends GcomAction {
	...
	public ActionForward execute(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest httpServletRequest,
		...
		String descricao = inserirTipoServicoReferenciaActionForm.getDescricao();

		String abreviatura = inserirTipoServicoReferenciaActionForm.getAbreviatura();

		String indicadorExistenciaOsReferencia = inserirTipoServicoReferenciaActionForm.getIndicadorExistenciaOsReferencia();

		String situacaoOSAntes = null;

		if(indicadorExistenciaOsReferencia != null
						&& indicadorExistenciaOsReferencia
										.equals(ServicoTipoReferencia.INDICADOR_EXISTENCIA_OS_REFERENCIA_ATIVO.toString())){
			if(inserirTipoServicoReferenciaActionForm.getSituacaoOSAntes() == null
							|| inserirTipoServicoReferenciaActionForm.getSituacaoOSAntes().equals(
											"" + ConstantesSistema.NUMERO_NAO_INFORMADO)){
				throw new ActionServletException("atencao.indicador_os_incompativel_os_antes");
			}

			if(inserirTipoServicoReferenciaActionForm.getSituacaoOSApos() == null
							|| inserirTipoServicoReferenciaActionForm.getSituacaoOSApos().equals(
											"" + ConstantesSistema.NUMERO_NAO_INFORMADO)){
				throw new ActionServletException("atencao.indicador_os_incompativel_os_apos");

			}
		}

		if(inserirTipoServicoReferenciaActionForm.getSituacaoOSAntes() != null
						&& !inserirTipoServicoReferenciaActionForm.getSituacaoOSAntes().toString().equals(
										new Integer(ConstantesSistema.NUMERO_NAO_INFORMADO).toString())){

			situacaoOSAntes = inserirTipoServicoReferenciaActionForm.getSituacaoOSAntes();
			servicoTipoReferencia.setSituacaoOsReferenciaAntes(new Integer(situacaoOSAntes));
		...
		if(inserirTipoServicoReferenciaActionForm.getSituacaoOSApos() != null
						&& !inserirTipoServicoReferenciaActionForm.getSituacaoOSApos().toString().equals(
										new Integer(ConstantesSistema.NUMERO_NAO_INFORMADO).toString())){

			situacaoOSApos = inserirTipoServicoReferenciaActionForm.getSituacaoOSApos();
			servicoTipoReferencia.setSituacaoOsReferenciaApos(new Integer(situacaoOSApos));
		}

		String tipoServico = inserirTipoServicoReferenciaActionForm.getTipoServico();
		...
		servicoTipoReferencia.setIndicadorExistenciaOsReferencia(new Short(indicadorExistenciaOsReferencia));
		...
		servicoTipoReferencia.setIndicadorUso(new Short(ConstantesSistema.INDICADOR_USO_ATIVO));
		...
		if(sessao.getAttribute("semMenu") != null){
			...
			montarPaginaSucesso(httpServletRequest, "Tipo de Serviço de Referência " + descricao + " inserido com sucesso.",
							"Inserir outro Tipo de Serviço de Referência", "exibirInserirTipoServicoReferenciaAction.do?menu=sim");
		}
