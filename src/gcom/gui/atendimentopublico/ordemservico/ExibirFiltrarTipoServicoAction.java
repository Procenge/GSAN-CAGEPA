 ...
 */
public class ExibirFiltrarTipoServicoAction
				extends GcomAction {
	...
	public ActionForward execute(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest httpServletRequest,
		...
		if(redirecionarPagina != null && !redirecionarPagina.equals("")){
			...
			retorno = actionMapping.findForward("filtrarTipoServico");
		}
		...
		if(form.getIdTipoServicoReferencia() != null && !form.getIdTipoServicoReferencia().trim().equals("")){
			...
			filtroServicoTipoReferencia.adicionarParametro(new ParametroSimples(FiltroServicoTipoReferencia.ID, form
							.getIdTipoServicoReferencia()));
	...
	private void getAtividadesTipoServico(FiltrarTipoServicoActionForm form, Fachada fachada){
		...
		if(idAtividadesTipoServico != null && !idAtividadesTipoServico.trim().equals("")){
			...
			if(colecaoServicoTipoAtividade != null && !colecaoServicoTipoAtividade.isEmpty()){
				...
				if((servicoTipoAtividade != null && !servicoTipoAtividade.equals(""))){
					form.setAtividadesTipoServico(servicoTipoAtividade.getId().toString());
					form.setDsAtividadesTipoServico(servicoTipoAtividade.getDescricao());
	 ...
	 * @param atividades
	 */
	private void setColecaoAtividades(FiltrarTipoServicoActionForm form, Atividade atividade){
	...
	private void getTipoServicoMaterial(FiltrarTipoServicoActionForm form, Fachada fachada){
		...
		if(idTipoServicoMaterial != null && !idTipoServicoMaterial.trim().equals("")){
			...
			filtroMaterial.adicionarParametro(new ParametroSimples(FiltroMaterial.ID, idTipoServicoMaterial));

			// Pesquisa de acordo com os parâmetros informados no filtro

			Collection colecaoServicoTipoMaterial = Fachada.getInstancia().pesquisar(filtroMaterial, Material.class.getName());

			// Verifica se a pesquisa retornou algum objeto para a coleção
			if(colecaoServicoTipoMaterial != null && !colecaoServicoTipoMaterial.isEmpty()){

				Material servicoTipoMaterial = (Material) Util.retonarObjetoDeColecao(colecaoServicoTipoMaterial);
	 ...
	 * @param materiais
	 */
	private void setColecaoMateriais(FiltrarTipoServicoActionForm form, Material servicoTipoMaterial){
