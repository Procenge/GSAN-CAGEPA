 ...
 */
public class ExibirAtualizarOrdemServicoAction
				extends GcomAction {

	public ActionForward execute(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest httpServletRequest,
					HttpServletResponse httpServletResponse){

		ActionForward retorno = actionMapping.findForward("exibirAtualizarOrdemServico");
		...
		if(httpServletRequest.getParameter("primeiraVez") != null && !httpServletRequest.getParameter("primeiraVez").equals("")){
			if(httpServletRequest.getParameter("ehPopup") != null && !httpServletRequest.getParameter("ehPopup").equals("")){
				sessao.setAttribute("ehPopup", httpServletRequest.getParameter("ehPopup"));
			}
			...
			if(ordemServico.getCobrancaDocumento() != null){
				form.setNumeroDocumentoCobranca(ordemServico.getCobrancaDocumento().getId() + "");
			}
			form.setDataGeracao(Util.formatarData(ordemServico.getDataGeracao()));
			...
			if(ordemServico.getServicoTipo() != null && !ordemServico.getServicoTipo().equals("")){
				...
				form.setDescricaoServicoTipo(ordemServico.getServicoTipo().getDescricao());
				ServicoTipo servicoTipo = fachada.pesquisarSevicoTipo(ordemServico.getServicoTipo().getId());
				...
				if(ordemServico.getServicoTipoReferencia() != null && !ordemServico.getServicoTipoReferencia().equals("")){
					ServicoTipo st = fachada.pesquisarSevicoTipo(ordemServico.getServicoTipoReferencia().getId());
					...
					form.setIdServicoTipoReferencia("" + ordemServico.getServicoTipoReferencia().getId());
					form.setDescricaoServicoTipoReferencia(ordemServico.getServicoTipoReferencia().getDescricao());
				}

			}

			if(ordemServico.getOsReferencia() != null){
				OrdemServico os = fachada.pesquisarOrdemServico(ordemServico.getOsReferencia().getId());
			...
			form.setPrioridadeOriginal(ordemServico.getServicoTipoPrioridadeOriginal().getDescricao());
			form.setPrioridadeAtual(ordemServico.getServicoTipoPrioridadeAtual().getDescricao() + "");
			...
			if(ordemServicoUnidade != null){
				form.setUnidadeGeracaoId(ordemServicoUnidade.getUnidadeOrganizacional().getId() + "");
				form.setUnidadeGeracaoDescricao(ordemServicoUnidade.getUnidadeOrganizacional().getDescricao());
				form.setUsuarioGeracaoId(ordemServicoUnidade.getUsuario().getId() + "");
				form.setUsuarioGeracaoNome(ordemServicoUnidade.getUsuario().getNomeUsuario());
			}
			if(ordemServico.getDataEmissao() != null){
				form.setDataUltimaEmissao(Util.formatarData(ordemServico.getDataEmissao()));
			}
			// fim dos dados gerais

			form.setDescricaoPrioridadeServicoOriginal(ordemServico.getServicoTipoPrioridadeOriginal().getDescricao());

			Integer IdPrioridadeServicoAtual = Util.converterStringParaInteger(form.getIdPrioridadeServicoAtual());

			if(Util.validarNumeroMaiorQueZERO(IdPrioridadeServicoAtual)){
				form.setIdPrioridadeServicoAtual(IdPrioridadeServicoAtual.toString());
			}else{
				if(ordemServico.getServicoTipoPrioridadeAtual() != null){
					form.setIdPrioridadeServicoAtual(ordemServico.getServicoTipoPrioridadeAtual().getId().toString());
				}
			}
			...
			if(ra != null){
				ObterDadosRegistroAtendimentoHelper registroAtendimentoHelper = fachada.obterDadosRegistroAtendimento(ra.getId());
				...
				fachada.verificarUnidadeUsuario(ordemServico.getId(), usuarioLogado);
				httpServletRequest.setAttribute("registroAtendimentoHelper", registroAtendimentoHelper);
				httpServletRequest.setAttribute("unidadeAtualRA", fachada.obterUnidadeAtualRA(ra.getId()));
			}
		...
		if(idServicoTipo != null && !idServicoTipo.equals("") && (descricaoServicoTipo == null || descricaoServicoTipo.equals(""))){
			...
			if(ordemServico.getOsReferencia() != null && !ordemServico.getOsReferencia().equals("")){
				form.setIdOrdemServicoReferencia("" + ordemServico.getOsReferencia().getId());
			}

			Integer idServicoTipoInteger = Util.converterStringParaInteger(form.getIdServicoTipo());
		...
		String descricaoOsReferencia = form.getDescricaoOrdemServicoReferencia();
		if(idOSReferencia != null && !idOSReferencia.equals("") && (descricaoOsReferencia == null || descricaoOsReferencia.equals(""))){
			...
			if(os != null && !os.equals("")){
				...
				httpServletRequest.setAttribute("nomeCampo", "idOrdemServicoReferencia");
				httpServletRequest.setAttribute("idOrdemServicoReferenciaNaoEncontrado", "exception");
				form.setDescricaoServicoTipo("Ordem Serviço Referência Inexistente");
		...
		String descricaoServicoTipoReferencia = form.getDescricaoServicoTipoReferencia();
		if(idServicoTipoReferencia != null && !idServicoTipoReferencia.equals("")
						&& (descricaoServicoTipoReferencia == null || descricaoServicoTipoReferencia.equals(""))){

			Integer idTipoServicoReferenciaInteger = Util.converterStringParaInteger(form.getIdServicoTipoReferencia());
