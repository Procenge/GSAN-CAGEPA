 ...
 * @date 26/07/2006
 */
public class ExibirInserirServicoTipoAction
				extends GcomAction {
	...
	public ActionForward execute(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest httpServletRequest,
		...
		if("reset".equalsIgnoreCase(httpServletRequest.getParameter("action"))){
			...
			sessao.removeAttribute("colecaoServicoTipoValorLocalidade");
		}

		// Alteração do caso de uso UC0410, OC1123944
		sessao.setAttribute("indicadorParametroTramite", true);
		// Fim da Alteração do caso de uso UC0410, OC1123944
		...
		if("removeRowTableServicoTipoReferencia".equalsIgnoreCase(form.getMethod())){
			sessao.removeAttribute("servicoTipoReferencia");
			form.setMethod("");
		}

		if("addServicoTipoAtividade".equalsIgnoreCase(form.getMethod())){
			form.addServicoTipoAtividade();
			form.setMethod(null);
		}

		if("removeServicoTipoAtividade".equalsIgnoreCase(form.getMethod())){
			form.removeServicoTipoAtividade();
		}

		if("addServicoTipoValorLocalidade".equalsIgnoreCase(form.getMethod())){
			form.addServicoTipoValorLocalidade();
			form.setMethod(null);
		}

		if("removeServicoTipoValorLocalidade".equalsIgnoreCase(form.getMethod())){
			form.removeServicoTipoValorLocalidade();
		}

		if("addServicoTipoMaterial".equalsIgnoreCase(form.getMethod())){
			form.addServicoTipoMaterial();
			form.setMethod("");
		}

		if("removeServicoTipoMaterial".equalsIgnoreCase(form.getMethod())){
			form.removeServicoTipoMaterial();
		}

		if("removeAllServicoTipoAtividade".equalsIgnoreCase(form.getMethod())){
		...
		if(atualizarIdLocalidade != null && !atualizarIdLocalidade.equals("") && servicoValorLocalidade != null
			...
			for(Iterator iter = form.getServicoTipoValorLocalidade().iterator(); iter.hasNext();){
			...
			form.setServicoTipoValorLocalidade(colecaoAtualizada);
		}
		...
		sessao.setAttribute("colecaoServicoTipoValorLocalidade", form.getServicoTipoValorLocalidade());
