 ...
 */
public class AtualizarEquipeAction
				extends GcomAction {
	...
	public ActionForward execute(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest httpServletRequest,
		...
		if(idPerfilServico != null && !idPerfilServico.trim().equals("")){
			...
			if(colecaoPerfilServico == null || colecaoPerfilServico.isEmpty()){
				throw new ActionServletException("atencao.pesquisa_inexistente", null, "Serviço Perfil Tipo");
			}else{
		...
		if(sessao.getAttribute("equipeAtualizar") != null){
			...
			if(servicoPerfilTipo != null){
				if(servicoPerfilTipo.getComponentesEquipe().intValue() != colecaoEquipeComponentes.size()){
					throw new ActionServletException("atencao.quantidade.componentes.diferente.permitido");
				}
			}

			Usuario usuarioLogado = (Usuario) sessao.getAttribute("usuarioLogado");

			// Aualiza a Equipe e seus componentes
			fachada.atualizarEquipe(equipe, colecaoEquipeComponentes, usuarioLogado);

			// [FS008] Monta a página de sucesso
			montarPaginaSucesso(httpServletRequest, "Equipe " + equipe.getNome() + " atualizada com sucesso.",
							"Realizar outra manutenção de Equipe", "exibirFiltrarEquipeAction.do?menu=sim");
