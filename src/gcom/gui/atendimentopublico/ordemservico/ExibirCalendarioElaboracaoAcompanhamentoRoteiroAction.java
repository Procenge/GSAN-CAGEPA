 ...
 * @date 20/09/2006
 */
public class ExibirCalendarioElaboracaoAcompanhamentoRoteiroAction
				extends GcomAction {
	...
	public ActionForward execute(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest httpServletRequest,
		...
		ActionForward retorno = actionMapping.findForward("calendarioElaboracaoAcompanhamentoRoteiroAction");
		...
		String dataRoteiroParametro = null;
		...
		dataRoteiroParametro = (String) httpServletRequest.getParameter("dataRoteiro");

		// Recupera a data enviada no parâmetro
		// quando clica no link da data
		if(!Util.isVazioOuBranco(dataRoteiroParametro)){
			Date dataParametro = new Date();
			Integer mesCalendarioAux = null;
			String mesParametro = "";

			dataParametro = Util.converteStringParaDate(dataRoteiroParametro);

			// Recupera o Ano/Mês
			Integer dataRoteiroParametroAux = Util.recuperaAnoMesDaData(dataParametro);

			// Recupera o ano
			anoCalendario = dataRoteiroParametroAux.toString().substring(0, 4);

			// Recupera o mês
			mesParametro = dataRoteiroParametroAux.toString().substring(4, 6);

			// Subtrai o mês para enviar para o calendário
			mesCalendarioAux = Integer.valueOf(mesParametro) - 1;

			mesCalendario = mesCalendarioAux.toString();
		...
		Collection colecaoProgramacaoRoteiro = fachada.consultarProgramacaoRoteiro(mesAnoReferencia, usuario.getUnidadeOrganizacional()
						.getId());

		if(httpServletRequest.getParameter("elaboracao") != null){
			sessao.setAttribute("elaboracao", "elaboracao");
			exibirCalendarioElaboracaoAcompanhamentoRoteiroActionForm.setCabecalho("Elaboração");
			exibirCalendarioElaboracaoAcompanhamentoRoteiroActionForm.setTitulo("elaboração");
		}else{
			sessao.setAttribute("acompanhamento", "acompanhamento");
			exibirCalendarioElaboracaoAcompanhamentoRoteiroActionForm.setCabecalho("Acompanhamento");
			exibirCalendarioElaboracaoAcompanhamentoRoteiroActionForm.setTitulo("acompanhamento");
		}
		...
		if(colecaoProgramacaoRoteiro != null){
			...
			while(iteratorRoteiro.hasNext()){
				...
				programacaoRoteiro = (ProgramacaoRoteiro) iteratorRoteiro.next();

				Date dataRoteiro = programacaoRoteiro.getDataRoteiro();
				Integer qtdOrdemServicoProgramacao = fachada.verificarExistenciaOSProgramacao(programacaoRoteiro.getId());
		...
		System.out.println("Exibir Calendario de " + exibirCalendarioElaboracaoAcompanhamentoRoteiroActionForm.getCabecalho() + " = "
