 ...
 * @date 14/08/2006
 */
public class ExibirGerarOrdemServicoInserirRAAction
				extends GcomAction {
	...
	public ActionForward execute(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest httpServletRequest,
		...
		if(httpServletRequest.getParameter("caminhoRetornoOS") != null && !httpServletRequest.getParameter("caminhoRetornoOS").equals("")){
			sessao.setAttribute("caminhoRetornoOS", httpServletRequest.getParameter("caminhoRetornoOS"));
		}
		// caso venha do encerrar os popup
		String veioEncerrarOS = (String) httpServletRequest.getAttribute("veioEncerrarOS");
		...
		if(veioEncerrarOS != null && !veioEncerrarOS.equals("")){
			...
			filtroServicoTipo.adicionarParametro(new ParametroSimples(FiltroServicoTipo.INDICADOR_FISCALIZACAO_SERVICO_TIPO_REF,
			...
			if(colecaoServicoTipo == null || colecaoServicoTipo.isEmpty()){
				throw new ActionServletException("atencao.servico_tipo.sem_fiscalizacao");
			}
			sessao.setAttribute("colecaoServicosTipo", colecaoServicoTipo);
			// se vier o id da RA então seta na ordem de serviço
			// caso venha do encerrar os popup
			String numeroRA = (String) httpServletRequest.getAttribute("numeroRA");
			if(numeroRA == null){
				// caso venha do encerrar os
			...
			if(numeroRA != null && !numeroRA.equals("")){
				...
				fachada.verificarUnidadeUsuario(registroAtendimento, usuarioLogado);
				form.getOrdemServico().setRegistroAtendimento(registroAtendimento);
			}
			// se vier o id da OS então seta o id da os como OSReferencia então
			// seta na ordem de serviço
			// caso venha do encerrar os popup
			String numeroOS = (String) httpServletRequest.getAttribute("numeroOS");
			if(numeroOS == null){
				// caso venha do encerrar os
			...
			if(numeroOS != null && !numeroOS.equals("")){
				...
				Integer idOrdemServicoReferencia = Util.converterStringParaInteger(numeroOS);
				...
				if(Util.validarNumeroMaiorQueZERO(idOrdemServicoReferencia)){
					...
					sessao.setAttribute("osFiscalizacao", form.getOrdemServico());
				}
			}
		}else{

			String idTipoSolicitacao = (String) httpServletRequest.getParameter("idTipoSolicitacao");
			if(idTipoSolicitacao != null && !idTipoSolicitacao.equals("")){
				boolean existe = false;
				FiltroSolicitacaoTipo filtroSolicitacaoTipo = new FiltroSolicitacaoTipo();
				filtroSolicitacaoTipo.adicionarParametro(new ParametroSimples(FiltroSolicitacaoTipo.ID, new Integer(idTipoSolicitacao)));
				...
				if(colecaoSolicitacaoTipo != null && !colecaoSolicitacaoTipo.isEmpty()){
					...
					if(colecaoServicoTipo != null && !colecaoServicoTipo.isEmpty()){
						existe = true;
						Integer idServicoTipo = ((ServicoTipo) Util.retonarObjetoDeColecao(colecaoServicoTipo)).getId();
						...
						if(servicoTipo.getValor() != null){
							valorServicoOriginal = servicoTipo.getValor().toString();
						}
						if(servicoTipo.getServicoTipoPrioridade() != null){
							...
							form.getOrdemServico().setServicoTipoPrioridadeOriginal(servicoTipoPrioridadeOriginal);
						}
						...
						// Serviço Tipo
						Integer idServicoTipoReferencia = Util.converterStringParaInteger(httpServletRequest.getParameter("idServicoTipo"));
						...
						httpServletRequest.setAttribute("solicitacaoEspecificacaoOperacionalAgua", "SIM");
				...
				if(!existe){
					// Serviço Tipo
					Integer idServicoTipo = Util.converterStringParaInteger(httpServletRequest.getParameter("idServicoTipo"));
					String valorServicoOriginal = null;
					Integer idServicoTipoPrioridadeOriginal = null;
					String descricaoServicoTipoPrioridadeOriginal = null;
				...
				if(httpServletRequest.getParameter("idServicoTipo") != null && !httpServletRequest.getParameter("idServicoTipo").equals("")){
					// Serviço Tipo
					idServicoTipo = Util.converterStringParaInteger(httpServletRequest.getParameter("idServicoTipo"));
				}
				if(form.getIdServicoTipo() != null && !form.getIdServicoTipo().equals("")){
					// Serviço Tipo
					idServicoTipo = Util.converterStringParaInteger(form.getIdServicoTipo());
				}
				...
				// Ordem de Serviço Referência
				if(sessao.getAttribute("veioEncerrarOS") == null || sessao.getAttribute("veioEncerrarOS").equals("")){
					Integer idOrdemServicoReferencia = Util.converterStringParaInteger(form.getIdOrdemServicoReferencia());
					...
					// Serviço Tipo Referência
					if(form.getIdServicoTipoReferencia() != null && !form.getIdServicoTipoReferencia().equals("")){
						Integer idServicoTipoReferencia = Util.converterStringParaInteger(form.getIdServicoTipoReferencia());
		...
		if(form.getIdPrioridadeServicoAtual() != null && !form.getIdPrioridadeServicoAtual().equals("")){
			ServicoTipoPrioridade servicoTipoPrioridade = new ServicoTipoPrioridade();
			servicoTipoPrioridade.setId(new Integer(form.getIdPrioridadeServicoAtual()));
			form.getOrdemServico().setServicoTipoPrioridadeAtual(servicoTipoPrioridade);
		}else{
			form.getOrdemServico().setServicoTipoPrioridadeAtual(null);
		}

		if(httpServletRequest.getParameter("close") != null){
			if(sessao.getAttribute("caminhoRetornoOS") != null && !sessao.getAttribute("caminhoRetornoOS").equals("")){
				httpServletRequest.setAttribute("close", sessao.getAttribute("caminhoRetornoOS"));
			}else{
