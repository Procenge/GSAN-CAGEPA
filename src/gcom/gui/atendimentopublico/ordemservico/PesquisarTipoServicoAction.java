...
public class PesquisarTipoServicoAction
				extends GcomAction {
	...
	public ActionForward execute(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest httpServletRequest,
		...
		String idUnidadeOrganizacionalDestinoStr = pesquisarTipoServicoActionForm.getIdUnidadeOrganizacionalDestino();
		...
		if((valorServicoInicial != null && !valorServicoInicial.trim().equalsIgnoreCase(""))
						|| (tempoMedioExecucaoFinal != null && !tempoMedioExecucaoFinal.trim().equalsIgnoreCase(""))){
		...
		// Código do Serviço
		if(codigoServico != null && !codigoServico.trim().equalsIgnoreCase("") && !codigoServico.trim().equalsIgnoreCase("0")
		...
		if(pesquisarTipoServicoActionForm.getArrayAtividade() != null && !pesquisarTipoServicoActionForm.getArrayAtividade().isEmpty()){
			peloMenosUmParametroInformado = true;
		}

		if(pesquisarTipoServicoActionForm.getArrayMateriais() != null && !pesquisarTipoServicoActionForm.getArrayMateriais().isEmpty()){
			peloMenosUmParametroInformado = true;
		}
		...
		if(!Util.isVazioOuBranco(idUnidadeOrganizacionalDestinoStr)){
			...
			idUnidadeOrganizacionalDestino = Integer.valueOf(idUnidadeOrganizacionalDestinoStr);
		}
		...
		Integer totalRegistros = fachada.filtrarSTCount(servicoTipo, pesquisarTipoServicoActionForm.getAtividades(),
		...
		Collection servicosTipo = fachada.filtrarST(servicoTipo, pesquisarTipoServicoActionForm.getAtividades(),
						pesquisarTipoServicoActionForm.getArrayMateriais(), valorServicoInicial, valorServicoFinal,
						tempoMedioExecucaoInicial, tempoMedioExecucaoFinal, tipoPesquisa, tipoPesquisaAbreviada,
						(Integer) httpServletRequest.getAttribute("numeroPaginasPesquisa"), idUnidadeOrganizacionalDestino);
