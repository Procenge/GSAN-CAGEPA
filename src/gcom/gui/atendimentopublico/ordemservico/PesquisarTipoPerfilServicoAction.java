 ...
 * @date 01/08/2006
 */
public class PesquisarTipoPerfilServicoAction
				extends GcomAction {
	...
	public ActionForward execute(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest httpServletRequest,
		...
		if(idServicoPerfil != null && !idServicoPerfil.trim().equalsIgnoreCase("")){
			filtroServicoPerfilTipo.adicionarParametro(new ParametroSimples(FiltroServicoPerfilTipo.ID, new Integer(idServicoPerfil)));
		...
		if(descricaoServicoPerfil != null && !descricaoServicoPerfil.trim().equalsIgnoreCase("")){
			...
			if(tipoPesquisa != null && tipoPesquisa.equals(ConstantesSistema.TIPO_PESQUISA_COMPLETA.toString())){
				filtroServicoPerfilTipo.adicionarParametro(new ComparacaoTextoCompleto(FiltroServicoPerfilTipo.DESCRICAO,
								descricaoServicoPerfil));
			}else{
				filtroServicoPerfilTipo.adicionarParametro(new ComparacaoTexto(FiltroServicoPerfilTipo.DESCRICAO, descricaoServicoPerfil));
			}
		}
		...
		if(abreviaturaServicoPerfil != null && !abreviaturaServicoPerfil.trim().equalsIgnoreCase("")){
			...
			if(tipoPesquisaAbreviada != null && tipoPesquisaAbreviada.equals(ConstantesSistema.TIPO_PESQUISA_COMPLETA.toString())){
				filtroServicoPerfilTipo.adicionarParametro(new ComparacaoTextoCompleto(FiltroServicoPerfilTipo.DESCRICAO_ABREVIADA,
								abreviaturaServicoPerfil));
			}else{
		...
		if(componenteEquipe != null && !componenteEquipe.trim().equalsIgnoreCase("")){
			peloMenosUmParametroInformado = true;
			filtroServicoPerfilTipo.adicionarParametro(new ParametroSimples(FiltroServicoPerfilTipo.COMPONENTES_EQUIPE, componenteEquipe));
		}

		if(equipeEspecial != null && !equipeEspecial.trim().equalsIgnoreCase("")){
			peloMenosUmParametroInformado = true;
			filtroServicoPerfilTipo
							.adicionarParametro(new ParametroSimples(FiltroServicoPerfilTipo.ID_EQUIPES_COMPONENTES, equipeEspecial));
		}

		if(veiculoPropio != null && !veiculoPropio.trim().equalsIgnoreCase("") && !veiculoPropio.equals("3")){
			peloMenosUmParametroInformado = true;
			filtroServicoPerfilTipo.adicionarParametro(new ParametroSimples(FiltroServicoPerfilTipo.VEICULO_PROPIO, veiculoPropio));
		}
		...
		if(servicosPerfilTipo != null && !servicosPerfilTipo.isEmpty()){
			...
			Map resultado = controlarPaginacao(httpServletRequest, retorno, filtroServicoPerfilTipo, ServicoPerfilTipo.class.getName());
			...
			httpServletRequest.getSession(false).setAttribute("servicosPerfilTipo", servicosPerfilTipo);

		}else{
			throw new ActionServletException("atencao.pesquisa.nenhumresultado", null, "servicoTipoPerfil");
		}
