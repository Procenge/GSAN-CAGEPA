...
public class ConcluirElaborarOrdemServicoRoteiroCriteriosAction
				extends GcomAction {
	...
	public ActionForward execute(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest httpServletRequest,
		...
		montarPaginaSucesso(httpServletRequest, "Elaboração do Roteiro de Programação de Ordens de Serviços efetuado com sucesso",
						"Efetuar outra Elaboração do Roteiro", "exibirElaborarOrdemServicoRoteiroCriteriosAction.do?menu=sim&dataRoteiro="
										+ elaborarActionForm.getDataRoteiro());
	 ...
	 * @author Rafael Pinto
	 * @date 17/08/2006
	 *       TODO Regra deveria estar no Controlador.
	 * @param colecao
	 *            de OS ProgramacaoHelper
			...
			while(iteraOsProgramacao.hasNext()){
				...
				if(ordemServicoProgramacao.getId() == null
					...
					if(Util.compararData(dataRoteiroElaboracao, dataRoteiro) != 0){
						...
						// Ex: dataRoteiroElaboracao = 10/01 e dataRoteiro(DataFinalProgramacao) =
						// 12/01
						// Ex: 10/01 até 12/01
						while(Util.compararData(calendar.getTime(), dataRoteiro) == -1){

							// 2.Ordem de serviço programação com a data do roteiro da elaboração
							ProgramacaoRoteiro programacaoRoteiro = new ProgramacaoRoteiro(ordemServicoProgramacao.getProgramacaoRoteiro()
											.getDataRoteiro(), ordemServicoProgramacao.getProgramacaoRoteiro().getUltimaAlteracao(),
							...
							OrdemServicoProgramacao ordemServicoProgramacaoNova = new OrdemServicoProgramacao(ordemServicoProgramacao
											.getId(), sequencialAnterior, ordemServicoProgramacao.getIndicadorAtivo(),
											ordemServicoProgramacao.getIndicadorEquipePrincipal(), ordemServicoProgramacao
															.getUltimaAlteracao(), ordemServicoProgramacao.getEquipe(),
											ordemServicoProgramacao.getUsuarioProgramacao(),
											ordemServicoProgramacao.getUsuarioFechamento(), programacaoRoteiro, ordemServicoProgramacao
															.getOsProgramNaoEncerMotivo(), ordemServicoProgramacao.getOrdemServico());
