...
import gcom.atendimentopublico.ordemservico.FiltroServicoTipoTramite;
...
public class FiltrarTipoServicoAction
				extends GcomAction {
	...
	public ActionForward execute(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest httpServletRequest,
		...
		ActionForward retorno = actionMapping.findForward("retornarFiltroTipoServico");
		...
		// valor
		if(valorInicial != null && !valorInicial.trim().equals("") && valorFinal != null && !valorFinal.trim().equals("")){
			peloMenosUmParametroInformado = true;
		...
		if(indicadorFiscalizacaoInfracao != null && !indicadorFiscalizacaoInfracao.trim().equals("")){
			peloMenosUmParametroInformado = true;
			filtroTipoServico.adicionarParametro(new ParametroSimples(FiltroTipoServico.INDICADOR_FISCALIZACAO_INFRACAO,
							indicadorFiscalizacaoInfracao));
		}
		// indicadorVistoria
		...
		// Tempo Medio
		if(tempoMedioIncial != null && !tempoMedioIncial.trim().equals("") && tempoMedioFinal != null && !tempoMedioFinal.trim().equals("")){
			peloMenosUmParametroInformado = true;
		...
		if(!Util.isVazioOuBranco(idUnidadeOrganizacionalDestino)){
			...
			Collection<Integer> idsServicoTipo = new ArrayList<Integer>();

			FiltroServicoTipoTramite filtroServicoTipoTramite = new FiltroServicoTipoTramite();
			filtroServicoTipoTramite.adicionarParametro(new ParametroSimples(FiltroServicoTipoTramite.UNIDADE_ORGANIZACIONAL_DESTINO_ID,
			...
			if(!Util.isVazioOrNulo(colecaoServicoTipoTramite)){
				...
				for(ServicoTipoTramite servicoTipoTramite : colecaoServicoTipoTramite){
					servicoTipo = servicoTipoTramite.getServicoTipo();

					if(servicoTipo != null){
						idServicoTipo = servicoTipo.getId();

						if(!idsServicoTipo.contains(idServicoTipo)){
							idsServicoTipo.add(idServicoTipo);
						}
					}
				}
			}

			if(Util.isVazioOrNulo(idsServicoTipo)){
				idsServicoTipo.add(-1);
			}

			filtroTipoServico.adicionarParametro(new ParametroSimplesColecao(FiltroTipoServico.ID, idsServicoTipo));
		}
