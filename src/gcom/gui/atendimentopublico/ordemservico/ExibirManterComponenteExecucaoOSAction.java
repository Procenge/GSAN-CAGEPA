 ...
 * @date 23/10/2006
 */
public class ExibirManterComponenteExecucaoOSAction
				extends GcomAction {

	public ActionForward execute(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest httpServletRequest,
					HttpServletResponse httpServletResponse){

		ActionForward retorno = actionMapping.findForward("manterComponenteExecucaoOS");
		...
		if(numeroOS != null && !numeroOS.equalsIgnoreCase("")){
			...
			form.setDataExecucao(Util.formatarData(Util.converteStringInvertidaSemBarraParaDate(dataExecucaoEquipeComponente)));
			...
			form.setDataRoteiro(dataRoteiro);
			...
			if(!Util.isVazioOrNulo(colecaoEquipeComponentes)){
				...
				while(iteratorColecaoEquipeComponentes.hasNext()){
					...
					if(equipeComponentes.getFuncionario() != null){
						...
						this.informarOsExecucaoEquipeComponentes(Util.converterStringParaInteger(form.getIdAtividade()), form
										.getDataExecucao(), form.getHoraInicio(), form.getHoraFim(), equipeComponentes.getEquipe().getId(),
										colecaoSessao, equipeComponentes.getFuncionario().getId(), equipeComponentes
														.getIndicadorResponsavel(), null, fachada, true);
					}else{

						this.informarOsExecucaoEquipeComponentes(Util.converterStringParaInteger(form.getIdAtividade()), form
										.getDataExecucao(), form.getHoraInicio(), form.getHoraFim(), equipeComponentes.getEquipe().getId(),
										colecaoSessao, null, equipeComponentes.getIndicadorResponsavel(), equipeComponentes
														.getComponentes(), fachada, true);
					}
				}
			}
		...
		if(pesquisarFuncionario != null && !pesquisarFuncionario.equalsIgnoreCase("")){
			...
			sessao.setAttribute("caminhoRetornoTelaPesquisaFuncionario", "exibirManterComponenteExecucaoOSAction");
			httpServletRequest.setAttribute("nomeCampo", "idFuncionario");
		}else{
		...
		if(adicionarComponente != null && !adicionarComponente.equalsIgnoreCase("")){
			...
			this.informarOsExecucaoEquipeComponentes(Util.converterStringParaInteger(form.getIdAtividade()), form.getDataExecucao(), form
							.getHoraInicio(), form.getHoraFim(), form.getIdEquipe(), colecaoManterDadosAtividadesOrdemServicoHelper,
							idFuncionario, Short.parseShort(form.getResponsavel()), nomeComponente, fachada, false);
		...
		if(removerComponente != null && !removerComponente.equalsIgnoreCase("")){
			...
			this.removerOsExecucaoEquipeComponentes(Util.converterStringParaInteger(form.getIdAtividade()), form.getDataExecucao(), form
							.getHoraInicio(), form.getHoraFim(), form.getIdEquipe(), colecaoSessao, fachada, Long
							.parseLong(removerComponente));
	...
	private void informarOsExecucaoEquipeComponentes(Integer idAtividade, String dataExecucao, String horaInicio, String horaFim,
					Integer idEquipe, Collection colecaoSessao, Integer idFuncionario, short responsavel, String nomeComponente,
					Fachada fachada, boolean carregamentoInicial){
					...
					while(iteratorColecaoOSAtividadePeriodoExecucaoHelper.hasNext()){
						...
						if(osAtividadePeriodoExecucaoHelper.getOsAtividadePeriodoExecucao().getDataInicioFormatada().equals(dataExecucao)
										&& osAtividadePeriodoExecucaoHelper.getOsAtividadePeriodoExecucao().getHoraInicioFormatada()
														.equals(horaInicio)
										&& osAtividadePeriodoExecucaoHelper.getOsAtividadePeriodoExecucao().getHoraFimFormatada().equals(
														horaFim)){
							...
							if(osExecucaoEquipeHelper.getOsExecucaoEquipe().getEquipe().getId().intValue() == idEquipe.intValue()){
								...
								if(colecaoOSExecucaoEquipeComponentes != null && !colecaoOSExecucaoEquipeComponentes.isEmpty()){
									...
									if(!carregamentoInicial){
										...
										while(iteratorColecaoOSExecucaoEquipeComponentes.hasNext()){
											...
											if(idFuncionario != null){
												...
												if(osExecucaoEquipeComponentes.getFuncionario() != null
																&& osExecucaoEquipeComponentes.getFuncionario().getId().equals(
																				idFuncionario)){

													throw new ActionServletException("atencao.equipe_componente_ja_informado");
												}

											}else{

												if(osExecucaoEquipeComponentes.getNomeComponente() != null
																&& osExecucaoEquipeComponentes.getNomeComponente().equalsIgnoreCase(
																				nomeComponente)){
											...
											if(osExecucaoEquipeComponentes.getIndicadorResponsavel() == ConstantesSistema.INDICADOR_USO_ATIVO
															.shortValue()){
										...
										if(responsavelInformado && responsavel == ConstantesSistema.INDICADOR_USO_ATIVO.shortValue()){

											throw new ActionServletException("atencao.responsavel_ja_informado");
										}
	...
	private OsExecucaoEquipeComponentes gerarOsExecucaoEquipeComponentes(OsExecucaoEquipe osExecucaoEquipe, Integer idFuncionario,
					String nomeComponente, short responsavel, Fachada fachada){

		OsExecucaoEquipeComponentes osExecucaoEquipeComponentes = new OsExecucaoEquipeComponentes();
		...
		if(idFuncionario != null){
			...
			if(colecaoFuncionario == null || colecaoFuncionario.isEmpty()){
				...
				throw new ActionServletException("atencao.label_inexistente", null, "Funcionário");
	...
	private void removerOsExecucaoEquipeComponentes(Integer idAtividade, String dataExecucao, String horaInicio, String horaFim,
