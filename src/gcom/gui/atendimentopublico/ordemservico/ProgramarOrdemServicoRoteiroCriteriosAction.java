...
public class ProgramarOrdemServicoRoteiroCriteriosAction
				extends GcomAction {
	...
	public ActionForward execute(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest httpServletRequest,
		...
		if(tipoAcao != null && tipoAcao.equals("R")){
			...
			if(idOs == null){
				osProgramadas = this.removerTodasOrdemServicoProgramada(sessao, elaborarActionForm.getProgramadas());

				// Remove as OS selecionada na tela
			}else{
				osProgramadas = this.removerOrdemServicoProgramada(sessao, new Integer(idOs), elaborarActionForm.getProgramadas());
			...
			if(idsEquipeSelecionada.length == 0 || idsEquipeSelecionada[0].intValue() == ConstantesSistema.NUMERO_NAO_INFORMADO
							&& idsOsSelecionada.length == 0 || idsOsSelecionada[0].intValue() == ConstantesSistema.NUMERO_NAO_INFORMADO){
			...
			if(idsOsSelecionada.length == 0 || idsOsSelecionada[0].intValue() == ConstantesSistema.NUMERO_NAO_INFORMADO){
			...
			int idEquipePrincipal = new Integer(httpServletRequest.getParameter("principal")).intValue();
			...
			for(Integer idOs : idsOsSelecionada){
				...
				for(Integer idEquipe : idsEquipeSelecionada){
					...
					OrdemServico ordemServico = retornaOrdemServicoDaColecaoProgramada(sessao, idOs, equipe.getNome());
					...
					if(ordemServico == null){
						...
						String dataProgramacaoSelecionada = httpServletRequest
										.getParameter("dataFinalProgramacaoSelecionadas" + os.getId());
			...
			while(iter.hasNext()){
				...
				if(osProgramacao.getOrdemServico().getId().intValue() == idOs.intValue()){
					...
					httpServletRequest.setAttribute("colecaoAlertaEquipeLogradouro", helper.getColecaoAlertaEquipeDeLogradouro());
	 ...
	 * @date 17/08/2006
	 * @param sessao
	 *            ,id da Os e numero de OS programadas
		...
		while(iteraChave.hasNext()){
			...
			while(iteraOsProgramacao.hasNext()){
				...
				if(ordemServicoProgramacao.getOrdemServico().getId().intValue() == idOs.intValue()){
					...
					String dataRoteiro = Util.formatarData(ordemServicoProgramacao.getProgramacaoRoteiro().getDataRoteiro());
	 ...
	 * esse metodo eh usado no momento que eh feito a remoção da Os na HashMap de
	 * OSProgramacaoHelper
	 * 
	 * @author Rafael Pinto
	 * @date 17/08/2006
	 * @param sessao
	 *            ,ordem de servico e data de roteiro
	 */
	private void adcionaOrdemServicoNaColecaoSelecionada(HttpSession sessao, OrdemServico ordemServico, String dataRoteiro){
	 ...
	 * @date 17/08/2006
	 * @param sessao
	 *            ,id da OS e chave da equipe(nome da Equipe)
	 * @return OrdemServico
	 */
	 ...
	 * @date 17/08/2006
	 * @param sessao
	 *            e ordemServicoProgramacao
	 */
	private void montaOrdemServicoProgramacao(HttpSession sessao, OrdemServicoProgramacao ordemServicoProgramacao,
					Set<Equipe> colecaoEquipesPorOS){
	...
	private void preparaBarraCargaTrabalho(HttpSession sessao, Date dataRoteiro){
		...
		while(itera.hasNext()){
			...
			while(iteraOsProgramacao.hasNext()){
				...
				if(osProgramacao.getId() != null){
					...
					ObterOSDistribuidasPorEquipeHelper obterOSDistribuidasPorEquipeHelper = new ObterOSDistribuidasPorEquipeHelper();

					obterOSDistribuidasPorEquipeHelper.setIdOS(osProgramacao.getOrdemServico().getId());
					obterOSDistribuidasPorEquipeHelper.setDataFinalProgramacao(osProgramacao.getProgramacaoRoteiro().getDataRoteiro());
					...
					if(!mapEquipeOsDistribuidas.containsKey(chaveNome)){
						...
						Set<ObterOSDistribuidasPorEquipeHelper> colecaoOsDistribuidas = (HashSet) mapEquipeOsDistribuidas.get(chaveNome);
		...
		while(itera.hasNext()){
			...
			Set<ObterOSDistribuidasPorEquipeHelper> colecaoOsDistribuidas = (HashSet) mapEquipeOsDistribuidas.get(key);

			ObterCargaTrabalhoEquipeHelper obterCargaTrabalhoEquipeHelper = Fachada.getInstancia().obterCargaTrabalhoEquipe(equipe.getId(),
							colecaoIdsOSProgramadas, colecaoOsDistribuidas, dataRoteiro);
			...
			if(colecaoOsDistribuidas != null){
				qtidadeOs += colecaoOsDistribuidas.size();
			}
