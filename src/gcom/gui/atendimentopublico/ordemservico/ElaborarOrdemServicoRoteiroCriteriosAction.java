...
public class ElaborarOrdemServicoRoteiroCriteriosAction
				extends GcomAction {
	...
	public ActionForward execute(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest httpServletRequest,
		...
		ActionForward retorno = actionMapping.findForward("elaborarOrdemServico");

		HttpSession sessao = httpServletRequest.getSession(false);

		// Form
		ElaborarOrdemServicoRoteiroCriteriosActionForm elaborarActionForm = (ElaborarOrdemServicoRoteiroCriteriosActionForm) actionForm;
		...
		PesquisarOrdemServicoHelper pesquisarOrdemServicoHelper = this.montaPesquisarOrdemServicoHelper(elaborarActionForm);

		// Pesquisa a partir do filtro
		Collection<OrdemServico> colecaoOrdemServico = Fachada.getInstancia().pesquisarOrdemServicoElaborarProgramacao(
						pesquisarOrdemServicoHelper);
	 ...
	 */
	private PesquisarOrdemServicoHelper montaPesquisarOrdemServicoHelper(ElaborarOrdemServicoRoteiroCriteriosActionForm elaborarActionForm){

		// Unidade Lotacao
		Integer idUnidadeLotacao = Integer.valueOf(elaborarActionForm.getUnidadeLotacao());

		// Origem Servico
		Integer origemServico = Integer.valueOf(elaborarActionForm.getOrigemServicos());

		// Criterio Seleção
		Integer criterio = Integer.valueOf(elaborarActionForm.getCriterioSelecao());
		...
		String indicadorProcessoAdmJud = elaborarActionForm.getIndicadorProcessoAdmJud();
		...
		if((diaAtrasoFinal != null && !diaAtrasoFinal.equals("")) && (diaAtrasoInicial != null && !diaAtrasoInicial.equals(""))){

			Calendar calendar = Calendar.getInstance();

			int dia = Integer.valueOf(diaAtrasoInicial).intValue();

			calendar.add(Calendar.DAY_OF_MONTH, -dia);
			dataAtrasoInicial = calendar.getTime();

			dia = Integer.valueOf(diaAtrasoFinal).intValue();
		...
		}else if((diaAtrasoFinal != null && !diaAtrasoFinal.equals("")) || (diaAtrasoInicial != null && !diaAtrasoInicial.equals(""))){

			Calendar calendar = Calendar.getInstance();

			int dia = ConstantesSistema.NUMERO_NAO_INFORMADO;
			if(diaAtrasoInicial != null && !diaAtrasoInicial.equals("")){
				dia = Integer.valueOf(diaAtrasoInicial).intValue();
			}else{
				dia = Integer.valueOf(diaAtrasoFinal).intValue();
			}
		...
		if(elaborarActionForm.getPeriodoAtendimentoInicial() != null && !elaborarActionForm.getPeriodoAtendimentoInicial().equals("")){

			dataAtendimentoInicial = Util.converteStringParaDate(elaborarActionForm.getPeriodoAtendimentoInicial());

			dataAtendimentoFinal = null;
			if(elaborarActionForm.getPeriodoAtendimentoFinal() != null && !elaborarActionForm.getPeriodoAtendimentoFinal().equals("")){

				dataAtendimentoFinal = Util.converteStringParaDate(elaborarActionForm.getPeriodoAtendimentoFinal());
			}else{
		...
		if(elaborarActionForm.getPeriodoGeracaoInicial() != null && !elaborarActionForm.getPeriodoGeracaoInicial().equals("")){

			dataGeracaoInicial = Util.converteStringParaDate(elaborarActionForm.getPeriodoGeracaoInicial());

			dataGeracaoFinal = null;

			if(elaborarActionForm.getPeriodoGeracaoFinal() != null && !elaborarActionForm.getPeriodoGeracaoFinal().equals("")){

				dataGeracaoFinal = Util.converteStringParaDate(elaborarActionForm.getPeriodoGeracaoFinal());
		...
		if(elaborarActionForm.getPeriodoClienteInicial() != null && !elaborarActionForm.getPeriodoClienteInicial().equals("")){

			dataClienteInicial = Util.converteStringParaDate(elaborarActionForm.getPeriodoClienteInicial());

			dataClienteFinal = null;

			if(elaborarActionForm.getPeriodoClienteFinal() != null && !elaborarActionForm.getPeriodoClienteFinal().equals("")){

				dataClienteFinal = Util.converteStringParaDate(elaborarActionForm.getPeriodoClienteFinal());
		...
		if(elaborarActionForm.getPeriodoAgenciaInicial() != null && !elaborarActionForm.getPeriodoAgenciaInicial().equals("")){

			dataAgenciaInicial = Util.converteStringParaDate(elaborarActionForm.getPeriodoAgenciaInicial());

			dataAgenciaFinal = null;

			if(elaborarActionForm.getPeriodoAgenciaFinal() != null && !elaborarActionForm.getPeriodoAgenciaFinal().equals("")){

				dataAgenciaFinal = Util.converteStringParaDate(elaborarActionForm.getPeriodoAgenciaFinal());
	 ...
	 * @date 17/08/2006
	 * @param colecao
	 *            de ordem servico e data de roteiro
	 ...
	 * @date 17/08/2006
	 * @param sessao
	 *            ,data do roteiro,ElaborarOrdemServicoRoteiroCriteriosActionForm
	 */
	private void montaOrdemServicoProgramacao(HttpSession sessao, Date dataRoteiro, Integer idUnidadeLotacao){
		...
		if(colecaoOrdemServicoProgramacao != null && !colecaoOrdemServicoProgramacao.isEmpty()){
			...
			while(itera.hasNext()){
				...
				if(ordemServico.getSituacao() == OrdemServico.SITUACAO_EXECUCAO_EM_ANDAMENTO.shortValue()){
					helper.setSituacao("" + ordemServico.getSituacao());
					helper.setSituacaoDescricaoAbreviada("" + OrdemServico.SITUACAO_DESC_ABREV_EXECUCAO_EM_ANDAMENTO);
				}
