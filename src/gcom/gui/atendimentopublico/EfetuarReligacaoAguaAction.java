 ...
 * @date 07/07/2006
 */
public class EfetuarReligacaoAguaAction
				extends GcomAction {
	...
	public ActionForward execute(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest httpServletRequest,
		...
		RegistradorOperacao registradorOperacao = new RegistradorOperacao(Operacao.OPERACAO_RELIGACAO_AGUA_EFETUAR,
						new UsuarioAcaoUsuarioHelper(usuario, UsuarioAcao.USUARIO_ACAO_EFETUOU_OPERACAO));

		Operacao operacao = new Operacao();
		operacao.setId(Operacao.OPERACAO_RELIGACAO_AGUA_EFETUAR);
		...
		if(sessao.getAttribute("ordemServico") == null){
			...
			String idOrdemServico = efetuarReligacaoAguaActionForm.getIdOrdemServico();
			...
			if(idOrdemServico != null && !idOrdemServico.trim().equals("")){
				...
				try{
					...
					ex.setUrlBotaoVoltar("/gsan/exibirEfetuarReligacaoAguaAction.do");
					throw ex;
				}
		...
		String idFuncionario = efetuarReligacaoAguaActionForm.getIdFuncionario();

		if(ordemServico != null){ // && efetuarReligacaoAguaActionForm.getIdTipoDebito() != null){
			...
			if(efetuarReligacaoAguaActionForm.getValorDebito() != null){
				String valorDebito = efetuarReligacaoAguaActionForm.getValorDebito().toString().replace(".", "");
			...
			if(valorPercentual != null){
				ordemServico.setPercentualCobranca(new BigDecimal(efetuarReligacaoAguaActionForm.getPercentualCobranca()));
			}
		...
		LigacaoAgua ligacaoAgua = getFachada().pesquisarLigacaoAgua(imovel.getId());
		...
		// validar Data Corte
		if(efetuarReligacaoAguaActionForm.getDataReligacao() != null && !efetuarReligacaoAguaActionForm.getDataReligacao().equals("")){
			Date data = Util.converteStringParaDate(efetuarReligacaoAguaActionForm.getDataReligacao());
		...
		if(efetuarReligacaoAguaActionForm.getVeioEncerrarOS().equalsIgnoreCase("FALSE")){
		...
		if(retorno.getName().equalsIgnoreCase("telaSucesso")){
			montarPaginaSucesso(httpServletRequest, "Religação de Água para o imóvel " + imovel.getId() + " efetuada com sucesso",
							"Efetuar outra Religação de Água", "exibirEfetuarReligacaoAguaAction.do?menu=sim");
		}
