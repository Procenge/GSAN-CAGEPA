 ...
 * @date 15/01/2007
 */
public class RegistroAtendimentoTramitacaoPopupAction
				extends GcomAction {
	...
	public ActionForward execute(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest httpServletRequest,
		...
		unidadeDestino.setId(Integer.parseInt(form.getIdUnidadeDestino()));
		unidadeDestino.setDescricao(form.getDescricaoUnidadeDestino());

		// [FS0006] Valida Data
		// [FS0007] Valida Hora
		// [FS0008] Valida Unidade Destino
		fachada.validarConjuntoTramitacao(ids, dataHoraTramitacaoObjetoDate, Integer.parseInt(form.getIdUnidadeDestino()));
		...
		if(ids != null && ids.length != 0){
			for(int i = 0; i < ids.length; i++){
				...
				if(sessao.getAttribute("tramites") != null && !sessao.getAttribute("tramites").equals("")){
					...
					if(tramitesSessao != null && !tramitesSessao.isEmpty()){
						...
						while(iteratorTramite.hasNext()){
							...
							// Caso exita na colecão da sessão o registro de atendimento selecionado
							// atualiza o tramite existente.
							if(tramiteColecao.getRegistroAtendimento().getId().equals(Integer.parseInt(idsTramitacao[0]))){
								...
								// Motivo do Tramite
								if(form.getMotivoTramite() != null && !form.getMotivoTramite().equals("")
												&& !form.getMotivoTramite().equals("-1")){
									MotivoTramite motivoTramite = new MotivoTramite();
									motivoTramite.setId(new Integer(form.getMotivoTramite()));
									tramiteColecao.setMotivoTramite(motivoTramite);
								}
								// Data da tramitação
				...
				if(!achou){
					...
					unidadeAtual.setId(Integer.parseInt(idsTramitacao[1]));
					...
					// Motivo do Tramite
					if(form.getMotivoTramite() != null && !form.getMotivoTramite().equals("") && !form.getMotivoTramite().equals("-1")){
						MotivoTramite motivoTramite = new MotivoTramite();
						motivoTramite.setId(new Integer(form.getMotivoTramite()));
					...
					// indicador de tramite unicamente para RA ou parta RA em conjunto com OS
					tramite.setIndicadorTramiteApenasRA(form.getIndicadorTramiteApenasRA());
