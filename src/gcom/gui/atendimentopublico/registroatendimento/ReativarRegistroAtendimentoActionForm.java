 ...
 * @date 17/08/2006
 */
public class ReativarRegistroAtendimentoActionForm
	...
	public RegistroAtendimento setFormValues(RegistroAtendimento ra, String parametro){
		...
		if(getTempoEsperaInicial() != null && !getTempoEsperaInicial().equals("")){
			Date dataEsperaInicial = Util.converteStringParaDateHora(getDataAtendimentoReativado() + " " + getTempoEsperaInicial() + ":00");
			ra.setDataInicioEspera(dataEsperaInicial);
			Date dataEsperaFinal = Util.converteStringParaDateHora(getDataAtendimentoReativado() + " " + getTempoEsperaFinal() + ":00");
		...
		if(getSenhaAtendimento() != null && !getSenhaAtendimento().equals("")){
			ra.setSenhaAtendimento(Integer.parseInt(getSenhaAtendimento()));
		}
		// Especificação
		...
		Date dataPrevistaObjetoDate = new Date();
		if(parametro.equals(SistemaParametro.INDICADOR_EMPRESA_DESO.toString())){
			dataPrevistaObjetoDate = Util.converteStringParaDate(getDataPrevistaReativado());
		}else{
			dataPrevistaObjetoDate = Util.converteStringParaDateHora(getDataPrevistaReativado() + ":00");
		}
	...
	/**
	 * @param resetarReativar
	 *            O resetarReativar a ser setado.
	 */
