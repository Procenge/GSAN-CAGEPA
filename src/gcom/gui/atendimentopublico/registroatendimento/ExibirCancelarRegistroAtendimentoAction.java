 ...
 * @date 17/10/2014
 */
public class ExibirCancelarRegistroAtendimentoAction
				extends GcomAction {
	...
	public ActionForward execute(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest httpServletRequest,
		...
		ActionForward retorno = actionMapping.findForward("cancelarRegistroAtendimento");
		...
		// Reseta Tramitação
		if(form.getResetarEncerramento().equalsIgnoreCase("true")){
			form.resetarEncerramento();
		}
		...
		if(httpServletRequest.getParameter("numerosRAs") != null){
			form.setMotivoEncerramentoId("-1");
			form.setNumerosRAs(httpServletRequest.getParameter("numerosRAs").toString());
		}

		form.setDataEncerramento(Util.formatarData(new Date()));
		form.setHoraEncerramento(Util.formatarHoraSemSegundos(new Date()));
		getMotivoEncerramentoCollection(form, sessao);
	...
	private void getMotivoEncerramentoCollection(EncerrarRegistroAtendimentoActionForm form, HttpSession sessao){
		...
		filtroAtendimentoMotivoEncerramento.adicionarParametro(new ParametroSimplesDiferenteDe(
						FiltroAtendimentoMotivoEncerramento.INDICADOR_DUPLICIDADE, ConstantesSistema.SIM));
		filtroAtendimentoMotivoEncerramento.adicionarParametro(new ParametroSimplesDiferenteDe(
