...
public class ReativarRegistroAtendimentoAction
				extends GcomAction {
	...
	public ActionForward execute(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest httpServletRequest,
		 ...
		 */
		fachada.validarInserirRegistroAtendimentoDadosGerais(form.getDataAtendimentoReativado(), form.getHoraAtendimentoReativado(), form
						.getTempoEsperaInicial(), form.getTempoEsperaFinal(), form.getIdUnidadeAtendimento(), null, form
						.getEspecificacaoId(), null);
		...
		form.setFormValues(ra, this.getParametroCompanhia(httpServletRequest).toString());
		...
		// Incluir o novo Registro de Atendimento
		/*
		 * Integer idRAGerado = fachada.reativarRegistroAtendimento(ra,
		 * Util.converterStringParaInteger(form.getIdUnidadeAtendimento()),
		 * usuario.getId(),Util.converterStringParaInteger(form.getIdClienteSolicitante()) ,
		 * form.getIdRaSolicitante() ,Util.converterStringParaInteger(form.getIdUnidadeDestino()) ,
		 * form.getParecerUnidadeDestino(), os);
		 */

		Integer[] idRAGerado = fachada.reativarRegistroAtendimento(ra, Util.converterStringParaInteger(form.getIdUnidadeAtendimento()),
						usuario.getId(), Util.converterStringParaInteger(form.getIdClienteSolicitante()), form.getIdRaSolicitante(), Util
										.converterStringParaInteger(form.getIdUnidadeDestino()), form.getParecerUnidadeDestino(), Util
										.converterStringParaInteger(form.getTipoSolicitacaoId()));
		...
		// if
		// (fachada.gerarOrdemServicoOpcional(Util.converterStringParaInteger(form.getEspecificacaoId()))){

		if(fachada.gerarOrdemServicoAutomatica(ra.getSolicitacaoTipoEspecificacao().getId())){

			montarPaginaSucesso(httpServletRequest, "RA - Registro de Atendimento de código " + idRAGerado[0]
							+ " e Ordem de Serviço de código "
							+ idRAGerado[1] + " inseridos com sucesso.", "Voltar", "exibirConsultarRegistroAtendimentoAction.do?numeroRA="
							+ idRAGerado[0]);
		}else{

			montarPaginaSucesso(httpServletRequest, "RA - Registro de Atendimento de código " + idRAGerado[0] + " inserido com sucesso.",
							"Voltar", "exibirConsultarRegistroAtendimentoAction.do?numeroRA=" + idRAGerado[0]);
		}

		/*
		 * }else{
		 * montarPaginaSucesso(httpServletRequest, "RA - Registro de Atendimento de código "
		 * + idRAGerado + " inserido com sucesso.", "", "");
		 * }
		 */
	...
	private void validarUnidadeAtendimento(ReativarRegistroAtendimentoActionForm form, Fachada fachada, Usuario usuario){
		...
		Short indicadorAutorizacao = fachada.obterIndicadorAutorizacaoManutencaoRA(Util.converterStringParaInteger(form
						.getIdUnidadeAtendimento()), usuario.getId());
		if(indicadorAutorizacao == null || indicadorAutorizacao.equals(ConstantesSistema.NAO)){
			throw new ActionServletException("atencao.unidade_nao_autorizada", null, form.getUnidadeAtendimento());
		}
	}
