...
public class ExibirResultadoDetalhadoProgramacaoAbastecimentoManutencaoAction
	...
	public ActionForward execute(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest httpServletRequest,
		...
		String sistemaAbastecimento = (String) sessao.getAttribute("sistemaAbastecimento");

		resultadoDetalhadoProgramacaoAbastecimentoManutencaoActionForm.setSistemaAbastecimento(sistemaAbastecimento);

		String zonaAbastecimento = (String) sessao.getAttribute("zonaAbastecimento");

		resultadoDetalhadoProgramacaoAbastecimentoManutencaoActionForm.setZonaAbastecimento(zonaAbastecimento);

		String municipio = (String) sessao.getAttribute("municipio");

		resultadoDetalhadoProgramacaoAbastecimentoManutencaoActionForm.setMunicipio(municipio);

		String bairro = (String) sessao.getAttribute("bairro");

		resultadoDetalhadoProgramacaoAbastecimentoManutencaoActionForm.setBairro(bairro);

		String bairroArea = (String) sessao.getAttribute("bairroArea");

		resultadoDetalhadoProgramacaoAbastecimentoManutencaoActionForm.setAreaBairro(bairroArea);

		String codigoBairro = (String) sessao.getAttribute("idBairro");

		resultadoDetalhadoProgramacaoAbastecimentoManutencaoActionForm.setCodigoBairro(codigoBairro);

		String codigoMunicipio = (String) sessao.getAttribute("idMunicipio");

		resultadoDetalhadoProgramacaoAbastecimentoManutencaoActionForm.setCodigoMunicipio(codigoMunicipio);

		String codigoSistemaAbastecimento = (String) sessao.getAttribute("codigoSistemaAbastecimento");

		resultadoDetalhadoProgramacaoAbastecimentoManutencaoActionForm.setCodigoSistemaAbastecimento(codigoSistemaAbastecimento);

		String codigoZonaAbastecimento = (String) sessao.getAttribute("codigoZonaAbastecimento");

		resultadoDetalhadoProgramacaoAbastecimentoManutencaoActionForm.setCodigoZonaAbastecimento(codigoZonaAbastecimento);

		String codigoAreaBairro = (String) sessao.getAttribute("areaBairro");

		resultadoDetalhadoProgramacaoAbastecimentoManutencaoActionForm.setCodigoAreaBairro(codigoAreaBairro);
		...
		if(sessao.getAttribute("pesquisa") != null){
			...
			colecaoProgramacaoAbastecimento = (Collection) sessao.getAttribute("colecaoProgramacaoAbastecimento");
			colecaoProgramacaoManutencao = (Collection) sessao.getAttribute("colecaoProgramacaoManutencao");
		}else{
			String mesAnoReferencia = (String) sessao.getAttribute("mesAnoReferencia");
			colecaoProgramacaoAbastecimento = fachada.consultarProgramacaoAbastecimento(codigoMunicipio, codigoBairro, codigoAreaBairro,
							mesAnoReferencia);
			colecaoProgramacaoManutencao = fachada.consultarProgramacaoManutencao(codigoMunicipio, codigoBairro, codigoAreaBairro,
							mesAnoReferencia);
		}
		...
		if(mes < 10){
			...
			dataReferencia = diaCalendario + "/0" + mes.toString() + "/" + anoCalendario;

			String teste = anoCalendario + "0" + mes.toString() + diaCalendario;

			dataReferenciaFormatada = Util.converteStringInvertidaSemBarraParaDate(teste);

		}else{
			dataReferencia = diaCalendario + "/" + mes.toString() + "/" + anoCalendario;

			dataReferenciaFormatada = Util.converteStringParaDate(dataReferencia);
		}

		resultadoDetalhadoProgramacaoAbastecimentoManutencaoActionForm.setDataReferencia(dataReferencia);

		if(colecaoProgramacaoAbastecimento != null && !colecaoProgramacaoAbastecimento.isEmpty()){

			Iterator colecaoProgramacaoAbastecimentoIterator = colecaoProgramacaoAbastecimento.iterator();
			...
			while(colecaoProgramacaoAbastecimentoIterator.hasNext()){
				...
				abastecimentoProgramacao = (AbastecimentoProgramacao) colecaoProgramacaoAbastecimentoIterator.next();

				dataInicioAbastecimento = abastecimentoProgramacao.getDataInicio();
				dataFimAbastecimento = abastecimentoProgramacao.getDataFim();

				if(dataReferenciaFormatada.compareTo(dataInicioAbastecimento) >= 0
								&& dataReferenciaFormatada.compareTo(dataFimAbastecimento) <= 0){
					break;
				}else{
			...
			if(abastecimentoProgramacao != null){
				...
				if((dataInicioAbastecimento.compareTo(dataReferenciaFormatada) < 0)
								&& ((dataFimAbastecimento.compareTo(dataReferenciaFormatada) > 0))){

					abastecimentoProgramacao.setDataInicio(dataReferenciaFormatada);
					abastecimentoProgramacao.setDataFim(dataReferenciaFormatada);

					abastecimentoProgramacao.setHoraInicio(Util.formatarDataInicial(dataReferenciaFormatada));

					abastecimentoProgramacao.setHoraFim(Util.formatarDataFinal(dataReferenciaFormatada));

					colecaoProgramacaoAbastecimento.removeAll(colecaoProgramacaoAbastecimento);

					colecaoProgramacaoAbastecimento.add(abastecimentoProgramacao);

					httpServletRequest.setAttribute("colecaoProgramacaoAbastecimento", colecaoProgramacaoAbastecimento);
				...
				if((dataInicioAbastecimento.compareTo(dataFimAbastecimento) == 0)
								&& (dataInicioAbastecimento.compareTo(dataReferenciaFormatada) == 0)
				...
				if(dataInicioAbastecimento.compareTo(dataFimAbastecimento) != 0){
					...
					}else if(dataFimAbastecimento.compareTo(dataReferenciaFormatada) == 0){
		...
		if(colecaoProgramacaoManutencao != null && !colecaoProgramacaoManutencao.isEmpty()){
			...
			Iterator colecaoProgramacaoManutencaoIterator = colecaoProgramacaoManutencao.iterator();
			...
			while(colecaoProgramacaoManutencaoIterator.hasNext()){
				...
				manutencaoProgramacao = (ManutencaoProgramacao) colecaoProgramacaoManutencaoIterator.next();
			...
			if(manutencaoProgramacao != null){
				...
				if((dataInicioManutencao.compareTo(dataReferenciaFormatada) < 0)
								&& ((dataFimManutencao.compareTo(dataReferenciaFormatada) > 0))){

					manutencaoProgramacao.setDataInicio(dataReferenciaFormatada);
					manutencaoProgramacao.setDataFim(dataReferenciaFormatada);

					colecaoProgramacaoManutencao.removeAll(colecaoProgramacaoManutencao);

					colecaoProgramacaoManutencao.add(manutencaoProgramacao);

					httpServletRequest.setAttribute("colecaoProgramacaoManutencao", colecaoProgramacaoManutencao);
