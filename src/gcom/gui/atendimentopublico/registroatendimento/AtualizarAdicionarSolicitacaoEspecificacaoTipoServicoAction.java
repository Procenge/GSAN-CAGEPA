 ...
 * @date 22/11/2006
 */
public class AtualizarAdicionarSolicitacaoEspecificacaoTipoServicoAction
				extends GcomAction {
	...
	public ActionForward execute(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest httpServletRequest,
		...
		ActionForward retorno = actionMapping.findForward("exibirAtualizarAdicionarSolicitacaoEspecificacao");
		...
		if(atualizarAdicionarSolicitacaoEspecificacaoActionForm.getIdTipoServico() != null
						&& !atualizarAdicionarSolicitacaoEspecificacaoActionForm.getIdTipoServico().equals("")){
			FiltroServicoTipo filtroServicoTipo = new FiltroServicoTipo();
			filtroServicoTipo.adicionarParametro(new ParametroSimples(FiltroServicoTipo.ID,
							atualizarAdicionarSolicitacaoEspecificacaoActionForm.getIdTipoServico()));
			...
			if(servicoTipoEncontrado != null && !servicoTipoEncontrado.isEmpty()){
				ServicoTipo servicoTipo = (ServicoTipo) ((List) servicoTipoEncontrado).get(0);
				especificacaoServicoTipoPK.setIdServicoTipo(servicoTipo.getId());
				especificacaoServicoTipo.setServicoTipo(servicoTipo);
			}else{
				throw new ActionServletException("atencao.pesquisa_inexistente", null, "Serviço Tipo");
			}

		}else{
			throw new ActionServletException("atencao.required", null, "Tipo do Serviço");
		}

		// id do tipo de servico
		if(atualizarAdicionarSolicitacaoEspecificacaoActionForm.getOrdemExecucao() != null
						&& !atualizarAdicionarSolicitacaoEspecificacaoActionForm.getOrdemExecucao().equals("")){
			Short ordemExecucao = new Short(atualizarAdicionarSolicitacaoEspecificacaoActionForm.getOrdemExecucao());
			// [SF0004] - Validar Valor Ordem de Serviço 1ª parte
			fachada.verificarExistenciaOrdemExecucao(colecaoEspecificacaoServicoTipo, ordemExecucao);
		...
		especificacaoServicoTipo.setIndicadorGeracaoAutomatica(ConstantesSistema.NAO);

		Boolean encontrou = Boolean.FALSE;

		for(EspecificacaoServicoTipo especSercTipo : (HashSet<EspecificacaoServicoTipo>) colecaoEspecificacaoServicoTipo){

			Integer idServicoTipo = especSercTipo.getComp_id().getIdServicoTipo();

			if(idServicoTipo.equals(especificacaoServicoTipoPK.getIdServicoTipo())){
