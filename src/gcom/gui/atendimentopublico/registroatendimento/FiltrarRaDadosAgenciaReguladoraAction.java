...
public class FiltrarRaDadosAgenciaReguladoraAction
				extends GcomAction {
	...
	public ActionForward execute(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest httpServletRequest,
		...
		if(motivoReclamacao != null && !motivoReclamacao.trim().equalsIgnoreCase(String.valueOf(ConstantesSistema.NUMERO_NAO_INFORMADO))){

			peloMenosUmParametroInformado = true;

			filtroRaDadosAgenciaReguladora.adicionarParametro(new ParametroSimples(FiltroRaDadosAgenciaReguladora.MOTIVO_RECLAMACAO_ID,
							motivoReclamacao));
		}

		// Motivo Encerramento do Atendimento

		if(motivoEncerramentoAtendimento != null
						&& !motivoEncerramentoAtendimento.trim().equalsIgnoreCase(String.valueOf(ConstantesSistema.NUMERO_NAO_INFORMADO))){

			peloMenosUmParametroInformado = true;

			filtroRaDadosAgenciaReguladora.adicionarParametro(new ParametroSimples(FiltroRaDadosAgenciaReguladora.MOTIVO_ENCERRAMENTO_ID,
		...
		if(indicadorSituacaoAgencia != null && !indicadorSituacaoAgencia.equalsIgnoreCase("")
			...
			filtroRaDadosAgenciaReguladora.adicionarParametro(new ParametroSimples(
							FiltroRaDadosAgenciaReguladora.INDICADOR_SITUACAO_AGENCIA, indicadorSituacaoAgencia));
		...
		if(periodoReclamacaoInicio != null && !periodoReclamacaoInicio.trim().equals("") && periodoReclamacaoFim != null
						&& !periodoReclamacaoFim.trim().equals("")){

			peloMenosUmParametroInformado = true;

			filtroRaDadosAgenciaReguladora.adicionarParametro(new Intervalo(FiltroRaDadosAgenciaReguladora.DATA_RECLAMACAO, Util
							.converteStringParaDate(periodoReclamacaoInicio), Util.converteStringParaDate(periodoReclamacaoFim)));

		}

		// Período do Retorno

		if(periodoRetornoInicio != null && !periodoRetornoInicio.trim().equals("") && periodoRetornoFim != null
						&& !periodoRetornoFim.trim().equals("")){

			peloMenosUmParametroInformado = true;

			filtroRaDadosAgenciaReguladora.adicionarParametro(new Intervalo(FiltroRaDadosAgenciaReguladora.DATA_RETORNO, Util
							.converteStringParaDate(periodoRetornoInicio), Util.converteStringParaDate(periodoRetornoFim)));

		}

		// Motivo do Retorno para Agência Reguladora

		if(motivoRetornoAgencia != null
						&& !motivoRetornoAgencia.trim().equalsIgnoreCase(String.valueOf(ConstantesSistema.NUMERO_NAO_INFORMADO))){

			peloMenosUmParametroInformado = true;

			filtroRaDadosAgenciaReguladora.adicionarParametro(new ParametroSimples(FiltroRaDadosAgenciaReguladora.MOTIVO_RETORNO_ID,
							motivoRetornoAgencia));
		}
