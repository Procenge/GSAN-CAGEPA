 ...
 * @date 10/01/2007
 */
public class ExibirRegistroAtendimentoTramitacaoPopupAction
				extends GcomAction {
	...
	public ActionForward execute(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest httpServletRequest,
		...
		if(consultaUnidadeDestino != null && !consultaUnidadeDestino.trim().equalsIgnoreCase("")
			...
			Collection colecaoUnidadeDestino = fachada.pesquisar(filtroUnidadeOrganizacional, UnidadeOrganizacional.class.getName());
		...
		if(consultaUsuarioResponsavel != null && !consultaUsuarioResponsavel.trim().equalsIgnoreCase("")
						&& (Integer.parseInt(consultaUsuarioResponsavel)) == 1){

			// Filtro para obter o Unidade Superior ativo de id informado
			FiltroUsuario filtroUsuario = new FiltroUsuario();

			filtroUsuario.adicionarParametro(new ParametroSimples(FiltroUsuario.ID, new Integer(form.getIdUsuarioResponsavel())));
			...
			if(colecaoUsuarios != null && !colecaoUsuarios.isEmpty()){
				...
				// Obtém o objeto da coleção pesquisada
				Usuario usuarioResponsavel = (Usuario) Util.retonarObjetoDeColecao(colecaoUsuarios);
		...
		if(httpServletRequest.getParameter("tipoConsulta") != null && !httpServletRequest.getParameter("tipoConsulta").equals("")){
			...
			if(httpServletRequest.getParameter("tipoConsulta").equals("usuario")){

				form.setIdUsuarioResponsavel(httpServletRequest.getParameter("idCampoEnviarDados"));
				form.setDescricaoUsuarioResponsavel(httpServletRequest.getParameter("descricaoCampoEnviarDados"));

			}
		}

		if(Util.isVazioOuBranco(form.getIndicadorTramiteApenasRA())){
			form.setIndicadorTramiteApenasRA("1");
		}
