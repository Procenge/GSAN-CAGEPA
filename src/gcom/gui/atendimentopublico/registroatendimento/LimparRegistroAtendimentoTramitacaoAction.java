 ...
 * @date 15/01/2007
 */
public class LimparRegistroAtendimentoTramitacaoAction
				extends GcomAction {
		...
		if(ids != null && ids.length != 0){
			...
			for(int i = 0; i < ids.length; i++){
				...
				if(tramites != null && !tramites.isEmpty()){
					...
					while(iteratorTramite.hasNext()){
						...
						if(tramiteColecao.getRegistroAtendimento().getId().equals(Integer.parseInt(idsTramitacao[0]))){
							// Unidade Destino
							tramiteColecao.setUnidadeOrganizacionalDestino(null);
				...
				if(achou){
					...
					// Unidade Destino
					atualizarUnidadeDestinoColecao(Integer.parseInt(idsTramitacao[0]), sessao);
				}
			}
		}
