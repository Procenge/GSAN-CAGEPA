 ...
 * @date 05/08/2006
 */
public class PesquisarTipoSolicitacaoEspecificacoesAction
				extends GcomAction {
	...
	public ActionForward execute(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest httpServletRequest,
		...
		ActionForward retorno = actionMapping.findForward("listaPesquisarTipoSolicitacaoEspecificacoesResultado");
		...
		String descricao = pesquisarTipoSolicitacaoEspecificacoesActionForm.getDescricao();

		String solicitacaoTipoGrupo = pesquisarTipoSolicitacaoEspecificacoesActionForm.getSolicitacaoTipoGrupo();

		String indicadorFaltaAgua = pesquisarTipoSolicitacaoEspecificacoesActionForm.getIndicadorFaltaAgua();
		...
		if(descricao != null && !descricao.trim().equalsIgnoreCase("")){
			...
			filtroSolicitacaoTipo.adicionarParametro(new ComparacaoTexto(FiltroSolicitacaoTipo.DESCRICAO, descricao));

		}

		// Verifica se o campo Grupo do Tipo da Solicitação foi informado

		if(solicitacaoTipoGrupo != null && !solicitacaoTipoGrupo.trim().equalsIgnoreCase("" + ConstantesSistema.NUMERO_NAO_INFORMADO)){
		...
		if(indicadorFaltaAgua != null && !indicadorFaltaAgua.trim().equals("")){
			peloMenosUmParametroInformado = true;
			filtroSolicitacaoTipo.adicionarParametro(new ParametroSimples(FiltroSolicitacaoTipo.INDICADOR_FALTA_AGUA, indicadorFaltaAgua));
		...
		Collection colecaoSolicitacaoTipo = (Collection) fachada.pesquisar(filtroSolicitacaoTipo, SolicitacaoTipo.class.getName());

		if(colecaoSolicitacaoTipo == null || colecaoSolicitacaoTipo.isEmpty()){
			throw new ActionServletException("atencao.naocadastrado", null, "Solicitação de Especificações");
		}
