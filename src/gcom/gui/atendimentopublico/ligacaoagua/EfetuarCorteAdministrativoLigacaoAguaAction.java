...
public class EfetuarCorteAdministrativoLigacaoAguaAction
				extends GcomAction {
	...
	public ActionForward execute(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest httpServletRequest,
		...
		String tipoCorteId = corteAdministrativoLigacaoAguaActionForm.getTipoCorte();
		OrdemServico ordemServico = (OrdemServico) sessao.getAttribute("ordemServico");
		Operacao operacao = new Operacao();
		operacao.setId(Operacao.OPERACAO_CORTE_ADMINISTRATIVO_LIGACAO_AGUA_EFETUAR);
		...
		if(corteAdministrativoLigacaoAguaActionForm.getDataCorte() != null && corteAdministrativoLigacaoAguaActionForm.getDataCorte() != ""){

			dataCorteAdministrativo = Util.converteStringParaDate(corteAdministrativoLigacaoAguaActionForm.getDataCorte());
		}else{
		...
		corteTipo.setId(Integer.valueOf(tipoCorteId));
		...
		if(corteAdministrativoLigacaoAguaActionForm.getMotivoNaoCobranca() != null
						&& !corteAdministrativoLigacaoAguaActionForm.getMotivoNaoCobranca().equals("-1")){
			servicoNaoCobrancaMotivo = new ServicoNaoCobrancaMotivo();
			servicoNaoCobrancaMotivo.setId(Integer.valueOf(corteAdministrativoLigacaoAguaActionForm.getMotivoNaoCobranca()));
		}
		ordemServico.setServicoNaoCobrancaMotivo(servicoNaoCobrancaMotivo);

		BigDecimal valorAtual = new BigDecimal(0);
		if(corteAdministrativoLigacaoAguaActionForm.getValorDebito() != null){
			String valorDebito = corteAdministrativoLigacaoAguaActionForm.getValorDebito().toString().replace(".", "");
		...
		if(corteAdministrativoLigacaoAguaActionForm.getPercentualCobranca() != null
						&& !corteAdministrativoLigacaoAguaActionForm.getPercentualCobranca().equals("")){
			ordemServico.setPercentualCobranca(new BigDecimal(corteAdministrativoLigacaoAguaActionForm.getPercentualCobranca()));
		}
		// ordemServico.setUltimaAlteracao(corteAdministrativoLigacaoAguaActionForm.getDataConcorrencia());

		String idFuncionario = corteAdministrativoLigacaoAguaActionForm.getIdFuncionario();
		...
		if(ligacaoAgua.getMotivoCorte() != null && ligacaoAgua.getMotivoCorte().getIndicadorCorteFaltaPagamento().equals(new Short("1"))){
		...
		}else if(ligacaoAgua.getMotivoCorte() != null && ligacaoAgua.getMotivoCorte().getIndicadorCorteInfracao().equals(new Short("1"))){
		...
		}else if(ligacaoAgua.getMotivoCorte() != null && ligacaoAgua.getMotivoCorte().getIndicadorCortePedido().equals(new Short("1"))){
		...
		if(corteAdministrativoLigacaoAguaActionForm.getVeioEncerrarOS().equalsIgnoreCase("true")){
		...
		if(corteAdministrativoLigacaoAguaActionForm.getQuantidadeParcelas() != null
						&& !corteAdministrativoLigacaoAguaActionForm.getQuantidadeParcelas().equals("")){
			dadosHelper.setQtdeParcelas(Integer.valueOf(corteAdministrativoLigacaoAguaActionForm.getQuantidadeParcelas()).intValue());
		}else{
		...
		if(ordemServico.getServicoTipo().getDebitoTipo() != null && ordemServico.getServicoNaoCobrancaMotivo() == null){
			...
			try{
				...
				ex.setUrlBotaoVoltar("/gsan/exibirEfetuarCorteAdministrativoLigacaoAguaAction.do");
				throw ex;
			}
		}
		...
		if(retorno.getName().equalsIgnoreCase("telaSucesso")){
			...
			montarPaginaSucesso(httpServletRequest, "Corte Administrativo da Ligação de Água do imóvel " + imovel.getId()
							+ " efetuada com Sucesso ", "Efetuar outro Corte Administrativo da Ligação de Água",
