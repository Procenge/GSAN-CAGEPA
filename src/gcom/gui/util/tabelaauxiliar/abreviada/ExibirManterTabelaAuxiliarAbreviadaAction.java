 ...
 */
public class ExibirManterTabelaAuxiliarAbreviadaAction
				extends GcomAction {
	...
	public ActionForward execute(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest httpServletRequest,
		...
		int tamMaxCampoDescricaoAbreviada = 5;
		...
		FiltroTabelaAuxiliarAbreviada filtroTabelaAuxiliarAbreviada = (FiltroTabelaAuxiliarAbreviada) httpServletRequest
						.getAttribute("filtroTabelaAuxiliarAbreviada");

		// Verifica se o filtro foi informado pela página de filtragem da tabela
		// auxiliar abreviada
		if(filtroTabelaAuxiliarAbreviada != null){
			...
			// de filtragem
			filtroTabelaAuxiliarAbreviada = (FiltroTabelaAuxiliarAbreviada) sessao.getAttribute("filtroTabelaAuxiliarAbreviada");

			if(filtroTabelaAuxiliarAbreviada == null){
				filtroTabelaAuxiliarAbreviada = new FiltroTabelaAuxiliarAbreviada();
			}
			if(fachada.registroMaximo(tabelaAuxiliarAbreviada.getClass()) > ConstantesSistema.NUMERO_MAXIMO_REGISTROS_MANUTENCAO){
		...
		if(retorno.getName().equalsIgnoreCase("manterTabelaAuxiliarAbreviada")){
			...
			Map resultado = controlarPaginacao(httpServletRequest, retorno, filtroTabelaAuxiliarAbreviada, pacoteNomeObjeto);
			tabelasAuxiliaresAbreviadas = (Collection) resultado.get("colecaoRetorno");
			retorno = (ActionForward) resultado.get("destinoActionForward");

			// tabelasAuxiliaresAbreviadas = fachada.pesquisarTabelaAuxiliar(
			// filtroTabelaAuxiliarAbreviada,
			// TabelaAuxiliarAbreviada.class.getName());
			/*
			 * if (tabelasAuxiliaresAbreviadas == null ||
			 * tabelasAuxiliaresAbreviadas.isEmpty()) { //Nenhum atividade
			 * cadastrado throw new
			 * ActionServletException("atencao.naocadastrado", null, titulo); }
			 * //Verifica o numero de objetos retornados if
			 * (tabelasAuxiliaresAbreviadas.size() >
			 ...
			 * sessao.setAttribute("tabelasAuxiliaresAbreviadas",
			 * tabelasAuxiliaresAbreviadas); //Envia o path do pacote na sessão
			...
			if(tabelasAuxiliaresAbreviadas != null && !tabelasAuxiliaresAbreviadas.isEmpty()){
				...
				if(tabelasAuxiliaresAbreviadas.size() == 1
					...
					if(httpServletRequest.getParameter("atualizar") != null){
						...
						TabelaAuxiliarAbreviada tabelaAuxiliarabreviada = (TabelaAuxiliarAbreviada) tabelasAuxiliaresAbreviadas.iterator()
										.next();
