 ...
 */
public class ExibirManterTabelaAuxiliarFaixaInteiroAction
				extends GcomAction {
	...
	public ActionForward execute(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest httpServletRequest,
		...
		ActionForward retorno = actionMapping.findForward("manterTabelaAuxiliarFaixaInteiro");
		...
		if(sessao.getAttribute("tabela") != null){
			tabelaAuxiliarFaixaInteiro = (TabelaAuxiliarFaixaInteiro) sessao.getAttribute("tabela");
		}
		...
		if(httpServletRequest.getAttribute("filtroTabelaAuxiliarFaixaInteiro") != null){
			...
			sessao.setAttribute("filtroTabelaAuxiliarFaixaInteiro", filtroTabelaAuxiliarFaixaInteiro);
		}else{
			...
			if(fachada.registroMaximo(tabelaAuxiliarFaixaInteiro.getClass()) > ConstantesSistema.NUMERO_MAXIMO_REGISTROS_MANUTENCAO){
				...
				// é chamada
				retorno = actionMapping.findForward("filtroTabelaAuxiliarFaixaInteiro");
		...
		if(retorno.getName().equalsIgnoreCase("manterTabelaAuxiliarFaixaInteiro")){
			// Seta a ordenação desejada do filtro
			filtroTabelaAuxiliarFaixaInteiro.setCampoOrderBy(FiltroTabelaAuxiliarFaixaInteiro.MENOR_FAIXA);
			// Pesquisa de tabelas auxiliares

			// Aciona o controle de paginação para que sejam pesquisados apenas
			// os registros que aparecem na página
			Map resultado = controlarPaginacao(httpServletRequest, retorno, filtroTabelaAuxiliarFaixaInteiro, pacoteNomeObjeto);
			tabelasAuxiliaresFaixaInteiro = (Collection) resultado.get("colecaoRetorno");
			...
			if(tabelasAuxiliaresFaixaInteiro != null && !tabelasAuxiliaresFaixaInteiro.isEmpty()){
				...
				if(tabelasAuxiliaresFaixaInteiro.size() == 1
					...
					if(httpServletRequest.getParameter("atualizar") != null){
						retorno = actionMapping.findForward("atualizarTabelaAuxiliarFaixaInteiro");
						TabelaAuxiliarFaixaInteiro tabelaAuxiliarFaixaInteiroAux = (TabelaAuxiliarFaixaInteiro) tabelasAuxiliaresFaixaInteiro
										.iterator().next();
						httpServletRequest.setAttribute("id", tabelaAuxiliarFaixaInteiroAux.getId().toString());

					}else{
						sessao.setAttribute("tabelasAuxiliaresFaixaInteiro", tabelasAuxiliaresFaixaInteiro);
					}
				}else{
					sessao.setAttribute("tabelasAuxiliaresFaixaInteiro", tabelasAuxiliaresFaixaInteiro);
				}
		...
		sessao.setAttribute("funcionalidadeTabelaAuxiliarFaixaInteiroManter", funcionalidadeTabelaAuxiliarFaixaInteiroManter);
