 ...
 */
public class ExibirAtualizarTabelaAuxiliarFaixaRealAction
				extends GcomAction {
	...
	public ActionForward execute(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest httpServletRequest,
		...
		ActionForward retorno = actionMapping.findForward("atualizarTabelaAuxiliarFaixaReal");
		...
		int tamMaxCampoVolumeMenorFaixa = 9;

		int tamMaxCampoVolumeMaiorFaixa = 9;
		...
		if(httpServletRequest.getAttribute("desfazer") == null){
			...
			Collection tabelasAuxiliaresFaixaReais = fachada.pesquisarTabelaAuxiliar(filtroTabelaAuxiliarFaixaReal, pacoteNomeObjeto);

			// Caso a coleção esteja vazia, indica erro inesperado
			if(tabelasAuxiliaresFaixaReais == null || tabelasAuxiliaresFaixaReais.isEmpty()){
				throw new ActionServletException("erro.sistema");
			}

			Iterator iteratorTabelaAuxiliarFaixaReal = tabelasAuxiliaresFaixaReais.iterator();

			// A tabela auxiliar abreviada que será atualizada
			TabelaAuxiliarFaixaReal tabelaAuxiliarFaixaReal = (TabelaAuxiliarFaixaReal) iteratorTabelaAuxiliarFaixaReal.next();

			// Manda a tabela auxiliar na sessão
			sessao.setAttribute("tabelaAuxiliarFaixaReal", tabelaAuxiliarFaixaReal);
			...
			if(tela.equalsIgnoreCase("piscinaVolumeFaixa") || tela.equalsIgnoreCase("reservatorioVolumeFaixa")){
				...
				if(tabelaAuxiliarFaixaReal.getIndicadorUso().intValue() == ConstantesSistema.INDICADOR_USO_ATIVO.intValue()){
			...
			sessao.setAttribute("funcionalidadeTabelaAuxiliarFaixaRealFiltrar", dados.getFuncionalidadeTabelaFaixaRealFiltrar());

			sessao.setAttribute("tamMaxCampoVolumeMenorFaixa", new Integer(tamMaxCampoVolumeMenorFaixa));
			sessao.setAttribute("tamMaxCampoVolumeMaiorFaixa", new Integer(tamMaxCampoVolumeMaiorFaixa));
		...
		if(httpServletRequest.getAttribute("desfazer") != null){
			...
			// Pesquisa a tabela auxiliar no sistema
			Collection tabelasAuxiliaresFaixaReaisBase = fachada.pesquisarTabelaAuxiliar(filtroTabelaAuxiliarFaixaReal, pacoteNomeObjeto);

			// Caso a coleção esteja vazia, indica erro inesperado
			if(tabelasAuxiliaresFaixaReaisBase == null || tabelasAuxiliaresFaixaReaisBase.isEmpty()){
				throw new ActionServletException("erro.sistema");
			}

			Iterator iteratorTabelaAuxiliarFaixaRealBase = tabelasAuxiliaresFaixaReaisBase.iterator();

			// A tabela auxiliar abreviada que será atualizada
			TabelaAuxiliarFaixaReal tabelaAuxiliarFaixaRealBase = (TabelaAuxiliarFaixaReal) iteratorTabelaAuxiliarFaixaRealBase.next();

			// Manda a tabela auxiliar na sessão
			sessao.setAttribute("tabelaAuxiliarFaixaReal", tabelaAuxiliarFaixaRealBase);
