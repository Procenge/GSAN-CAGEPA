 ...
 */
public class ExibirManterTabelaAuxiliarFaixaRealAction
				extends GcomAction {
	...
	public ActionForward execute(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest httpServletRequest,
		...
		ActionForward retorno = actionMapping.findForward("manterTabelaAuxiliarFaixaReal");
		...
		if(sessao.getAttribute("tabela") != null){
			tabelaAuxiliarFaixaReal = (TabelaAuxiliarFaixaReal) sessao.getAttribute("tabela");
		}
		...
		if(httpServletRequest.getAttribute("filtroTabelaAuxiliarFaixaReal") != null){
			...
			sessao.setAttribute("filtroTabelaAuxiliarFaixaReal", filtroTabelaAuxiliarFaixaReal);
			...
			if(sessao.getAttribute("filtroTabelaAuxiliarFaixaReal") != null){
				filtroTabelaAuxiliarFaixaReal = (FiltroTabelaAuxiliarFaixaReal) sessao.getAttribute("filtroTabelaAuxiliarFaixaReal");
			}else{
			...
			if(fachada.registroMaximo(tabelaAuxiliarFaixaReal.getClass()) > ConstantesSistema.NUMERO_MAXIMO_REGISTROS_MANUTENCAO){
				...
				// é chamada
				retorno = actionMapping.findForward("filtroTabelaAuxiliarFaixaReal");
		...
		if(retorno.getName().equalsIgnoreCase("manterTabelaAuxiliarFaixaReal")){
			...
			filtroTabelaAuxiliarFaixaReal.setCampoOrderBy(FiltroTabelaAuxiliarFaixaReal.VOLUME_MENOR_FAIXA);
			// Pesquisa de tabelas auxiliares

			// Aciona o controle de paginação para que sejam pesquisados apenas
			// os registros que aparecem na página
			Map resultado = controlarPaginacao(httpServletRequest, retorno, filtroTabelaAuxiliarFaixaReal, pacoteNomeObjeto);
			tabelasAuxiliaresFaixaReais = (Collection) resultado.get("colecaoRetorno");
			...
			if(tabelasAuxiliaresFaixaReais != null && !tabelasAuxiliaresFaixaReais.isEmpty()){
				...
				if(tabelasAuxiliaresFaixaReais.size() == 1
					...
					if(httpServletRequest.getParameter("atualizar") != null){
						retorno = actionMapping.findForward("atualizarTabelaAuxiliarFaixaReal");
						TabelaAuxiliarFaixaReal tabelaAuxiliarFaixaRealAux = (TabelaAuxiliarFaixaReal) tabelasAuxiliaresFaixaReais
										.iterator().next();
						httpServletRequest.setAttribute("id", tabelaAuxiliarFaixaRealAux.getId().toString());

					}else{
						sessao.setAttribute("tabelasAuxiliaresFaixaReais", tabelasAuxiliaresFaixaReais);
					}
				}else{
					sessao.setAttribute("tabelasAuxiliaresFaixaReais", tabelasAuxiliaresFaixaReais);
				}
		...
		sessao.setAttribute("funcionalidadeTabelaAuxiliarAbreviadaManter", funcionalidadeTabelaAuxiliarFaixaRealManter);
