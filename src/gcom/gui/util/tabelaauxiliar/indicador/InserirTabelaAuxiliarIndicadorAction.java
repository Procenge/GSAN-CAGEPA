 ...
 */
public class InserirTabelaAuxiliarIndicadorAction
				extends GcomAction {
	...
	public ActionForward execute(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest httpServletRequest,
		...
		TabelaAuxiliarIndicador tabelaAuxiliarIndicador = (TabelaAuxiliarIndicador) sessao.getAttribute("tabela");

		if(tabelaAuxiliarIndicador instanceof QuadraPerfil){ // indica que está atualizando o Perfil
			// de Quadra
			if(form.getDescricao().length() > 20){
				throw new ActionServletException("atencao.quadra_perfil.descricao.tamanho_maximo");
			}
		}
		...
		filtroTabelaAuxiliarIndicador.adicionarParametro(new ParametroSimples(FiltroTabelaAuxiliarIndicador.DESCRICAO,
						tabelaAuxiliarIndicador.getDescricao()));

		filtroTabelaAuxiliarIndicador.adicionarParametro(new ParametroSimples(FiltroTabelaAuxiliarIndicador.INDICADOR_BAIXA_RENDA,
						tabelaAuxiliarIndicador.getIndicadorBaixaRenda().toString()));

		Collection colecaoTabelaAuxiliarIndicador = fachada.pesquisar(filtroTabelaAuxiliarIndicador, TabelaAuxiliarIndicador.class
						.getName());
		...
		if(retorno.getName().equalsIgnoreCase("telaSucesso")){
			montarPaginaSucesso(httpServletRequest, ((String) sessao.getAttribute("titulo")) + " " + tabelaAuxiliarIndicador.getDescricao()
							+ " inserido(a) com sucesso.", "Inserir outro(a) " + ((String) sessao.getAttribute("titulo")), ((String) sessao
							.getAttribute("funcionalidadeTabelaAuxiliarIndicadorInserir")));
		}
