 ...
 */
public class ExibirAtualizarTabelaAuxiliarIndicadorAction
				extends GcomAction {
	...
	public ActionForward execute(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest httpServletRequest,
		...
		ActionForward retorno = actionMapping.findForward("atualizarTabelaAuxiliarIndicador");
		...
		if(httpServletRequest.getAttribute("desfazer") == null){
			...
			filtroTabelaAuxiliarIndicador.adicionarParametro(new ParametroSimples(FiltroTabelaAuxiliarIndicador.ID, id));

			// Pesquisa a tabela auxiliar no sistema
			Collection tabelasAuxiliaresIndicador = fachada.pesquisarTabelaAuxiliar(filtroTabelaAuxiliarIndicador, pacoteNomeObjeto);

			// Caso a coleção esteja vazia, indica erro inesperado
			if(tabelasAuxiliaresIndicador == null || tabelasAuxiliaresIndicador.isEmpty()){
				throw new ActionServletException("erro.sistema");
			}

			Iterator iteratorTabelaAuxiliarIndicador = tabelasAuxiliaresIndicador.iterator();

			// A tabela auxiliar abreviada que será atualizada
			TabelaAuxiliarIndicador tabelaAuxiliarIndicador = (TabelaAuxiliarIndicador) iteratorTabelaAuxiliarIndicador.next();

			// Manda a tabela auxiliar na sessão
			sessao.setAttribute("tabelaAuxiliarIndicador", tabelaAuxiliarIndicador);

			String tela = (String) sessao.getAttribute("tela");

			if(tela.equalsIgnoreCase("quadraPerfil")){

				if(tabelaAuxiliarIndicador.getIndicadorUso().intValue() == ConstantesSistema.INDICADOR_USO_ATIVO.intValue()){
			...
			if(tela.equalsIgnoreCase("quadraPerfil")){
				...
				if(tabelaAuxiliarIndicador.getIndicadorBaixaRenda().intValue() == ConstantesSistema.INDICADOR_USO_ATIVO.intValue()){
			...
			sessao.setAttribute("funcionalidadeTabelaAuxiliarIndicadorFiltrar", dados.getFuncionalidadeTabelaIndicadorFiltrar());
		...
		if(httpServletRequest.getAttribute("desfazer") != null){
			...
			// Pesquisa a tabela auxiliar no sistema
			Collection tabelasAuxiliaresIndicadorBase = fachada.pesquisarTabelaAuxiliar(filtroTabelaAuxiliarIndicador, pacoteNomeObjeto);

			// Caso a coleção esteja vazia, indica erro inesperado
			if(tabelasAuxiliaresIndicadorBase == null || tabelasAuxiliaresIndicadorBase.isEmpty()){
				throw new ActionServletException("erro.sistema");
			}

			Iterator iteratorTabelaAuxiliarIndicadorBase = tabelasAuxiliaresIndicadorBase.iterator();

			// A tabela auxiliar abreviada que será atualizada
			TabelaAuxiliarIndicador tabelaAuxiliarIndicadorBase = (TabelaAuxiliarIndicador) iteratorTabelaAuxiliarIndicadorBase.next();

			// Manda a tabela auxiliar na sessão
			sessao.setAttribute("tabelaAuxiliarIndicadorBase", tabelaAuxiliarIndicadorBase);
