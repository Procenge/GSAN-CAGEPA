 ...
 */
public class ExibirManterTabelaAuxiliarIndicadorAction
				extends GcomAction {
	...
	public ActionForward execute(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest httpServletRequest,
		...
		ActionForward retorno = actionMapping.findForward("manterTabelaAuxiliarIndicador");
		...
		if(httpServletRequest.getAttribute("filtroTabelaAuxiliarIndicador") != null){
			...
			if(sessao.getAttribute("filtroTabelaAuxiliarIndicador") != null){
				filtroTabelaAuxiliarIndicador = (FiltroTabelaAuxiliarIndicador) sessao.getAttribute("filtroTabelaAuxiliarIndicador");
			}else{
				filtroTabelaAuxiliarIndicador = new FiltroTabelaAuxiliarIndicador();
			}
			if(fachada.registroMaximo(TabelaAuxiliarIndicador.class) > ConstantesSistema.NUMERO_MAXIMO_REGISTROS_MANUTENCAO){
				// Se o limite de registros foi atingido, a página de filtragem
				// é chamada
				retorno = actionMapping.findForward("filtroTabelaAuxiliarIndicador");
		...
		if(retorno.getName().equalsIgnoreCase("manterTabelaAuxiliarIndicador")){
			...
			filtroTabelaAuxiliarIndicador.setCampoOrderBy(FiltroTabelaAuxiliarIndicador.DESCRICAO);
			// Pesquisa de tabelas auxiliares

			// Aciona o controle de paginação para que sejam pesquisados apenas
			// os registros que aparecem na página
			Map resultado = controlarPaginacao(httpServletRequest, retorno, filtroTabelaAuxiliarIndicador, pacoteNomeObjeto);
			tabelasAuxiliaresIndicador = (Collection) resultado.get("colecaoRetorno");
			...
			if(tabelasAuxiliaresIndicador != null && !tabelasAuxiliaresIndicador.isEmpty()){
				...
				if(tabelasAuxiliaresIndicador.size() == 1
					...
					if(httpServletRequest.getParameter("atualizar") != null){
						retorno = actionMapping.findForward("atualizarTabelaAuxiliarIndicador");
						TabelaAuxiliarIndicador tabelaAuxiliarIndicadorAux = (TabelaAuxiliarIndicador) tabelasAuxiliaresIndicador
										.iterator().next();
						httpServletRequest.setAttribute("id", tabelaAuxiliarIndicadorAux.getId().toString());

					}else{
						sessao.setAttribute("tabelasAuxiliaresIndicador", tabelasAuxiliaresIndicador);
					}
				}else{
					sessao.setAttribute("tabelasAuxiliaresIndicador", tabelasAuxiliaresIndicador);
				}
		...
		sessao.setAttribute("funcionalidadeTabelaAuxiliarIndicadorManter", funcionalidadeTabelaAuxiliarIndicadorManter);
