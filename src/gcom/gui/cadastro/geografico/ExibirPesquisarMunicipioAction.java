...
public class ExibirPesquisarMunicipioAction
				extends GcomAction {
	...
	public ActionForward execute(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest httpServletRequest,
		...
		if(idRegiao != null && idRegiao.intValue() != ConstantesSistema.NUMERO_NAO_INFORMADO){
			...
			if(httpServletRequest.getParameter("indicadorUsoTodos") == null){
				...
				filtroMicrorregiao.adicionarParametro(new ParametroSimples(FiltroMicrorregiao.INDICADOR_USO,
								ConstantesSistema.INDICADOR_USO_ATIVO));
			}

			microrregioes = fachada.pesquisar(filtroMicrorregiao, Microrregiao.class.getName());

			if(microrregioes == null || microrregioes.isEmpty()){
				throw new ActionServletException("atencao.pesquisa.nenhumresultado", null, "microrregioes");
			}else{
			...
			regiaoDesenvolvimentos = fachada.pesquisar(filtroRegiaoDesenvolvimento, RegiaoDesenvolvimento.class.getName());

			if(regiaoDesenvolvimentos == null || regiaoDesenvolvimentos.isEmpty()){
				throw new ActionServletException("atencao.pesquisa.nenhumresultado", null, "regiao desenvolvimento");
			}else{
				sessao.setAttribute("regiaoDesenvolvimentos", regiaoDesenvolvimentos);
			}

			filtroRegiao.adicionarParametro(new ParametroSimples(FiltroRegiao.INDICADOR_USO, ConstantesSistema.INDICADOR_USO_ATIVO));
			regioes = fachada.pesquisar(filtroRegiao, Regiao.class.getName());

			if(regioes == null || regioes.isEmpty()){
				throw new ActionServletException("atencao.pesquisa.nenhumresultado", null, "regiao");
			}else{
			...
			if(microrregioes == null || microrregioes.isEmpty()){
				throw new ActionServletException("atencao.pesquisa.nenhumresultado", null, "microrregiao");
			}else{
				sessao.setAttribute("microrregioes", microrregioes);
			}

			unidadesFederacao = fachada.pesquisar(filtroUnidadeFederacao, UnidadeFederacao.class.getName());
			...
			// parametros
			if(httpServletRequest.getParameter("caminhoRetornoTelaPesquisaMunicipio") != null){
				sessao.setAttribute("caminhoRetornoTelaPesquisaMunicipio", httpServletRequest
