 ...
 */
public class FiltrarBairroAction
				extends GcomAction {
	...
	public ActionForward execute(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest httpServletRequest,
		...
		if(nome != null && !nome.trim().equalsIgnoreCase("")){
			...
			if(tipoPesquisa != null && tipoPesquisa.equals(ConstantesSistema.TIPO_PESQUISA_COMPLETA.toString())){
				filtroBairro.adicionarParametro(new ComparacaoTextoCompleto(FiltroBairro.NOME, nome));
			}else{
				filtroBairro.adicionarParametro(new ComparacaoTexto(FiltroBairro.NOME, nome));
			}
		}
		...
		if(indicadorUso != null && !indicadorUso.trim().equalsIgnoreCase("")){
			peloMenosUmParametroInformado = true;
			filtroBairro.adicionarParametro(new ParametroSimples(FiltroBairro.INDICADOR_USO, indicadorUso));
		}
		...
		Collection colecaoBairro = (Collection) fachada.pesquisar(filtroBairro, Bairro.class.getName());

		if(colecaoBairro != null && !colecaoBairro.isEmpty()){
			if(atualizar != null && colecaoBairro.size() == 1){
				Bairro bairro = (Bairro) colecaoBairro.iterator().next();
				httpServletRequest.setAttribute("idRegistroAtualizacao", bairro.getId());
