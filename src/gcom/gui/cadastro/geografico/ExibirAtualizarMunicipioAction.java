...
public class ExibirAtualizarMunicipioAction
				extends GcomAction {
	...
	public ActionForward execute(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest httpServletRequest,
		...
		filtroMicrorregiao
						.adicionarParametro(new ParametroSimples(FiltroMicrorregiao.INDICADOR_USO, ConstantesSistema.INDICADOR_USO_ATIVO));

		Collection colecaoMicrorregiao = fachada.pesquisar(filtroMicrorregiao, Microrregiao.class.getName());
		...
		filtroRegiaoDesenvolvimento.adicionarParametro(new ParametroSimples(FiltroRegiaoDesenvolvimento.INDICADOR_USO,
						ConstantesSistema.INDICADOR_USO_ATIVO));

		Collection colecaoRegiaoDesenv = fachada.pesquisar(filtroRegiaoDesenvolvimento, RegiaoDesenvolvimento.class.getName());
		...
		if(idMunicipio == null){
			...
			}else if(httpServletRequest.getParameter("idMunicipio") != null){
				idMunicipio = (String) httpServletRequest.getParameter("idMunicipio").toString();
			}
		}

		// ------Inicio da parte que verifica se vem da página de municipio_manter.jsp
		...
		if(municipio == null){
			...
			if(idMunicipio != null && !idMunicipio.equals("")){
				...
				filtroMunicipio.adicionarCaminhoParaCarregamentoEntidade("agencia.banco");
		...
		if(municipio != null){
			if(httpServletRequest.getParameter("recarregar") == null){
				atualizarMunicipioActionForm.setCodigoMunicipio(municipio.getId().toString());

				atualizarMunicipioActionForm.setNomeMunicipio(municipio.getNome());

				if(municipio.getDdd() != null){
					atualizarMunicipioActionForm.setCodigoDdd(municipio.getDdd().toString());
				}else{
					atualizarMunicipioActionForm.setCodigoDdd("");
				}

				if(municipio.getCepInicio() != null){
					atualizarMunicipioActionForm.setCepInicial(municipio.getCepInicio().toString());
				}else{
					atualizarMunicipioActionForm.setCepInicial("");
				}

				if(municipio.getCepFim() != null){
					atualizarMunicipioActionForm.setCepFinal(municipio.getCepFim().toString());
				}else{
					atualizarMunicipioActionForm.setCepFinal("");
				}

				atualizarMunicipioActionForm.setUnidadeFederacao(municipio.getUnidadeFederacao().getId().toString());

				atualizarMunicipioActionForm.setMicroregiao(municipio.getMicrorregiao().getId().toString());

				atualizarMunicipioActionForm.setRegiaoDesenv(municipio.getRegiaoDesenvolvimento().getId().toString());

				if(municipio.getDataConcessaoInicio() != null){
					atualizarMunicipioActionForm.setDataInicioConcessao(Util.formatarData(municipio.getDataConcessaoInicio()));
				}else{
					atualizarMunicipioActionForm.setDataInicioConcessao("");
				}

				if(municipio.getDataConcessaoFim() != null){
					atualizarMunicipioActionForm.setDataFimConcessao(Util.formatarData(municipio.getDataConcessaoFim()));
				}else{
					atualizarMunicipioActionForm.setDataFimConcessao("");
				}

				atualizarMunicipioActionForm.setIndicadorUso("" + municipio.getIndicadorUso());
			}
			sessao.setAttribute("municipio", municipio);
		}
		// ------ Fim da parte que verifica se vem da página de municipio_manter.jsp

		// Inserir Banco e Agencia para Deso
		if(getParametroCompanhia(httpServletRequest) == SistemaParametro.INDICADOR_EMPRESA_DESO.shortValue()){
			if(municipio != null && httpServletRequest.getParameter("recarregar") == null){
				atualizarMunicipioActionForm.setNomePrefeitura(municipio.getNomePrefeitura());
				atualizarMunicipioActionForm.setEnderecoPrefeitura(municipio.getEnderecoPrefeitura());

				atualizarMunicipioActionForm.setNumeroCnpjPrefeitura(municipio.getNumeroCnpjPrefeitura());

				if(municipio.getCepPadrao() != null && !municipio.getCepPadrao().equals("")){
					atualizarMunicipioActionForm.setCepPadrao(municipio.getCepPadrao().toString());
				}

				if(municipio.getNumeroContaBancaria() != null && !municipio.getNumeroContaBancaria().equals("")){
					atualizarMunicipioActionForm.setNumeroContaBancaria(municipio.getNumeroContaBancaria().toString());
				}
				atualizarMunicipioActionForm.setNomePrefeito(municipio.getNomePrefeito());
				atualizarMunicipioActionForm.setNumeroCpfPrefeito(municipio.getNumeroCpfPrefeito());
				atualizarMunicipioActionForm.setNomePartidoPrefeito(municipio.getNomePartidoPrefeito());
				atualizarMunicipioActionForm.setNacionalidadePrefeito(municipio.getNacionalidadePrefeito());
				atualizarMunicipioActionForm.setEstadoCivilPrefeito(municipio.getEstadoCivilPrefeito());

				if(municipio.getAgencia() != null){
					atualizarMunicipioActionForm.setAgencia(municipio.getAgencia().getId().toString());
					atualizarMunicipioActionForm.setBanco(municipio.getAgencia().getBanco().getId().toString());
				}
			}
			...
			if(atualizarMunicipioActionForm.getBanco() != null && !Integer.valueOf(atualizarMunicipioActionForm.getBanco()).equals(-1)){
				idBanco = Integer.valueOf(atualizarMunicipioActionForm.getBanco());
			}

			FiltroAgencia filtroAgencia = new FiltroAgencia();
			filtroAgencia.setCampoOrderBy(FiltroAgencia.NOME_AGENCIA);
