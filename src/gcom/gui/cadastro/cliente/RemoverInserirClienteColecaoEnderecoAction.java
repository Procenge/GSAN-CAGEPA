 ...
 */
public class RemoverInserirClienteColecaoEnderecoAction
				extends GcomAction {
		...
		if(colecaoEnderecos != null){
			...
			while(iterator.hasNext()){
				...
				for(int i = 0; i < codigoEnderecos.length; i++){
					if(obterTimestampIdObjeto(clienteEndereco) == (Long.parseLong(codigoEnderecos[i]))){
						// Verifica se o endereço removido era o principal para
						// adicionar o indicador de principal
						// para o primeiro da lista

						if(obterTimestampIdObjeto(clienteEndereco) == (((Long) clienteActionForm.get("enderecoCorrespondenciaSelecao"))
										.longValue())
							...
							httpServletRequest.setAttribute("enderecoCorrespondenciaSelecao", new Long(
