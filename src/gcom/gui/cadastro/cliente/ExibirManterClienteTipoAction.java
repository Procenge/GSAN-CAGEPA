 ...
 * @since 18/06/2007
 */
public class ExibirManterClienteTipoAction
				extends GcomAction {
	...
	public ActionForward execute(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest httpServletRequest,
		...
		FiltroClienteTipo filtroClienteTipo = (FiltroClienteTipo) sessao.getAttribute("filtroClienteTipo");

		// Aciona o controle de paginação para que sejam pesquisados apenas
		// os registros que aparecem na página
		Collection colecaoClienteTipo = new ArrayList();
		if(filtroClienteTipo != null && !filtroClienteTipo.equals("")){
			Map resultado = controlarPaginacao(httpServletRequest, retorno, filtroClienteTipo, ClienteTipo.class.getName());
			colecaoClienteTipo = (Collection) resultado.get("colecaoRetorno");
		...
		if(colecaoClienteTipo != null && !colecaoClienteTipo.isEmpty()){
			...
			if(colecaoClienteTipo.size() == 1
				...
				if(httpServletRequest.getAttribute("atualizar") != null){
					retorno = actionMapping.findForward("exibirAtualizarClienteTipo");
					ClienteTipo clienteTipo = (ClienteTipo) colecaoClienteTipo.iterator().next();
					sessao.setAttribute("clienteTipo", clienteTipo);
				}else{
