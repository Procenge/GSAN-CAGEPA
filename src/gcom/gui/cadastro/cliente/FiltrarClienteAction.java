 ...
 * @date 23/07/2005
 */
public class FiltrarClienteAction
				extends Action {
	...
	public ActionForward execute(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest httpServletRequest,
		...
		ActionForward retorno = actionMapping.findForward("retornarFiltroCliente");
		...
		if(sessao.getAttribute("idClienteAtualizacaoCpfCnpj") != null){
			...
			sessao.removeAttribute("idClienteAtualizacaoCpfCnpj");
			filtrarClienteActionForm.setCodigoClienteFiltro(id);
		...
		filtroCliente.adicionarCaminhoParaCarregamentoEntidade("cliente.clienteTipo");
		...
		if(indicadorContaBraille != null && !indicadorContaBraille.trim().equalsIgnoreCase(ConstantesSistema.TODOS.toString())){
			peloMenosUmParametroInformado = true;
			filtroCliente.adicionarParametro(new ParametroSimples(FiltroCliente.INDICADOR_CONTA_BRAILLE, indicadorContaBraille));
		}

		if(nome != null && !nome.trim().equalsIgnoreCase("")){
			peloMenosUmParametroInformado = true;
			if(tipoPesquisa != null && tipoPesquisa.equals(ConstantesSistema.TIPO_PESQUISA_COMPLETA.toString())){
				filtroCliente.adicionarParametro(new ComparacaoTextoCompleto(FiltroCliente.NOME, nome));
			}else{
		...
		if(nomeMae != null && !nomeMae.trim().equalsIgnoreCase("")){
			...
			if(tipoPesquisaNomeMae != null && tipoPesquisaNomeMae.equals(ConstantesSistema.TIPO_PESQUISA_COMPLETA.toString())){
				filtroCliente.adicionarParametro(new ComparacaoTextoCompleto(FiltroCliente.NOME_MAE, nomeMae));
			}else{
		...
		if(idLogradouro != null && !idLogradouro.trim().equalsIgnoreCase("")){
			peloMenosUmParametroInformado = true;
			filtroCliente.adicionarParametro(new ParametroSimplesColecao(FiltroCliente.LOGRADOURO, idLogradouro));
		}
