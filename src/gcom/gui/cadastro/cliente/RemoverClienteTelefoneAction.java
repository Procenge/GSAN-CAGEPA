 ...
 */
public class RemoverClienteTelefoneAction
				extends GcomAction {
	...
	public ActionForward execute(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest httpServletRequest,
		...
		}else if(httpServletRequest.getParameter("tela").trim().equals("atualizar")){
		...
		if(ids != null && ids.length != 0){
			if(colecaoClienteFone != null && !colecaoClienteFone.isEmpty()){
				...
				Iterator iteratorColecaoClienteFone = colecaoClienteFone.iterator();

				while(iteratorColecaoClienteFone.hasNext()){
					clienteFone = (ClienteFone) iteratorColecaoClienteFone.next();
					for(int i = 0; i < ids.length; i++){
						if(clienteFone.getDddTelefone() != null && clienteFone.getDddTelefone().equals(ids[i])){
							// Verifica se o clienteFone removido era o
							// principal para adicionar o indicador de principal
							// para o primeiro da lista
							if(clienteActionForm.get("indicadorTelefonePadrao") != null
											&& obterTimestampIdObjeto(clienteFone) == (((Long) clienteActionForm
															.get("indicadorTelefonePadrao"))
											.longValue())
											&& colecaoClienteFone.size() > 1){

								Iterator iteratorTemp = colecaoClienteFone.iterator();
								// Pega o primeiro da lista
								ClienteFone clienteFonePrimeiroDaLista = (ClienteFone) iteratorTemp.next();

								// Verifica se o primeiro da lista é o mesmo que
								// será removido
								if(clienteFonePrimeiroDaLista.equals(clienteFone)){
									// Seta como principal o segundo da lista
									clienteFonePrimeiroDaLista = (ClienteFone) iteratorTemp.next();
								}
