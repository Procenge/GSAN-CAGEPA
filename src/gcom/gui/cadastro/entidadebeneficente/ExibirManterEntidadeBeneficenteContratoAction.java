...
public class ExibirManterEntidadeBeneficenteContratoAction
				extends GcomAction {

	/**
	 * Manter Contrato Entidade Beneficente
	 * 
	...
	public ActionForward execute(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest httpServletRequest,
		...
		FiltroEntidadeBeneficenteContrato filtro = (FiltroEntidadeBeneficenteContrato) sessao
						.getAttribute("filtroEntidadeBeneficenteContrato");

		// Se for preciso carregar outra entidade usar
		// "filtroEntidadeBeneficente.adicionarCaminhoParaCarregamentoEntidade"

		// Aciona o controle de paginação para que sejam pesquisados apenas os registros que
		// aparecem na página
		Map resultado = controlarPaginacao(httpServletRequest, retorno, filtro, EntidadeBeneficenteContrato.class.getName());
		Collection colecaoEntidadeBeneficenteContrato = (Collection) resultado.get("colecaoRetorno");
		...
		if(colecaoEntidadeBeneficenteContrato != null && !colecaoEntidadeBeneficenteContrato.isEmpty()){
			...
			if(colecaoEntidadeBeneficenteContrato.size() == 1
				...
				if(httpServletRequest.getParameter("checkAtualizar") != null){
					retorno = actionMapping.findForward("atualizarEntidadeBeneficenteContrato");
					EntidadeBeneficenteContrato entidadeBeneficenteContrato = (EntidadeBeneficenteContrato) colecaoEntidadeBeneficenteContrato
									.iterator().next();
					sessao.setAttribute("objetoEntidadeBeneficenteContrato", entidadeBeneficenteContrato);

				}else{
					httpServletRequest.setAttribute("colecaoEntidadeBeneficenteContrato", colecaoEntidadeBeneficenteContrato);
				}
			}else{
				httpServletRequest.setAttribute("colecaoEntidadeBeneficenteContrato", colecaoEntidadeBeneficenteContrato);
			}
