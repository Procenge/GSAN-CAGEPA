 ...
 * @date 20/09/2006
 */
public class ExibirCalendarioDiaVencimentoAction
				extends GcomAction {
	...
	public ActionForward execute(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest httpServletRequest,
		...
		ActionForward retorno = actionMapping.findForward("calendarioDiaVencimentoAction");

		Fachada fachada = Fachada.getInstancia();

		HttpSession sessao = httpServletRequest.getSession(false);

		ExibirCalendarioDiaVencimentoActionForm form = (ExibirCalendarioDiaVencimentoActionForm) actionForm;
		...
		// verifica se usuario possue permissão especial
		boolean temPermissaoInformarVencimentoAlternativoNovaData = fachada
						.verificarPermissaoInformarVencimentoAlternativoNovaData(usuario);

		Integer novoDiaVencimento = 0;
		Collection colecaoNovoDiaVencimento = new ArrayList();

		if(temPermissaoInformarVencimentoAlternativoNovaData){
			for(int i = 1; i <= 30; i++){
				colecaoNovoDiaVencimento.add(i);
			}
		}else{
			SistemaParametro sistemaParametro = fachada.pesquisarParametrosDoSistema();
			String dias = sistemaParametro.getNumeroMaxDiasVencimentoAlternativo();
			String[] arrayDias = dias.split("\\;");
			for(int i = 0; i < arrayDias.length; i++){
				novoDiaVencimento = Util.converterStringParaInteger(arrayDias[i]);
				colecaoNovoDiaVencimento.add(novoDiaVencimento);
			}
		...
		if(colecaoNovoDiaVencimento != null){

			Iterator it = colecaoNovoDiaVencimento.iterator();
			Date dataEntrada;
			...
			while(it.hasNext()){
				...
				while(i < ultimoDia){
					...
					if(colecaoNovoDiaVencimento.size() == 0){
						...
						vetorEstilos[dia - 1] = ConstantesSistema.SITUACAO_ROTEIRO_ACOMPANHAMENTO_FECHAMENTO_NULO;
					}else{

						// obterDia.setTime(dataEntrada);
						// vetorEstilos[obterDia.get(Calendar.DAY_OF_MONTH) - 1] =
						// ConstantesSistema.SITUACAO_ROTEIRO_ACOMPANHAMENTO_FECHAMENTO_PREENCHIDO;
						vetorEstilos[dia - 1] = ConstantesSistema.SITUACAO_ROTEIRO_ACOMPANHAMENTO_FECHAMENTO_PREENCHIDO;
					}
