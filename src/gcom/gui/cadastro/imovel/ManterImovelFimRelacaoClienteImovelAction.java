 ...
 * @since 17/04/2006
 */
public class ManterImovelFimRelacaoClienteImovelAction
				extends GcomAction {
	...
	public ActionForward execute(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest httpServletRequest,
		...
		Collection colecaoClientesImoveisFimRelacao = (Collection) sessao.getAttribute("colecaoClientesImoveisFimRelacao");
		...
		// relação e o motivo.
		if(colecaoClientesImoveisFimRelacao != null && !colecaoClientesImoveisFimRelacao.isEmpty()){
			Iterator clienteImovelIterator = colecaoClientesImoveisFimRelacao.iterator();
			String idMotivoFimRelacao = fimRelacaoClienteImovelActionForm.getIdMotivo();
			...
			clienteImovelFimRelacao.setId(Util.converterStringParaInteger(idMotivoFimRelacao));
			...
			while(clienteImovelIterator.hasNext()){
				...
				if(clienteImovel.getDataInicioRelacao() != null){
					if(dataFimRelacao.before(clienteImovel.getDataInicioRelacao())){
			...
			clienteImovelIterator = colecaoClientesImoveisFimRelacao.iterator();

			String alerta = "";

			while(clienteImovelIterator.hasNext()){
				ClienteImovel clienteImovel = (ClienteImovel) clienteImovelIterator.next();
				clienteImovel.setClienteImovelFimRelacaoMotivo(clienteImovelFimRelacao);
				clienteImovel.setDataFimRelacao(dataFimRelacao);

				// verifica se o tipo do cliente é usuário
				if(clienteImovel.getClienteRelacaoTipo().getId().shortValue() == ConstantesSistema.CLIENTE_IMOVEL_TIPO_USUARIO.shortValue()){
		...
		// atencao.cliente.em.processo.negativacao
