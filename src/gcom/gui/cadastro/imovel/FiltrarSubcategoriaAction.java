 ...
 */
public class FiltrarSubcategoriaAction
				extends GcomAction {
	...
	public ActionForward execute(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest httpServletRequest,
		...
		if(id != null && id.intValue() != ConstantesSistema.NUMERO_NAO_INFORMADO){
			peloMenosUmParametroInformado = true;
			filtroSubcategoria.adicionarParametro(new ParametroSimples(FiltroSubCategoria.CATEGORIA_ID, id));
		}
		if(descricao != null && !descricao.trim().equalsIgnoreCase("")){
			peloMenosUmParametroInformado = true;
			filtroSubcategoria.adicionarParametro(new ComparacaoTexto(FiltroSubCategoria.DESCRICAO, descricao));
		}
		if(indicadorUso != null && !indicadorUso.trim().equalsIgnoreCase("")){
			peloMenosUmParametroInformado = true;
			filtroSubcategoria.adicionarParametro(new ParametroSimples(FiltroBairro.INDICADOR_USO, indicadorUso));
		}
		...
		if(!peloMenosUmParametroInformado){
			...
			if(colecaoSubcategoria != null && !colecaoSubcategoria.isEmpty()){
				if(atualizar != null && colecaoSubcategoria.size() == 1){
					...
					httpServletRequest.setAttribute("idRegistroAtualizacao", subcategoria.getId());

					retorno = actionMapping.findForward("exibirAtualizarSubcategoria");

				}else{
					retorno = actionMapping.findForward("retornarFiltroSubcategoria");
				}
			}else{
				throw new ActionServletException("atencao.pesquisa.nenhumresultado");
			}

			httpServletRequest.setAttribute("filtroSubcategoria", filtroSubcategoria);
		}
