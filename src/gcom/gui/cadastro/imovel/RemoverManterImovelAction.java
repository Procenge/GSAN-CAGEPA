 ...
 */
public class RemoverManterImovelAction
				extends GcomAction {
	...
	public ActionForward execute(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest httpServletRequest,
		...
		for(int i = 0; i < ids.length; i++){
			...
			if(colecaoImovel != null && !colecaoImovel.isEmpty()){
				...
				if((imovel.getLigacaoAgua() != null && imovel.getLigacaoAgua().getHidrometroInstalacaoHistorico() != null)
					...
					// Não é possível excluir o(s) imóvel(is) selecionado(s) devido a existência de
					// hidrômetro na ligação de e/ou no poço
				...
				if((imovel.getIndicadorImovelCondominio() != null && imovel.getIndicadorImovelCondominio().shortValue() == Imovel.IMOVEL_CONDOMINIO
								.shortValue())
								& (imovelPesquisadoVinculados != null && !imovelPesquisadoVinculados.isEmpty())){
					// Não é possível excluir o(s) imóvel(is) selecionado(s) devido a ser(em)
					// imóvel(is) condomínio, com outros imóveis associados
					throw new ActionServletException("atencao.imovel.ser.condominio.outros.imoveis");
				}
		...
		if(retorno.getName().equalsIgnoreCase("telaSucesso")){
			montarPaginaSucesso(httpServletRequest, ids.length + " Imóvel(is) removido(s) com sucesso.",
							"Realizar outra Manutenção de Imóvel", "exibirFiltrarImovelAction.do");
		}
