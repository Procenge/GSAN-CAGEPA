 ...
 */
public class RemoverImovelSubCategoriaAction
				extends GcomAction {
		...
		if(colecaoImovelSubCategorias != null && !colecaoImovelSubCategorias.equals("")){
			...
			while(imovelSubcategoriaIterator.hasNext()){
				...
				for(int i = 0; i < removerImovelSubCategoria.length; i++){
					if(imovelSubcategoria.getUltimaAlteracao().getTime() == Long.parseLong(removerImovelSubCategoria[i])){
						if(!(colecaoImovelSubcategoriasRemoviadas.contains(imovelSubcategoria))){
							colecaoImovelSubcategoriasRemoviadas.add(imovelSubcategoria);
							...
							if(getParametroCompanhia(httpServletRequest).equals(SistemaParametro.INDICADOR_EMPRESA_DESO)){
								if(verificarExclusaoConsumoFaixaAreaCategoria(colecaoImovelSubCategorias, imovelSubcategoria.getComp_id()
												.getSubcategoria().getCategoria())){
									removerConsumoFaixaAreaCategoria(sessao, imovelSubcategoria, indicadorContratoConsumo);
								}
							}
						}
					}
			...
			sessao.setAttribute("colecaoImovelSubcategoriasRemoviadas", colecaoImovelSubcategoriasRemoviadas);
		}

		return retorno;
	}

	private void removerConsumoFaixaAreaCategoria(HttpSession sessao, ImovelSubcategoria imovelSubcategoria,
