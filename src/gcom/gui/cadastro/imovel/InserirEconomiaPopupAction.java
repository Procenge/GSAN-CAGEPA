 ...
 */
public class InserirEconomiaPopupAction
				extends GcomAction {
	...
	public ActionForward execute(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest httpServletRequest,
		...
		ImovelSubcategoria imovelSubCategoria = (ImovelSubcategoria) sessao.getAttribute("imovelSubCategoria");

		Fachada fachada = Fachada.getInstancia();

		Collection imovelEconomias = null;

		if(imovelSubCategoria.getImovelEconomias() == null || imovelSubCategoria.getImovelEconomias().equals("")){
		...
		if(imovelEconomias.size() <= imovelSubCategoria.getQuantidadeEconomias()){
			...
			// modificados
			colecaoImovelEconomiasModificadas = (Collection) sessao.getAttribute("colecaoImovelEconomiasModificadas");
			...
			Integer areaContruidaFaixaForm = (new Integer(economiaPopupActionForm.getIdAreaConstruidaFaixa().toString()));

			if(areaContruidaFaixaForm != null && areaContruidaFaixaForm.intValue() != ConstantesSistema.NUMERO_NAO_INFORMADO){
				...
				if(economiaPopupActionForm.getAreaConstruida() != null && !economiaPopupActionForm.getAreaConstruida().equals("")){
					...
					filtroAreaConstruidaFaixa.adicionarParametro(new MaiorQue(FiltroAreaConstruidaFaixa.MAIOR_FAIXA, areaConstruida));

					filtroAreaConstruidaFaixa.adicionarParametro(new MenorQue(FiltroAreaConstruidaFaixa.MENOR_FAIXA, areaConstruida));
			...
			String complementoEndereco = (String) economiaPopupActionForm.getComplementoEndereco();
			...
			if(economiaPopupActionForm.getNumeroMorador() != null && !economiaPopupActionForm.getNumeroMorador().equalsIgnoreCase("")){
				numeroMoradores = (new Short(economiaPopupActionForm.getNumeroMorador().toString()));
			}
			...
			if(economiaPopupActionForm.getNumeroPontosUtilizacao() != null
				...
				numeroPontoUtilizacao = (new Short(economiaPopupActionForm.getNumeroPontosUtilizacao().toString()));
			}
			...
			if(economiaPopupActionForm.getNumeroIptu() != null && !economiaPopupActionForm.getNumeroIptu().equals("")){
				...
				// digitado no
				// mesmo municipio
				fachada.verificarExistenciaIPTU(colecaoImovelEconomiasModificadas, imovel, economiaPopupActionForm.getNumeroIptu(),
								new Date());

				numeroIptu = (new BigDecimal(economiaPopupActionForm.getNumeroIptu().toString()));

			}

			Long numeroContratoEnergia = null;
			if(economiaPopupActionForm.getNumeroCelpe() != null && !economiaPopupActionForm.getNumeroCelpe().equals("")){

				// verifica se existe no imovel, no imovelEconomia ou na coleção
				// que será
				// inserida se existe algum iptu cadastrado com o numero da
				// celpe digitado
				fachada.verificarExistenciaCelpe(colecaoImovelEconomiasModificadas, imovel, economiaPopupActionForm.getNumeroCelpe(),
								new Date());
				numeroContratoEnergia = (new Long(economiaPopupActionForm.getNumeroCelpe().toString()));

			}

			ImovelEconomia imovelEconomia = new ImovelEconomia(complementoEndereco, numeroMoradores, numeroPontoUtilizacao, numeroIptu,
							numeroContratoEnergia, areaConstruida, new Date(), areaConstruidaFaixa, imovelSubCategoria, new HashSet(
			...
			if(!imovelEconomias.contains(imovelEconomia)){
				...
				imovelSubCategoria.setImovelEconomias(new HashSet(imovelEconomias));
				colecaoImovelEconomiasModificadas.add(imovelEconomia);

			}else{
				throw new ActionServletException("atencao.ja_cadastrado.imovel_economia");
			}
			...
			sessao.setAttribute("colecaoImovelEconomiasModificadas", colecaoImovelEconomiasModificadas);
			...
			// com a coleção de cliente, imovel e tipo da relação
			sessao.setAttribute("contIdentificadorTemp", new Integer(imovelEconomias.size() + 1));

		}else{
			throw new ActionServletException("atencao.quantidade_ultrapassada.imovel_economia", null, new Integer(imovelSubCategoria
