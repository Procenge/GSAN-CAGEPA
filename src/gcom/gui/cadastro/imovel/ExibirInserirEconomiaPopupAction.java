 ...
 */
public class ExibirInserirEconomiaPopupAction
				extends GcomAction {
	...
	public ActionForward execute(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest httpServletRequest,
		...
		if(httpServletRequest.getParameter("codigoImovelSubCategoria") != null
			...
			while(imovelSubCategoriaIterator.hasNext()){
				...
				if(imovelSubCategoria.getComp_id().getSubcategoria().getId().equals(Integer.parseInt(codigoImovelSubCategoria))){
					...
					if(imovelSubCategoria.getImovelEconomias() != null && !imovelSubCategoria.getImovelEconomias().equals("")){
						...
						if(imovelSubCategoria.getImovelEconomias().size() == imovelSubCategoria.getQuantidadeEconomias()){
							...
							sessao.setAttribute("contIdentificadorTemp", new Integer(imovelSubCategoria.getImovelEconomias().size() + 1));
						}
					}else{
		...
		if(economiaPopupActionForm.getIdCliente() == null || economiaPopupActionForm.getIdCliente().toString().trim().equalsIgnoreCase("")){
			...
			FiltroClienteRelacaoTipo filtroClienteRelacaoTipo = new FiltroClienteRelacaoTipo(FiltroClienteRelacaoTipo.DESCRICAO);
			...
			if(clientesRelacoesTipos == null || clientesRelacoesTipos.equals("")){
				...
				throw new ActionServletException("atencao.pesquisa.nenhumresultado", null, "cliente relação tipo");
			}else{
		...
		if(httpServletRequest.getAttribute("i") == null){
			...
			economiaPopupActionForm.setIdAreaConstruidaFaixa("-1");
		}
