 ...
 */
public class AdicionarInformarEconomiaPopupAction
				extends GcomAction {
	...
	public ActionForward execute(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest httpServletRequest,
		...
		if(economiaPopupActionForm.getBotaoAdicionar() != null && !economiaPopupActionForm.getBotaoAdicionar().equals("")){
			...
			if(economiaPopupActionForm.getBotaoAdicionar().equals("inserir")){
				...
				if(imovelEconomias.size() + 1 > imovelSubCategoria.getQuantidadeEconomias()){
					throw new ActionServletException("atencao.quantidade_ultrapassada.imovel_economia", null, new Integer(
									imovelSubCategoria.getImovelEconomias().size()).toString());
				}
			}
		}
		...
		ImovelEconomia imovelEconomia = (ImovelEconomia) sessao.getAttribute("imovelEconomia");
		...
		try{
			dataInicioRelacao = dataFormato.parse(economiaPopupActionForm.getDataInicioClienteImovelRelacao());
		...
		if(economiaPopupActionForm.getIdCliente() != null && !economiaPopupActionForm.getIdCliente().trim().equalsIgnoreCase("")){
			...
			if(clientes != null && !clientes.isEmpty()){
				...
				clienteRelacaoTipo.setId(new Integer(economiaPopupActionForm.getClienteRelacaoTipo()));
				// recupera a descricao do tipo do cliente imovel
				clienteRelacaoTipo.setDescricao(economiaPopupActionForm.getTextoSelecionadoEconomia());

				// inicializa o cliente imovel
				ClienteImovelEconomia clienteImovelEconomia = new ClienteImovelEconomia(dataInicioRelacao, null, new Date(), null, cliente,
								clienteRelacaoTipo, null);

				if(!colecaoClientesImoveisEconomia.contains(clienteImovelEconomia)){

					if(imovelEconomia != null && !imovelEconomia.equals("")){
						if(imovelEconomia.getClienteImovelEconomias().contains(clienteImovelEconomia)){
					...
					if(clienteImovelEconomia.getClienteRelacaoTipo().getId().shortValue() == ClienteRelacaoTipo.USUARIO.shortValue()){
						if(economiaPopupActionForm.getIdClienteImovelUsuario() == null
										|| (economiaPopupActionForm.getIdClienteImovelUsuario().equals(""))){

							economiaPopupActionForm.setIdClienteImovelUsuario(cliente.getId().toString());
							// adiciona o cliente imovel na coleção de
							// imovelClientesNovos
							colecaoClientesImoveisEconomia.add(clienteImovelEconomia);
						}else{
							throw new ActionServletException("atencao.ja_cadastradado.cliente_imovel_usuario");
						}
						...
						if(imovelEconomia != null){
							Collection clientesImoveisEconoiasCadastradas = imovelEconomia.getClienteImovelEconomias();
							// cria um iterator da coleção de clientes imoveis
							// economia
							Iterator clienteImovelEconomiaCadastradaIterator = clientesImoveisEconoiasCadastradas.iterator();

							// verifica cada cliente imovel economia
							while(clienteImovelEconomiaCadastradaIterator.hasNext()){
								...
								if(clienteImovelEconomiaCadastrada.getCliente().getId().equals(clienteImovelEconomia.getCliente().getId())){
									// verifica se esse cliente imovel economia
									// cadastrado é propietário
									if(clienteImovelEconomiaCadastrada.getClienteRelacaoTipo().getId().shortValue() == ClienteRelacaoTipo.PROPRIETARIO
													.shortValue()){
										throw new ActionServletException("atencao.ja_cadastrado_cliente_imovel_propietario");
									}
								}
							}
						}
