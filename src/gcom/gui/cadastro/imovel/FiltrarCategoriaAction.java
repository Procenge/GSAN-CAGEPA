 ...
 */
public class FiltrarCategoriaAction
				extends GcomAction {
	...
	public ActionForward execute(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest httpServletRequest,
		...
		ActionForward retorno = actionMapping.findForward("retornarFiltroCategoria");
		...
		if(id != null && !id.trim().equalsIgnoreCase("")){
			...
			filtroCategoria.adicionarParametro(new ParametroSimples(FiltroCategoria.CODIGO, new Integer(id)));
		}
		if(tipoCategoria != null && !tipoCategoria.trim().equalsIgnoreCase("" + ConstantesSistema.NUMERO_NAO_INFORMADO)){
			peloMenosUmParametroInformado = true;
			filtroCategoria.adicionarParametro(new ParametroSimples(FiltroCategoria.TIPO_CATEGORIA, new Integer(tipoCategoria)));
		}
		if(descricao != null && !descricao.trim().equalsIgnoreCase("")){
			peloMenosUmParametroInformado = true;
			filtroCategoria.adicionarParametro(new ComparacaoTexto(FiltroCategoria.DESCRICAO, descricao));
		}
		...
		if(!peloMenosUmParametroInformado){
			...
			if(categoriasRetorno.isEmpty()){
				throw new ActionServletException("atencao.pesquisa.nenhumresultado", null, "exceção de categoria");
			}else{
				...
				if(atualizar != null && categoriasRetorno.size() == 1){
					...
					httpServletRequest.setAttribute("idRegistroAtualizacao", categoria.getId());

					retorno = actionMapping.findForward("atualizarCategoria");

				}else{
					retorno = actionMapping.findForward("retornarFiltroCategoria");
				}
			}
