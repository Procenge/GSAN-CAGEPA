 ...
 * @date 26/03/2006
 */
public class ExibirInserirImovelSituacaoAction
				extends GcomAction {
	...
	public ActionForward execute(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest httpServletRequest,
		...
		ActionForward retorno = actionMapping.findForward("situacaoImovelInserir");
		...
		Collection<ImovelSituacaoTipo> collectionImovelSituacaoTipo = fachada.pesquisar(filtroImovelSituacaoTipo, ImovelSituacaoTipo.class
						.getName());
		if(collectionImovelSituacaoTipo == null || collectionImovelSituacaoTipo.isEmpty()){
			throw new ActionServletException("atencao.entidade_sem_dados_para_selecao", null, "Tabela Imóvel Situação Tipo");
		}
		httpServletRequest.setAttribute("collectionImovelSituacaoTipo", collectionImovelSituacaoTipo);
		...
		Collection<LigacaoAguaSituacao> collectionLigacaoAguaSituacao = fachada.pesquisar(filtroLigacaoAguaSituacao,
						LigacaoAguaSituacao.class.getName());

		if(collectionLigacaoAguaSituacao == null || collectionLigacaoAguaSituacao.isEmpty()){
			throw new ActionServletException("atencao.entidade_sem_dados_para_selecao", null, "Tabela Ligação Água Situação");
		}
		httpServletRequest.setAttribute("collectionLigacaoAguaSituacao", collectionLigacaoAguaSituacao);
		...
		Collection<LigacaoEsgotoSituacao> collectionLigacaoEsgotoSituacao = fachada.pesquisar(filtroLigacaoEsgotoSituacao,
						LigacaoEsgotoSituacao.class.getName());

		if(collectionLigacaoEsgotoSituacao == null || collectionLigacaoEsgotoSituacao.isEmpty()){
			throw new ActionServletException("atencao.entidade_sem_dados_para_selecao", null, "Tabela Ligação Esgoto Situação");
