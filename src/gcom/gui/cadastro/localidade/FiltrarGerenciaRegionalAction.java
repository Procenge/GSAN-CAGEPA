 ...
 * @date 05/08/2006
 */
public class FiltrarGerenciaRegionalAction
				extends GcomAction {
	...
	public ActionForward execute(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest httpServletRequest,
		...
		ActionForward retorno = actionMapping.findForward("exibirManterGerenciaRegional");
		...
		if(gerenciaRegionalID != null && !gerenciaRegionalID.trim().equalsIgnoreCase("")){
			...
			filtroGerenciaRegional.adicionarParametro(new ParametroSimples(FiltroGerenciaRegional.ID, gerenciaRegionalID));
		}
		...
		if(gerenciaRegionalNome != null && !gerenciaRegionalNome.trim().equalsIgnoreCase("")){
			...
			if(tipoPesquisa != null && tipoPesquisa.equals(ConstantesSistema.TIPO_PESQUISA_COMPLETA.toString())){
				...
				filtroGerenciaRegional.adicionarParametro(new ComparacaoTextoCompleto(FiltroGerenciaRegional.NOME, gerenciaRegionalNome));
			}else{
				filtroGerenciaRegional.adicionarParametro(new ComparacaoTexto(FiltroGerenciaRegional.NOME, gerenciaRegionalNome));
			}
		}

		// Verifica se o campo descrição abreviatura foi informado
		if(gerenciaRegionalNomeAbre != null && !gerenciaRegionalNomeAbre.trim().equalsIgnoreCase("")){

			peloMenosUmParametroInformado = true;

			filtroGerenciaRegional.adicionarParametro(new ComparacaoTexto(FiltroGerenciaRegional.NOME_ABREVIADO, gerenciaRegionalNomeAbre));
		...
		filtroGerenciaRegional.adicionarCaminhoParaCarregamentoEntidade("logradouroCep.logradouro.logradouroTipo");
		filtroGerenciaRegional.adicionarCaminhoParaCarregamentoEntidade("logradouroCep.logradouro.logradouroTitulo");
		filtroGerenciaRegional.adicionarCaminhoParaCarregamentoEntidade("logradouroBairro.bairro.municipio.unidadeFederacao");
