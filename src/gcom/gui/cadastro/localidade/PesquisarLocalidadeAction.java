...
public class PesquisarLocalidadeAction
				extends GcomAction {
	...
	public ActionForward execute(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest httpServletRequest,
		...
		String descricao = (String) pesquisarActionForm.get("descricaoLocalidade");
		String idGerencia = (String) pesquisarActionForm.get("idGerenciaRegional");
		String idUnidadeNegocio = (String) pesquisarActionForm.get("idUnidadeNegocio");
		...
		if(descricao != null && !descricao.trim().equalsIgnoreCase("")){
			if(tipoPesquisa != null && tipoPesquisa.equals(ConstantesSistema.TIPO_PESQUISA_COMPLETA.toString())){
				filtroLocalidade.adicionarParametro(new ComparacaoTextoCompleto(FiltroLocalidade.DESCRICAO, descricao));
			}else{
				filtroLocalidade.adicionarParametro(new ComparacaoTexto(FiltroLocalidade.DESCRICAO, descricao));
			}

		}

		if(idGerencia != null && !idGerencia.trim().equalsIgnoreCase("") && !idGerencia.trim().equalsIgnoreCase("-1")){
			filtroLocalidade.adicionarParametro(new ParametroSimples(FiltroLocalidade.ID_GERENCIA, new Integer(idGerencia)));
		}

		if(idUnidadeNegocio != null && !idUnidadeNegocio.trim().equalsIgnoreCase("") && !idUnidadeNegocio.trim().equalsIgnoreCase("-1")){
		...
		if(idElo != null && !idElo.equals("")){
			filtroLocalidade.adicionarParametro(new ParametroSimples(FiltroLocalidade.ID_ELO, new Integer(idElo)));
		}
		...
		if(localidades != null && !localidades.isEmpty()){
			...
			Map resultado = controlarPaginacao(httpServletRequest, retorno, filtroLocalidade, Localidade.class.getName());
