...
public class ExibirFiltrarQuadraAction
				extends GcomAction {
	...
	public ActionForward execute(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest httpServletRequest,
		...
		if(objetoConsulta != null && !objetoConsulta.trim().equalsIgnoreCase("")){
			...
			bairroCD = filtrarQuadraActionForm.getBairroCD();

			switch(Integer.parseInt(objetoConsulta)){

				// Localidade
				case 1:
					pesquisarLocalidade(filtrarQuadraActionForm, fachada, httpServletRequest);
					break;

				// Setor Comercial
				case 2:
					pesquisarLocalidade(filtrarQuadraActionForm, fachada, httpServletRequest);
					pesquisarSetorComercial(filtrarQuadraActionForm, fachada, httpServletRequest);
					break;

				// Bairro
				case 3:
					pesquisarBairro(filtrarQuadraActionForm, fachada, httpServletRequest);
					break;
	...
	/**
	 * Método responsável por pesquisar um bairro. Será utilizado o município do setor comercial.
	 * 
	 * @author virginia melo
	 * @param filtrarQuadraActionForm
	 * @param fachada
	 * @param httpServletRequest
	 */
	private void pesquisarBairro(FiltrarQuadraActionForm filtrarQuadraActionForm, Fachada fachada, HttpServletRequest httpServletRequest){
		...
		if(setorComercialCD == null || setorComercialCD.trim().equalsIgnoreCase("")){
			...
			filtroSetor.adicionarParametro(new ParametroSimples(FiltroSetorComercial.ID_LOCALIDADE, localidadeID));
			filtroSetor.adicionarParametro(new ParametroSimples(FiltroSetorComercial.CODIGO_SETOR_COMERCIAL, setorComercialCD));
			colecaoPesquisa = fachada.pesquisar(filtroSetor, SetorComercial.class.getName());
	...
	private void pesquisarLocalidade(FiltrarQuadraActionForm filtrarQuadraActionForm, Fachada fachada, HttpServletRequest httpServletRequest){
	...
	private void pesquisarSetorComercial(FiltrarQuadraActionForm filtrarQuadraActionForm, Fachada fachada,
		...
		if(localidadeID == null || localidadeID.trim().equalsIgnoreCase("")){
			...
			filtrarQuadraActionForm.setSetorComercialCD("");
			filtrarQuadraActionForm.setSetorComercialID("");
			filtrarQuadraActionForm.setSetorComercialNome("Informe o setor comercial.");
			httpServletRequest.setAttribute("corSetorComercial", "exception");
			httpServletRequest.setAttribute("nomeCampo", "setorComercialCD");
		}else{
