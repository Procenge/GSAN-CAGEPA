...
public class ExibirInserirTarifaSocialDadosCartaoMultiplasEconomiasAction
	...
	public ActionForward execute(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest httpServletRequest,
		...
		ActionForward retorno = actionMapping.findForward("inserirTarifaSocialDadosCartao");
		...
		Collection colecaoTarifaSocialCartaoTipo = fachada.pesquisar(filtro, TarifaSocialCartaoTipo.class.getName());
		...
		if(colecaoTarifaSocialCartaoTipo == null || colecaoTarifaSocialCartaoTipo.isEmpty()){
			throw new ActionServletException("atencao.naocadastrado", null, "tipo do cartão");
		}
		...
		httpServletRequest.setAttribute("colecaoTarifaSocialCartaoTipo", colecaoTarifaSocialCartaoTipo);
		...
		String clienteImovelEconomiaID = httpServletRequest.getParameter("clienteImovelEconomia");

		Integer idCliente = fachada.pesquisarClienteImovelEconomia(new Integer(clienteImovelEconomiaID));

		Collection colecaotarifaSocialDadoEconomia = fachada.pesquisarClientesUsuarioExistenteTarifaSocial(idCliente);

		if(colecaotarifaSocialDadoEconomia != null && !colecaotarifaSocialDadoEconomia.isEmpty()){
			TarifaSocialDadoEconomia tarifaSocialDadoEconomia = (TarifaSocialDadoEconomia) colecaotarifaSocialDadoEconomia.iterator()
							.next();
			...
			if(tarifaSocialDadoEconomia.getImovel() != null){
				tarifaSocialCartaoActionForm.setIdImovel(tarifaSocialDadoEconomia.getImovel().getId().toString());
			}

			if(tarifaSocialDadoEconomia.getTarifaSocialRevisaoMotivo() != null){
				tarifaSocialCartaoActionForm.setMotivoRevisao(tarifaSocialDadoEconomia.getTarifaSocialRevisaoMotivo().getDescricao());
			}

			FiltroTarifaSocialExclusaoMotivo filtroTarifaSocialExclusaoMotivo = new FiltroTarifaSocialExclusaoMotivo();
			filtroTarifaSocialExclusaoMotivo.adicionarParametro(new ParametroSimples(FiltroTarifaSocialExclusaoMotivo.INDICADOR_USO,
							ConstantesSistema.INDICADOR_USO_ATIVO));
			filtroTarifaSocialExclusaoMotivo.setCampoOrderBy(FiltroTarifaSocialExclusaoMotivo.DESCRICAO);

			Collection colecaoTarifaSocialExclusaoMotivo = fachada.pesquisar(filtroTarifaSocialExclusaoMotivo,
		...
		if(sessao.getAttribute("colecaoClienteImovelEconomia") != null){
			Collection colecaoClienteImovelEconomiaMemoria = (Collection) sessao.getAttribute("colecaoClienteImovelEconomia");

			TarifaSocialDadoEconomia tarifaSocialDadoEconomia = buscarTarifaSocialDadoEconomiaSelecionada(
							colecaoClienteImovelEconomiaMemoria, clienteImovelEconomiaID);

			if(tarifaSocialDadoEconomia != null){
				httpServletRequest.setAttribute("tarifaSocialDadoEconomia", tarifaSocialDadoEconomia);
		...
		if(imovelEconomia != null){
			...
			if(imovelEconomia.getNumeroCelpe() != null){
				tarifaSocialCartaoActionForm.setNumeroContratoCelpe(imovelEconomia.getNumeroCelpe().toString());
			}else{
			...
			if(imovelEconomia.getNumeroIptu() != null){
				tarifaSocialCartaoActionForm.setNumeroIPTU(imovelEconomia.getNumeroIptu().toString());
			}else{
			...
			if(imovelEconomia.getAreaConstruida() != null){
				String areaConstruidaFormatada = Util.formatarMoedaReal(imovelEconomia.getAreaConstruida());
				tarifaSocialCartaoActionForm.setAreaConstruida(areaConstruidaFormatada);
			}else{
			...
			if(imovelEconomia.getAreaConstruidaFaixa() != null){
				tarifaSocialCartaoActionForm.setAreaConstruidaFaixa(imovelEconomia.getAreaConstruidaFaixa().getId().toString());
			}else{
				tarifaSocialCartaoActionForm.setAreaConstruidaFaixa("" + ConstantesSistema.NUMERO_NAO_INFORMADO);
			}
		...
		httpServletRequest.setAttribute("clienteImovelEconomiaID", clienteImovelEconomiaID);
	 ...
	 * @return
	 */
	private TarifaSocialDadoEconomia buscarTarifaSocialDadoEconomiaSelecionada(Collection colecao, String idObjeto){
		...
		if(colecao != null && !colecao.isEmpty() && idObjeto != null && !idObjeto.equals("")){
			...
			while(colecaoIterator.hasNext()){
				...
				if(clienteImovelEconomia.getId().intValue() == new Integer(idObjeto).intValue()){
					colecaoTarifaSocialDadoEconomia = clienteImovelEconomia.getImovelEconomia().getTarifaSocialDadoEconomias();

					if(colecaoTarifaSocialDadoEconomia != null && !colecaoTarifaSocialDadoEconomia.isEmpty()){
						retorno = (TarifaSocialDadoEconomia) Util.retonarObjetoDeColecao(colecaoTarifaSocialDadoEconomia);
					}
