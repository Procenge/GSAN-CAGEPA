 ...
 */
public class AtualizarDadosTarifaSocialAction
				extends ControladorAlteracaoGcomAction {

	public static final String CASO_USO = AtualizarDadosTarifaSocialAction.class.getName() + ".do";

	public static final String ACAO_EXIBIR = CASO_USO + "?" + ControladorGcomAction.PARAMETRO_ACAO + "="
					+ ControladorGcomAction.PARAMETRO_ACAO_EXIBIR;

	public static final String ACAO_PROCESSAR = CASO_USO + "?" + ControladorGcomAction.PARAMETRO_ACAO + "="
					+ ControladorGcomAction.PARAMETRO_ACAO_PROCESSAR;

	public ActionForward concluir(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest request,
	...
	public ActionForward exibirAuxiliar(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest request,
					HttpServletResponse response) throws Exception{
		...
		if(otd != null){
			...
			while(it.hasNext()){
				if(otd.getQuantidadeEconomia() == 1){
					...
					if(obj != null){
						if(obj.getAreaConstruida() != null) form.setAreaConstruida((obj.getAreaConstruida() + "").replace('.', ','));
						if(obj.getAreaConstruidaFaixa() != null) form.setAreaConstruidaFaixa(obj.getAreaConstruidaFaixa().getId() + "");
						if(obj.getNumeroIptu() != null && obj.getNumeroIptu().intValue() != 0) form.setNumeroIPTU(obj.getNumeroIptu() + "");
						if(obj.getNumeroCelpe() != null && obj.getNumeroCelpe().intValue() != 0) form.setNumeroContratoCelpe(obj
										.getNumeroCelpe()
										+ "");
	...
	public ActionForward processarAuxiliar(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest request,
					HttpServletResponse response) throws Exception{

		AtualizarDadosTarifaSocialActionForm form = (AtualizarDadosTarifaSocialActionForm) actionForm;

		MostrarDadosTarifaSocialActionForm f = (MostrarDadosTarifaSocialActionForm) getControladorGcomActionForm(request,
						"MostrarDadosTarifaSocialActionForm");
		...
		if(otd != null){
			...
			while(it.hasNext()){
				if(otd.getQuantidadeEconomia() == 1){
					...
					Imovel obj = (Imovel) clienteEconomia.getImovel();
					if(obj != null){
						if(!"".equals(form.getAreaConstruida().trim())) obj.setAreaConstruida(new BigDecimal(form.getAreaConstruida()
										.replace(',', '.')));
		...
		BigDecimal valorRendaFamiliar = (!"".equals(form.getValorRendaFamiliar().trim())) ? new BigDecimal(form.getValorRendaFamiliar())
						: null;
		String tarifaSocialCartaoTipo = (!"".equals(form.getTipoCartao().trim()) && form.getTipoCartao() != null) ? form.getTipoCartao()
						: null;
		String tipoRenda = (!"".equals(form.getRendaTipo().trim())) ? form.getRendaTipo() : null;

		Fachada.getInstancia().verificarPreenchimentoInserirDadosTarifaSocial(numeroCelpe, areaConstruida, numeroIPTU, idImovel,
						numeroCartaoSocial, dataValidadeCartaoSocial, numeroParcelasCartaoSocial, consumoMedio, valorRendaFamiliar,
						tarifaSocialCartaoTipo, tipoRenda);
	...
	public Object consultarObjetoSistema(String[] chavesPrimarias, HttpServletRequest request, ControladorAlteracaoGcomActionForm actionForm)
					throws Exception{

		AtualizarDadosTarifaSocialActionForm form = (AtualizarDadosTarifaSocialActionForm) actionForm;

		// MostrarDadosTarifaSocialActionForm f = (MostrarDadosTarifaSocialActionForm)
		// getControladorGcomActionForm(request,"MostrarDadosTarifaSocialActionForm");
	...
	public void carregarColecao(ControladorAlteracaoGcomActionForm actionForm){
		...
		//
		// AtualizarDadosTarifaSocialActionForm form = (AtualizarDadosTarifaSocialActionForm)
		// actionForm;
		//
		// Collection collTarifaSocialCartaoTipo = Fachada.getInstancia().pesquisar(new
		// FiltroTarifaSocialCartaoTipo(FiltroTarifaSocialCartaoTipo.DESCRICAO),TarifaSocialCartaoTipo.class.getName());
		// form.setCollTipoCartao(collTarifaSocialCartaoTipo);
		//
		// Collection collAreaConstruida = Fachada.getInstancia().pesquisar(new
		// FiltroAreaConstruidaFaixa(FiltroAreaConstruidaFaixa.MAIOR_FAIXA),AreaConstruidaFaixa.class.getName());
		// form.setCollAreaConstruida(collAreaConstruida);
		//
		// Collection collRendaTipo = Fachada.getInstancia().pesquisar(new
		// FiltroRendaTipo(FiltroRendaTipo.DESCRICAO),RendaTipo.class.getName());
		// form.setCollRendaTipo(collRendaTipo);
		//
	}

	private void validarDuplicidadeContratoCELPE(String numeroCELPE, Imovel imovel, ImovelEconomia imovelEconomia,
					HttpServletRequest request) throws ActionServletException{

		// MostrarDadosTarifaSocialActionForm form = (MostrarDadosTarifaSocialActionForm)
		// getControladorGcomActionForm(request,"MostrarDadosTarifaSocialActionForm");
	...
	private void validarDuplicidadeContratoIPTU(String numeroIPTU, Imovel imovel, ImovelEconomia imovelEconomia, HttpServletRequest request)
	...
	private void validarDuplicidadeCartaoProgramaSocial(HttpServletRequest request, String numeroCartaoProgramaSocial,
					String tarifaSocialCartaoTipo) throws ActionServletException{

		// MostrarDadosTarifaSocialActionForm f = (MostrarDadosTarifaSocialActionForm)
		// getControladorGcomActionForm(request,"MostrarDadosTarifaSocialActionForm");
		if(!"".equals(numeroCartaoProgramaSocial.trim()) && numeroCartaoProgramaSocial != null && tarifaSocialCartaoTipo != null
						&& !"".equals(tarifaSocialCartaoTipo.trim())){
			...
			filtroTarifaSocialDadoEconomia.adicionarCaminhoParaCarregamentoEntidade(FiltroTarifaSocialDadoEconomia.IMOVEL_ECONOMEIA);
			filtroTarifaSocialDadoEconomia
							.adicionarCaminhoParaCarregamentoEntidade(FiltroTarifaSocialDadoEconomia.TARIFA_SOCIAL_DADO_IMOVEL);

			filtroTarifaSocialDadoEconomia
							.adicionarParametro(new ParametroSimples("numeroCartaoProgramaSocial", numeroCartaoProgramaSocial));
			...
			if(coll != null && !coll.isEmpty()){
				...
				while(it.hasNext()){
					...
					// if
					// (!f.getIdRegistroAtualizacao().equals(tarifaSocialDadoEconomia.getTarifaSocialDado().getImovel().getId()
					// + "")) {
					// throw new
					// ActionServletException("atencao.duplicidade_cartao_programa_social",null,tarifaSocialDadoEconomia.getTarifaSocialDado().getImovel().getId().toString());
					// }
				}
			}
		}
	...
	public Object gerarObject(ControladorAlteracaoGcomActionForm actionForm, HttpServletRequest request){
		...
		if(!"".equals(form.getId().trim())) tarifaSocialDadoEconomia.setId(new Integer(form.getId()));
		if(!"".equals(form.getNumeroCartaoProgramaSocial().trim())) tarifaSocialDadoEconomia.setNumeroCartaoProgramaSocial(new Long(form
		...
		if(!"".equals(form.getNumeroMesesAdesao().trim())) tarifaSocialDadoEconomia.setNumeroMesesAdesao(new Short(form
						.getNumeroMesesAdesao()));

		if(!"".equals(form.getValorRendaFamiliar().trim())) tarifaSocialDadoEconomia.setValorRendaFamiliar(new BigDecimal(form
						.getValorRendaFamiliar()));

		if(!"".equals(form.getConsumoCelpe().trim())) tarifaSocialDadoEconomia.setConsumoCelpe(new Integer(form.getConsumoCelpe()));

		if(!"".equals(form.getTipoCartao().trim()) && form.getTipoCartao() != null){

			if((!"".equals(form.getTipoCartao().trim()) && tarifaSocialDadoEconomia.getTarifaSocialCartaoTipo() != null)
							&& tarifaSocialDadoEconomia.getTarifaSocialCartaoTipo().getId().intValue() != new Integer(form.getTipoCartao())
											.intValue() || (tarifaSocialDadoEconomia.getTarifaSocialCartaoTipo() == null)){
				...
				Collection collCartaoTipo = Fachada.getInstancia().pesquisar(filtroTarifaSocialCartaoTipo,
		...
		if(!"".equals(form.getRendaTipo().trim())){
			...
			Collection collRendaTipo = Fachada.getInstancia().pesquisar(filtroRendaTipo, RendaTipo.class.getName());
	...
	public void montarFormulario(Object obj, ControladorAlteracaoGcomActionForm actionForm){
		...
		if(obj instanceof TarifaSocialDadoEconomia){
			...
			if(tarifaSocialDadoEconomia != null){
				if(tarifaSocialDadoEconomia.getRendaTipo() != null) form.setRendaTipo(tarifaSocialDadoEconomia.getRendaTipo().getId()
								.toString());

				if(tarifaSocialDadoEconomia.getTarifaSocialCartaoTipo() != null) form.setTipoCartao(tarifaSocialDadoEconomia
								.getTarifaSocialCartaoTipo().getId().toString());
			}
			if(tarifaSocialDadoEconomia.getId() != null) form.setId(tarifaSocialDadoEconomia.getId() + "");
			if(tarifaSocialDadoEconomia.getNumeroCartaoProgramaSocial() != null) form
							.setNumeroCartaoProgramaSocial(tarifaSocialDadoEconomia.getNumeroCartaoProgramaSocial() + "");
			if(tarifaSocialDadoEconomia.getDataValidadeCartao() != null) form.setDataValidadeCartao(gcom.util.Util
							.formatarData(tarifaSocialDadoEconomia.getDataValidadeCartao()));
			if(tarifaSocialDadoEconomia.getNumeroMesesAdesao() != null) form.setNumeroMesesAdesao(tarifaSocialDadoEconomia
							.getNumeroMesesAdesao()
							+ "");
			if(tarifaSocialDadoEconomia.getConsumoCelpe() != null) form.setConsumoCelpe(tarifaSocialDadoEconomia.getConsumoCelpe() + "");
			if(tarifaSocialDadoEconomia.getValorRendaFamiliar() != null) form.setValorRendaFamiliar((tarifaSocialDadoEconomia
							.getValorRendaFamiliar() + "").replace('.', ','));
			if(tarifaSocialDadoEconomia.getTarifaSocialCartaoTipo() != null) form.setTipoCartao(tarifaSocialDadoEconomia
							.getTarifaSocialCartaoTipo().getId()
							+ "");
			if(tarifaSocialDadoEconomia.getRendaTipo() != null) form.setRendaTipo(tarifaSocialDadoEconomia.getRendaTipo().getId() + "");

			Collection coll = Fachada.getInstancia().pesquisar(new FiltroSistemaParametro(), SistemaParametro.class.getName());
			if(coll != null && !coll.isEmpty()){
				form.setSalarioMinimo(((SistemaParametro) coll.iterator().next()).getValorSalarioMinimo().toString());
				form.setConsumoEnergiaMaximoTarifaSocial(((SistemaParametro) coll.iterator().next()).getConsumoEnergiaMaximoTarifaSocial()
								.toString());
