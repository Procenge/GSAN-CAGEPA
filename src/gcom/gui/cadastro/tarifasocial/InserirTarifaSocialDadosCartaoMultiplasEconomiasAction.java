 ...
 * @author rodrigo
 */
public class InserirTarifaSocialDadosCartaoMultiplasEconomiasAction
				extends GcomAction {
	...
	public ActionForward execute(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest httpServletRequest,
		...
		ActionForward retorno = actionMapping.findForward("inserirTarifaSocialDadosCartaoMultiplasEconomias");
		...
		String clienteImovelEconomiaID = httpServletRequest.getParameter("clienteImovelEconomiaID");
		...
		String[] dado = fachada.verificarPreenchimentoInserirDadosTarifaSocialMultiplas(numeroCelpeFormatado, areaConstruidaFormatada,
						numeroIptuFormatado, imovelEconomiaAtualizado.getId(), form.getNumero(), form.getDataValidade(), form
										.getNumeroParcelas(), new Integer(form.getNumeroCelpe().trim().equals("") ? "0" : form
										.getNumeroCelpe()), form.getValorRendaFamiliar().trim().equals("") ? Util
										.formatarMoedaRealparaBigDecimal("0") : Util.formatarMoedaRealparaBigDecimal(form
										.getValorRendaFamiliar()), form.getTipoCartao(), form.getTipoRenda());
		...
		if(form.getNumero() != null && !form.getNumero().equals("")){
			tarifaSocialDadoEconomia.setNumeroCartaoProgramaSocial(new Long(form.getNumero()));
			...
			filtroTarifaSocialCartaoTipo.adicionarParametro(new ParametroSimples(FiltroTarifaSocialCartaoTipo.ID, form.getTipoCartao()));

			colecaoPesquisa = fachada.pesquisar(filtroTarifaSocialCartaoTipo, TarifaSocialCartaoTipo.class.getName());

			tarifaSocialCartaoTipo = (TarifaSocialCartaoTipo) Util.retonarObjetoDeColecao(colecaoPesquisa);
		...
		// Consumo Celpe
		if(form.getNumeroCelpe() != null && !form.getNumeroCelpe().equals("")){
			tarifaSocialDadoEconomia.setConsumoCelpe(new Integer(form.getNumeroCelpe()));
		}

		// Número de parcelas
		if(form.getNumeroParcelas() != null && !form.getNumeroParcelas().equals("")){
			tarifaSocialDadoEconomia.setNumeroMesesAdesao(new Short(form.getNumeroParcelas()));
		}

		if(form.getValorRendaFamiliar() != null && !form.getValorRendaFamiliar().equals("")){
			...
			filtroRendaTipo.adicionarParametro(new ParametroSimples(FiltroRendaTipo.ID, form.getTipoRenda()));

			colecaoPesquisa = fachada.pesquisar(filtroRendaTipo, RendaTipo.class.getName());

			rendaTipo = (RendaTipo) Util.retonarObjetoDeColecao(colecaoPesquisa);
		...
		if(numeroIptu != null && !numeroIptu.trim().equals("")){
			...
			if(colecaoImovelEconomiaAtualizar != null && !colecaoImovelEconomiaAtualizar.isEmpty()){
				...
				while(colecaoImovelEconomiaAtualizarIterator.hasNext()){
					...
					ImovelEconomia imovelEconomia = (ImovelEconomia) colecaoImovelEconomiaAtualizarIterator.next();

					// Verifica se o número de contrato da companhia já foi usado em outra economia
					// do imóvel
					if(imovelEconomia.getNumeroIptu() != null && imovelEconomia.getNumeroIptu().equals(numeroIptuFormatado)
									&& !imovelEconomia.getId().equals(imovelEconomiaAtualizado.getId())){
						throw new ActionServletException("atencao.imovel.iptu_jacadastrado", null, imovelSessao.getId().toString());
					}
				}
			}

			idImovel = fachada.verificarNumeroIptu(numeroIptuFormatado, null, imovelEconomiaAtualizado.getId(), imovelEconomiaAtualizado
							.getImovelSubcategoria().getComp_id().getImovel().getSetorComercial().getMunicipio().getId());
		...
		if(numeroContratoCompanhiaEletrica != null && !numeroContratoCompanhiaEletrica.trim().equals("")){
			...
			if(colecaoImovelEconomiaAtualizar != null && !colecaoImovelEconomiaAtualizar.isEmpty()){
				...
				while(colecaoImovelEconomiaAtualizarIterator.hasNext()){
					...
					if(imovelEconomia.getNumeroCelpe() != null
									&& imovelEconomia.getNumeroCelpe().equals(numeroContratoCompanhiaEletricaFormatado)
									&& !imovelEconomia.getId().equals(imovelEconomiaAtualizado.getId())){
			...
			idImovel = fachada.verificarNumeroCompanhiaEletrica(numeroContratoCompanhiaEletricaFormatado, null, imovelEconomiaAtualizado
							.getId());
		...
		if(form.getIdImovel() != null && !form.getIdImovel().equals("")){
			if(form.getMotivoExclusao() != null && !form.getMotivoExclusao().equals("" + ConstantesSistema.NUMERO_NAO_INFORMADO)){
				...
				// String idTarifaSocialDadoEconomia = (String)
				// sessao.getAttribute("idTarifaSocialDadoEconomia");
		...
		if(chave.trim().equals("")){
			...
			retornarColecaoAtualizada(colecaoClienteImovelEconomia, clienteImovelEconomiaID, tarifaSocialDadoEconomia);
	 ...
	 * @return
	 */
	private void retornarColecaoAtualizada(Collection colecao, String idObjeto, TarifaSocialDadoEconomia tarifaSocial){

		ClienteImovelEconomia clienteImovelEconomia = null;
		Set colecaoTarifaSocialDadoEconomia;

		if(colecao != null && !colecao.isEmpty() && idObjeto != null && !idObjeto.equals("")){
			Iterator colecaoIterator = colecao.iterator();

			while(colecaoIterator.hasNext()){
				clienteImovelEconomia = (ClienteImovelEconomia) colecaoIterator.next();

				if(clienteImovelEconomia.getId().intValue() == new Integer(idObjeto).intValue()){
					colecaoTarifaSocialDadoEconomia = new HashSet();
					colecaoTarifaSocialDadoEconomia.add(tarifaSocial);

					clienteImovelEconomia.getImovelEconomia().setTarifaSocialDadoEconomias(colecaoTarifaSocialDadoEconomia);
				}else{
					...
					if(colecaoTarifasSociais != null && !colecaoTarifasSociais.isEmpty()){
						...
						while(colecaoTarifasSociaisIterator.hasNext()){
							...
							if(tarifaSocialDadoEconomia.getNumeroCartaoProgramaSocial() != null
											&& tarifaSocial.getNumeroCartaoProgramaSocial() != null
											&& tarifaSocialDadoEconomia.getNumeroCartaoProgramaSocial().longValue() == tarifaSocial
															.getNumeroCartaoProgramaSocial().longValue()){

								if(tarifaSocialDadoEconomia.getTarifaSocialCartaoTipo() != null
												&& tarifaSocial.getTarifaSocialCartaoTipo() != null
												&& tarifaSocialDadoEconomia.getTarifaSocialCartaoTipo().getId().equals(
																tarifaSocial.getTarifaSocialCartaoTipo().getId())){
									throw new ActionServletException("atencao.cartao_programa_social_duplicidade");
								}
							}
						}
