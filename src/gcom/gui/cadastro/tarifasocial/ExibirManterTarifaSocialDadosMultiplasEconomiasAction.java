 ...
 * @author rodrigo
 */
public class ExibirManterTarifaSocialDadosMultiplasEconomiasAction
				extends GcomAction {
	...
	public ActionForward execute(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest httpServletRequest,
		...
		ActionForward retorno = actionMapping.findForward("manterTarifaSocialMultiplasEconomias");
		...
		if(sessao.getAttribute("atualizar") != null){
			colecaoTarifaSocialHelper = (ArrayList) sessao.getAttribute("colecaoTarifaSocialHelper");
			sessao.removeAttribute("atualizar");
		}else{
			colecaoTarifaSocialHelper = (ArrayList) fachada.pesquisarDadosClienteEconomiaTarifaSocial(new Integer(idImovel));
			...
			if(idTarifaSocial != null && !idTarifaSocial.equals("")){
				...
				if(!removeu){
					...
					TarifaSocialDadoEconomia tarifaSocialDadoEconomia = fachada.pesquisarTarifaSocial(new Integer(idTarifaSocial));

					if(tarifaSocialDadoEconomia.getTarifaSocialExclusaoMotivo().getIndicadorPermiteRecadastramentoImovel().equals(
									TarifaSocialExclusaoMotivo.INDICADOR_PERMITE_RECADASTRAMENTO_NAO)){
					...
					if(tarifaSocialDadoEconomia.getQuantidadeRecadastramento() != null){
						qtdeRecadastramentos = tarifaSocialDadoEconomia.getQuantidadeRecadastramento().intValue() + 1;
					}

					tarifaSocialDadoEconomia.setQuantidadeRecadastramento(new Short("" + qtdeRecadastramentos));
					...
					while(colecaoTarifaSocialHelperIterator.hasNext()){
						...
						if(tarifaSocialHelper.getTarifaSocialDadoEconomia().getId().equals(tarifaSocialDadoEconomia.getId())){

							Cliente cliente = tarifaSocialHelper.getClienteImovelEconomia().getCliente();

							Collection colecaoImovel = fachada.verificarClienteCadastradoManterTarifaSocialMultiplasEconomias(cliente
											.getId(), tarifaSocialDadoEconomia.getImovelEconomia().getId());

							if(colecaoImovel != null && !colecaoImovel.isEmpty()){

								Imovel imovel = (Imovel) colecaoImovel.iterator().next();

								throw new ActionServletException("atencao.usuario.ja_cadastrado_tarifasocial.sem.encerramento", null,
							...
							// Caso o usuário esteja na tarifa social em outro imóvel com motivo de
							// revisão
							if(colecaoClientesUsuarioExistenteTarifaSocial != null
											&& !colecaoClientesUsuarioExistenteTarifaSocial.isEmpty()){
								httpServletRequest.setAttribute("abrirPopupExclusao", true);
								...
								colecaoTarifasSociaisRecadastradas.add(tarifaSocialDadoEconomia);
