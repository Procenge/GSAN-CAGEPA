 ...
 * @date
 */
public class ConsultaImovelTarifaSocialAction
				extends ControladorConsultaGcomAction {

	public static final String CASO_USO = ConsultaImovelTarifaSocialAction.class.getName() + ".do";
	...
	public Collection pesquisarObjetoSistema(Filtro filtro) throws Exception{
		 ...
		 * Collection coll =
		 * Fachada.getInstancia().pesquisarImovelTarfiaSocial(filtroClienteImovel);
		 * if (coll == null || coll.isEmpty()) {
		 * throw new ActionServletException("atencao.naocadastrado", null,
		 * "Imóvel com dados da tarifa social");
		 * }
		 */
		return null;
	}

	public ActionForward remover(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest request,
		...
		if(("".equals(form.getTarifaSocialExclusaoMotivo()) || form.getTarifaSocialExclusaoMotivo() == null)
						&& request.getParameter("tarifaSocialExclusaoMotivo") != null){
			...
			for(int i = 0; i < ids.length; i++){
				try{
					Fachada.getInstancia()
									.removerImovelTarfiaSocial(new Integer(ids[i]), new Integer(form.getTarifaSocialExclusaoMotivo()));
		...
		request.setAttribute("caminhoFuncionalidade",
						"exibirFiltrarImovelAction.do?menu=sim&acao=exibir&redirecionar=ManterDadosTarifaSocial");
	...
	public ActionForward exibirRemover(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest request,
					HttpServletResponse response) throws Exception{
		...
		form.setCollTarifaSocialExclusaoMotivo(Fachada.getInstancia().pesquisar(filtroTarifaSocialExclusaoMotivo,
	...
	public ActionForward processarAuxiliar(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest request,
		...
		if(request.getAttribute("filtroImovelPreenchido") != null){
			...
			// seta na sessao
			request.getSession().setAttribute("filtroImovelPreenchido", filtroClienteImovel);
		}else{
			if(request.getSession().getAttribute("filtroImovelPreenchido") != null){
				filtroClienteImovel = (FiltroClienteImovel) request.getSession().getAttribute("filtroImovelPreenchido");
			}else{
		...
		// tela
		int totalRegistros = Fachada.getInstancia().pesquisarQuantidadeImovelTarfiaSocial(filtroClienteImovel);
		...
		Collection imoveis = Fachada.getInstancia().pesquisarImovelTarfiaSocial(filtroClienteImovel,
						((Integer) request.getAttribute("numeroPaginasPesquisa")));
	...
	public Filtro gerarFiltro(ControladorConsultaGcomActionForm actionForm){
		...
		// ConsultaImovelTarifaSocialActionForm form = (ConsultaImovelTarifaSocialActionForm)
		// actionForm;
		 ...
		 * "imovel.logradouro.logradouroTipo");
		 * filtroClienteImovel.adicionarCaminhoParaCarregamentoEntidade(
		 * "imovel.logradouro.logradouroTitulo");
		 * 
		 * filtroClienteImovel.adicionarCaminhoParaCarregamentoEntidade("imovel.enderecoReferencia");
		 * filtroClienteImovel.adicionarCaminhoParaCarregamentoEntidade(
		 * "imovel.quadra.bairro.municipio.unidadeFederacao");
		 ...
		 * ParametroSimples(FiltroClienteImovel.IMOVEL_ID, form.getMatricula()));
		 * }
		 * if (form.getIdLocalidade() != null && !"".equals(form.getIdLocalidade())) {
		 * filtroClienteImovel.adicionarParametro(new
		 * ParametroSimples(FiltroClienteImovel.LOCALIDADE_ID, form.getIdLocalidade()));
		 * }
		 * if (form.getIdSetorComercial() != null && !"".equals(form.getIdSetorComercial())) {
		 * filtroClienteImovel.adicionarParametro(new
		 * ParametroSimples(FiltroClienteImovel.SETOR_COMERCIAL_ID, form.getIdSetorComercial()));
		 * }
		 * if (form.getIdQuadra() != null && !"".equals(form.getIdQuadra())) {
		 * filtroClienteImovel.adicionarParametro(new
		 * ParametroSimples(FiltroClienteImovel.QUADRA_NUMERO, form.getLote()));
		 * }
		 * if (form.getLote() != null && !"".equals(form.getLote())) {
		 * filtroClienteImovel.adicionarParametro(new ParametroSimples(FiltroClienteImovel.LOTE,
		 * form.getLote()));
		 * }
		 * if (form.getSubLote() != null && !"".equals(form.getSubLote())) {
		 * filtroClienteImovel.adicionarParametro(new ParametroSimples(FiltroClienteImovel.SUBLOTE,
		 * form.getSubLote()));
		 * }
		 * if (form.getCodigoCliente() != null && !"".equals(form.getCodigoCliente())) {
		 * filtroClienteImovel.adicionarParametro(new
		 * ParametroSimples(FiltroClienteImovel.CLIENTE_ID, form.getCodigoCliente()));
		 * }
		 * if (form.getIdMunicipio() != null && !"".equals(form.getIdMunicipio())) {
		 * filtroClienteImovel.adicionarParametro(new
		 * ParametroSimples(FiltroClienteImovel.MUNICIPIO_SETOR_COMERICAL_CODIGO,
		 * form.getIdMunicipio()));
		 * }
		 * if (form.getCep() != null && !"".equals(form.getCep())) {
		 * filtroClienteImovel.adicionarParametro(new
		 * ParametroSimples(FiltroClienteImovel.CEP_CODIGO, form.getCep()));
		 * }
		 * if (form.getIdBairro() != null && !"".equals(form.getIdBairro())) {
		 * filtroClienteImovel.adicionarParametro(new
		 * ParametroSimples(FiltroClienteImovel.BAIRRO_CODIGO, form.getIdBairro()));
		 * }
		 * if (form.getLogradouro() != null && !"".equals(form.getLogradouro())) {
		 * filtroClienteImovel.adicionarParametro(new
		 * ParametroSimples(FiltroClienteImovel.LOGRADOURO_NOME, form.getLogradouro()));
		 * }
		 * if (form.getIndicadorUso() != null && !"".equals(form.getIndicadorUso())) {
		 * filtroClienteImovel.adicionarParametro(new
		 * ParametroSimples(FiltroClienteImovel.INDICADOR_USO, form.getIndicadorUso()));
		 * }
		 */
