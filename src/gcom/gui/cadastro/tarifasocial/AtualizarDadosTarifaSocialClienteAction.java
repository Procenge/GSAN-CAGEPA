 ...
 */
public class AtualizarDadosTarifaSocialClienteAction
				extends ControladorInclusaoGcomAction {

	public static final String CASO_USO = AtualizarDadosTarifaSocialClienteAction.class.getName() + ".do";
	...
	public ActionForward exibirAuxiliar(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest request,
		...
		AtualizarDadosTarifaSocialActionForm f2 = (AtualizarDadosTarifaSocialActionForm) getControladorGcomActionForm(request,
					...
					while(it.hasNext()){
						...
						if(f2.getId().equals(clienteEconomia.getTarifaSocialDadoEconomia().getId() + "")){
							...
							filtroClienteImovelEconomia.adicionarParametro(new ParametroSimples(
											FiltroClienteImovelEconomia.IMOVEL_ECONOMIA_ID, clienteEconomia.getImovelEconomia().getId()));
		...
		if(clienteUsuario != null){
			...
			if(coll != null){
				...
				while(it.hasNext()){
					...
					if((form.getIdImovelEconomia().equals(clienteEconomia.getTarifaSocialDadoEconomia().getId() + ""))
									&& !clienteUsuario.getId().equals(clienteEconomia.getCliente().getId())){
	...
	public ActionForward removerCliente(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest request,
					HttpServletResponse response) throws Exception{
	...
	public ActionForward adicionarCliente(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest request,
					HttpServletResponse response) throws Exception{
		...
		filtroClienteRelacaoTipo.adicionarParametro(new ParametroSimples(FiltroClienteRelacaoTipo.CLIENTE_RELACAO_TIPO_ID, form
		...
		if(ClienteRelacaoTipo.USUARIO.equals(new Short(form.getClienteRelacaoTipo()))){
			Fachada.getInstancia().verificarClienteUsuarioEmOutroEconomia(null, new Integer(form.getIdImovelEconomia()),
		...
		try{
			...
			if(coll != null && !coll.isEmpty()) cliente = (Cliente) coll.iterator().next();
		...
		if(clientes != null){
			...
			while(it.hasNext()){
				...
				if(ClienteRelacaoTipo.USUARIO.equals(clienteRelacaoTipo.getId())
								&& ClienteRelacaoTipo.USUARIO.equals(clienteImovel.getClienteRelacaoTipo().getId())
								&& clienteImovel.getDataFimRelacao() == null){
					throw new ActionServletException("atencao.tarifasocial.cliente_usuario_duplicidade");
				}

				if(clienteImovel.getCliente().getId().intValue() == ci.getCliente().getId().intValue()
								&& clienteImovel.getDataFimRelacao() == null
								&& clienteImovel.getClienteRelacaoTipo().getId().equals(clienteRelacaoTipo.getId())){
					if(ClienteRelacaoTipo.USUARIO.equals(clienteRelacaoTipo.getId())){
						throw new ActionServletException("atencao.tarifasocial.cliente_usuario_duplicidade");
	...
	public ActionForward pesquisaCliente(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest request,
					HttpServletResponse response) throws Exception{
		...
		try{
			...
			if(idCliente.intValue() == 0) throw new ActionServletException("atencao.tarifasocial.codigo_cliente_iqual_zero");
	...
	public ActionForward mostraMotivo(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest request,
					HttpServletResponse response) throws Exception{
		...
		// colecao de clientes que fora solicitados para remover e que ainda noa foram persistido no
		// banco
		...
		if(collParaRemover.isEmpty()){
			...
			request.getSession().setAttribute("clienteImovelEconomia", collParaRemover);

			form.setCollClienteImovelFimRelacaoMotivo(Fachada.getInstancia().pesquisar(new FiltroClienteImovelFimRelacaoMotivo(),
	...
	/*
	 * public ActionForward mostraMotivo(ActionMapping actionMapping, ActionForm actionForm,
	 * HttpServletRequest request, HttpServletResponse response) throws Exception {
	 * AtualizarDadosTarifaSocialClienteActionForm form =
	 * (AtualizarDadosTarifaSocialClienteActionForm) actionForm;
	 * String[] posicaoParaRemover = form.getPosicaoParaRemover();
	 * form.setCollClienteImovelFimRelacaoMotivo(Fachada.getInstancia().pesquisar(new
	 * FiltroClienteImovelFimRelacaoMotivo(),ClienteImovelFimRelacaoMotivo.class.getName()));
	 * return actionMapping.findForward(ControladorGcomAction.FORWARD_EXIBIR);
	 * }
	 */
	...
	public void carregarColecao(ControladorGcomActionForm actionForm){
		...
		Collection collClienteRelacaoTipo = Fachada.getInstancia().pesquisar(
						new FiltroClienteRelacaoTipo(FiltroClienteRelacaoTipo.DESCRICAO), ClienteRelacaoTipo.class.getName());
