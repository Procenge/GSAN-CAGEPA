 ...
 * @since 16/01/2007
 */
public class ExibirRemoverClienteImovelTarifaSocialAction
				extends GcomAction {
	...
	public ActionForward execute(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest httpServletRequest,
		...
		Collection colecaoFimRelacaoMotivo = fachada.pesquisar(filtroClienteImovelFimRelacaoMotivo, ClienteImovelFimRelacaoMotivo.class
						.getName());
		...
		atualizarDadosTarifaSocialClienteActionForm.setDataFimRelacao(Util.formatarData(dataAtual));
		atualizarDadosTarifaSocialClienteActionForm.setClienteImovelFimRelacaoMotivo("" + ConstantesSistema.NUMERO_NAO_INFORMADO);

		String[] posicaoRemover = atualizarDadosTarifaSocialClienteActionForm.getPosicaoParaRemover();
		...
		if(sessao.getAttribute("colecaoClienteImovel") != null){
			...
			while(i < posicaoRemover.length){
				...
				if(posicao != null && !posicao.equals("")){
					...
					while(colecaoClienteImovelIterator.hasNext()){
						ClienteImovel clienteImovel = (ClienteImovel) colecaoClienteImovelIterator.next();
						...
						if(posicao.equals("" + a)){
							...
							// ainda não esteja na base
								...
								if(!existePeloMenosUmNaBase){
									retorno = actionMapping.findForward("exibirDadosCliente");
								}

								TarifaSocialHelper tarifaSocialHelperAtualizar = (TarifaSocialHelper) sessao
												.getAttribute("tarifaSocialHelperAtualizar");

								Collection colecaoClientesInseridos = tarifaSocialHelperAtualizar.getColecaoClientesInseridos();
					...
					colecaoClienteImovelIterator = colecaoClienteImovel.iterator();
				}
			...
			sessao.setAttribute("colecaoClienteImovelRemover", colecaoClienteImovelRemover);
			...
			while(i < posicaoRemover.length){
				...
				if(posicao != null && !posicao.equals("")){
					...
					while(colecaoClienteImovelEconomiaIterator.hasNext()){
						ClienteImovelEconomia clienteImovelEconomia = (ClienteImovelEconomia) colecaoClienteImovelEconomiaIterator.next();
						...
						if(posicao.equals("" + a)){
							...
							// ainda não esteja na base
								...
								tarifaSocialHelperAtualizar.setColecaoClientesEconomiaInseridos(colecaoClientesEconomiaInseridos);
								sessao.setAttribute("tarifaSocialHelperAtualizar", tarifaSocialHelperAtualizar);
			...
			sessao.setAttribute("colecaoClienteImovelEconomiaRemover", colecaoClienteImovelEconomiaRemover);
		}
