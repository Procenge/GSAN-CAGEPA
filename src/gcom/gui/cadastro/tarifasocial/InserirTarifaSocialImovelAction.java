 ...
 * @author rodrigo
 */
public class InserirTarifaSocialImovelAction
				extends GcomAction {
	...
	public ActionForward execute(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest httpServletRequest,
		...
		if(idImovel != null && !idImovel.equals("")){
			...
			Collection clienteImoveis = fachada.pesquisar(filtroClienteImovel, ClienteImovel.class.getName());
			...
			if(clienteImoveis == null || clienteImoveis.isEmpty()){
				...
				clienteImovelObj = (ClienteImovel) clienteImoveis.iterator().next();

				if(clienteImovelObj.getImovel().getImovelPerfil() != null
								&& clienteImovelObj.getImovel().getImovelPerfil().getId().equals(ImovelPerfil.TARIFA_SOCIAL)){
			...
			int quantidadeEconomiasImovel = fachada.obterQuantidadeEconomias(imovel);
			...
			if(ParametroCadastro.P_VERIFICAR_ECONOMIA_E_PONTO_UTILIZACAO.executar().equals(ConstantesSistema.SIM.toString())){
				if(quantidadeEconomiasImovel > 1){
					throw new ActionServletException("atencao.nao_cadastrar_tarifa_social_mais_1_economia", idImovel);
				}
				...
				// Pesquisa o imóvel para saber a sua quantidade de pontos de utilização
				imovel = (Imovel) fachada.pesquisar(filtroImovel, Imovel.class.getName()).iterator().next();

				if(imovel.getNumeroPontosUtilizacao() != null && imovel.getNumeroPontosUtilizacao() > 7){
					throw new ActionServletException("atencao.nao_cadastrar_tarifa_social_mais_7_pontos_utilizacao", idImovel);
				}
			...
			inserirTarifaSocialActionForm.set("qtdEconomia", new Integer(quantidadeEconomiasImovel));
