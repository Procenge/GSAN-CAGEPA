 ...
 */
public class MostrarDadosTarifaSocialAction
				extends ControladorInclusaoGcomAction {

	public static final String CASO_USO = MostrarDadosTarifaSocialAction.class.getName() + ".do";
	...
	public ActionForward exibirAuxiliar(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest request,
		...
		fachada
						.verificarExistenciaRegistroAtendimentoTarifaSocial(idImovel,
										"atencao.manter_tarifa_social_existencia_registro_atendimento");
		...
		if(quantidade == 1){
			...
			while(it.hasNext()){
				...
				filtroTarifaSocialDadoEconomia.adicionarParametro(new ParametroSimples(FiltroTarifaSocialDadoEconomia.IMOVEL_ECONOMEIA,
								imovelEconomia.getId()));
	...
	public ActionForward reexibir(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest request,
					HttpServletResponse response) throws Exception{
	...
	public ActionForward processarAuxiliar(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest request,
		...
		if(collTarifaSocial != null){
			...
			while(it.hasNext()){
				 ...
				 * if (coll != null && !coll.isEmpty()) {
				 * TarifaSocialDadoEconomia temp = (TarifaSocialDadoEconomia)
				 * coll.iterator().next();
					...
					while(it.hasNext()){
						...
						if(otd.getQuantidadeEconomia() == 1){
							...
							if(otdClienteEconomia.getImovelEconomia() != null){
								percistirClienteImovelEconomia(tarifa.getId(), otdClienteImovelEconomia, otdClienteEconomia
												.getImovelEconomia());
							}
						}
					}
		...
		return this.telaSucesso(actionMapping, request, "Dados da Tarifa Social do imóvel de matrícula " + form.getIdRegistroAtualizacao()
						+ " foram atualizados com sucesso.", "Realizar outra Manutenção de Tarifa Social",
						ConsultaImovelTarifaSocialAction.ACAO_EXIBIR);
	}
	...
	public Filtro gerarFiltro(ControladorGcomActionForm actionForm){
		...
		FiltroTarifaSocialDadoEconomia filtroTarifaSocialDadoEconomia = new FiltroTarifaSocialDadoEconomia();
		filtroTarifaSocialDadoEconomia.adicionarParametro(new ParametroSimples(FiltroTarifaSocialDadoEconomia.IMOVEL_ID, form
						.getIdRegistroAtualizacao()));
	...
	public void montarFormulario(Object obj, ControladorGcomActionForm actionForm){
		...
		if(obj instanceof Cliente){
			...
			if(cliente.getNome() != null) form.setNomeCliente(cliente.getNome());
			if(cliente.getNome() != null) form.setComplementoEndereco("");
			if(cliente.getCpf() != null) form.setCpf(cliente.getCpf());
			if(cliente.getRg() != null) form.setRg(cliente.getRg());
			if(cliente.getDataEmissaoRg() != null) form.setDataEmissao(Util.formatarData(cliente.getDataEmissaoRg()));
			if(cliente.getOrgaoExpedidorRg() != null) form.setOrgaoExpedidor(cliente.getOrgaoExpedidorRg().getDescricao());
	 ...
	 * form.getIdRegistroAtualizacao()));
	 * filtroClienteImovel.adicionarParametro(new
	 * ParametroSimples(FiltroClienteImovel.CLIENTE_RELACAO_TIPO, ClienteRelacaoTipo.USUARIO));
	 * filtroClienteImovel.adicionarCaminhoParaCarregamentoEntidade("cliente.unidadeFederacao");
	 * filtroClienteImovel.adicionarCaminhoParaCarregamentoEntidade("cliente.orgaoExpedidorRg");
	 * Collection collClienteImovel =
	 * Fachada.getInstancia().pesquisarClienteImovel(filtroClienteImovel);
	 * if (collClienteImovel != null && !collClienteImovel.isEmpty()) {
	 * ClienteImovelSimplificado clienteImovel = (ClienteImovelSimplificado)
	 * collClienteImovel.iterator().next();
	 * FiltroCliente filtroCliente = new FiltroCliente();
	...
	private void criarWizard(HttpServletRequest request){
		...
		StatusWizard statusWizard = new StatusWizard("AtualizarDadosTarifaSocialWizardAction", "AtualizarDadosTarifaSocialAction",
						"AtualizarDadosTarifaSocialClienteAction", "");

		statusWizard.inserirNumeroPaginaCaminho(statusWizard.new StatusWizardItem(1, "TarifaSocialTarifaA.gif", "TarifaSocialTarifaD.gif",
						"exibirPopUpDadosTarifaSocial", "processarPopUpDadosTarifaSocial"));

		statusWizard.inserirNumeroPaginaCaminho(statusWizard.new StatusWizardItem(2, "ClienteA.gif", "ClienteD.gif", "exibirPopUpCliente",
						"processarPopUpCliente"));

		// manda o statusWizard para a sessao
		request.getSession().setAttribute("statusWizard", statusWizard);
