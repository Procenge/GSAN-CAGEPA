 ...
 */
public class AtualizarTarifaSocialAction
				extends GcomAction {
	...
	public ActionForward execute(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest httpServletRequest,
		...
		ActionForward retorno = actionMapping.findForward("avancar");
		...
		if(tipoCartaoProgramaSocial != null && !tipoCartaoProgramaSocial.equals("" + ConstantesSistema.NUMERO_NAO_INFORMADO)){
			...
			if(tarifaSocialHelper.getClienteImovel() != null){
				...
				fachada.verificarDuplicidadeCartaoProgramaSocial(new Long(numeroCartaoProgramaSocial), tarifaSocialCartaoTipo,
		...
		if(tarifaSocialHelper.getClienteImovel() != null){
			...
			fachada.verificarPreenchimentoManterDadosTarifaSocial(numeroContratoCompanhiaEletricaFormatado, areaConstruidaFormatada,
							numeroIptuFormatado, idImovelTarifaSocial, numeroCartaoProgramaSocial, dataValidadeCartao, numeroParcelas,
							consumoMedioFormatado, valorRendaFormatada, tipoCartaoProgramaSocial, tipoRenda);
		...
		// Caso seja de múltiplas economias
			...
			idMunicipio = tarifaSocialHelper.getClienteImovelEconomia().getImovelEconomia().getImovelSubcategoria().getComp_id()
			...
			imovelEconomia.setImovelSubcategoria(tarifaSocialHelper.getClienteImovelEconomia().getImovelEconomia().getImovelSubcategoria());
			...
			if(numeroIptu != null && !numeroIptu.trim().equals("")){
				...
				if(colecaoTarifaSocialHelper != null && !colecaoTarifaSocialHelper.isEmpty()){
					...
					while(colecaoTarifaSocialHelperIterator.hasNext()){
						...
						TarifaSocialHelper tarifaSocialHelperVerificacao = (TarifaSocialHelper) colecaoTarifaSocialHelperIterator.next();

						// Verifica se o número de contrato da companhia já foi usado em outra
						// economia do imóvel
						if(tarifaSocialHelperVerificacao.getClienteImovelEconomia().getImovelEconomia().getNumeroIptu() != null
										&& tarifaSocialHelperVerificacao.getClienteImovelEconomia().getImovelEconomia().getNumeroIptu()
														.equals(numeroIptuFormatado)
										&& !tarifaSocialHelperVerificacao.getTarifaSocialDadoEconomia().getId().equals(
			...
			if(numeroContratoCompanhiaEletrica != null && !numeroContratoCompanhiaEletrica.trim().equals("")){
				...
				if(colecaoTarifaSocialHelper != null && !colecaoTarifaSocialHelper.isEmpty()){
					...
					while(colecaoTarifaSocialHelperIterator.hasNext()){
						...
						// Verifica se o número de contrato da companhia já foi usado em outra
						// economia do imóvel
						if(tarifaSocialHelperVerificacao.getClienteImovelEconomia().getImovelEconomia().getNumeroCelpe() != null
										&& tarifaSocialHelperVerificacao.getClienteImovelEconomia().getImovelEconomia().getNumeroCelpe()
														.equals(numeroContratoCompanhiaEletricaFormatado)
										&& !tarifaSocialHelperVerificacao.getTarifaSocialDadoEconomia().getId().equals(
														tarifaSocialDadoEconomia.getId())){
							throw new ActionServletException("atencao.imovel.numero_celpe_jacadastrado", null, imovelSessao.getId()
											.toString());
						}
					}
				}
			...
			// [FS0008] - Verificar Preenchimento dos Campos
			fachada.verificarPreenchimentoManterDadosTarifaSocialMultiplasEconomias(numeroContratoCompanhiaEletricaFormatado,
							areaConstruidaFormatada, numeroIptuFormatado, idImovelTarifaSocial, numeroCartaoProgramaSocial,
							dataValidadeCartao, numeroParcelas, consumoMedioFormatado, valorRendaFormatada, tipoCartaoProgramaSocial,
							tipoRenda);
