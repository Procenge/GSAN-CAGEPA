 ...
 * @author rodrigo
 */
public class RemoverInserirTarifaSocialColecaoAction
				extends GcomAction {
	...
	public ActionForward execute(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest httpServletRequest,
		...
		ActionForward retorno = actionMapping.findForward("inserirTarifaSocialDadosUmaEconomia");
		...
		if(remover != null && !remover.equals("")){
			...
			if(sessao.getAttribute("colecaoDadosTarifaSocial") != null){
				colecaoRemocao = (Collection) sessao.getAttribute("colecaoDadosTarifaSocial");
			}else if(sessao.getAttribute("colecaoClienteImovelEconomia") != null){
				colecaoRemocao = (Collection) sessao.getAttribute("colecaoClienteImovelEconomia");
			...
			if(colecaoRemocao.size() <= 1){
				while(itColecaoRemocao.hasNext()){
					tarifaSocialDadoEconomia = (TarifaSocialDadoEconomia) itColecaoRemocao.next();

					if(tarifaSocialDadoEconomia.getUltimaAlteracao().toString().equals(remover)){
			...
			// Para mais de uma economia
				...
				while(itColecaoRemocao.hasNext()){
					...
					clienteImovelEconomia = (ClienteImovelEconomia) itColecaoRemocao.next();
					...
					if(colecaoTarifaSocialDadoMultiplasEconomia != null && !colecaoTarifaSocialDadoMultiplasEconomia.isEmpty()){
						...
						if(tarifaSocialDadoEconomia.getUltimaAlteracao().toString().equals(remover)){
							colecaoTarifaSocialDadoMultiplasEconomia.remove(tarifaSocialDadoEconomia);
							break;
						}
					}
				}
