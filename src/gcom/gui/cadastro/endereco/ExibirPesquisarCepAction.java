 ...
 * @date 05/05/2006
 */
public class ExibirPesquisarCepAction
				extends GcomAction {
	...
	public ActionForward execute(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest httpServletRequest,
		...
		if(httpServletRequest.getParameter("voltaFiltro") == null && httpServletRequest.getParameter("pesquisaMunicipio") == null){
			pesquisarCepActionForm.setIdMunicipio("");
			pesquisarCepActionForm.setNomeMunicipio("");
			pesquisarCepActionForm.setNomeLogradouro("");
			pesquisarCepActionForm.setCodigoLado(ConstantesSistema.INVALIDO_ID.toString());
			sessao.removeAttribute("travarFaixa");
			sessao.removeAttribute("faixa");
		}

		if(httpServletRequest.getParameter("limpar") == null && httpServletRequest.getParameter("pesquisaMunicipio") == null){

			pesquisarCepActionForm.setIdMunicipio("");
			pesquisarCepActionForm.setNomeMunicipio("");
			pesquisarCepActionForm.setNomeLogradouro("");
			pesquisarCepActionForm.setFaixaInicial("");
			pesquisarCepActionForm.setCodigoLado(ConstantesSistema.INVALIDO_ID.toString());
		}
		/*
		...
		if(httpServletRequest.getParameter("idMunicipioDefinido") != null){
			idMunicipio = httpServletRequest.getParameter("idMunicipioDefinido");
		}else{

			if(pesquisarCepActionForm.getIdMunicipio() != null){

				idMunicipio = (String) pesquisarCepActionForm.getIdMunicipio();
		...
		if(idMunicipio != null && !idMunicipio.equals("")){
			...
			if(colecaoMunicipio != null && !colecaoMunicipio.isEmpty()){
				...
				pesquisarCepActionForm.setIdMunicipio(String.valueOf(municipio.getId()));
				pesquisarCepActionForm.setNomeMunicipio(municipio.getNome());
			}

		}else if(pesquisarCepActionForm.getIdMunicipio() != null && !pesquisarCepActionForm.getIdMunicipio().equals("")){
			...
			if(colecaoMunicipio != null && !colecaoMunicipio.isEmpty()){
				...
				pesquisarCepActionForm.setIdMunicipio("");
				pesquisarCepActionForm.setNomeMunicipio("MUNICÍPIO INEXISTENTE");

				httpServletRequest.setAttribute("idMunicipioNaoEncontrado", "OK");
		...
		if(httpServletRequest.getParameter("caminhoRetornoTelaInformarEndereco") != null){

			sessao.setAttribute("caminhoRetornoTelaInformarEndereco", httpServletRequest.getParameter("caminhoRetornoTelaInformarEndereco"));
		...
		if(httpServletRequest.getParameter("tipoConsulta") != null && !httpServletRequest.getParameter("tipoConsulta").equals("")){
			...
			if(httpServletRequest.getParameter("tipoConsulta").equals("municipio")){
				...
				pesquisarCepActionForm.setIdMunicipio(httpServletRequest.getParameter("idCampoEnviarDados"));

				pesquisarCepActionForm.setNomeMunicipio(httpServletRequest.getParameter("descricaoCampoEnviarDados"));
			}
		}

		if(httpServletRequest.getParameter("numeroJsp") != null && !httpServletRequest.getParameter("numeroJsp").equals("")){

			Integer numero = retornarFaixa(httpServletRequest.getParameter("numeroJsp"));

			if(numero != null){

				pesquisarCepActionForm.setFaixaInicial(numero.toString());
				httpServletRequest.setAttribute("travarFaixa", true);
				httpServletRequest.removeAttribute("numeroJsp");
				sessao.setAttribute("travarFaixa", true);
			}

		}else if(pesquisarCepActionForm.getFaixaInicial() == null){

			pesquisarCepActionForm.setFaixaInicial("");
			httpServletRequest.removeAttribute("travarFaixa");
			sessao.removeAttribute("travarFaixa");
			sessao.removeAttribute("faixa");

		}

		if(sessao.getAttribute("travarFaixa") != null){

			boolean travaFaixa = (Boolean) sessao.getAttribute("travarFaixa");

			if(travaFaixa == true){

				if(sessao.getAttribute("faixa") != null){

					String numeroFaixa = (String) sessao.getAttribute("faixa");

					pesquisarCepActionForm.setFaixaInicial(numeroFaixa);
	...
	private static Integer retornarFaixa(String faixa){
		...
		for(int i = 0; i < faixa.length(); i++){
			...
			caractere = faixa.charAt(i);
			digitoValido = false;

			if(caractere.equals('0')){
				saidaString = saidaString + caractere;
				digitoValido = true;
			}
		...
		if(saidaString != null && !saidaString.equals("")){

			saidaFaixaInt = Integer.parseInt(saidaString);
