 ...
 * @date 19/12/2007
 */
public class ExibirInformarExpurgoPagamentoAction
				extends GcomAction {
	...
	public ActionForward execute(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest httpServletRequest,
		...
		ActionForward retorno = actionMapping.findForward("informarExpurgoPagamento");
		...
		String botaoConsultar = httpServletRequest.getParameter("botaoConsultar");

		if(botaoConsultar != null){

			Object[] colecaoDadosPagamento = fachada.gerarColecaoDadosPagamentoPelaData(informarExpurgoPagamentoActionForm
							.getDataPagamento(), Util.converterStringParaInteger(informarExpurgoPagamentoActionForm.getIdCliente()));

			Collection colecaoExpurgado = new ArrayList();

			Collection colecaoNaoExpurgado = new ArrayList();

			if(colecaoDadosPagamento != null && !colecaoDadosPagamento.equals("")){
			...
			informarExpurgoPagamentoActionForm.setQuantidadePagamentosExpurgados("" + colecaoExpurgado.size());

			informarExpurgoPagamentoActionForm.setQuantidadePagamentosNaoExpurgados("" + colecaoNaoExpurgado.size());
		...
		String nomeCliente = informarExpurgoPagamentoActionForm.getNomeCliente();

		if(idCliente != null && !idCliente.equals("") && (nomeCliente == null || nomeCliente.equals(""))){
			...
			if(colecaoCliente != null && !colecaoCliente.isEmpty()){
				...
				informarExpurgoPagamentoActionForm.setNomeCliente(cliente.getNome());
			}else{
				informarExpurgoPagamentoActionForm.setNomeCliente("");
				informarExpurgoPagamentoActionForm.setNomeCliente("CLIENTE INEXISTENTE");
