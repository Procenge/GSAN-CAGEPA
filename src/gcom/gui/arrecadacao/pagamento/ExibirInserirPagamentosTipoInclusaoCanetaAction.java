 ...
 * @date 16/02/2006
 */
public class ExibirInserirPagamentosTipoInclusaoCanetaAction
				extends GcomAction {
	...
	public ActionForward execute(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest httpServletRequest,
		...
		// Caso contrário levanta uma exceção para o usuário indicando que a data de pagamento não é
		// uma data válida
		...
		// Recupera a coleção dos objetos que armazenam as informações necessárias dos códigos de
		// barra
		Collection<InserirPagamentoViaCanetaHelper> colecaoInserirPagamentoViaCanetaHelper = (Collection<InserirPagamentoViaCanetaHelper>) sessao
						.getAttribute("colecaoInserirPagamentoViaCanetaHelper");
		...
		if(colecaoInserirPagamentoViaCanetaHelper == null){
			...
			if(codigoBarraRemocao != null && !codigoBarraRemocao.trim().equalsIgnoreCase("")){
				...
				lacoWhile: while(iteratorRemocaoCodigoBarra.hasNext()){
					...
					InserirPagamentoViaCanetaHelper inserirPagamentoViaCanetaHelperParaRemocao = (InserirPagamentoViaCanetaHelper) iteratorRemocaoCodigoBarra
									.next();
		...
		// Caso o código de barras foi indicado por leitura ótica, recupera todos os campos do
		// código de barra
		// Caso contrário recupera os campos de código de barra digitado e seus dígitos
		// verificadores
		...
		codigoBarra = codigoBarraDigitadoCampo1 + codigoBarraDigitadoCampo2 + codigoBarraDigitadoCampo3 + codigoBarraDigitadoCampo4;
		...
		if(codigoBarra != null && !codigoBarra.trim().equalsIgnoreCase("") && codigoBarra.trim().length() == 44){
			...
			if(codigoBarraRemocao == null || codigoBarraRemocao.trim().equalsIgnoreCase("")){
				...
				if(codigoBarraLeituraOtica == null || codigoBarraLeituraOtica.equalsIgnoreCase("")){
					...
					this.validarDigitoVerificador(codigoBarraDigitadoCampo1, codigoBarraDigitadoDigitoVerificadorCampo1,
									codigoBarraDigitadoCampo2, codigoBarraDigitadoDigitoVerificadorCampo2, codigoBarraDigitadoCampo3,
									codigoBarraDigitadoDigitoVerificadorCampo3, codigoBarraDigitadoCampo4,
									codigoBarraDigitadoDigitoVerificadorCampo4);
				}

				// Caso o código de barra já tenha sido informado pelo usuário e esteja na lista de
				// documentos
				...
				String subStringIdPagamento = codigoBarra.substring(19, 44);
				RegistroHelperCodigoBarras registroHelperCodigoBarras = fachada.distribuirDadosCodigoBarras(codigoBarra);

				// [SB0003] - Processar pagamento com Código de Barra
				PagamentoHelperCodigoBarras pagamentoHelperCodigoBarras = fachada.processarPagamentosCodigoBarras(
								registroHelperCodigoBarras, dataPagamento, Integer.valueOf(idFormaArrecadacao), sistemaParametro,
								subStringIdPagamento, null);
				...
				if(indicadorAceitacaoRegistroMovimento == 1 && descricaoOcorrenciaMovimento.equalsIgnoreCase("OK")){
					...
					/*
					 * Colocado por Raphael Rossiter em 30/10/2007 OBJ: Gerar as devolucões
					 */
					...
					// Chama o caso de uso [UC0264] para recuperar todos os dados contidos no
					// códigode barras
					...
					valorPagamento = (Util.formatarMoedaRealparaBigDecimal(distribuirDadosCodigoBarras.getValorPagamento()))
									.divide(new BigDecimal("100.00"));
					// valorPagamento = new
					// BigDecimal(distribuirDadosCodigoBarras.getValorPagamento());

					inserirPagamentoViaCanetaHelper.setCodigoBarra(codigoBarraDigitadoCampo1 + codigoBarraDigitadoCampo2
									+ codigoBarraDigitadoCampo3 + codigoBarraDigitadoCampo4);
		...
		// Seta na sessão a coleção de documento de códigos de barra informados e a coleção de
		// pagamentos gerados
	 ...
	 * @return
	 */
	private boolean verificarExistenciaDocumentoNaLista(String codigoBarra,
	 ...
	 * @return
	 */
	private void validarDigitoVerificador(String campo1, String dvCampo1, String campo2, String dvCampo2, String campo3, String dvCampo3,
					String campo4, String dvCampo4){

		// Cria as variáveis quevai armazenar os dígitos verificadores do módulo 11 e 10
		// respectivamente
		...
		if((campo1.substring(2, 3)).equals("6")){
			...
			// ou não for igual a 8(oito), levanta uma exceção indicando que a indicação do módulo é
			// inválida
		}else{
