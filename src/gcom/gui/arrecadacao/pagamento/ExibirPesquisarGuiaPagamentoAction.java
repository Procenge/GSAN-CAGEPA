...
/**
 * Action que define o pré-processamento da página de pesquisa de guia de pagamento para cliente ou
 * imóvel
 * 
 * @author Pedro Alexandre
 * @date 07/03/2006
 */
public class ExibirPesquisarGuiaPagamentoAction
				extends GcomAction {
	...
	public ActionForward execute(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest httpServletRequest,
		...
		// PesquisarGuiaPagamentoActionForm pesquisarGuiaPagamentoActionForm =
		// (PesquisarGuiaPagamentoActionForm) actionForm;

		// Caso o código do imóvel tenha sido informado pelo caso de uso que chamou a tela de
		// pesquisar guia de pagamento,
		...
		if(idImovel != null && !idImovel.trim().equalsIgnoreCase("")){
			...
			// Caso o código do cliente tenha sido informado pelo caso de uso que chamou a tela de
			// pesquisar guia de pagamento,
			// Caso contrário levanta aexceção para o usuário indicando que o imóvel ou o cliente
		...
		if(colecaoSituacaoGuiaPagamento == null || colecaoSituacaoGuiaPagamento.isEmpty()){
			...
			// Manda a coleção de situação de guia de pagamento no request para a página de
			// pesquisar
			httpServletRequest.setAttribute("colecaoSituacaoGuiaPagamento", colecaoSituacaoGuiaPagamento);
		}

		// Cria o filtro para pesquisar ao tipos de débito sem limite de registros de guia de
		...
		if(colecaoTipoDebito == null || colecaoTipoDebito.isEmpty()){
			...
			// Manda a coleção de tipo de débito de guia de pagamento no request para a página de
			// pesquisar
			httpServletRequest.setAttribute("colecaoTipoDebito", colecaoTipoDebito);
		}

		if(httpServletRequest.getParameter("caminhoRetornoTelaPesquisaGuiaPagamento") != null){
			sessao.setAttribute("caminhoRetornoTelaPesquisaGuiaPagamento", httpServletRequest
