 ...
 * @date 13/08/2008
 *       Alteração na Consulta para Consultar a Guia e suas Prestações
 */
public class PesquisarGuiaPagamentoAction
				extends GcomAction {
	...
	public ActionForward execute(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest httpServletRequest,
		...
		String dataVencimentoGuiaPagamentoInicialEmString = (String) pesquisarGuiaPagamentoActionForm
						.getDataVencimentoGuiaPagamentoInicial();
		...
		filtroGuiaPagamentoPrestacao.adicionarCaminhoParaCarregamentoEntidade("guiaPagamento.documentoTipo");
		filtroGuiaPagamentoPrestacao.setEntidadeComChaveComposta(true);

		// Seta a ordenação do resultado da pesquisa, pelo tipo de débito
		// filtroGuiaPagamento.setCampoOrderBy(FiltroGuiaPagamento.DEBITO_TIPO_ID,
		// FiltroGuiaPagamento.NUMERO_PRESTACAO_DEBITO);
		filtroGuiaPagamentoPrestacao.setCampoOrderBy(FiltroGuiaPagamentoPrestacao.GUIA_PAGAMENTO_ID,
						FiltroGuiaPagamentoPrestacao.DEBITO_TIPO_ID, FiltroGuiaPagamentoPrestacao.NUMERO_PRESTACAO);
		...
		if(idImovel != null && !idImovel.trim().equalsIgnoreCase("")){
			// Seta o mapeamento de retorno para a tela de resultado da pesquisa de guia de
			// pagamento
			// de imóveis
			...
			// Cria o filtro de cliente e seta as entidades necessárias para o retornar na pesquisa
			// de cliente
			...
			// Seta o nome do cliente no form para exibição na página de resultado de guias de
			...
			if(colecaoEnderecoCliente != null && !colecaoEnderecoCliente.isEmpty()){
				...
				// Seta no form o endereço do cliente para exibição na página de resultado da
				// pesquisa de guias de pagamento de cliente
			...
			filtroGuiaPagamentoPrestacao.adicionarParametro(new ParametroSimples(FiltroGuiaPagamentoPrestacao.GUIA_PAGAMENTO_CLIENTE_ID,
							idCliente));
		}else{
		...
		if(dataGeracaoGuiaPagamentoInicialEmString != null
						&& !dataGeracaoGuiaPagamentoInicialEmString.toString().trim().equalsIgnoreCase("")){

			// [FS0001]converte a data inicial informada no formato string para um objeto do tipo
			// date
			try{
			...
			if(dataGeracaoGuiaPagamentoFinalEmString == null || dataGeracaoGuiaPagamentoFinalEmString.trim().equalsIgnoreCase("")){
				...
				// [FS0001] Converte a data final informada no formato string para um objeto do tipo
				// date
				try{
		...
		if(dataGeracaoGuiaPagamentoFinalEmString != null && !dataGeracaoGuiaPagamentoFinalEmString.toString().trim().equalsIgnoreCase("")){
			...
			// Seta no filtro para retornar as guias de pagamento que estiver entre a data de
			// geração inicial e final
			// filtroGuiaPagamento.adicionarParametro(new
			// Intervalo(FiltroGuiaPagamento.EMISSAO_GUIA_PAGAMENTO,
			// dataGeracaoGuiaPagamentoInicial, dataGeracaoGuiaPagamentoFinal));
			filtroGuiaPagamentoPrestacao.adicionarParametro(new Intervalo(FiltroGuiaPagamentoPrestacao.DATA_EMISSAO,
							dataGeracaoGuiaPagamentoInicial, dataGeracaoGuiaPagamentoFinal));
		}
		...
		if(dataVencimentoGuiaPagamentoInicialEmString != null
						&& !dataVencimentoGuiaPagamentoInicialEmString.toString().trim().equalsIgnoreCase("")){

			// [FS0001] Converte a data de vencimento inicial informada no formato string para um
			// objeto do tipo date
			try{
			...
			if(dataVencimentoGuiaPagamentoFinalEmString == null || dataVencimentoGuiaPagamentoFinalEmString.trim().equalsIgnoreCase("")){
				...
				// [FS0001] Converte a data de vencimento final informada no formato string para um
				// objeto do tipo date
				try{
		...
		if(dataVencimentoGuiaPagamentoFinalEmString != null
						&& !dataVencimentoGuiaPagamentoFinalEmString.toString().trim().equalsIgnoreCase("")){
			// Indica que o usuário informou um parâmetro para pesquisar
			peloMenosUmParametroInformado = true;
			// filtroGuiaPagamento.adicionarParametro(new
			// Intervalo(FiltroGuiaPagamento.DATA_VENCIMENTO, dataVencimentoGuiaPagamentoInicial,
			// dataVencimentoGuiaPagamentoFinal));
			filtroGuiaPagamentoPrestacao.adicionarParametro(new Intervalo(FiltroGuiaPagamentoPrestacao.DATA_VENCIMENTO,
							dataVencimentoGuiaPagamentoInicial, dataVencimentoGuiaPagamentoFinal));
		}

		// Caso o usuário indicar os tipos de débitos para pesquisar as guias
		if(idTipoDebito != null && !idTipoDebito[0].equals("" + ConstantesSistema.NUMERO_NAO_INFORMADO) && idTipoDebito.length > 0){
			// Indica que o usuário informou um parâmetro para pesquisar
			peloMenosUmParametroInformado = true;
			...
			for(int i = 0; i < idTipoDebito.length; i++){
				if(!(new Integer(idTipoDebito[i]).equals(new Integer(ConstantesSistema.NUMERO_NAO_INFORMADO)))){
					...
					if(idTipoDebito.length == 1){
						// filtroGuiaPagamento.adicionarParametro(new
						// ParametroSimples(FiltroGuiaPagamento.DEBITO_TIPO_ID,idTipoDebito[i]));
						filtroGuiaPagamentoPrestacao.adicionarParametro(new ParametroSimples(FiltroGuiaPagamentoPrestacao.DEBITO_TIPO_ID,
										idTipoDebito[i]));
					}else{
						if(i == 0){
							// filtroGuiaPagamento.adicionarParametro(new
							// ParametroSimples(FiltroGuiaPagamento.DEBITO_TIPO_ID,idTipoDebito[i],
							// ParametroSimples.CONECTOR_OR,idTipoDebito.length ));
							filtroGuiaPagamentoPrestacao.adicionarParametro(new ParametroSimples(
											FiltroGuiaPagamentoPrestacao.DEBITO_TIPO_ID, idTipoDebito[i], ParametroSimples.CONECTOR_OR,
											idTipoDebito.length));
						}else{
							if(i == (idTipoDebito.length - 1)){
								...
								// ParametroSimples(FiltroGuiaPagamento.DEBITO_TIPO_ID,idTipoDebito[i]));
								filtroGuiaPagamentoPrestacao.adicionarParametro(new ParametroSimples(
												FiltroGuiaPagamentoPrestacao.DEBITO_TIPO_ID, idTipoDebito[i]));
							}else{
								// filtroGuiaPagamento.adicionarParametro(new
								// ParametroSimples(FiltroGuiaPagamento.DEBITO_TIPO_ID,idTipoDebito[i],
								// ParametroSimples.CONECTOR_OR));
								filtroGuiaPagamentoPrestacao
												.adicionarParametro(new ParametroSimples(FiltroGuiaPagamentoPrestacao.DEBITO_TIPO_ID,
																idTipoDebito[i], ParametroSimples.CONECTOR_OR));
							}
						}
					}
				}
		...
		if(idSituacaoGuiaPagamento != null && !idSituacaoGuiaPagamento[0].equals("" + ConstantesSistema.NUMERO_NAO_INFORMADO)
			...
			for(int i = 0; i < idSituacaoGuiaPagamento.length; i++){
				if(!(new Integer(idSituacaoGuiaPagamento[i]).equals(new Integer(ConstantesSistema.NUMERO_NAO_INFORMADO)))){
					...
					if(idSituacaoGuiaPagamento.length == 1){
						filtroGuiaPagamentoPrestacao.adicionarParametro(new ParametroSimples(
										FiltroGuiaPagamentoPrestacao.DEBITO_CREDITO_SITUACAO_ATUAL_ID, idSituacaoGuiaPagamento[i]));
					}else{
						if(i == 0){
							filtroGuiaPagamentoPrestacao.adicionarParametro(new ParametroSimples(
											FiltroGuiaPagamentoPrestacao.DEBITO_CREDITO_SITUACAO_ATUAL_ID, idSituacaoGuiaPagamento[i],
											ParametroSimples.CONECTOR_OR, idSituacaoGuiaPagamento.length));
						}else{
		...
		// Realiza um tratamento nas Prestações Obtidas, para exibição por Prestação (Considerando
		// os tipos de débitos para cada uma)
		Map<String, GuiaPagamentoPrestacaoHelper> mapGuiaPagamento = new TreeMap();
		Collection<GuiaPagamentoPrestacao> colecaoGuiasPagamento = fachada.pesquisar(filtroGuiaPagamentoPrestacao,
		...
		for(Iterator iterator = colecaoGuiasPagamento.iterator(); iterator.hasNext();){
			...
			GuiaPagamentoPrestacaoHelper prestacaoHelper = new GuiaPagamentoPrestacaoHelper();
			prestacaoHelper.setId(Long.valueOf(guiaPrestacao.getComp_id().getGuiaPagamentoId().hashCode()
							+ guiaPrestacao.getComp_id().getNumeroPrestacao().hashCode()));

			if(!mapGuiaPagamento.containsKey(prestacaoHelper.getId().toString())){ // Nova Guia e
				// Prestação
				prestacaoHelper.setIdGuiaPagamento(guiaPrestacao.getComp_id().getGuiaPagamentoId());
				prestacaoHelper.setIdDocumentoTipo(DocumentoTipo.GUIA_PAGAMENTO);
				if(guiaPrestacao.getGuiaPagamento().getDocumentoTipo() != null){
					prestacaoHelper.setDescricaoDocumentoTipo(guiaPrestacao.getGuiaPagamento().getDocumentoTipo()
									.getDescricaoDocumentoTipo());
				}else{
					prestacaoHelper.setDescricaoDocumentoTipo("DOCUMENTO NÂO IDENTIFICADO");
				}
				prestacaoHelper.setDataEmissao(guiaPrestacao.getDataEmissao());
				prestacaoHelper.setDataVencimento(guiaPrestacao.getDataVencimento());
				prestacaoHelper.setNumeroPrestacao(guiaPrestacao.getComp_id().getNumeroPrestacao());
				prestacaoHelper.setNumeroPrestacaoTotal(guiaPrestacao.getGuiaPagamento().getNumeroPrestacaoTotal());
				prestacaoHelper.setValorTotalPorPrestacao(guiaPrestacao.getValorPrestacao());
				prestacaoHelper.setDescricaoDebitoCreditoSituacao(guiaPrestacao.getDebitoCreditoSituacao()
								.getDescricaoDebitoCreditoSituacao());

				mapGuiaPagamento.put(prestacaoHelper.getId().toString(), prestacaoHelper);
			}else{ // Mesma Guia e Prestação - acumular valor
				GuiaPagamentoPrestacaoHelper prestacaoHelperMap = (GuiaPagamentoPrestacaoHelper) mapGuiaPagamento.get(prestacaoHelper
								.getId().toString());

				prestacaoHelperMap.setValorTotalPorPrestacao(prestacaoHelperMap.getValorTotalPorPrestacao().add(
								guiaPrestacao.getValorPrestacao()));
		...
		if(mapGuiaPagamento.values() == null || mapGuiaPagamento.values().isEmpty()){
			...
			sessao.setAttribute("colecaoGuiasPagamentoPrestacao", mapGuiaPagamento.values());
		}
