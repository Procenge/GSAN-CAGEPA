 ...
 * @date 15/03/2006
 */
public class InserirPagamentosAction
				extends GcomAction {
	...
	public ActionForward execute(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest httpServletRequest,
		...
		Collection<InserirPagamentoManualHelper> colecaoPagamentoInserirHelper = null;
		...
		// [SB0002] - Inserir Pagamento Manual
		if(tipoInclusao.equalsIgnoreCase("manual")){
			 ...
			 */
			colecaoPagamentoInserirHelper = (Collection) sessao
							.getAttribute("colecaoPagamentoInserirHelper");
			if(!Util.isVazioOrNulo(colecaoPagamentoInserirHelper)){

				for(InserirPagamentoManualHelper inserirPagamentoManualHelper : colecaoPagamentoInserirHelper){

					colecaoPagamentos.add(inserirPagamentoManualHelper.getPagamento());
				}
			}

			try{

				idPagamento = fachada.inserirPagamentos(colecaoPagamentos, usuarioLogado, avisoBancario, usuarioConfirmou);

			}catch(InserirPagamentoException e){

				httpServletRequest.setAttribute("caminhoActionConclusao", "/gsan/inserirPagamentosTipoInclusaoManualAction.do");
				retorno = montarPaginaConfirmacaoWizard(e.getMessage(), httpServletRequest, actionMapping,
								new String[] {e.getValorPagamento(), e.getDescricaoPagamentoSituacao()});

				return retorno;
			}

			// Caso o tipo de inclusão seja por caneta óptica
		}else if(tipoInclusao.equalsIgnoreCase("caneta")){
			...
			// Recupera a coleção de documentos da sessão contendo o código de barras e o valor do
			// pagamento
			...
			try{
				...
				idPagamento = fachada.inserirPagamentosCodigoBarras(colecaoPagamentos, colecaoDevolucoes, usuarioLogado, avisoBancario,
								usuarioConfirmou);
			}catch(InserirPagamentoException e){

				httpServletRequest.setAttribute("caminhoActionConclusao", "/gsan/inserirPagamentosTipoInclusaoCanetaAction.do");
				retorno = montarPaginaConfirmacaoWizard(e.getMessage(), httpServletRequest, actionMapping,
								new String[] {e.getDescricaoPagamentoSituacao()});

				return retorno;
			}

		}else{
			// //Caso o tipo de inclusão não tenha sido informado, levanta uma exceção para o
			// usuário
		...
		if(retorno.getName().equalsIgnoreCase("telaSucesso")){
			...
			if(Util.isNaoNuloBrancoZero(avisoBancario) && Util.isNaoNuloBrancoZero(avisoBancario.getArrecadador())){
				if(avisoBancario.getArrecadador().getIndicadorCaixaEmpresa().equals(ConstantesSistema.SIM)){
					...
					sessao.setAttribute("colecaoPagamento", colecaoPagamentos);

					montarPaginaSucessoAutenticacao(httpServletRequest, "Pagamento inserido com sucesso.", "Inserir outro Pagamento",
									"exibirInserirPagamentosAction.do?menu=sim",
									"exibirAtualizarPagamentosAction.do?idRegistroInseridoAtualizar=" + idPagamento,
									"Atualizar Pagamento Inserido", "exibirAutenticarPagamentoAction.do?colecaoPagamentos="
													+ colecaoPagamentos, "Autenticar Pagamentos Inseridos.");
				}else{
					montarPaginaSucesso(httpServletRequest, "Pagamento inserido com sucesso.", "Inserir outro Pagamento",
									"exibirInserirPagamentosAction.do?menu=sim",
									"exibirAtualizarPagamentosAction.do?idRegistroInseridoAtualizar=" + idPagamento,
