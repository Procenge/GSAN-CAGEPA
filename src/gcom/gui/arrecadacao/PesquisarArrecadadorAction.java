 ...
 */
public class PesquisarArrecadadorAction
				extends GcomAction {
	...
	public ActionForward execute(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest httpServletRequest,
		...
		String inscricaoEstadual = (String) pesquisarArrecadadorActionForm.getInscricaoEstadual();
		String idLocalidade = (String) pesquisarArrecadadorActionForm.getIdLocalidade();
		String idCliente = (String) pesquisarArrecadadorActionForm.getIdCliente();
		String idImovel = (String) pesquisarArrecadadorActionForm.getIdImovel();
		Short indicadorCaixaEmpresa = pesquisarArrecadadorActionForm.getIndicadorCaixaEmpresa();
		...
		if(inscricaoEstadual != null && !inscricaoEstadual.trim().equalsIgnoreCase("")){
			...
			filtroArrecadador.adicionarParametro(new ComparacaoTexto(FiltroArrecadador.INSCRICAO_ESTATAL, inscricaoEstadual));
		}

		if(idLocalidade != null && !idLocalidade.trim().equalsIgnoreCase("")){
			peloMenosUmParametroInformado = true;
			filtroArrecadador.adicionarParametro(new ParametroSimples(FiltroArrecadador.LOCALIDADE_ID, idLocalidade));
		}
		...
		if(indicadorCaixaEmpresa != null && !indicadorCaixaEmpresa.equals(ConstantesSistema.ZERO)){
			peloMenosUmParametroInformado = true;
			filtroArrecadador.adicionarParametro(new ParametroSimples(FiltroArrecadador.INDICADOR_CAIXA_EMPRESA, indicadorCaixaEmpresa));
		}
		...
		filtroArrecadador.adicionarCaminhoParaCarregamentoEntidade("imovel.localidade");

		filtroArrecadador.adicionarCaminhoParaCarregamentoEntidade("imovel");

		filtroArrecadador.adicionarCaminhoParaCarregamentoEntidade("cliente");

		if(Util.isNaoNuloBrancoZero(sessao.getAttribute("tipo"))){
		...
		if(colecaoArrecadadores == null || colecaoArrecadadores.isEmpty()){
			...
			throw new ActionServletException("atencao.pesquisa.nenhumresultado", null, "Arrecadador");
