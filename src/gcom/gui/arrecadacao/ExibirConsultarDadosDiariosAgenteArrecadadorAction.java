 ...
 **/
public class ExibirConsultarDadosDiariosAgenteArrecadadorAction
				extends GcomAction {
	...
	public ActionForward execute(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest httpServletRequest,
		...
		ActionForward retorno = actionMapping.findForward("exibirConsultarDadosDiariosAgente");
		...
		// colecaoArrecadacaoDadosDiariosArrecadador =
		...
		for(Object object : colecaoArrecadacaoDadosDiariosArrecadador){
			...
			BigDecimal valorDespesaBancaria = calcularValorDespesaBancaria(arrecadacaoDadosDiariosHelper, fachada, actionForm);

			arrecadacaoDadosDiariosHelper.setValorDespesaBancaria(valorDespesaBancaria);
	...
	private BigDecimal calcularValorDespesaBancaria(ArrecadacaoDadosDiariosHelper arrecadacaoDadosDiariosHelper, Fachada fachada,
					ActionForm actionForm){

		BigDecimal valorDespesaBancaria = BigDecimal.ZERO;
		...
		List<ArrecadadorContratoTarifaHelper> listaArrecadadorContratoTarifa = fachada
						.pesquisarArrecadadorContratoTarifaPorArrecadacaoDadosDiarios(arrecadacaoDadosDiariosHelper.getArrecadador()
										.getId(), arrecadacaoDadosDiariosHelper.getAnoMesReferenciaArrecadacao(), localidade, idElo,
										setorComercial, idGerenciaRegional, unidadeNegocioId, idsImovelPerfil, idsLigacaoAgua,
										idsLigacaoEsgoto, idsCategoria, idsEsferaPoder, idsDocumentosTipos);

		if(!Util.isVazioOrNulo(listaArrecadadorContratoTarifa)){
			for(ArrecadadorContratoTarifaHelper arrecadadorContratoTarifaHelper : listaArrecadadorContratoTarifa){

				if(arrecadadorContratoTarifaHelper.getValorTarifa() != null
								&& !arrecadadorContratoTarifaHelper.getValorTarifa().equals(BigDecimal.ZERO)){

					valorDespesaBancaria = valorDespesaBancaria.add(arrecadadorContratoTarifaHelper.getValorTarifa().multiply(
									new BigDecimal(arrecadadorContratoTarifaHelper.getQuantidadePagamentos())));

					// calculoDespesaBancaria = arrecadadorContratoTarifa.getValorTarifa().multiply(
					// new BigDecimal(arrecadacaoDadosDiariosHelper.getQuantidadePagamentos()));
					// valorDespesaBancaria = valorDespesaBancaria.add(calculoDespesaBancaria);

				}else if(arrecadadorContratoTarifaHelper.getPercentualTarifa() != null
								&& !arrecadadorContratoTarifaHelper.getPercentualTarifa().equals(BigDecimal.ZERO)){

					valorDespesaBancaria = valorDespesaBancaria.add((arrecadadorContratoTarifaHelper.getValorPagamentos()
									.multiply(arrecadadorContratoTarifaHelper.getPercentualTarifa())).divide(new BigDecimal(100)));

					// calculoDespesaBancaria =
					// arrecadacaoDadosDiariosHelper.getValorPagamentos().multiply(
					// arrecadadorContratoTarifa.getPercentualTarifa());
					// valorDespesaBancaria = valorDespesaBancaria.divide(new BigDecimal(100));
	...
	private ArrecadacaoDadosDiariosHelper converter(ArrecadacaoDadosDiarios arrecadacaoDadosDiarios){
		...
		ArrecadacaoDadosDiariosHelper arrecadacaoDadosDiariosHelper = new ArrecadacaoDadosDiariosHelper(arrecadacaoDadosDiarios.getId(),
						...
						arrecadacaoDadosDiarios.getUnidadeNegocio(), null);
