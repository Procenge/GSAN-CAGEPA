 ...
 *         21/03/2014
 */
public class ExibirManterBancoAction
				extends GcomAction {
	...
	public ActionForward execute(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest httpServletRequest,
		...
		FiltroBanco filtro = (FiltroBanco) sessao.getAttribute("filtroBanco");

		Map resultado = controlarPaginacao(httpServletRequest, retorno, filtro, Banco.class.getName());
		Collection colecaoBanco = (Collection) resultado.get("colecaoRetorno");
		retorno = (ActionForward) resultado.get("destinoActionForward");

		if(colecaoBanco != null && !colecaoBanco.isEmpty()){

			if(colecaoBanco.size() == 1
				 ...
				 * ExibirAtualizarBancoAction e em caso negativo manda
				 * a coleção pelo request.
				 */

				if(httpServletRequest.getParameter("checkAtualizar") != null){
					retorno = actionMapping.findForward("atualizarBanco");
					Banco banco = (Banco) colecaoBanco.iterator().next();
					sessao.setAttribute("objetoBanco", banco);
