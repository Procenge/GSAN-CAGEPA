...
public class ExibirProcessoDoisInserirAvisoBancarioAction
				extends GcomAction {
	...
	public ActionForward execute(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest httpServletRequest,
		...
		ActionForward retorno = actionMapping.findForward("exibirInserirAvisoBancarioProcessoDois");
		...
		if((sessao.getAttribute("arrecadador") != null && !arrecadadorVolta.getId().toString().equalsIgnoreCase(idArrecadador))
						|| (data != null && !data.equalsIgnoreCase("") && !data.equalsIgnoreCase(dataLancamento))){
			form.setTipoAviso("");
		...
		if(idArrecadador != null && !idArrecadador.trim().equals("")){
			...
			collectionArrecadador = fachada.pesquisar(filtroArrecadador, Arrecadador.class.getName());

			if(!collectionArrecadador.isEmpty()){
				Iterator iterator = collectionArrecadador.iterator();
				if(iterator.hasNext()) arrecadador = (Arrecadador) iterator.next();
				...
				throw new ActionServletException("atencao.arrecadador.codigo.inexistente", null, idArrecadador);
			}
		}
		...
		if(dataLancamento != null && !dataLancamento.equals("")){
			...
			if(dataLancamentoFormatada.after(dataAtual)){
				...
				throw new ActionServletException("atencao.data.lancamento.posterior", null, dataCorrente);

			}else{
				filtroAvisoBancario.adicionarParametro(new ComparacaoTexto(FiltroAvisoBancario.DATA_LANCAMENTO, dataLancamento));
			}

		}

		// Pesquisa de aviso bancario
		Collection<AvisoBancario> collectionAvisoBancario = fachada.pesquisar(filtroAvisoBancario, AvisoBancario.class.getName());
		...
		if(collectionAvisoBancario == null || collectionAvisoBancario.isEmpty()){
			...
			if(numeroPagina.equals("3")){
				retorno = actionMapping.findForward("exibirInserirAvisoBancarioProcessoDoisParaUm");

			}else{

				retorno = actionMapping.findForward("exibirInserirAvisoBancarioProcessoDoisParaTres");
				Short numeroSequencial = fachada.pesquisarValorMaximoNumeroSequencial(dataLancamentoFormatada, "" + arrecadador.getId());
			...
			filtroAvisoBancario.adicionarParametro(new ParametroNulo(FiltroAvisoBancario.DATA_REALIZADA));
			// Inicializacao de filtrode aviso bancario

			// Pesquisa de aviso bancario
			filtroAvisoBancario.setCampoOrderBy(FiltroAvisoBancario.NUMERO_SEQUENCIAL);
			collectionAvisoBancario = fachada.pesquisar(filtroAvisoBancario, AvisoBancario.class.getName());
			// Pesquisa de aviso bancario

			// Validacao de collection nao nula e nao vazia
			if(collectionAvisoBancario != null && !collectionAvisoBancario.isEmpty()){
				httpServletRequest.setAttribute("collectionAvisoBancario", collectionAvisoBancario);
