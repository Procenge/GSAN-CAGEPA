...
public class InserirAgenciaBancariaAction
				extends GcomAction {
	...
	public ActionForward execute(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest httpServletRequest,
		...
		if(colecaoEnderecos != null && !colecaoEnderecos.isEmpty()){
			Agencia agenciaEndereco = (Agencia) Util.retonarObjetoDeColecao(colecaoEnderecos);
		...
		if(Util.validarNumeroMaiorQueZERO(inserirAgenciaBancariaActionForm.getBancoID())){
			// Constrói o filtro para pesquisa do serviço tipo referência
			FiltroBanco filtroBanco = new FiltroBanco();
			filtroBanco.adicionarParametro(new ParametroSimples(FiltroBanco.ID, inserirAgenciaBancariaActionForm.getBancoID()));

			Collection colecaoBancos = (Collection) fachada.pesquisar(filtroBanco, Banco.class.getName());
		...
		if(ramal != null && !ramal.equalsIgnoreCase("")){
			...
			if(telefone == null || telefone.equalsIgnoreCase("")){
				throw new ActionServletException("atencao.telefone_agencia_bancaria_nao_informado");
		...
		if(bancoID != null && !bancoID.equalsIgnoreCase(String.valueOf(ConstantesSistema.NUMERO_NAO_INFORMADO))){
			...
			colecaoPesquisa = fachada.pesquisar(filtroBanco, Banco.class.getName());
		...
		filtroAgencia.adicionarParametro(new ParametroSimples(FiltroAgencia.CODIGO_AGENCIA, agenciaInserir.getCodigoAgencia()));
		if(agenciaInserir.getBanco() != null && agenciaInserir.getBanco().getId() != null){
			filtroAgencia.adicionarParametro(new ParametroSimples(FiltroAgencia.BANCO_ID, agenciaInserir.getBanco().getId()));
		}

		// Verificar existência da Agência Bancária
		colecaoPesquisa = fachada.pesquisar(filtroAgencia, Agencia.class.getName());

		if(colecaoPesquisa != null && !colecaoPesquisa.isEmpty()){
			// Gerencia Regional já existe
			throw new ActionServletException("atencao.pesquisa_agencia_ja_cadastrada", null, codigo);
		}else{
			Integer idAgencia = null;

			idAgencia = fachada.inserirAgenciaBancaria(agenciaInserir);

			montarPaginaSucesso(httpServletRequest, "Agência Bancaria de código  " + agenciaInserir.getCodigoAgencia()
							+ " inserida com sucesso.", "Inserir outra Agência Bancaria", "exibirInserirAgenciaBancariaAction.do?menu=sim",
							"exibirAtualizarAgenciaBancariaAction.do?inserir=sim&idRegistroAtualizacao=" + idAgencia,
							"Atualizar Agencia Bancaria Inserida");
