 ...
 * @date 05/08/2006
 */
public class FiltrarAgenciaBancariaAction
				extends GcomAction {

	/**
	 * Este caso de uso permite Pesquisar uma agência bancária
	 * [UC0517] Filtrar Agência Bancária
	 * 
	 * @author Thiago Tenório
	 * @date 31/07/2006
	 * @param actionMapping
	...
	public ActionForward execute(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest httpServletRequest,
		...
		ActionForward retorno = actionMapping.findForward("exibirManterAgenciaBancaria");
		...
		if(codigo != null && !codigo.trim().equalsIgnoreCase("")){
			...
			filtroAgencia.adicionarParametro(new ComparacaoTexto(FiltroAgencia.CODIGO_AGENCIA, codigo));
		...
		if(nome != null && !nome.trim().equalsIgnoreCase("")){
			...
			if(tipoPesquisa != null && tipoPesquisa.equals(ConstantesSistema.TIPO_PESQUISA_COMPLETA.toString())){
				filtroAgencia.adicionarParametro(new ComparacaoTextoCompleto(FiltroAgencia.NOME_AGENCIA, nome));
			}else{
				filtroAgencia.adicionarParametro(new ComparacaoTexto(FiltroAgencia.NOME_AGENCIA, nome));
			}
		...
		if(bancoID != null && !bancoID.trim().equalsIgnoreCase("" + ConstantesSistema.NUMERO_NAO_INFORMADO)){
		...
		if(!peloMenosUmParametroInformado){
			...
			filtroAgencia.setCampoOrderBy(FiltroAgencia.CODIGO_AGENCIA);
		}
