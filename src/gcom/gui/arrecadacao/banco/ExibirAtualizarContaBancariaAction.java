 ...
 * @date 30/10/2006
 */
public class ExibirAtualizarContaBancariaAction
				extends GcomAction {
	...
	public ActionForward execute(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest httpServletRequest,
		...
		ActionForward retorno = actionMapping.findForward("atualizarContaBancaria");
		...
		if(sessao.getAttribute("contaBancariaAtualizar") == null){
			...
			if(sessao.getAttribute("contaBancaria") != null){
				...
				contaBancaria = (ContaBancaria) sessao.getAttribute("contaBancaria");

				sessao.setAttribute("idContaBancaria", contaBancaria.getId().toString());
				...
				if(httpServletRequest.getParameter("idRegistroAtualizacao") == null
								|| httpServletRequest.getParameter("idRegistroAtualizacao").equals("")){
					contaBancaria = (ContaBancaria) sessao.getAttribute("objetoContaBancaria");
				}else{
					idContaBancaria = (String) httpServletRequest.getParameter("idRegistroAtualizacao");
					sessao.setAttribute("idRegistroAtualizacao", idContaBancaria);
				}
				...
				if(idContaBancaria != null){
					...
					Collection colecaoContaBancaria = fachada.pesquisar(filtroContaBancaria, ContaBancaria.class.getName());
					...
					contaBancaria = (ContaBancaria) colecaoContaBancaria.iterator().next();

				}

				if(idContaBancaria == null){
					if(sessao.getAttribute("idRegistroAtualizacao") != null){
						idContaBancaria = (String) sessao.getAttribute("idRegistroAtualizacao");
					}else{
						contaBancaria = (ContaBancaria) sessao.getAttribute("contaBancaria");
				...
				Collection colecaoContaBancaria = (Collection) fachada.pesquisar(filtroContaBancaria, ContaBancaria.class.getName());

				contaBancaria = (ContaBancaria) colecaoContaBancaria.iterator().next();

			}

			atualizarContaBancariaActionForm.setContaBanco(contaBancaria.getNumeroConta());

			if(contaBancaria.getNumeroContaContabil() != null){
				atualizarContaBancariaActionForm.setContaContabil(contaBancaria.getNumeroContaContabil().toString());
			}else{
				atualizarContaBancariaActionForm.setContaContabil("");
			}

			atualizarContaBancariaActionForm.setBanco(contaBancaria.getAgencia().getBanco().getId().toString());

			atualizarContaBancariaActionForm.setAgenciaBancaria(contaBancaria.getAgencia().getId().toString());
		...
		if(httpServletRequest.getParameter("desfazer") != null && httpServletRequest.getParameter("desfazer").equalsIgnoreCase("S")){
			...
			if(sessao.getAttribute("idRegistroAtualizacao") != null && !sessao.getAttribute("idRegistroAtualizacao").equals("")){
				contaBancariaID = (String) sessao.getAttribute("idRegistroAtualizacao");
			}

			if((contaBancariaID == null) && (id == null)){

				ContaBancaria contaBancaria = (ContaBancaria) sessao.getAttribute("contaBancaria");
				...
				if(contaBancaria != null){
					...
					if(contaBancaria.getAgencia() != null && contaBancaria.getAgencia().getBanco() != null
									&& contaBancaria.getAgencia().getBanco().getId() != null){
						atualizarContaBancariaActionForm.setBanco(contaBancaria.getAgencia().getBanco().getId().toString());
					}
			...
			if(idContaBancaria != null){
				...
				ContaBancaria contaBancaria = (ContaBancaria) colecaoContaBancaria.iterator().next();
		...
		httpServletRequest.setAttribute("colecaoContaBancariaTela", sessao.getAttribute("colecaoContaBancariaTipoValorTela"));
