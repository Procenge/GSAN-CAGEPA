...
public class ExibirProcessoUmInserirAvisoBancarioAction
				extends GcomAction {
	...
	public ActionForward execute(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest httpServletRequest,
		...
		// Vem do popup pesquisar Arrecadador quando o inserir aviso Bancário tiver sido chamado do
		// inserir pagamento.
		if(httpServletRequest.getParameter("descricaoCampoEnviarDados2") != null){
			httpServletRequest.setAttribute("montarPopUp", httpServletRequest.getParameter("descricaoCampoEnviarDados3"));
		}
		// --------------------------------------------------------------

		ActionForward retorno = actionMapping.findForward("exibirInserirAvisoBancarioProcessoUm");
		...
		if(Util.isVazioOuBranco(idArrecadador)){
			if(httpServletRequest.getParameter("idCampoEnviarDados") != null){
				idArrecadador = httpServletRequest.getParameter("idCampoEnviarDados");
			}
		}
		...
		if((sessao.getAttribute("arrecadador") != null && !arrecadadorVolta.getId().toString().equalsIgnoreCase(idArrecadador))
						|| (sessao.getAttribute("data") != null && !data.equalsIgnoreCase("") && !data.equalsIgnoreCase(dataLancamento))){
			form.setTipoAviso("");
		...
		if(idArrecadador != null && !idArrecadador.trim().equals("")){
			...
			if(!collectionArrecadador.isEmpty()){
				...
				if(arrecadador.getCliente() != null){
					form.setCodigoArrecadador(idArrecadador);
