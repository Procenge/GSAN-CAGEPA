 ...
 * @date 10/03/2006
 */
public class AtualizarAvisoBancarioAction
				extends GcomAction {
	...
	public ActionForward execute(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest request,
		...
		// ------------ REGISTRAR TRANSAÇÃO ----------------
		RegistradorOperacao registradorOperacao = new RegistradorOperacao(Operacao.OPERACAO_AVISO_BANCARIO_ATUALIZAR,
		...
		if(form.getNumeroDocumento() != null && !"".equals(form.getNumeroDocumento().trim())) avisoBancario.setNumeroDocumento(Integer
						.parseInt(form.getNumeroDocumento()));
		if(form.getDataRealizacao() != null && !"".equals(form.getDataRealizacao().trim())){
			...
			Date dataLancamento = (Date) sessao.getAttribute("dataLancamento");
			...
			if(data.getTime() > new Date(System.currentTimeMillis()).getTime()){
				throw new ActionServletException("atencao.avisoBancario.data_realizacao_menor_que_hoje", null, Util.formatarData(new Date(
								System.currentTimeMillis())));
			}
			if(data.getTime() < dataLancamento.getTime()){
				throw new ActionServletException("atencao.avisoBancario.data_realizacao_menor_que_data_lancamento",
								Util.formatarData(data), Util.formatarData(dataLancamento));
			}
			avisoBancario.setDataRealizada(Util.converteStringParaDate(form.getDataRealizacao()));
		}
		if(form.getValorArrecadacao() != null && !"".equals(form.getValorArrecadacao().trim())) avisoBancario
						.setValorArrecadacaoInformado(Util.formatarMoedaRealparaBigDecimal(form.getValorArrecadacao().trim()));
		if(form.getValorDevolucao() != null && !"".equals(form.getValorDevolucao().trim())) avisoBancario.setValorDevolucaoInformado(Util
						.formatarMoedaRealparaBigDecimal(form.getValorDevolucao().trim()));
		...
		if(collAvisoDeducoes != null && !collAvisoDeducoes.isEmpty()){
			...
			while(it.hasNext()){
				...
				if(request.getParameter("posicaoAvisoDeducao_" + i) != null){
					avisoDeducoes.setValorDeducao(Util.formatarMoedaRealparaBigDecimal(request.getParameter("posicaoAvisoDeducao_" + i)
									.toString().trim()));
					somatorioValorDeducao = somatorioValorDeducao.add((Util.formatarMoedaRealparaBigDecimal(request.getParameter(
									"posicaoAvisoDeducao_" + i).toString().trim())));
				}
				i++;
			}
		}
		...
		if(collAvisoAcerto != null && !collAvisoAcerto.isEmpty()){
			...
			while(it.hasNext()){
				...
				if(request.getParameter("posicaoAvisoAcerto_" + i) != null){
					avisoAcerto.setValorAcerto(Util.formatarMoedaRealparaBigDecimal(request.getParameter("posicaoAvisoAcerto_" + i)
									.toString().trim()));
				}
				i++;
			}
		...
		if(form.getTipoAviso().equals(AvisoBancario.INDICADOR_CREDITO.toString())){

			if(!Util.isVazioOuBranco(form.getValorArrecadacao())){

				valorRealizado = Util.formatarMoedaRealparaBigDecimal(form.getValorArrecadacao()).add(
								Util.formatarMoedaRealparaBigDecimal(form.getValorAcertosArrecadacao())).subtract(somatorioValorDeducao);
			}
		}else{
			...
			if(!Util.isVazioOuBranco(form.getValorArrecadacao())){
				...
				valorRealizado = Util.formatarMoedaRealparaBigDecimal(form.getValorArrecadacao()).add(
								Util.formatarMoedaRealparaBigDecimal(form.getValorAcertosDevolucao())).add(somatorioValorDeducao);
			}
		}

		avisoBancario.setValorRealizado(valorRealizado);

		Fachada.getInstancia().atualizarAvisoBancario(avisoBancario, collAvisoDeducoes, collAvisoDeducoesRemover, collAvisoAcerto,
						collAvisoAcertoRemover, this.getUsuarioLogado(request));
		...
		request.setAttribute("mensagemPaginaSucesso", "Aviso Bancário de código " + avisoBancario.getId() + " com Data de Lançamento "
						+ Util.formatarData(avisoBancario.getDataLancamento()) + " e seqüencial " + avisoBancario.getNumeroSequencial()
						+ " do arrecadador " + avisoBancario.getArrecadador().getCliente().getNome() + " atualizado com sucesso. ");
