...
public class ExibirAtualizarArrecadadorAction
				extends GcomAction {

	public ActionForward execute(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest httpServletRequest,
					HttpServletResponse httpServletResponse){

		ActionForward retorno = actionMapping.findForward("atualizarArrecadador");
		...
		String idArrecadador = httpServletRequest.getParameter("idRegistroAtualizacao");

		if(idArrecadador == null){
			if(httpServletRequest.getAttribute("idRegistroAtualizacao") == null){
				idArrecadador = (String) sessao.getAttribute("idRegistroAtualizacao");
			}else{
				idArrecadador = (String) httpServletRequest.getAttribute("idRegistroAtualizacao").toString();
			}
		...
		if(objetoConsulta != null && !objetoConsulta.equals("")){
			...
			// Validamos o cliente
			if(atualizarArrecadadorActionForm.getIdCliente() != null && !atualizarArrecadadorActionForm.getIdCliente().trim().equals("")){
				FiltroCliente filtro = new FiltroCliente();

				filtro.adicionarParametro(new ParametroSimples(FiltroCliente.ID, atualizarArrecadadorActionForm.getIdCliente()));
				...
				if(colCliente == null || !colCliente.iterator().hasNext()){
					...
					atualizarArrecadadorActionForm.setNomeCliente("Cliente inexistente");
					httpServletRequest.setAttribute("existeCliente", "exception");

				}else{
					Cliente cliente = (Cliente) Util.retonarObjetoDeColecao(colCliente);
					atualizarArrecadadorActionForm.setIdCliente(cliente.getId().toString());

					atualizarArrecadadorActionForm.setNomeCliente(cliente.getNome());
				}
			}

			if(atualizarArrecadadorActionForm.getIdImovel() != null && !atualizarArrecadadorActionForm.getIdImovel().trim().equals("")){
				// Validamos o imovel
				...
				if(colImovel == null || colImovel.isEmpty()){
					...
					atualizarArrecadadorActionForm.setInscricaoImovel("Imóvel inexistente");
					...
					inscricaoImovel = fachada.pesquisarInscricaoImovel(imovel.getId(), true);

					atualizarArrecadadorActionForm.setIdImovel(imovel.getId().toString());

					atualizarArrecadadorActionForm.setInscricaoImovel(inscricaoImovel);
				}
			}
		}else{
			...
			if(idImovel == null || idImovel.trim().equals("")){
				...
				if(arrecadador.getImovel() != null && !arrecadador.getImovel().getId().toString().trim().equals("")){
					...
					atualizarArrecadadorActionForm.setIdImovel(arrecadador.getImovel().getId().toString());
			...
			atualizarArrecadadorActionForm.setIdAgente(arrecadador.getCodigoAgente().toString());

			atualizarArrecadadorActionForm.setInscricaoEstadual(arrecadador.getNumeroInscricaoEstadual());

			// 1.5. Caixa de empresa (Sim ou Não) (ARRC_ICCAIXAEMPRESA da tabela ARRECADADOR);
			atualizarArrecadadorActionForm.setIndicadorCaixaEmpresa(arrecadador.getIndicadorCaixaEmpresa());

			sessao.setAttribute("arrecadador", arrecadador);

			if(sessao.getAttribute("colecaoArrecadador") != null){
				sessao.setAttribute("caminhoRetornoVoltar", "/gsan/filtrarArrecadadorAction.do");
			}else{
