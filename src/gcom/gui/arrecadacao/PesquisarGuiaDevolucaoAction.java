 ...
 * @create 02/03/2006
 */
public class PesquisarGuiaDevolucaoAction
				extends GcomAction {
	...
	public ActionForward execute(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest httpServletRequest,
		...
		if(codigoImovel != null && !codigoImovel.equals("")){
			retorno = actionMapping.findForward("pesquisarGuiaDevolucaoResultadoImovel");
		}else{
			retorno = actionMapping.findForward("pesquisarGuiaDevolucaoResultadoCliente");
		}

		String dataEmissaoGuiaInicio = (String) pesquisarGuiaDevolucaoActionForm.getDataEmissaoGuiaInicio();
		String dataEmissaoGuiaFim = (String) pesquisarGuiaDevolucaoActionForm.getDataEmissaoGuiaFim();
		String dataValidadeGuiaInicio = (String) pesquisarGuiaDevolucaoActionForm.getDataValidadeGuiaInicio();
		String dataValidadeGuiaFim = (String) pesquisarGuiaDevolucaoActionForm.getDataValidadeGuiaFim();
		String[] idsSituacaoGuia = (String[]) pesquisarGuiaDevolucaoActionForm.getSituacaoGuia();
		String[] idsTipoCredito = (String[]) pesquisarGuiaDevolucaoActionForm.getTipoCredito();
		String[] idsTipoDocumento = (String[]) pesquisarGuiaDevolucaoActionForm.getTipoDocumento();

		// Validando datas dos campos dataEmissaoGuia Inicio e Fim
		if((dataEmissaoGuiaInicio.trim().length() == 10) && (dataEmissaoGuiaFim.trim().length() == 10)){

			Calendar calendarInicio = new GregorianCalendar();
			Calendar calendarFim = new GregorianCalendar();

			calendarInicio.set(Calendar.DAY_OF_MONTH, new Integer(dataEmissaoGuiaInicio.substring(0, 2)).intValue());
			calendarInicio.set(Calendar.MONTH, new Integer(dataEmissaoGuiaInicio.substring(3, 5)).intValue());
			calendarInicio.set(Calendar.YEAR, new Integer(dataEmissaoGuiaInicio.substring(6, 10)).intValue());

			calendarFim.set(Calendar.DAY_OF_MONTH, new Integer(dataEmissaoGuiaFim.substring(0, 2)).intValue());
			calendarFim.set(Calendar.MONTH, new Integer(dataEmissaoGuiaFim.substring(3, 5)).intValue());
			calendarFim.set(Calendar.YEAR, new Integer(dataEmissaoGuiaFim.substring(6, 10)).intValue());
			// joga exessão
		...
		if((dataValidadeGuiaInicio.trim().length() == 10) && (dataValidadeGuiaFim.trim().length() == 10)){

			Calendar calendarInicio = new GregorianCalendar();
			Calendar calendarFim = new GregorianCalendar();

			calendarInicio.set(Calendar.DAY_OF_MONTH, new Integer(dataValidadeGuiaInicio.substring(0, 2)).intValue());
			calendarInicio.set(Calendar.MONTH, new Integer(dataValidadeGuiaInicio.substring(3, 5)).intValue());
			calendarInicio.set(Calendar.YEAR, new Integer(dataValidadeGuiaInicio.substring(6, 10)).intValue());

			calendarFim.set(Calendar.DAY_OF_MONTH, new Integer(dataValidadeGuiaFim.substring(0, 2)).intValue());
			calendarFim.set(Calendar.MONTH, new Integer(dataValidadeGuiaFim.substring(3, 5)).intValue());
			calendarFim.set(Calendar.YEAR, new Integer(dataValidadeGuiaFim.substring(6, 10)).intValue());
			// joga exessão
			if(calendarFim.compareTo(calendarInicio) < 0){
				throw new ActionServletException("atencao.data_fim_menor_inicio");
			}
		...
		if((codigoImovel != null) && (!codigoImovel.trim().equals(""))){
			...
			filtroGuiaDevolucao.adicionarParametro(new ParametroSimples(FiltroGuiaDevolucao.IMOVEL_ID, codigoImovel));
			...
			filtroGuiaDevolucao.adicionarParametro(new ParametroSimples(FiltroGuiaDevolucao.CLIENTE_ID, codigoCliente));
			...
			if(pesquisarGuiaDevolucaoActionForm.getEnderecoCliente() == null
				...
				Collection clienteEncontrado = fachada.pesquisar(filtroClienteEndereco, ClienteEndereco.class.getName());
				...
				if(clienteEncontrado != null && !clienteEncontrado.isEmpty()){
					...
					if(clienteEndereco.getCliente().getClienteTipo().getIndicadorPessoaFisicaJuridica() == 1){
						if(clienteEndereco.getCliente().getCpfFormatado() != null){
							pesquisarGuiaDevolucaoActionForm.setCpfCnpj("" + clienteEndereco.getCliente().getCpfFormatado());
						}
					}else{
						if(clienteEndereco.getCliente().getCnpjFormatado() != null){
							pesquisarGuiaDevolucaoActionForm.setCpfCnpj("" + clienteEndereco.getCliente().getCnpjFormatado());
						}
					}
					if(clienteEndereco.getCliente().getProfissao() != null){
						pesquisarGuiaDevolucaoActionForm.setProfissao("" + clienteEndereco.getCliente().getProfissao().getDescricao());
					}
					...
					pesquisarGuiaDevolucaoActionForm.setEnderecoCliente(""
									+ ((ClienteEndereco) ((List) clienteEncontrado).get(0)).getEnderecoFormatado());
					...
					if(pesquisarGuiaDevolucaoActionForm.getClienteFone() == null
						...
						if(colecaoClienteFone != null && !colecaoClienteFone.isEmpty()){
							...
							pesquisarGuiaDevolucaoActionForm.setClienteFone(""
											+ ((ClienteFone) ((List) colecaoClienteFone).get(0)).getTelefone());
						}
					}
				}
			}
		...
		if((dataEmissaoGuiaInicio != null) && (!dataEmissaoGuiaInicio.trim().equals(""))){
			filtroGuiaDevolucao.adicionarParametro(new Intervalo(FiltroGuiaDevolucao.DATA_EMISSAO, dataEmissaoGuiaInicio,
							dataEmissaoGuiaFim));
		}
		if((dataValidadeGuiaInicio != null) && (!dataValidadeGuiaInicio.trim().equals(""))){
			filtroGuiaDevolucao.adicionarParametro(new Intervalo(FiltroGuiaDevolucao.DATA_VALIDADE, dataValidadeGuiaInicio,
							dataValidadeGuiaFim));
		}
		int i = 0;
		if(idsTipoCredito != null){
			...
			while(i < idsTipoCredito.length){
				...
				if(!idsTipoCredito[i].equals("") && new Integer(idsTipoCredito[i]) != ConstantesSistema.NUMERO_NAO_INFORMADO){
					if(i + 1 < idsTipoCredito.length){
						filtroGuiaDevolucao.adicionarParametro(new ParametroSimples(FiltroGuiaDevolucao.CREDITO_TIPO_ID, idsTipoCredito[i],
										ParametroSimples.CONECTOR_OR, idsTipoCredito.length));
					}else{
						filtroGuiaDevolucao
										.adicionarParametro(new ParametroSimples(FiltroGuiaDevolucao.CREDITO_TIPO_ID, idsTipoCredito[i]));
					}
				}
				i++;
			}
		...
		if(idsTipoDocumento != null){
			...
			while(i < idsTipoDocumento.length){
				...
				if(!idsTipoDocumento[i].equals("") && new Integer(idsTipoDocumento[i]) != ConstantesSistema.NUMERO_NAO_INFORMADO){
					if(i + 1 < idsTipoDocumento.length){
						filtroGuiaDevolucao.adicionarParametro(new ParametroSimples(FiltroGuiaDevolucao.DOCUMENTO_TIPO_ID,
										idsTipoDocumento[i], ParametroSimples.CONECTOR_OR, idsTipoDocumento.length));
					}else{
						filtroGuiaDevolucao.adicionarParametro(new ParametroSimples(FiltroGuiaDevolucao.DOCUMENTO_TIPO_ID,
										idsTipoDocumento[i]));
					}
		...
		if(idsSituacaoGuia != null){
			...
			while(i < idsSituacaoGuia.length){
				...
				if(!idsSituacaoGuia[i].equals("") && new Integer(idsSituacaoGuia[i]) != ConstantesSistema.NUMERO_NAO_INFORMADO){
					if(i + 1 < idsSituacaoGuia.length){
						filtroGuiaDevolucao.adicionarParametro(new ParametroSimples(FiltroGuiaDevolucao.DEBITO_CREDITO_SITUACAO_ATUAL_ID,
										idsSituacaoGuia[i], ParametroSimples.CONECTOR_OR, idsSituacaoGuia.length));
					}else{
						filtroGuiaDevolucao.adicionarParametro(new ParametroSimples(FiltroGuiaDevolucao.DEBITO_CREDITO_SITUACAO_ATUAL_ID,
										idsSituacaoGuia[i]));
					}
		...
		// Filtrando os dados...
		Collection<GuiaDevolucao> colecaoGuiaDevolucao = null;// fachada.pesquisar(filtroGuiaDevolucao,
		// GuiaDevolucao.class.getName());

		Map resultado = controlarPaginacao(httpServletRequest, retorno, filtroGuiaDevolucao, GuiaDevolucao.class.getName());
		colecaoGuiaDevolucao = (Collection) resultado.get("colecaoRetorno");
		...
		if(colecaoGuiaDevolucao == null){
			...
			if(!colecaoGuiaDevolucao.isEmpty()){
				...
				ClienteImovel dadosClienteImovel = (ClienteImovel) ((List) colecaoClienteImovel).get(0);

				if((codigoImovel != null) && (!codigoImovel.trim().equals(""))){
					if(dadosClienteImovel.getImovel().getId() != null && !dadosClienteImovel.getImovel().getId().equals("")){
						pesquisarGuiaDevolucaoActionForm.setCodigoImovel("" + dadosClienteImovel.getImovel().getId());
					}
					pesquisarGuiaDevolucaoActionForm.setInscricao("" + fachada.pesquisarInscricaoImovel(new Integer(codigoImovel), true));
					if(dadosClienteImovel.getImovel().getLigacaoAguaSituacao() != null){
						pesquisarGuiaDevolucaoActionForm.setSituacaoAgua(""
										+ dadosClienteImovel.getImovel().getLigacaoAguaSituacao().getDescricao());
					}
					if(dadosClienteImovel.getImovel().getLigacaoEsgotoSituacao() != null){
						pesquisarGuiaDevolucaoActionForm.setSituacaoEsgoto(""
										+ dadosClienteImovel.getImovel().getLigacaoEsgotoSituacao().getDescricao());
					}
				}
				if((codigoCliente != null) && (!codigoCliente.trim().equals(""))){
					if(dadosClienteImovel.getCliente() != null){
						pesquisarGuiaDevolucaoActionForm.setNomeCliente("" + dadosClienteImovel.getCliente().getNome());
						pesquisarGuiaDevolucaoActionForm.setCodigoCliente("" + dadosClienteImovel.getCliente().getId());
					}
					if(dadosClienteImovel.getCliente().getClienteTipo().getIndicadorPessoaFisicaJuridica() == 1){
						if(dadosClienteImovel.getCliente().getCpfFormatado() != null){
							pesquisarGuiaDevolucaoActionForm.setCpfCnpj("" + dadosClienteImovel.getCliente().getCpfFormatado());
						}
					}else{
						if(dadosClienteImovel.getCliente().getCnpjFormatado() != null){
							pesquisarGuiaDevolucaoActionForm.setCpfCnpj("" + dadosClienteImovel.getCliente().getCnpjFormatado());
						}
					}
					if(dadosClienteImovel.getCliente().getProfissao() != null){
						pesquisarGuiaDevolucaoActionForm.setProfissao("" + dadosClienteImovel.getCliente().getProfissao().getDescricao());
					}
					if(dadosClienteImovel.getCliente().getRamoAtividade() != null){
						pesquisarGuiaDevolucaoActionForm.setRamoAtividade(""
										+ dadosClienteImovel.getCliente().getRamoAtividade().getDescricao());
					}
				}
				if(dadosClienteImovel.getCliente() != null){
					pesquisarGuiaDevolucaoActionForm.setNomeCliente("" + dadosClienteImovel.getCliente().getNome());
				...
				sessao.setAttribute("colecaoGuiaDevolucao", colecaoGuiaDevolucao);
			}else{
				throw new ActionServletException("atencao.colecao_vazia");
			}
		}
