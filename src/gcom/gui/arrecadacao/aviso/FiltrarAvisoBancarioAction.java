...
public class FiltrarAvisoBancarioAction
				extends GcomAction {
	...
	public ActionForward execute(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest httpServletRequest,
		...
		String idConcessionaria = filtrarAvisoBancarioActionForm.getIdConcessionaria();
		String arrecadadorCodAgente = filtrarAvisoBancarioActionForm.getArrecadadorCodAgente();
		String tipoServico = filtrarAvisoBancarioActionForm.getTipoServico();
		String cdConvenio = filtrarAvisoBancarioActionForm.getCdConvenio();
		...
		if((idConcessionaria != null) && (!idConcessionaria.trim().equals(""))
						&& !idConcessionaria.equals(ConstantesSistema.VALOR_NAO_INFORMADO)){
			avisoBancarioHelper.setIdConcessionaria(Integer.parseInt(idConcessionaria));
			peloMenosUmParametroInformado = true;
		}

		if((arrecadadorCodAgente != null) && (!arrecadadorCodAgente.trim().equals(""))){
			avisoBancarioHelper.setCodigoAgenteArrecadador(new Short(arrecadadorCodAgente));
			peloMenosUmParametroInformado = true;
		}

		if((tipoServico != null) && (!tipoServico.trim().equals("")) && !tipoServico.equals(ConstantesSistema.VALOR_NAO_INFORMADO)){
			avisoBancarioHelper.setTipoServico(tipoServico);
			peloMenosUmParametroInformado = true;
		}

		if((cdConvenio != null) && (!cdConvenio.trim().equals(""))){
			avisoBancarioHelper.setCdConvenio(cdConvenio);
			peloMenosUmParametroInformado = true;
		}

		if((dataLancamentoInicioString != null) && (!dataLancamentoInicioString.equals(""))){
		...
		if((tipoAviso != null) && (!tipoAviso.trim().equals("")) && (!tipoAviso.trim().equals("3"))){
		...
		if((dataPrevisaoCreditoDebitoInicioString != null) && (!dataPrevisaoCreditoDebitoInicioString.trim().equals(""))){
		...
		if((dataRealizacaoCreditoDebitoInicioString != null) && (!dataRealizacaoCreditoDebitoInicioString.trim().equals(""))){
		...
		if(Util.verificarIdNaoVazio(filtrarAvisoBancarioActionForm.getAviso())){

			avisoBancarioHelper.setIndicadorAbertoFechado(Util.obterShort(filtrarAvisoBancarioActionForm.getAviso()));
			peloMenosUmParametroInformado = true;
		}else{

			avisoBancarioHelper.setIndicadorAbertoFechado(ConstantesSistema.AVISO_ABERTO_FECHADO);
		}
		...
		if(collectionAvisoBancario == null || collectionAvisoBancario.isEmpty()){
			...
			if(atualizar != null && totalRegistros == 1){
				...
				httpServletRequest.setAttribute("idAvisoBancario", avisoBancario2Helper.getAvisoBancario().getId());
				if(sessao.getAttribute("acao") != null){
					retorno = actionMapping.findForward("efetuarAnaliseAvisoBancario");
				}else{
					retorno = actionMapping.findForward("atualizarAvisoBancario");
				}
			...
			sessao.setAttribute("collectionAvisoBancario", collectionAvisoBancario);
			sessao.setAttribute("avisoBancarioHelper", avisoBancarioHelper);
			sessao.setAttribute("idConcessionaria", idConcessionaria);
	...
	private void validacaoFinal(FiltrarAvisoBancarioActionForm form){
		...
		if(form.getDataLancamentoInicio() != null && form.getDataLancamentoFim() != null){
			if(!form.getDataLancamentoInicio().equals("") && !form.getDataLancamentoFim().equals("")){

				dataLancamentoInicio = Util.converteStringParaDate(form.getDataLancamentoInicio());
				dataLancamentoFim = Util.converteStringParaDate(form.getDataLancamentoFim());
		...
		if(form.getPeriodoArrecadacaoInicio() != null && form.getPeriodoArrecadacaoFim() != null){
			if(!form.getPeriodoArrecadacaoInicio().equals("") && !form.getPeriodoArrecadacaoFim().equals("")){

				periodoArrecadacaoInicio = Integer.parseInt(Util.formatarMesAnoParaAnoMesSemBarra(form.getPeriodoArrecadacaoInicio()));
				periodoArrecadacaoFim = Integer.parseInt(Util.formatarMesAnoParaAnoMesSemBarra(form.getPeriodoArrecadacaoFim()));
		...
		if(form.getDataPrevisaoCreditoDebitoInicio() != null && form.getDataPrevisaoCreditoDebitoFim() != null){
			if(!form.getDataPrevisaoCreditoDebitoInicio().equals("") && !form.getDataPrevisaoCreditoDebitoFim().equals("")){

				dataPrevisaoCreditoDebitoInicio = Util.converteStringParaDate(form.getDataPrevisaoCreditoDebitoInicio());
				dataPrevisaoCreditoDebitoFim = Util.converteStringParaDate(form.getDataPrevisaoCreditoDebitoFim());
		...
		if(form.getIntervaloValorPrevistoInicio() != null && form.getIntervaloValorPrevistoFim() != null){
			if(!form.getIntervaloValorPrevistoInicio().equals("") && !form.getIntervaloValorPrevistoFim().equals("")){

				intervaloValorPrevistoInicio = Util.formatarMoedaRealparaBigDecimal(form.getIntervaloValorPrevistoInicio());
				intervaloValorPrevistoFim = Util.formatarMoedaRealparaBigDecimal(form.getIntervaloValorPrevistoFim());
		...
		// período de previsão do crédito/débito final
		if(form.getDataRealizacaoCreditoDebitoInicio() != null && form.getDataRealizacaoCreditoDebitoFim() != null){
			if(!form.getDataRealizacaoCreditoDebitoInicio().equals("") && !form.getDataRealizacaoCreditoDebitoFim().equals("")){

				dataRealizacaoCreditoDebitoInicio = Util.converteStringParaDate(form.getDataRealizacaoCreditoDebitoInicio());
				dataRealizacaoCreditoDebitoFim = Util.converteStringParaDate(form.getDataRealizacaoCreditoDebitoFim());
		...
		if(form.getIntervaloValorRealizadoInicio() != null && form.getIntervaloValorRealizadoFim() != null){
			if(!form.getIntervaloValorRealizadoInicio().equals("") && !form.getIntervaloValorRealizadoFim().equals("")){

				intervaloValorRealizadoInicio = Util.formatarMoedaRealparaBigDecimal(form.getIntervaloValorRealizadoInicio());
				intervaloValorRealizadoFim = Util.formatarMoedaRealparaBigDecimal(form.getIntervaloValorRealizadoFim());

			}
		}
		fachada.validacaoFinal(dataLancamentoInicio, dataLancamentoFim, periodoArrecadacaoInicio, periodoArrecadacaoFim,
						dataPrevisaoCreditoDebitoInicio, dataPrevisaoCreditoDebitoFim, intervaloValorPrevistoInicio,
						intervaloValorPrevistoFim, dataRealizacaoCreditoDebitoInicio, dataRealizacaoCreditoDebitoFim,
