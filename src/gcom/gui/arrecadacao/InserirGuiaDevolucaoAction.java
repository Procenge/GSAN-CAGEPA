 ...
 * @date 10/03/2006
 */
public class InserirGuiaDevolucaoAction
				extends GcomAction {
	...
	public ActionForward execute(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest httpServletRequest,
		...
		// Ordem de Serviço: Caso informada, incluir na guia, caso contrario, nula.
		if(idOrdemServico != null && !idOrdemServico.trim().equals("")){
			...
			if(ordemServico == null){
				...
				fachada.validarExibirInserirGuiaDevolucao(null, ordemServico);
		...
		if(idRegistroAtendimento == null || idRegistroAtendimento.trim().equals("")){
			throw new ActionServletException("atencao.Informe_entidade", null, "RA - Registro de Atendimento");
		}else{
		...
		if(valorDevolucao != null && !valorDevolucao.trim().equals("")){
			...
			BigDecimal valorDevolucaoFormatado = new BigDecimal(valorDevolucao.replace(".", "").replace(",", "."));

			if(valorDevolucaoFormatado.equals(new BigDecimal("0.00")) || valorDevolucaoFormatado.equals(new BigDecimal("0"))){
		...
		if(colecaoRegistroAtendimento != null && !colecaoRegistroAtendimento.isEmpty()){
			...
			// RegistroAtendimento registroAtendimentoVerificacao = (RegistroAtendimento)
			// colecaoRegistroAtendimento.iterator().next();

			/*--<merge>--if (registroAtendimentoVerificacao.getCliente() != null && documentoTipo.getId().equals(DocumentoTipo.DEVOLUCAO_VALOR)) {

				if (idDebitoTipo != null && !idDebitoTipo.trim().equals("")) {

					FiltroGuiaDevolucao filtroGuiaDevolucao = new FiltroGuiaDevolucao();
					filtroGuiaDevolucao.adicionarParametro(new ParametroSimples(FiltroGuiaDevolucao.CLIENTE_ID,	registroAtendimentoVerificacao.getCliente().getId()));
					filtroGuiaDevolucao.adicionarParametro(new ParametroSimples(FiltroGuiaDevolucao.DOCUMENTO_TIPO_ID,DocumentoTipo.DEVOLUCAO_VALOR));
					filtroGuiaDevolucao.adicionarParametro(new ParametroSimples(FiltroGuiaDevolucao.DEBITO_TIPO_ID,idDebitoTipo));
					...
					if (colecaoGuiaDevolucao != null && !colecaoGuiaDevolucao.isEmpty()) {
						...
						if (confirmado == null || !confirmado.trim().equalsIgnoreCase("ok")) {
							...
							httpServletRequest.setAttribute("caminhoActionConclusao","/gsan/inserirGuiaDevolucaoAction.do");
							
							// Monta a página de confirmação do wizard para perguntar se o usuário quer inserir
							// o pagamento mesmo sem a conta existir para a referência e o imóvel informados
			...
			throw new ActionServletException("atencao.pesquisa_inexistente", null, "RA - Registro de Atendimento");
		}
		...
		montarPaginaSucesso(httpServletRequest, "Guia de Devolução " + id + " inserida com sucesso.", "Inserir outra Guia de Devolução",
						"exibirInserirGuiaDevolucaoAction.do?menu=sim",
						"gerarRelatorioGuiaDevolucaoActionInserir.do?idGuiaDevolucao=" + id, "Emitir Guia de Devolução");
