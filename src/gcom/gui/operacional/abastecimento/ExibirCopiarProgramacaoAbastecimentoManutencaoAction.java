...
public class ExibirCopiarProgramacaoAbastecimentoManutencaoAction
				extends GcomAction {
	...
	public ActionForward execute(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest httpServletRequest,
		...
		if(tipoOperacao != null && !tipoOperacao.equals("") && tipoOperacao.equals("C")){
			...
			Collection colecaoProgramacaoAbastecimento = this.getFachada().consultarProgramacaoAbastecimento(idMunicipio, idBairro,
			...
			if(colecaoProgramacaoAbastecimento == null || colecaoProgramacaoAbastecimento.isEmpty()){
				...
				throw new ActionServletException("atencao.ja_existe_programacao_abastecimento_na_area", null, msg);
			}
			...
			}else if(objetoConsulta != null && !objetoConsulta.trim().equals("") && objetoConsulta.trim().equals("2")){
	...
	private void pesquisarMunicipio(InformarProgramacaoAbastecimentoManutencaoActionForm form){
		...
		filtroMunicipio.adicionarParametro(new ParametroSimples(FiltroMunicipio.ID, new Integer(form.getMunicipioCopiar())));
	 ...
	 * @date 13/11/2006
	 */
	private void pesquisarBairro(InformarProgramacaoAbastecimentoManutencaoActionForm form, HttpServletRequest httpServletRequest){
		...
		filtroBairro.adicionarParametro(new ParametroSimples(FiltroBairro.CODIGO, new Integer(form.getBairroCopiar())));
		...
		// Pesquisa de acordo com os parâmetros informados no filtro
		Collection colecaoBairro = this.getFachada().pesquisar(filtroBairro, Bairro.class.getName());
	...
	private void montarAreaBairroPorId(HttpServletRequest request, Integer id){
		...
		colecaoAreaBairro = this.getFachada().pesquisar(filtroBairroArea, BairroArea.class.getName());
	 ...
	 * @date 01/12/2006
	 */
	private void setaRequest(HttpServletRequest httpServletRequest, InformarProgramacaoAbastecimentoManutencaoActionForm form){

		// Municipio
		if(form.getMunicipioCopiar() != null && !form.getMunicipioCopiar().equals("") && form.getNomeMunicipioCopiar() != null
		...
		// Bairro
		if(form.getBairroCopiar() != null && !form.getBairroCopiar().equals("") && form.getNomeBairroCopiar() != null
						&& !form.getNomeBairroCopiar().equals("")){
	...
	private void pesquisarAreaBairro(Integer idArea, InformarProgramacaoAbastecimentoManutencaoActionForm form){
		...
		if(colecaoAreaBairro != null && !colecaoAreaBairro.isEmpty()){
			...
			BairroArea bairroArea = (BairroArea) Util.retonarObjetoDeColecao(colecaoAreaBairro);
