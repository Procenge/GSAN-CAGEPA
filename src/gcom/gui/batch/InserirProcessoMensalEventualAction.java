 ...
 * @created 24/07/2006
 */
public class InserirProcessoMensalEventualAction
				extends GcomAction {
	...
	public ActionForward execute(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest httpServletRequest,
		...
		Processo processo = (Processo) fachada.pesquisar(idProcesso, Processo.class);

		if(processo != null && processo.getProcessoTipo().getId().intValue() == ProcessoTipo.RELATORIO){

			boolean gerarRelatorioProcessoEventual = GerenciadorExecucaoTarefaRelatorio
							.verificarGeracaoRelatorioProcessoEventual(idProcesso);

			if(gerarRelatorioProcessoEventual){

				montarPaginaSucesso(httpServletRequest, "Processo Iniciado Inserido com Sucesso.", "Inserir outro Processo",
								"exibirInserirProcessoAction.do");
				return retorno;
			}else{

				throw new ActionServletException("atencao.pesquisa_inexistente", "Processo Eventual");
			}
		}
		...
		montarPaginaSucesso(httpServletRequest, "Processo Iniciado de código " + codigoProcessoIniciadoGerado + " inserido com sucesso.",
