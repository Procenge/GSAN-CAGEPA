 ...
 * @created 11/08/2006
 */
public class InserirProcessoFaturamentoComandadoAction
				extends GcomAction {
	...
	public ActionForward execute(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest httpServletRequest,
		...
		String[] idsProcessosFaturamentoAtividadeCronogramaPagina = httpServletRequest
						.getParameterValues("idFaturamentoAtividadeCronograma");

		Collection<Integer> idsFaturamentoAtividadeCronograma = new ArrayList();
		if(!Util.isVazioOrNulo(idsProcessosFaturamentoAtividadeCronogramaPagina)){

			for(int i = 0; i < idsProcessosFaturamentoAtividadeCronogramaPagina.length; i++){
				idsFaturamentoAtividadeCronograma.add(Integer.parseInt(idsProcessosFaturamentoAtividadeCronogramaPagina[i]));
			}
		}

		String[] idsFaturamentoSimulacaoComandoSelecionadosPagina = httpServletRequest.getParameterValues("idFaturamentoSimulacaoComando");

		Collection<Integer> idsFaturamentoSimulacaoComando = new ArrayList();

		if(!Util.isVazioOrNulo(idsFaturamentoSimulacaoComandoSelecionadosPagina)){

			for(int i = 0; i < idsFaturamentoSimulacaoComandoSelecionadosPagina.length; i++){
				idsFaturamentoSimulacaoComando.add(Integer.parseInt(idsFaturamentoSimulacaoComandoSelecionadosPagina[i]));
			}
		}
		...
		fachada.inserirProcessoIniciadoFaturamentoComandado(idsFaturamentoAtividadeCronograma, idsFaturamentoSimulacaoComando,
						dataHoraAgendamento,
