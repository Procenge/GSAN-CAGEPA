 ...
 * @date 26/10/2006
 */
public class ExibirRelatorioBatchAction
				extends GcomAction {
	...
	public ActionForward execute(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest httpServletRequest,
		...
		int idRelatorioGerado = converterStringToInt(httpServletRequest.getParameter("idRelatorioGerado"));
		FiltroRelatorioGerado filtroRelatorioGerado = new FiltroRelatorioGerado();
		filtroRelatorioGerado.adicionarParametro(new ParametroSimples(FiltroRelatorioGerado.ID, idRelatorioGerado));
		filtroRelatorioGerado.adicionarCaminhoParaCarregamentoEntidade("funcionalidadeIniciada");
		RelatorioGerado relatorioGerado = (RelatorioGerado) Util.retonarObjetoDeColecao(Fachada.getInstancia().pesquisar(
		...
		if(relatorioGerado == null){
			...
			return actionMapping.findForward("telaAtencaoPopup");
		}

		OutputStream out = null;
		try{

			TarefaRelatorio tarefaRelatorio = (TarefaRelatorio) IoUtil.transformarBytesParaObjeto(relatorioGerado
			...
			switch((Integer) tarefaRelatorio.getParametro("tipoFormatoRelatorio")){
				...
				case TarefaRelatorio.TIPO_CSV:
					httpServletResponse.addHeader("Content-Disposition", "attachment; filename=relatorio.csv");

					mimeType = "application/csv";
					break;

				case TarefaRelatorio.TIPO_ZIP:
					httpServletResponse.addHeader("Content-Disposition", "attachment; filename=relatorio.zip");
