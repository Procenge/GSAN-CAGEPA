 ...
 * @created 25/04/06
 */
public class AdicionarCriterioCobrancaRotaAction
				extends GcomAction {
	...
	public ActionForward execute(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest httpServletRequest,
		...
		if(idCobrancaAcao != null && !idCobrancaAcao.equalsIgnoreCase("")){
			...
			Collection<CobrancaAcao> collectionCobrancaAcao = fachada.pesquisar(filtroCobrancaAcao, CobrancaAcao.class.getName());
	...
	private void validacaoFinal(Collection collectionRotaAcaoCriterio, String idCobrancaAcao, String idCobrancaCriterio, Fachada fachada){

		// ação de cobrança é obrigatório.
		if((idCobrancaAcao == null) || (idCobrancaAcao.equals("" + ConstantesSistema.NUMERO_NAO_INFORMADO))){
			throw new ActionServletException("atencao.acao_cobranca_nao_informado");
		}

		// critério de cobrança é obrigatório.
		if(idCobrancaCriterio == null || idCobrancaCriterio.equalsIgnoreCase("")){
			throw new ActionServletException("atencao.criterio_cobranca_nao_informado");
		}else{
			...
			if(collectionRotaAcaoCriterio != null && !collectionRotaAcaoCriterio.isEmpty()){
				...
				while(iterator.hasNext()){
					...
					if(rotaAcaoCriterio.getCobrancaAcao().getId().equals(new Integer(idCobrancaAcao))){
						...
						throw new ActionServletException("atencao.acao_cobranca.ja_informada");
					}
	...
	private void verificaExistenciaCodCobrancaCriterio(String idDigitadoEnterCobrancaCriterio, Fachada fachada){
		...
		if(idDigitadoEnterCobrancaCriterio != null && !idDigitadoEnterCobrancaCriterio.trim().equals("")
			...
			if(cobrancaCriterios == null || cobrancaCriterios.isEmpty()){
				...
				throw new ActionServletException("atencao.pesquisa_inexistente", null, "Critério de Cobrança");
