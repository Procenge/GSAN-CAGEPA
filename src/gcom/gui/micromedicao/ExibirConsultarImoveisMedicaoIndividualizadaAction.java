 ...
 */
public class ExibirConsultarImoveisMedicaoIndividualizadaAction
				extends GcomAction {
			...
			if(codigoImovel != null && !codigoImovel.trim().equalsIgnoreCase("")){
				...
				if(codigoImovel != null && !codigoImovel.trim().equals("")){
					...
					if(idImovel == null || idImovel.intValue() == 0){
						...
						/*
						 * filtroImovel
						 * .adicionarCaminhoParaCarregamentoEntidade("lote");
						 * filtroImovel
						...
						if(!Util.isVazioOuBranco(imovelPesquisado)){
							Imovel imovelCondominio = (Imovel) Util.retonarObjetoDeColecao(imovelPesquisado);

							if(!Util.isVazioOuBranco(imovelCondominio)){
								...
								consultarImoveisMedicaoIndividualizadaActionForm.setInscricaoImovel(imovelCondominio
												.getInscricaoFormatada());
								if(imovelCondominio.getLigacaoAguaSituacao() != null){
									consultarImoveisMedicaoIndividualizadaActionForm.setSituacaoAgua(imovelCondominio
													.getLigacaoAguaSituacao().getDescricao());

								}
								if(imovelCondominio.getLigacaoEsgotoSituacao() != null){
									consultarImoveisMedicaoIndividualizadaActionForm.setSituacaoEsgoto(imovelCondominio
								...
								if(imovelCondominio.getLigacaoAguaSituacao() != null){
									...
									if(imovelCondominio.getLigacaoAguaSituacao().getIndicadorFaturamentoSituacao().equals(
								...
								if(!achou){
									if(imovelCondominio.getLigacaoEsgotoSituacao() != null){
										...
										if(imovelCondominio.getLigacaoEsgotoSituacao().getIndicadorFaturamentoSituacao().equals(
														LigacaoEsgotoSituacao.FATURAMENTO_ATIVO)){
											descricaoRateioTipo = fachada.pesquisarDescricaoRateioTipoLigacaoEsgoto(imovelCondominio
															.getId());
										}
									}
								}
								...
								filtroClienteImovel.adicionarParametro(new ParametroSimples(FiltroClienteImovel.IMOVEL_CONDOMINIO_ID,
												imovelCondominio.getId()));
								filtroClienteImovel.adicionarParametro(new ParametroNulo(FiltroClienteImovel.DATA_FIM_RELACAO));

								filtroClienteImovel.setCampoOrderBy(FiltroClienteImovel.NUMERO_SEGMENTO, FiltroClienteImovel.LOTE,
												FiltroClienteImovel.SUBLOTE);

								Collection<ClienteImovel> imovelPesquisadoVinculados = fachada.pesquisar(filtroClienteImovel,
												ClienteImovel.class.getName());

								consultarImoveisMedicaoIndividualizadaActionForm.setQuantidadeDeImoveisVinculados(""
												+ imovelPesquisadoVinculados.size());
