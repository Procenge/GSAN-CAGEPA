 ...
 * @date 22/07/2007
 */
public class FiltrarRoteiroEmpresaAction
				extends GcomAction {
	...
	public ActionForward execute(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest httpServletRequest,
		...
		String idSetorComercial = filtrarRoteiroEmpresaActionForm.getIdSetorComercial();
		String indicadorUso = filtrarRoteiroEmpresaActionForm.getIndicadorUso();

		// Verifica se o campo Empresa foi informado

		if(empresa != null && !empresa.trim().equalsIgnoreCase("" + ConstantesSistema.NUMERO_NAO_INFORMADO)){

			peloMenosUmParametroInformado = true;

			filtroRoteiroEmpresa.adicionarParametro(new ParametroSimples(FiltroRoteiroEmpresa.EMPRESA, empresa));
		...
		if(idLocalidade != null && !idLocalidade.trim().equalsIgnoreCase("")){
			...
			filtroRoteiroEmpresa.adicionarParametro(new ParametroSimples(FiltroRoteiroEmpresa.LOCALIDADE_ID, idLocalidade));
		}
		...
		if(idLeiturista != null && !idLeiturista.trim().equalsIgnoreCase("")){
			...
			filtroRoteiroEmpresa.adicionarParametro(new ParametroSimples(FiltroRoteiroEmpresa.LEITURISTA_ID, idLeiturista));
		}

		// Verifica se o setor comercial foi informado

		if(idSetorComercial != null && !idSetorComercial.trim().equalsIgnoreCase("" + ConstantesSistema.NUMERO_NAO_INFORMADO)){

			peloMenosUmParametroInformado = true;

			filtroRoteiroEmpresa.adicionarParametro(new ParametroSimples(FiltroRoteiroEmpresa.SETOR_COMERCIAL_ID, idSetorComercial));
		...
		if(indicadorUso != null && !indicadorUso.equalsIgnoreCase("") && !indicadorUso.equalsIgnoreCase("3")){
			...
			if(indicadorUso.equalsIgnoreCase(String.valueOf(ConstantesSistema.INDICADOR_USO_ATIVO))){
				filtroRoteiroEmpresa.adicionarParametro(new ParametroSimples(FiltroRoteiroEmpresa.INDICADOR_USO,
