 ...
 * @created 21/03/2006
 */
public class InserirRoteiroEmpresaAction
				extends GcomAction {
	...
	public ActionForward execute(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest httpServletRequest,
		...
		validacaoRoteiroEmpresa(idEmpresa, idLocalidade, idFaturamentoGrupo, idLeiturista, idQuadrasAdicionar, fachada, httpServletRequest);
		...
		if(idLeiturista != null && !idLeiturista.equals("" + ConstantesSistema.NUMERO_NAO_INFORMADO)){
			...
			// Validar se a empresa escolhida for diferente da empresa a qual pertence o funcionário
			// ou o cliente
			...
			if(leiturista.getEmpresa() != null && leiturista.getEmpresa().getId().intValue() != Integer.parseInt(idEmpresa)){
		...
		montarPaginaSucesso(httpServletRequest, "Roteiro Empresa (" + idRoteiro + ") inserido com sucesso.", "Inserir outro Roteiro",
						"exibirInserirRoteiroEmpresaAction.do?desfazer=S",
						"exibirAtualizarRoteiroEmpresaAction.do?idRegistroInseridoAtualizar=" + idRoteiro,
	...
	private void validacaoRoteiroEmpresa(String empresa, String idLocalidade, String idFaturamentoGrupo, String idLeiturista,
					String[] quadrasAdicionar, Fachada fachada, HttpServletRequest httpServletRequest){

		// Empresa é obrigatório.
		if((empresa == null) || (empresa.equals("" + ConstantesSistema.NUMERO_NAO_INFORMADO))){
			...
			httpServletRequest.setAttribute("nomeCampo", "empresa");
			throw new ActionServletException("atencao.nao.numerico", null, "Empresa");
		}
		...
		if(quadrasAdicionar == null || quadrasAdicionar.length == 0){
			throw new ActionServletException("atencao.quadras.informar");
		}

	}

	private void verificaExistenciaIdLocalidade(String idDigitadoEnterLocalidade, Fachada fachada, HttpServletRequest httpServletRequest){
		...
		if((idDigitadoEnterLocalidade != null && !idDigitadoEnterLocalidade.toString().trim().equalsIgnoreCase(""))){
			...
			if(idDigitadoEnterLocalidade != null && !idDigitadoEnterLocalidade.toString().trim().equalsIgnoreCase("")){
				...
				filtroLocalidade.adicionarParametro(new ParametroSimples(FiltroLocalidade.ID, new Integer(idDigitadoEnterLocalidade)));
			}
