 ...
 * @created 28/03/2006
 */
public class FiltrarRotaAction
				extends GcomAction {
	...
	public ActionForward execute(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest httpServletRequest,
		...
		String codigoLeiturista = rotaActionForm.getCodigoLeiturista();
		...
		if(codigoLeiturista != null && !codigoLeiturista.trim().equalsIgnoreCase("")){
			filtroRota.adicionarParametro(new ParametroSimples(FiltroRota.CODIGO_LEITURISTA_ID, new Integer(codigoLeiturista)));
			peloMenosUmParametroInformado = true;
		}

		if((indicadorUso != null && !indicadorUso.equals("" + ConstantesSistema.NUMERO_NAO_INFORMADO)) && (!indicadorUso.equals("3"))){

			filtroRota.adicionarParametro(new ParametroSimples(FiltroRota.INDICADOR_USO, indicadorUso));
	...
	private void verificaExistenciaCodSetorComercial(String idDigitadoEnterLocalidade, String codigoDigitadoEnterSetorComercial,
		...
		if((codigoDigitadoEnterSetorComercial != null && !codigoDigitadoEnterSetorComercial.toString().trim().equalsIgnoreCase(""))){
			...
			if(setorComerciais == null || setorComerciais.isEmpty()){
				...
				throw new ActionServletException(msg);
	...
	private void validacaoFinal(String codigoRota, String idLocalidade, String codigoSetorComercial, Fachada fachada,
					HttpServletRequest httpServletRequest){

		if(idLocalidade != null && !idLocalidade.equalsIgnoreCase("") && Util.validarValorNaoNumerico(idLocalidade)){
			// Localidade não numérico.
			httpServletRequest.setAttribute("nomeCampo", "idLocalidade");
			throw new ActionServletException("atencao.nao.numerico", null, "Localidade ");
		}

		if(codigoSetorComercial != null && !codigoSetorComercial.equalsIgnoreCase("") && Util.validarValorNaoNumerico(codigoSetorComercial)){
		...
		if(codigoRota != null && !codigoRota.equalsIgnoreCase("") && Util.validarValorNaoNumerico(codigoRota)){
			// Código da Rota não numérico.
			httpServletRequest.setAttribute("nomeCampo", "codigoRota");
			throw new ActionServletException("atencao.nao.numerico", null, "Código da Rota ");
		}
