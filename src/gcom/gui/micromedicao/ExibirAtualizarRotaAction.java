...
public class ExibirAtualizarRotaAction
				extends GcomAction {
	...
	public ActionForward execute(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest httpServletRequest,
		...
		Collection<RotaAcaoCriterio> collectionRotaAcaoCriterio = (Collection<RotaAcaoCriterio>) sessao
		...
		if((collectionRotaAcaoCriterio != null && collectionRotaAcaoCriterio.size() != 0)
			...
			Collection<CobrancaGrupo> collectionAcaoCobranca = fachada.pesquisar(filtroCobrancaAcao, CobrancaAcao.class.getName());
		...
		if(httpServletRequest.getParameter("reloadPage") == null || httpServletRequest.getParameter("reloadPage").equals("")){
			...
			exibirRota(idRota, inserirRotaActionForm, fachada, sessao, collectionRotaAcaoCriterio, httpServletRequest);
		...
		if(idDigitadoEnterLeiturista != null && !idDigitadoEnterLeiturista.equalsIgnoreCase("")
			...
			throw new ActionServletException("atencao.nao.numerico", null, "Leiturista ");
		}
		verificaExistenciaCodLeiturista(idDigitadoEnterLeiturista, inserirRotaActionForm, httpServletRequest, fachada);
	...
	private void exibirRota(String idRota, InserirRotaActionForm inserirRotaActionForm, Fachada fachada, HttpSession sessao,
					Collection collectionRotaAcaoCriterio, HttpServletRequest httpServletRequest){
		...
		Collection<Rota> collectionRota = fachada.pesquisar(filtroRota, Rota.class.getName());
		...
		if(collectionRota != null && !collectionRota.isEmpty()){
			...
			// inserirRotaActionForm.setDataAjusteLeitura(Util.formatarData(rota.getDataAjusteLeitura()));
			...
			if(rota.getLeiturista() != null){
				inserirRotaActionForm.setCodigoLeiturista("" + rota.getLeiturista().getId());
				...
				}else if(rota.getLeiturista().getCliente() != null){
			...
			if(collectionRotaAcaoCriterio == null || collectionRotaAcaoCriterio.isEmpty()){
				...
				collectionRotaAcaoCriterio = (Collection<RotaAcaoCriterio>) fachada.pesquisar(filtroRotaAcaoCriterio,
	...
	private void verificaExistenciaCodLeiturista(String idDigitadoEnterLeiturista, InserirRotaActionForm inserirRotaActionForm,
					HttpServletRequest httpServletRequest, Fachada fachada){
		...
		if(idDigitadoEnterLeiturista != null && !idDigitadoEnterLeiturista.trim().equals("")
			...
			// Recupera o leiturista informada pelo usuário
			// Varaiável que vai armazenar o leiturista digitada pelo usuário
			Leiturista leituristaDigitada = null;
			...
			Collection<Localidade> colecaoLeiturista = fachada.pesquisar(filtroLeiturista, Leiturista.class.getName());

			// Caso o Leiturista informada pelo usuário esteja cadastrada no sistema
			// Seta os dados do leiturista no form
			// Caso contrário seta as informações do leiturista para vazio
			// e indica ao usuário que o leiturista não existe

			// Caso exista o leiturista no sistema
			// Retorna para o usuário o leiturista retornada pela pesquisa
			// Caso contrário retorna um objeto nulo
			if(colecaoLeiturista != null && !colecaoLeiturista.isEmpty()){
				leituristaDigitada = (Leiturista) Util.retonarObjetoDeColecao(colecaoLeiturista);
			}

			if(leituristaDigitada != null){
				// a localidade foi encontrada
				inserirRotaActionForm.setCodigoLeiturista("" + (leituristaDigitada.getId()));
				inserirRotaActionForm.setNomeLeiturista(leituristaDigitada.getNomeLeiturista());
				httpServletRequest.setAttribute("idLeituristaNaoEncontrado", "true");
			}else{
				// a localidade não foi encontrada
				inserirRotaActionForm.setCodigoLeiturista("");
				httpServletRequest.setAttribute("idLeituristaNaoEncontrado", "exception");
