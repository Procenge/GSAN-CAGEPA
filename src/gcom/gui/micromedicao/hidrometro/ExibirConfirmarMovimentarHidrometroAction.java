 ...
 */
public class ExibirConfirmarMovimentarHidrometroAction
				extends GcomAction {
	...
	public ActionForward execute(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest httpServletRequest,
		...
		ActionForward retorno = actionMapping.findForward("confirmarMovimentarHidrometro");
		...
		Boolean pesquisar = Boolean.parseBoolean(httpServletRequest.getParameter("pesquisar"));

		Collection colecaoHidrometroSelecionado = new ArrayList();
		String numeroHidrometrosSelecionados = "";

		if(!pesquisar){

			colecaoHidrometroSelecionado = (Collection) httpServletRequest.getAttribute("colecaoHidrometroSelecionado");
			numeroHidrometrosSelecionados = (String) httpServletRequest.getAttribute("numeroHidrometrosSelecionados");

		}else{
			colecaoHidrometroSelecionado = (Collection) sessao.getAttribute("colecaoHidrometroSelecionado");
			numeroHidrometrosSelecionados = (String) sessao.getAttribute("numeroHidrometrosSelecionados");
		}

		// Obtém a descrição do local de armazenagem
		Hidrometro hidrometro = (Hidrometro) Util.retonarObjetoDeColecao(colecaoHidrometroSelecionado);
		String codigoDescricaoLocalArmazenagemAtual = "";
		if(hidrometro.getHidrometroLocalArmazenagem() != null){
			codigoDescricaoLocalArmazenagemAtual = hidrometro.getHidrometroLocalArmazenagem().getId().toString();
		...
		if(objetoConsulta != null && !objetoConsulta.trim().equalsIgnoreCase("") && (Integer.parseInt(objetoConsulta)) == 1){
			...
			filtroHidrometroLocalArmazenagem.adicionarParametro(new ParametroSimples(FiltroHidrometroLocalArmazenagem.ID, new Integer(
							confirmarMovimentarHidrometroActionForm.getIdLocalArmazenagemDestino()), ParametroSimples.CONECTOR_AND));
			...
			if(colecaoHidrometroLocalArmazenagem != null && !colecaoHidrometroLocalArmazenagem.isEmpty()){
				...
				confirmarMovimentarHidrometroActionForm.setIdLocalArmazenagemDestino(hidrometroLocalArmazenagem.getId().toString());
				confirmarMovimentarHidrometroActionForm.setLocalArmazenagemDescricaoDestino(hidrometroLocalArmazenagem.getDescricao());
				...
				confirmarMovimentarHidrometroActionForm.setIdLocalArmazenagemDestino("");
				confirmarMovimentarHidrometroActionForm.setLocalArmazenagemDescricaoDestino("LOCAL DE ARMAZENAGEM INEXISTENTE");
			}
		...
		Collection colecaoHidrometroMotivoMovimentacao = fachada.pesquisar(filtroHidrometroMotivoMovimentacao,
						HidrometroMotivoMovimentacao.class.getName());

		// Envia objeto no request
		httpServletRequest.setAttribute("colecaoHidrometroMotivoMovimentacao", colecaoHidrometroMotivoMovimentacao);
		// Envia objeto pela sessão
		sessao.setAttribute("codigoDescricaoLocalArmazenagemAtual", codigoDescricaoLocalArmazenagemAtual);

		// Data Corrente
		SimpleDateFormat formatoData = new SimpleDateFormat("dd/MM/yyyy");
		Calendar dataCorrente = new GregorianCalendar();

		httpServletRequest.setAttribute("dataMovimentacao", formatoData.format(dataCorrente.getTime()));
		...
		httpServletRequest.setAttribute("horaMovimentacao", formatoHora.format(horaCorrente.getTime()));

		sessao.setAttribute("numeroHidrometrosSelecionados", numeroHidrometrosSelecionados);
		sessao.setAttribute("colecaoHidrometroSelecionado", colecaoHidrometroSelecionado);
