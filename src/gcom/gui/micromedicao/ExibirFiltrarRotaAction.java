 ...
 * @created 27/03/2006
 */
public class ExibirFiltrarRotaAction
				extends GcomAction {
	...
	public ActionForward execute(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest httpServletRequest,
		...
		verificaExistenciaCodLocalidade(idDigitadoEnterLocalidade, rotaActionForm, httpServletRequest, fachada, sessao);
		...
		String idDigitadoEnterLeiturista = rotaActionForm.getCodigoLeiturista();
		...
		verificarExistenciaCodLeiturista(rotaActionForm.getEmpresaLeituristica(), idDigitadoEnterLeiturista, rotaActionForm,
						httpServletRequest, fachada);
		// -------Fim de parte que trata do código quando o usuário tecla enter

		// teste
		if(rotaActionForm.getIndicadorUso() == null || rotaActionForm.getIndicadorUso().equalsIgnoreCase("")){
			rotaActionForm.setIndicadorUso("3");
		}
	...
	private void verificaExistenciaCodLocalidade(String idDigitadoEnterLocalidade, RotaActionForm rotaActionForm,
		...
		if(idDigitadoEnterLocalidade != null && !idDigitadoEnterLocalidade.trim().equals("")
			...
			if(localidadeEncontrada != null){
				...
				rotaActionForm.setLocalidadeDescricao("LOCALIDADE INEXISTENTE");
				httpServletRequest.setAttribute("nomeCampo", "idLocalidade");
			}
	...
	private void verificaExistenciaCodSetorComercial(String idDigitadoEnterLocalidade, String idDigitadoEnterSetorComercial,
					RotaActionForm rotaActionForm, HttpServletRequest httpServletRequest, Fachada fachada, HttpSession sessao){
		...
		if((idDigitadoEnterSetorComercial != null && !idDigitadoEnterSetorComercial.toString().trim().equalsIgnoreCase(""))
			...
			if(setorComerciais != null && !setorComerciais.isEmpty()){
				...
				rotaActionForm.setSetorComercialDescricao("SETOR COMERCIAL INEXISTENTE");
				httpServletRequest.setAttribute("nomeCampo", "codigoSetorComercial");
			}
			if(httpServletRequest.getParameter("desfazer") == null){
				String indicadorAtualizar = httpServletRequest.getParameter("indicadorAtualizar");
	...
	private void verificarExistenciaCodLeiturista(String idDigitadoEnterEmpresaLeitura, String idDigitadoEnterLeiturista,
					RotaActionForm filtrarRotaActionForm, HttpServletRequest httpServletRequest, Fachada fachada){
		...
		if((idDigitadoEnterLeiturista != null && !idDigitadoEnterLeiturista.trim().equalsIgnoreCase(""))){
			...
			if(leituristas != null && !leituristas.isEmpty()){
				...
				filtrarRotaActionForm.setCodigoLeiturista("" + (leituristaEncontrado.getId()));

				if(leituristaEncontrado.getFuncionario() != null){
					filtrarRotaActionForm.setNomeLeiturista(leituristaEncontrado.getFuncionario().getNome());
				}else if(leituristaEncontrado.getCliente() != null){
					filtrarRotaActionForm.setNomeLeiturista(leituristaEncontrado.getCliente().getNome());
				}
				...
				// o setor comercial não foi encontrado
				filtrarRotaActionForm.setCodigoLeiturista("");
				httpServletRequest.setAttribute("idSetorComercialNaoEncontrada", "exception");
				filtrarRotaActionForm.setNomeLeiturista("AGENTE COMERCIAL INEXISTENTE");
