 ...
 * @since 12/01/2006
 */
public class DesfazerVinculoPopupAction
				extends GcomAction {
	...
	public ActionForward execute(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest httpServletRequest,
		...
		DesfazerVinculoPopupActionForm desfazerVinculoPopupActionForm = (DesfazerVinculoPopupActionForm) actionForm;

		String[] ids = desfazerVinculoPopupActionForm.getIdImovel();
		...
		for(int i = 0; i < ids.length; i++){
			...
			Boolean imovelEmProcessoDeFaturamento = fachada.verificarImovelEmProcessoDeFaturamento(Integer.valueOf(ids[i]));

			if(imovelEmProcessoDeFaturamento){
				FaturamentoGrupo fg = fachada.pesquisarGrupoImovel(Integer.valueOf(ids[i]));
				throw new ActionServletException("atencao.imovel.em_processo_faturamento", new String[] {ids[i], fg.getId().toString()});
			}
		...
		fachada.desfazerVinculo(imovel, ids, desvincular, usuarioLogado);
		...
		if(retorno.getName().equalsIgnoreCase("telaSucessoPopup")){
			montarPaginaSucesso(httpServletRequest, ids.length + " imóvel(is) desvinculado(s) do imóvel condomínio " + imovel.getId()
							+ " com sucesso.", "", "");
		}
