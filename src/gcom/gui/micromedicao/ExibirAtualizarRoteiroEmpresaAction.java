 ...
 * @date 28/06/2008
 *       Alteração de Mensagens de Leiturista para Termo:"Agente Comercial"
 */

public class ExibirAtualizarRoteiroEmpresaAction
				extends GcomAction {
	...
	public ActionForward execute(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest httpServletRequest,
		...
		filtroEmpresa.adicionarParametro(new ParametroSimples(FiltroEsferaPoder.INDICADOR_USO, ConstantesSistema.INDICADOR_USO_ATIVO));
		...
		if(httpServletRequest.getParameter("reloadPage") == null || httpServletRequest.getParameter("reloadPage").equals("")){
			...
			exibirRoteiroEmpresa(idRoteiroEmpresa, form, fachada, httpServletRequest);

		}else{
			idRoteiroEmpresa = (String) sessao.getAttribute("idRegistroAtualizacao");
			// Para o caso em que o Grupo foi alterado no Form, assim novas quadras serão definidas
			if(httpServletRequest.getParameter("grupoAlterado") == null && httpServletRequest.getParameter("localidadeAlterada") == null){
				carregarQuadrasRoteiroEmpresa(idRoteiroEmpresa, form, fachada, httpServletRequest);
				// setam atributos que foram escolhidos no form novamente, pois o
				// ExibirRoteiroEmpresa atribui valores para carregamento inicial,
				// baseado com o que existe no banco
				form.setFaturamentoGrupo(idFaturamentoGrupo);
			}
		}
		...
		if(httpServletRequest.getParameter("desfazer") != null && httpServletRequest.getParameter("desfazer").equalsIgnoreCase("S")){
			...
			// -------------- bt DESFAZER ---------------
			exibirRoteiroEmpresa(idRoteiroEmpresa, form, fachada, httpServletRequest);
			idFaturamentoGrupo = form.getFaturamentoGrupo();
		...
		String localidadeAlterada = httpServletRequest.getParameter("localidadeAlterada");
		String grupoFaturamentoAlterado = httpServletRequest.getParameter("grupoAlterado");

		if((localidadeAlterada != null && !localidadeAlterada.trim().equalsIgnoreCase(""))
						|| (grupoFaturamentoAlterado != null && !grupoFaturamentoAlterado.trim().equalsIgnoreCase(""))
						|| idRoteiroEmpresa != null){

			// Pesquisando Setor Comercial
			if(idLocalidade != null && !idLocalidade.trim().equalsIgnoreCase("") && idFaturamentoGrupo != null
							&& !idFaturamentoGrupo.trim().equalsIgnoreCase("")){

				colecaoSetorComercial = fachada.pesquisarSetoresComerciaisPorLocalidadeGrupoFaturamento(new Integer(idLocalidade),
								new Integer(idFaturamentoGrupo));

				sessao.setAttribute("colecaoSetorComercial", colecaoSetorComercial);

				if((colecaoSetorComercial == null || colecaoSetorComercial.isEmpty())
								&& httpServletRequest.getParameter("reloadPage") != null){ // Validar[a
					// somente
					// quando
					// houver
					// reload
					// da
					// página!
					this.reportarErros(httpServletRequest, "atencao.sem_setor_quadra_disponivel");
				}
			}
		...
		// Se não há setores comerciais a serem selecionados, não pode existir Setores selecionados
		// ou Coleção de Quadras
		if(colecaoSetorComercial == null || colecaoSetorComercial.isEmpty()){
			sessao.setAttribute("colecaoSetorComercial", new ArrayList());
		}
		...
		if(setoresSelecionados != null && setoresSelecionados.length > 0){
			colecaoSetoresSelecionados.clear();
			for(int i = 0; i < setoresSelecionados.length; i++){
				FiltroSetorComercial filtroSetorComercial = new FiltroSetorComercial();
				filtroSetorComercial.adicionarParametro(new ParametroSimples(FiltroSetorComercial.ID, setoresSelecionados[i]));

				Collection setor = fachada.pesquisar(filtroSetorComercial, SetorComercial.class.getName());
				colecaoSetoresSelecionados.add((SetorComercial) setor.iterator().next());
			}
		...
		if(idFaturamentoGrupo != null && !idFaturamentoGrupo.trim().equalsIgnoreCase("") && colecaoSetoresSelecionados.size() > 0
			...
			Integer intFaturGrupo = new Integer(idFaturamentoGrupo);
			// Retorna quadras
			...
			quadras.addAll(fachada.pesquisarQuadrasPorSetorComercialFaturamentoGrupo(Integer.parseInt(idLocalidade), idsSetores,
							intFaturGrupo));
		...
		// }
		if(colecaoSetoresSelecionados.isEmpty() || quadras == null || quadras.isEmpty()){
			httpServletRequest.setAttribute("colecaoQuadras", new ArrayList());
		}
		...
		if(idLocalidadeDigitado != null && !idLocalidadeDigitado.equalsIgnoreCase("")){
			...
			if(Util.validarValorNaoNumerico(idLocalidadeDigitado)){
				...
				verificaExistenciaCodLocalidade(idLocalidadeDigitado, form, httpServletRequest, fachada);
			}
		}
		...
		if(idDigitadoEnterLeiturista != null && !idDigitadoEnterLeiturista.equalsIgnoreCase("")
			...
			verificaExistenciaCodLeiturista(idDigitadoEnterLeiturista, form, httpServletRequest, fachada);
		}
	...
	private void exibirRoteiroEmpresa(String idRoteiroEmpresa, InserirRoteiroEmpresaActionForm form, Fachada fachada,
		...
		if(collectionRoteiro != null && !collectionRoteiro.isEmpty()){
			...
			this.carregarQuadrasRoteiroEmpresa(idRoteiroEmpresa, form, fachada, httpServletRequest);
			...
			int i = 0;
			for(Quadra quadra : quadras){ // Caso tenha mudado o grupo de Faturamento ou a
				// Localidade no form
				// Atributos nulos no form indicam carregamento inicial
				if((form.getFaturamentoGrupo() == null || form.getIdLocalidade() == null)){
					setoresSelecionados.add(quadra.getSetorComercial().getId() + "");
				}else if((form.getFaturamentoGrupo() != null || form.getIdLocalidade() != null)
								&& (form.getFaturamentoGrupo().equals(quadra.getRota().getFaturamentoGrupo().getId().toString()))
								&& (form.getIdLocalidade().equals(quadra.getSetorComercial().getLocalidade().getId().toString()))){
					setoresSelecionados.add(quadra.getSetorComercial().getId() + "");
				}
			...
			// pesquisarSetoresComerciais(quadra.getSetorComercial().getLocalidade().getId(),
	...
	/**
	 * Método que carrega as Quadras Existentes (já persistidas) para um determinado roteiro de
	 * Empresa
	 * 
	 ...
	 */
	private void carregarQuadrasRoteiroEmpresa(String idRoteiroEmpresa, InserirRoteiroEmpresaActionForm form, Fachada fachada,
					HttpServletRequest httpServletRequest){
		...
		if(collectionRoteiro != null && !collectionRoteiro.isEmpty()){
			...
			Collection quadrasSelecao = new ArrayList(); // Quadras que ficarâo disponíveis para
			// seleção
			String[] quadrasSelecionadas = new String[quadras.size()];
			int i = 0;
			for(Quadra quadra : quadras){ // Caso tenha mudado o grupo de Faturamento ou a
				// Localidade no form
				// Atributos nulos ou vazios no form indicam carregamento inicial
				if((form.getFaturamentoGrupo() == null || form.getFaturamentoGrupo().equalsIgnoreCase("") || form.getIdLocalidade() == null || form
								.getIdLocalidade().equalsIgnoreCase(""))){
					quadrasSelecionadas[i++] = quadra.getId() + "";
					quadrasSelecao.add(quadra);
				}else if((form.getFaturamentoGrupo() != null || form.getIdLocalidade() != null)
								&& (form.getFaturamentoGrupo().equals(quadra.getRota().getFaturamentoGrupo().getId().toString()))
								&& (form.getIdLocalidade().equals(quadra.getSetorComercial().getLocalidade().getId().toString()))){
					quadrasSelecionadas[i++] = quadra.getId() + "";
			...
			httpServletRequest.setAttribute("colecaoQuadras", quadrasSelecao);
	...
	private void verificaExistenciaCodLocalidade(String idDigitadoEnterLocalidade, InserirRoteiroEmpresaActionForm form,
	...
	private void pesquisarSetoresComerciais(int idLocalidade, Fachada fachada, HttpServletRequest request){
		...
		if(colecaoSetorComercial != null && !colecaoSetorComercial.isEmpty()){
			...
			request.setAttribute("colecaoSetoresSelecionados", new ArrayList());
	...
	private void verificaExistenciaCodLeiturista(String idDigitadoEnterLeiturista,
					InserirRoteiroEmpresaActionForm inserirRoteiroEmpresaActionForm, HttpServletRequest httpServletRequest, Fachada fachada){

		// Verifica se o código do Leiturista foi digitado
		if(idDigitadoEnterLeiturista != null && !idDigitadoEnterLeiturista.trim().equals("")
			...
			filtroLeiturista.adicionarParametro(new ParametroSimples(FiltroLeiturista.ID, idDigitadoEnterLeiturista));
			filtroLeiturista
							.adicionarParametro(new ParametroSimples(FiltroLeiturista.INDICADOR_USO, ConstantesSistema.INDICADOR_USO_ATIVO));

			Collection leituristaEncontrado = fachada.pesquisar(filtroLeiturista, Leiturista.class.getName());
			...
			if(leituristaEncontrado != null && leituristaEncontrado.size() > 0){
				...
				inserirRoteiroEmpresaActionForm.setIdLeiturista("" + leiturista.getId());
				...
				inserirRoteiroEmpresaActionForm.setNomeLeiturista("AGENTE COMERCIAL INEXISTENTE");
				httpServletRequest.setAttribute("nomeCampo", "idLeiturista");
			}
		}
