 ...
 */
public class ExibirInformarLeituraFiscalizacaoAction
				extends GcomAction {
	...
	public ActionForward execute(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest httpServletRequest,
		...
		ActionForward retorno = actionMapping.findForward("informarLeituraFiscalizacao");
		...
		if(httpServletRequest.getAttribute("idMedicaoTipo") != null){
			idMedicaoTipo = (String) httpServletRequest.getAttribute("idMedicaoTipo");
		}else{
			idMedicaoTipo = form.getMedicaoTipo();
		}

		filtroMedicaoHistorico.adicionarParametro(new ParametroSimples(FiltroMedicaoHistorico.MEDICAO_TIPO_ID, idMedicaoTipo));
		...
		Imovel imovel = this.pesquisarImovel(httpServletRequest, form, matricula, filtroMedicaoHistorico, sessao);
		...
		if(httpServletRequest.getAttribute("mesAnoReferencia") != null){
			mesAnoReferencia = (String) httpServletRequest.getAttribute("mesAnoReferencia");
		}else{
		...
		if(mesAnoValido == false){
			...
			throw new ActionServletException("atencao.mes_ano_referencia_faturamento_invalido", null, mesAnoReferencia);
		}
		...
		if((new Integer(anoMesReferencia)).intValue() > sistemaParametro.getAnoMesFaturamento().intValue()){

			throw new ActionServletException("atencao.mes_ano_referencia_faturamento_posterior_ano_mes_faturamento_sistema_parametro",
		...
		filtroMedicaoHistorico.adicionarCaminhoParaCarregamentoEntidade("leituraFiscalizacao");
		...
		Collection colecaoMedicaoHistorico = fachada.pesquisar(filtroMedicaoHistorico, MedicaoHistorico.class.getName());
		...
		if(colecaoMedicaoHistorico != null && !colecaoMedicaoHistorico.isEmpty()){
			...
			if(medicaoHistorico.getMedicaoTipo().getId().intValue() == MedicaoTipo.LIGACAO_AGUA.intValue()){
				...
				if(ligacaoAgua.getHidrometroInstalacaoHistorico() == null){
					...
					sessao.setAttribute("numeroDigitosLeitura", ligacaoAgua.getHidrometroInstalacaoHistorico().getHidrometro()
									.getNumeroDigitosLeitura());
				}

				if(medicaoHistorico.getLeituraAnormalidadeInformada() != null){

					form.setAnormalidadeNormal("" + medicaoHistorico.getLeituraAnormalidadeInformada().getDescricao());
				}else{
					form.setAnormalidadeNormal("");
				}

				form.setDataLeituraNormal("" + Util.formatarData(medicaoHistorico.getDataLeituraAtualInformada()));

				if(medicaoHistorico.getLeituraAtualInformada() != null){

					form.setLeituraNormal("" + medicaoHistorico.getLeituraAtualInformada());
				}else{
					form.setLeituraNormal("");
				}
				if(medicaoHistorico.getFuncionario() != null){
					form.setMatriculaLeituristaNormal("" + medicaoHistorico.getFuncionario().getId());
				}else{
				 ...
				 * Caso 2
				 */
				if((medicaoHistorico.getMedicaoTipo().getId().intValue() == MedicaoTipo.POCO.intValue())
								&& (imovel.getHidrometroInstalacaoHistorico() == null)){

					throw new ActionServletException("atencao.fiscalizao_poco_sem_hidrometro");

				}else{
					sessao.setAttribute("numeroDigitosLeitura", imovel.getHidrometroInstalacaoHistorico().getHidrometro()
									.getNumeroDigitosLeitura());
				}

				if(medicaoHistorico.getLeituraAnormalidadeInformada() != null) form.setAnormalidadeNormal(""
				...
				if(medicaoHistorico.getNumeroConsumoInformado() != null) form.setLeituraNormal(""
								+ medicaoHistorico.getNumeroConsumoInformado());
				else form.setLeituraNormal("");

				if(medicaoHistorico.getFuncionario() != null){
					if(medicaoHistorico.getFuncionario().getId().intValue() != 0) form.setMatriculaLeituristaNormal(""
									+ medicaoHistorico.getFuncionario().getId());
					else form.setMatriculaLeituristaNormal("");
				}
			}

			FiltroLeituraFiscalizacao filtroLeituraFiscalizacao = new FiltroLeituraFiscalizacao();

			filtroLeituraFiscalizacao.adicionarParametro(new ParametroSimples(FiltroLeituraFiscalizacao.ID, medicaoHistorico.getId()
							.toString()));
			Collection colecaoLeituraFiscalizacao = fachada.pesquisar(filtroLeituraFiscalizacao, LeituraFiscalizacao.class.getName());

			if(colecaoLeituraFiscalizacao != null && !colecaoLeituraFiscalizacao.isEmpty()){

				LeituraFiscalizacao leituraFiscalizacao = (LeituraFiscalizacao) colecaoLeituraFiscalizacao.iterator().next();

				form.setMatriculaLeituristaFiscalizacao(leituraFiscalizacao.getFuncionario().getId().toString());

				form.setDataLeituraFiscalizacao(Util.formatarData(leituraFiscalizacao.getdataLeituraEmpresa()));

				form.setAnormalidadeFiscalizacao(leituraFiscalizacao.getLeituraAnormalidade().getId().toString());

				form.setLeituraFiscalizacao("" + leituraFiscalizacao.getNumeroLeituraEmpresa());
			 ...
			 */
			throw new ActionServletException("atencao.imovel_sem_dados_faturamento", null, mesAnoReferencia);
		}
	...
	public void carregarCombos(HttpServletRequest httpServletRequest, InformarLeituraFiscalizacaoActionForm form){
		...
		colecaoMedicaoTipo = Fachada.getInstancia().pesquisar(filtroMedicaoTipo, MedicaoTipo.class.getName());

		if(colecaoMedicaoTipo == null || colecaoMedicaoTipo.isEmpty()){
			throw new ActionServletException("atencao.entidade_sem_dados_para_selecao", null, "MedicaoTipo");
		}

		httpServletRequest.setAttribute("colecaoMedicaoTipo", colecaoMedicaoTipo);
		...
		colecaoLeituraAnormalidade = Fachada.getInstancia().pesquisar(filtroLeituraAnormalidade, LeituraAnormalidade.class.getName());

		if(colecaoLeituraAnormalidade == null || colecaoLeituraAnormalidade.isEmpty()){
			throw new ActionServletException("atencao.entidade_sem_dados_para_selecao", null, "LeituraAnormalidade");
		}
	...
	public Imovel pesquisarImovel(HttpServletRequest httpServletRequest, InformarLeituraFiscalizacaoActionForm form, String matricula,
	...
	public void pesquisarDadosLeituraFiscalizacao(InformarLeituraFiscalizacaoActionForm form, MedicaoHistorico medicaoHistorico){

		FiltroLeituraFiscalizacao filtroLeituraFiscalizacao = new FiltroLeituraFiscalizacao();

		filtroLeituraFiscalizacao.adicionarParametro(new ParametroSimples(FiltroLeituraFiscalizacao.MEDICAO_HISTORICO_ID, medicaoHistorico
						.getId()));

		filtroLeituraFiscalizacao.adicionarCaminhoParaCarregamentoEntidade(FiltroLeituraFiscalizacao.FUNCIONARIO);

		filtroLeituraFiscalizacao.adicionarCaminhoParaCarregamentoEntidade(FiltroLeituraFiscalizacao.LEITURA_ANORMALIDADE);

		Collection colecaoLeituraFiscalizacao = Fachada.getInstancia().pesquisar(filtroLeituraFiscalizacao,
		...
		if(colecaoLeituraFiscalizacao != null && !colecaoLeituraFiscalizacao.isEmpty()){
			...
			form.setAnormalidadeFiscalizacao("" + leituraFiscalizacao.getLeituraAnormalidade().getDescricao());

			form.setDataLeituraFiscalizacao("" + Util.formatarData(leituraFiscalizacao.getdataLeituraEmpresa()));

			form.setLeituraFiscalizacao("" + leituraFiscalizacao.getNumeroLeituraEmpresa());

			form.setMatriculaLeituristaFiscalizacao("" + leituraFiscalizacao.getFuncionario().getId());
