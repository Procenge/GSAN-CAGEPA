 ...
 */
public class InformarLeituraFiscalizacaoAction
				extends GcomAction {
	...
	public ActionForward execute(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest httpServletRequest,
		...
		String matriculaLeituristaFiscalizacao = form.getMatriculaLeituristaFiscalizacao();

		LeituraFiscalizacao leituraFiscalizacao = null;

		if(sessao.getAttribute("leituraFiscalizacao") != null){

			leituraFiscalizacao = (LeituraFiscalizacao) sessao.getAttribute("leituraFiscalizacao");
		...
		filtroFuncionario.adicionarParametro(new ParametroSimples(FiltroFuncionario.ID, matriculaLeituristaFiscalizacao));
		 ...
		 */
		Short numeroDigitosLeitura = (Short) sessao.getAttribute("numeroDigitosLeitura");
		...
		if(!((Util.compararAnoMesReferencia(anoMesReferenciaInt, anoMesInteiro, "=")) || (Util.compararAnoMesReferencia(
						anoMesReferenciaInt + 1, anoMesInteiro, "=")))){

			throw new ActionServletException("atencao.data_leitura_incompativel_ciclo_faturamento");

		}

		leituraFiscalizacao.setdataLeituraEmpresa(Util.converteStringParaDate(dataLeituraFiscalizacao));

		LeituraAnormalidade leituraAnormalidade = new LeituraAnormalidade();

		if(!form.getAnormalidadeFiscalizacao().equalsIgnoreCase("-1")){
			leituraAnormalidade.setId(new Integer(form.getAnormalidadeFiscalizacao()));
		}else{
		...
		if(!form.getLeituraFiscalizacao().equalsIgnoreCase("")){
			...
			Integer numeroLeituraEmpresa = new Integer(form.getLeituraFiscalizacao());

			leituraFiscalizacao.setNumeroLeituraEmpresa(numeroLeituraEmpresa.intValue());

		}

		fachada.informarLeituraFiscalizacao(usuarioLogado, leituraFiscalizacao);

		montarPaginaSucesso(httpServletRequest, "Leitura de Fiscalização para o imóvel " + matricula + " informada com sucesso.",
						"Informar outra Fiscalização", "exibirFiltrarInformarLeituraFiscalizacaoAction.do?menu=sim");
