...
public class PesquisarLeituristaAction
				extends GcomAction {
	...
	public ActionForward execute(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest httpServletRequest,
		...
		if(idEmpresa != null && !idEmpresa.trim().equalsIgnoreCase("")
			...
			filtroLeiturista.adicionarParametro(new ParametroSimples(FiltroLeiturista.EMPRESA, new Integer(idEmpresa)));
			peloMenosUmParametroInformado = true;
		}
		if(idFuncionario != null && !idFuncionario.trim().equalsIgnoreCase("")){
			filtroLeiturista.adicionarParametro(new ParametroSimples(FiltroLeiturista.FUNCIONARIO, new Integer(idFuncionario)));
			peloMenosUmParametroInformado = true;
		}
		if(idCliente != null && !idCliente.trim().equalsIgnoreCase("")){
			filtroLeiturista.adicionarParametro(new ParametroSimples(FiltroLeiturista.CLIENTE, new Integer(idCliente)));
			peloMenosUmParametroInformado = true;
		}
		if(ddd != null && !ddd.trim().equalsIgnoreCase("")){
			filtroLeiturista.adicionarParametro(new ParametroSimples(FiltroLeiturista.DDD, new Integer(ddd)));
			peloMenosUmParametroInformado = true;
		}
		if(numeroTelefone != null && !numeroTelefone.trim().equalsIgnoreCase("")){
			filtroLeiturista.adicionarParametro(new ParametroSimples(FiltroLeiturista.TELEFONE, new Integer(numeroTelefone)));
			peloMenosUmParametroInformado = true;
		}
		...
		Collection leituristas = fachada.pesquisar(filtroLeiturista, Leiturista.class.getName());

		if(leituristas != null && !leituristas.isEmpty()){
			// Aciona o controle de paginação para que sejam pesquisados apenas
			// os registros que aparecem na página
			Map resultado = controlarPaginacao(httpServletRequest, retorno, filtroLeiturista, Leiturista.class.getName());
			leituristas = (Collection) resultado.get("colecaoRetorno");
			retorno = (ActionForward) resultado.get("destinoActionForward");

			sessao.setAttribute("leituristas", leituristas);
		}else{
			throw new ActionServletException("atencao.pesquisa.nenhumresultado", null, "Leiturista");
		}
