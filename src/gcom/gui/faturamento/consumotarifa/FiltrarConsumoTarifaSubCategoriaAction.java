 ...
 */
public class FiltrarConsumoTarifaSubCategoriaAction
				extends GcomAction {

	public ActionForward execute(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest httpServletRequest,
					HttpServletResponse httpServletResponse){

		ActionForward retorno = actionMapping.findForward("exibirManterConsumoTarifaSubCategoria");
		...
		if((dataVigenciaInicial.trim().length() == 10) && (dataVigenciaFinal.trim().length() == 10)){

			Calendar calendarInicio = new GregorianCalendar();
			Calendar calendarFim = new GregorianCalendar();

			calendarInicio.set(Calendar.DAY_OF_MONTH, new Integer(dataVigenciaInicial.substring(0, 2)).intValue());
			calendarInicio.set(Calendar.MONTH, new Integer(dataVigenciaInicial.substring(3, 5)).intValue());
			calendarInicio.set(Calendar.YEAR, new Integer(dataVigenciaInicial.substring(6, 10)).intValue());

			calendarFim.set(Calendar.DAY_OF_MONTH, new Integer(dataVigenciaFinal.substring(0, 2)).intValue());
			calendarFim.set(Calendar.MONTH, new Integer(dataVigenciaFinal.substring(3, 5)).intValue());
			calendarFim.set(Calendar.YEAR, new Integer(dataVigenciaFinal.substring(6, 10)).intValue());
			// joga exessão
		 ...
		 * .adicionarCaminhoParaCarregamentoEntidade(FiltroConsumoTarifaVigencia.
		 * CONSUMO_TARIFA_DESCRICAO);
		 */
		...
		// Descricao da Tarifa
		if(descTarifa != null && !descTarifa.trim().equalsIgnoreCase("")){
		...
		if(dataVigenciaInicial != null && !dataVigenciaInicial.trim().equalsIgnoreCase("")){
			...
			filtroConsumoTarifaVigencia.adicionarParametro(new MaiorQue(FiltroConsumoTarifaVigencia.DATA_VIGENCIA, dataInicial));
			parametroPesquisaInformando = true;
		}

		// data de Vigencia final
		if(dataVigenciaFinal != null && !dataVigenciaFinal.trim().equalsIgnoreCase("")){
			SimpleDateFormat formatoData = new SimpleDateFormat("dd/MM/yyyy");
			Date dataFinal = null;
			...
			filtroConsumoTarifaVigencia.adicionarParametro(new MenorQue(FiltroConsumoTarifaVigencia.DATA_VIGENCIA, dataFinal));
			parametroPesquisaInformando = true;
		}
		...
		filtroConsumoTarifaVigencia.setCampoOrderBy(FiltroConsumoTarifaVigencia.CONSUMO_TARIFA_DESCRICAO,
						FiltroConsumoTarifaVigencia.DATA_VIGENCIA);

		Map resultado = controlarPaginacao(httpServletRequest, retorno, filtroConsumoTarifaVigencia, ConsumoTarifaVigencia.class.getName());
		...
		if(colecaoFiltroConsumoTarifaVigencia != null && !colecaoFiltroConsumoTarifaVigencia.isEmpty()){
			...
			if(colecaoFiltroConsumoTarifaVigencia.size() == 1){
				if(atualizar != null){
					...
					retorno = actionMapping.findForward("atualizarConsumoTarifaVigenciaSubCategoria");

					ConsumoTarifaVigencia consumoTarifaVigencia = (ConsumoTarifaVigencia) colecaoFiltroConsumoTarifaVigencia.iterator()
									.next();
