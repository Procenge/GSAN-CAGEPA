...
public class AtualizarComandoAtividadeFaturamentoDataVencimentoAction
				extends GcomAction {
	...
	public ActionForward execute(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest httpServletRequest,
		...
		// (FaturamentoAtividadeCronograma) sessao
		...
		if(sessao.getAttribute("exibirCampoVencimentoGrupo") != null){
			...
			Iterator colecaoFaturamentoAtividadeCronogramaRotaUniaoIterator = colecaoFaturamentoAtividadeCronogramaRotaUniao.iterator();
			...
			while(colecaoFaturamentoAtividadeCronogramaRotaUniaoIterator.hasNext()){
				faturamentoAtivCronRota = (FaturamentoAtivCronRota) colecaoFaturamentoAtividadeCronogramaRotaUniaoIterator.next();

				// Formatação do nome do parâmetro
				descricaoParametro = "d" + faturamentoAtivCronRota.getRota().getId().toString();

				// Data informada pelo usuário
				dataVencimentoRotaJSP = (String) httpServletRequest.getParameter(descricaoParametro);

				if(dataVencimentoRotaJSP != null && !dataVencimentoRotaJSP.equalsIgnoreCase("")){

					try{
						dataVencimentoRota = formatoData.parse(dataVencimentoRotaJSP);

						mesDataVencimentoGrupo = dataCorrenteString.substring(3, 5);
						anoDataVencimentoGrupo = dataCorrenteString.substring(6, 10);

						mesDataVencimentoRota = dataVencimentoRotaJSP.substring(3, 5);
						anoDataVencimentoRota = dataVencimentoRotaJSP.substring(6, 10);

						if(dataCorrenteDate.after(dataVencimentoRota)){
							throw new ActionServletException("atencao.faturamento_data_rota_grupo_menor", null, dataCorrenteString);
						}else if((!mesDataVencimentoGrupo.equalsIgnoreCase(mesDataVencimentoRota))
										|| (!anoDataVencimentoGrupo.equalsIgnoreCase(anoDataVencimentoRota))){
							throw new ActionServletException("atencao.faturamento_data_rota_mes_ano_diferente");
						}
					}catch(ParseException ex){
						dataVencimentoRota = null;
					}

					faturamentoAtivCronRota.setDataContaVencimento(dataVencimentoRota);
				}
			}
		}
