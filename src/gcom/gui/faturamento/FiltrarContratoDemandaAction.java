...
public class FiltrarContratoDemandaAction
				extends GcomAction {
	...
	public ActionForward execute(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest httpServletRequest,
		...
		ActionForward retorno = actionMapping.findForward("exibirManterContratoDemanda");
		...
		String numeroContrato = filtrarContratoDemandaActionForm.getNumeroContrato();
		String idImovel = filtrarContratoDemandaActionForm.getIdImovel();
		String dataInicioContrato = filtrarContratoDemandaActionForm.getDataInicioContrato();
		String dataFimContrato = filtrarContratoDemandaActionForm.getDataFimContrato();
		...
		if(numeroContrato != null && !numeroContrato.trim().equalsIgnoreCase("")){
			...
			filtroContratoDemanda.adicionarParametro(new ComparacaoTexto(FiltroContratoDemanda.MUMEROCONTRATO, numeroContrato));
			...
			if(colecaoContratoDemanda == null || colecaoContratoDemanda.isEmpty()){
				throw new ActionServletException("atencao.pesquisa_inexistente", null, "Contrato de Demanda");
			}
		}
		...
		if(idImovel != null && !idImovel.trim().equals("")){
			...
			if(imovel == null){
				...
				peloMenosUmParametroInformado = true;
				filtroContratoDemanda.adicionarParametro(new ParametroSimples(FiltroContratoDemanda.IMOVEL, idImovel));
			}

		}

		if(dataInicioContrato != null && !dataInicioContrato.trim().equalsIgnoreCase("")){
			peloMenosUmParametroInformado = true;

			Date dataInicioFormatada = Util.converteStringParaDate(dataInicioContrato);

			filtroContratoDemanda.adicionarParametro(new MaiorQue(FiltroContratoDemanda.DATACONTRATOINICIO, dataInicioFormatada));
		}

		if(dataFimContrato != null && !dataFimContrato.trim().equalsIgnoreCase("")){
			peloMenosUmParametroInformado = true;

			Date dataFimFormatada = Util.converteStringParaDate(dataFimContrato);

			filtroContratoDemanda.adicionarParametro(new MenorQue(FiltroContratoDemanda.DATACONTRATOINICIO, dataFimFormatada));
		}
