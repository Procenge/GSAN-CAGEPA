...
public class AtualizarComandoAtividadeFaturamentoAction
				extends GcomAction {
	...
	public ActionForward execute(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest httpServletRequest,
		...
		if(colecaoFaturamentoAtividadeCronogramaRotaUniao == null || colecaoFaturamentoAtividadeCronogramaRotaUniao.isEmpty()){
		...
		Iterator colecaoFaturamentoAtividadeCronogramaRotaUniaoIt = colecaoFaturamentoAtividadeCronogramaRotaUniao.iterator();
		...
		while(colecaoFaturamentoAtividadeCronogramaRotaUniaoIt.hasNext()){
			...
			if(requestMap.get("data" + faturamentoAtivCronRota.getRota().getId().intValue()) != null){

				dataVencimentoRota = (requestMap.get("data" + faturamentoAtivCronRota.getRota().getId().intValue()))[0];

				if(dataVencimentoRota == null || dataVencimentoRota.equalsIgnoreCase("")){
					throw new ActionServletException("atencao.campo_texto.obrigatorio", null, "Data de vencimento da rota ");
				}

				try{
					if(!Util.isVazioOuBranco(dataVencimentoRota)){
				...
				if(dataVencimentoRotaJSPObjeto != null){
		...
		// Atualizar comando
		fachada.atualizarComandoAtividadeFaturamento(faturamentoAtividadeCronograma, colecaoFaturamentoAtividadeCronogramaRotaUniao,
						getUsuarioLogado(httpServletRequest));

		montarPaginaSucesso(httpServletRequest, "Comando de Atividade de Faturamento "
						...
						+ " referência "
						+ Util.formatarAnoMesParaMesAno(faturamentoAtividadeCronograma.getFaturamentoGrupoCronogramaMensal()
										.getFaturamentoGrupo().getAnoMesReferencia()) + " atualizado com sucesso.",
						"Realizar outra Manutenção de Comando de Atividade de Faturamento", "filtrarComandoAtividadeFaturamentoAction.do");
