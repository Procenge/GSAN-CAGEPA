 ...
 */
public class FiltrarMensagemContaAction
				extends GcomAction {
	...
	public ActionForward execute(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest httpServletRequest,
		...
		String referenciaFaturamento = filtrarMensagemContaActionForm.getReferenciaFaturamento();
		String atualizar = httpServletRequest.getParameter("atualizar");
		String mensagemConta01 = filtrarMensagemContaActionForm.getMensagemConta01();
		/*
		 * String mensagemConta02 = filtrarMensagemContaActionForm
		 * .getMensagemConta02();
		 * String mensagemConta03 = filtrarMensagemContaActionForm
		 * .getMensagemConta03();
		 */String grupoFaturamento = filtrarMensagemContaActionForm.getGrupoFaturamento();
		String gerenciaRegional = filtrarMensagemContaActionForm.getGerenciaRegional();
		String localidade = filtrarMensagemContaActionForm.getLocalidade();
		String setorComercial = filtrarMensagemContaActionForm.getSetorComercial();

		if((referenciaFaturamento == null || referenciaFaturamento.equalsIgnoreCase(""))
						&& (grupoFaturamento == null || grupoFaturamento.equalsIgnoreCase(""))
						&& (mensagemConta01 == null || mensagemConta01.equalsIgnoreCase(""))
						&& (gerenciaRegional == null || gerenciaRegional.equalsIgnoreCase(""))
						&& (localidade == null || localidade.equalsIgnoreCase(""))
						&& (setorComercial == null || setorComercial.equalsIgnoreCase(""))){
		...
		if(grupoFaturamento != null && !grupoFaturamento.equalsIgnoreCase("")){
			filtroContaMensagem.adicionarParametro(new ParametroSimples(FiltroContaMensagem.GRUPO_FATURAMENTO_ID, grupoFaturamento));

		}

		if(mensagemConta01 != null && !mensagemConta01.equalsIgnoreCase("")){
			filtroContaMensagem.adicionarParametro(new ComparacaoTexto(FiltroContaMensagem.MENSAGEM_CONTA_01, mensagemConta01));
		}
		...
		if(gerenciaRegional != null && !gerenciaRegional.equalsIgnoreCase("")){
			filtroContaMensagem.adicionarParametro(new ParametroSimples(FiltroContaMensagem.GERENCIA_REGIONAL_ID, gerenciaRegional));

		}

		if(localidade != null && !localidade.equalsIgnoreCase("")){
			filtroContaMensagem.adicionarParametro(new ParametroSimples(FiltroContaMensagem.LOCALIDADE_ID, localidade));

		}

		if(setorComercial != null && !setorComercial.equalsIgnoreCase("")){
			filtroContaMensagem.adicionarParametro(new ParametroSimples(FiltroContaMensagem.SETOR_COMERCIAL_CD, setorComercial));
		...
		Collection<ContaMensagem> colecaoContaMensagem = (Collection<ContaMensagem>) fachada.pesquisar(filtroContaMensagem,
		...
		if(colecaoContaMensagem != null && !colecaoContaMensagem.isEmpty()){
			if(colecaoContaMensagem.size() == 1 && atualizar != null){
				httpServletRequest.setAttribute("contaMensagemID", colecaoContaMensagem.iterator().next().getId());
				retorno = actionMapping.findForward("exibirAtualizarMensagemConta");
			}else{
				retorno = actionMapping.findForward("exibirManterMensagemConta");
			}
		}else{
