...
public class AdicionarDebitoCobradoContaAction
				extends GcomAction {
	...
	public ActionForward execute(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest httpServletRequest,
		...
		ActionForward retorno = actionMapping.findForward("exibirAdicionarDebitoCobradoConta");
		...
		AdicionarDebitoCobradoContaActionForm adicionarDebitoCobradoContaActionForm = (AdicionarDebitoCobradoContaActionForm) actionForm;
		...
		if(mesAnoDebito != null && !mesAnoDebito.equalsIgnoreCase("")){
			...
			// [FS0004] - Verifica ano e mês do faturamento
				...
				if(colecaoImovel == null || colecaoImovel.isEmpty()){
					throw new ActionServletException("atencao.adicionar_debito_ano_mes_debito_invalido");
				}else{
					...
					// if(!compararAnoMesReferencia(mesAnoFaturamentoImovel, new
					// Integer(mesAnoDebito), ">")){
					// throw new
					// ActionServletException("atencao.adicionar_debito_ano_mes_debito_invalido");
					// }else{

					if(compararAnoMesReferencia(new Integer(mesAnoDebito), mesAnoFaturamentoImovel, ">")){
						throw new ActionServletException("atencao.adicionar_debito_ano_mes_debito_invalido");
					}else{
		...
		// Caso a referência do débito não seja informada, atribuir o valor do
		// parâmetro da Referência do Faturamento ((PARM_AMREFERENCIAFATURAMENTO da tabela
		// SISTEMA_PARAMETROS).
		else{
			SistemaParametro sistemaParametro = fachada.pesquisarParametrosDoSistema();

			Integer amReferenciaFaturamento = sistemaParametro.getAnoMesFaturamento();

			objDebitoCobrado.setAnoMesReferenciaDebito(amReferenciaFaturamento);

			adicionarDebitoCobradoContaActionForm.setMesAnoDebito(Util.formatarAnoMesParaMesAno(amReferenciaFaturamento));
		}
		...
		// Caso a referência da cobrança do débito não seja informada, atribuir o valor do
		// parâmetro da Referência do Faturamento ((PARM_AMREFERENCIAFATURAMENTO da tabela
		// SISTEMA_PARAMETROS).
		else{
			SistemaParametro sistemaParametro = fachada.pesquisarParametrosDoSistema();
			...
			objDebitoCobrado.setAnoMesCobrancaDebito(amReferenciaFaturamento);

			adicionarDebitoCobradoContaActionForm.setMesAnoCobranca(Util.formatarAnoMesParaMesAno(amReferenciaFaturamento));
		}
		...
		filtroDebitoTipo.adicionarParametro(new ParametroSimples(FiltroDebitoTipo.ID, debitoTipoID));
