 ...
 * @date 02/03/2006
 */
public class PesquisarContaAction
				extends GcomAction {
	...
	public ActionForward execute(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest httpServletRequest,
		...
		// Caso o usuário informou o código do imóvel, pesquisa todas as contas relacionadas com o
		// imóvel
		...
		if(idImovel != null && !idImovel.trim().equalsIgnoreCase("")){
			...
			// Seta no form de pesquisar conta todos os dados de imóvel para exibição na página de
		...
		// Caso o usuário tenha informado a referência inicial da conta, pesquisa as contas entre a
		// referência inicial e final, replicando
		// a referência inicial na final caso essa não tenha sido informada
		// Caso contrário seta a referência final para vazio, e pesquisa as contas para todas as
		...
		if(referenciaContaInicial != null && !referenciaContaInicial.trim().equalsIgnoreCase("")){
			...
			if(referenciaContaFinal == null || referenciaContaFinal.trim().equalsIgnoreCase("")){
				...
				// [FS0002]Caso a referência final da conta seja anterior a inicial, levanta a
				// exceção para o
		...
		if(referenciaContaFinal != null && !referenciaContaFinal.trim().equalsIgnoreCase("")){
			...
			// Indica no filtro para pesquisar as contas no intervalo entre a referência inicial e a
			// final
		...
		// Caso o usuário informou a data de emissão da conta inicial, pesquisa as contas do imóvel
		// referentes a data de
		...
		if(dataEmissaoContaInicialEmString != null && !dataEmissaoContaInicialEmString.toString().trim().equalsIgnoreCase("")){
			...
			// Caso a data final de emissão da conta não foi informada, replica a data inicial na
			// final
		...
		if(dataEmissaoContaFinalEmString != null && !dataEmissaoContaFinalEmString.toString().trim().equalsIgnoreCase("")){
			...
			// Indica no filtro para pesquisar as contas no intervalo entre a data de emissão
			// inicial e a final
		...
		// Caso o usuário informou a data de vencimento da conta inicial, pesquisa as contas do
		// imóvel referentes a data de
		...
		if(dataVencimentoContaInicialEmString != null && !dataVencimentoContaInicialEmString.toString().trim().equalsIgnoreCase("")){
			...
			// Caso a data final de vencimento da conta não foi informada, replica a data inicial na
			// final
			// Caso contrário, formata a data final
		...
		if(dataVencimentoContaFinalEmString != null && !dataVencimentoContaFinalEmString.toString().trim().equalsIgnoreCase("")){
			...
			// Indica no filtro para pesquisar as contas no intervalo entre a data de vencimento
			// inicial e a final
			filtroConta
							.adicionarParametro(new Intervalo(FiltroConta.DATA_VENCIMENTO, dataVencimentoContaInicial,
		...
		if(idSituacaoConta != null && idSituacaoConta.length > 0){
			...
			// Laço para incluir no filtro todas as situações de conta informadas pelo usuário para
			// pesquisa
				...
				if(!(new Integer(idSituacaoConta[i]).equals(new Integer(ConstantesSistema.NUMERO_NAO_INFORMADO)))){
					if(i == 0){
						filtroConta.adicionarParametro(new ParametroSimples(FiltroConta.DEBITO_CREDITO_SITUACAO_ATUAL_ID, idSituacaoConta[i],
										ParametroSimples.CONECTOR_OR, idSituacaoConta.length));
					}else{
						if(i == (idSituacaoConta.length - 1)){
							...
							filtroConta.adicionarParametro(new ParametroSimples(FiltroConta.DEBITO_CREDITO_SITUACAO_ATUAL_ID,
											idSituacaoConta[i], ParametroSimples.CONECTOR_OR));
						}
					}
				}
			}
			...
			filtroConta
							.adicionarParametro(new ParametroSimples(FiltroConta.DEBITO_CREDITO_SITUACAO_ATUAL_ID,
		...
		Collection colecaoContasImovel = null; // =
		// fachada.pesquisar(filtroConta,Conta.class.getName());

		Map resultado = controlarPaginacao(httpServletRequest, retorno, filtroConta, Conta.class.getName());
		...
		if(colecaoContasImovel == null || colecaoContasImovel.isEmpty()){
			...
			// Caso o nº de contas retornadas pela pesquisa seja maior que o nº máximo de registros
			// permitidos
