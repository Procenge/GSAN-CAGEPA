...
public class InserirComandoAtividadeFaturamentoAction
				extends GcomAction {
	...
	public ActionForward execute(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest httpServletRequest,
		...
		if(grupoFaturamentoJSP == null || grupoFaturamentoJSP.equalsIgnoreCase(String.valueOf(ConstantesSistema.NUMERO_NAO_INFORMADO))){
		...
		if(atividadeFaturamentoJSP == null
			...
			throw new ActionServletException("atencao.faturamento_atividade_nao_informado");
		}
		...
		Collection colecaoFaturamentoAtividade = fachada.pesquisar(filtroFaturamentoAtividade, FaturamentoAtividade.class.getName());

		FaturamentoAtividade faturamentoAtividade = (FaturamentoAtividade) Util.retonarObjetoDeColecao(colecaoFaturamentoAtividade);
		...
		if((faturamentoAtividade.getId().equals(FaturamentoAtividade.FATURAR_GRUPO) || faturamentoAtividade.getId().equals(
		...
		String dataVencimentoGrupoJSP = inserirComandoAtividadeFaturamentoActionForm.getVencimentoGrupo();

		Date dataVencimentoGrupo = null;

		// ########## COMENTADA PARA CONTINUAR COM OS TESTES DE FATURAMENTO IMEDIATO ########

		// if((faturamentoAtividade.getId().equals(FaturamentoAtividade.FATURAR_GRUPO) ||
		// (faturamentoAtividade.getId()
		// .equals(FaturamentoAtividade.GERAR_FATURAMENTO_IMEDIATO)))
		// && sessao.getAttribute("exibirCampoVencimentoGrupo") != null){
		if((faturamentoAtividade.getId().equals(FaturamentoAtividade.FATURAR_GRUPO) || faturamentoAtividade.getId().equals(
						FaturamentoAtividade.GERAR_ARQUIVO_LEITURA))
						&& sessao.getAttribute("exibirCampoVencimentoGrupo") != null){

			if(dataVencimentoGrupoJSP != null && !dataVencimentoGrupoJSP.equalsIgnoreCase("")){

				try{
					dataVencimentoGrupo = formatoData.parse(dataVencimentoGrupoJSP);
					if(dataCorrenteGrupo.after(dataVencimentoGrupo)){
						throw new ActionServletException("atencao.faturamento_data_vencimento_grupo_menor", null, dataCorrente);
					}
				}catch(ParseException ex){
		...
		Integer faturamentoAtividadeCronogramaId = fachada.inserirComandoAtividadeFaturamento(faturamentoGrupo, faturamentoAtividade,
						colecaoRotasSituacao, dataVencimentoGrupo, getUsuarioLogado(httpServletRequest));

		montarPaginaSucesso(httpServletRequest, "Comando da Atividade de Faturamento " + faturamentoAtividade.getDescricao() + " do "
						+ faturamentoGrupo.getDescricaoAbreviada() + " referência "
						+ Util.formatarAnoMesParaMesAno(faturamentoGrupo.getAnoMesReferencia()) + " inserido com sucesso.",
						"Inserir outro Comando de Atividade de Faturamento", "exibirInserirComandoAtividadeFaturamentoAction.do",
