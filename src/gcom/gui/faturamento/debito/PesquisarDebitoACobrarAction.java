 ...
 * @date 13/03/2006
 */
public class PesquisarDebitoACobrarAction
				extends GcomAction {
	...
	public ActionForward execute(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest httpServletRequest,
		...
		// Seta o mapeamento de retorno para a tela de resultado da pesquisa de débitos a cobrar do
		// imóvel
		...
		// Caso o usuário informou o código do imóvel, pesquisa todos os débitos a cobrar
		// relacionados com o imóvel
		// caso contrário indica que o usuário não informou o imóvel
		if(idImovel != null && !idImovel.trim().equalsIgnoreCase("")){

			// Indica que o usuário informou um parâmetro para pesquisar os débitos a cobrar do
			// imóvel
			// peloMenosUmParametroInformado = true;

			// Cria o fitro de imóvel, e seta no filtro quais objetos necessários para a pesquisa de
			// imóvel
			...
			// Seta no form de pesquisar débito a cobrar todos os dados de imóvel para exibição na
		...
		// Caso o usuário tenha informado a referência inicial do débito a cobrar, pesquisa os
		// débito a cobrar entre a referência inicial e final, replicando
		// a referência inicial na final caso essa não tenha sido informada
		// Caso contrário seta a referência final para vazio, e pesquisa os débito a cobrar para
		...
		if(referenciaDebitoInicial != null && !referenciaDebitoInicial.trim().equalsIgnoreCase("")){
			...
			referenciaDebitoInicial = Util.formatarMesAnoParaAnoMesSemBarra((String) pesquisarDebitoACobrarActionForm
							.getReferenciaDebitoInicial());
			...
			if(referenciaDebitoFinal == null || referenciaDebitoFinal.trim().equalsIgnoreCase("")){
				...
				referenciaDebitoFinal = Util.formatarMesAnoParaAnoMesSemBarra((String) pesquisarDebitoACobrarActionForm
								.getReferenciaDebitoFinal());
		...
		if(referenciaDebitoFinal != null && !referenciaDebitoFinal.trim().equalsIgnoreCase("")){
			...
			// Indica no filtro para pesquisar os débitos a cobrar no intervalo entre a referência
			// inicial e a final
			filtroDebitoACobrar.adicionarParametro(new MaiorQue(FiltroDebitoACobrar.REFERENCIA_DEBITO, referenciaDebitoInicial,
		...
		// Caso o usuário informou a data de geração do débito a cobrar inicial, pesquisa os débitos
		// a cobrar do imóvel referentes a data de
		...
		if(dataGeracaoDebitoInicialEmString != null && !dataGeracaoDebitoInicialEmString.toString().trim().equalsIgnoreCase("")){
			...
			try{
				...
				// dataGeracaoDebitoInicialFormatoTimeStamp = new
				// Timestamp(dataGeracaoDebitoInicial.getTime());
			}catch(ParseException e){
				throw new ActionServletException("atencao.datageracaoinicial.invalida");
			}

			// Caso a data final de geração do débito não foi informada, replica a data inicial na
			// final
			...
			if(dataGeracaoDebitoFinalEmString == null || dataGeracaoDebitoFinalEmString.trim().equalsIgnoreCase("")){
				...
				// dataGeracaoDebitoFinalFormatoTimeStamp =
				// dataGeracaoDebitoInicialFormatoTimeStamp;
			}else{
				...
				try{
					...
					// dataGeracaoDebitoFinalFormatoTimeStamp = new
					// Timestamp(dataGeracaoDebitoFinal.getTime());
				}catch(ParseException e){
				...
				// levanta a exceção para o usuário indicando que a data final é anterior a data
				// inicial
		...
		if(dataGeracaoDebitoFinalEmString != null && !dataGeracaoDebitoFinalEmString.toString().trim().equalsIgnoreCase("")){
			...
			// Indica no filtro para pesquisar os débitos a cobrar no intervalo entre a data de
			// geração inicial e a final
			filtroDebitoACobrar.adicionarParametro(new Intervalo(FiltroDebitoACobrar.GERACAO_DEBITO, new Timestamp(dataGeracaoDebitoInicial
							.getTime()), new Timestamp(dataGeracaoDebitoFinal.getTime())));
		}
			...
			for(int i = 0; i < idTipoDebito.length; i++){
				if(!(new Integer(idTipoDebito[i]).equals(new Integer(ConstantesSistema.NUMERO_NAO_INFORMADO)))){
					...
					if(idTipoDebito.length == 1){
						...
						if(i == 0){
							filtroDebitoACobrar.adicionarParametro(new ParametroSimples(FiltroDebitoACobrar.DEBITO_TIPO_ID,
											idTipoDebito[i], ParametroSimples.CONECTOR_OR, idTipoDebito.length));
						}else{
		...
		if(idSituacaoDebitoACobrar != null && !idSituacaoDebitoACobrar[0].equals("" + ConstantesSistema.NUMERO_NAO_INFORMADO)
						&& idSituacaoDebitoACobrar.length > 0){
			// Indica que o usuário informou um parâmetro para pesquisar os débitos a cobrar do
			// imóvel
			peloMenosUmParametroInformado = true;

			// Laço para incluir no filtro todas as situações de débito a cobrar informadas pelo
				...
				if(!(new Integer(idSituacaoDebitoACobrar[i]).equals(new Integer(ConstantesSistema.NUMERO_NAO_INFORMADO)))){
					...
					if(idSituacaoDebitoACobrar.length == 1){
						filtroDebitoACobrar.adicionarParametro(new ParametroSimples(FiltroDebitoACobrar.DEBITO_CREDITO_SITUACAO_ATUAL_ID,
										idSituacaoDebitoACobrar[i]));
					}else{

						if(i == 0){
							filtroDebitoACobrar.adicionarParametro(new ParametroSimples(
											FiltroDebitoACobrar.DEBITO_CREDITO_SITUACAO_ATUAL_ID, idSituacaoDebitoACobrar[i],
											ParametroSimples.CONECTOR_OR, idSituacaoDebitoACobrar.length));
						}else{
		...
		Map resultado = controlarPaginacao(httpServletRequest, retorno, filtroDebitoACobrar, DebitoACobrar.class.getName());
		...
		if(colecaoDebitosACobrar == null || colecaoDebitosACobrar.isEmpty()){
			...
			// Caso o nº de débitos a cobrar retornados pela pesquisa seja maior que o nº máximo de
