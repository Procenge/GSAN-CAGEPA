 ...
 * @date 09/03/2006
 */
public class PesquisarTipoDebitoAction
				extends GcomAction {
	...
	public ActionForward execute(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest httpServletRequest,
		...
		String tipoPesquisa = (String) pesquisarTipoDebitoActionForm.getTipoPesquisa();
		...
		if(idTipoFinanciamento != null && !idTipoFinanciamento[0].equals("-1") && idTipoFinanciamento.length > 0){
			...
			for(int i = 0; i < idTipoFinanciamento.length; i++){
				if(!(new Integer(idTipoFinanciamento[i]).equals(new Integer(ConstantesSistema.NUMERO_NAO_INFORMADO)))){
					...
					if(idTipoFinanciamento.length == 1){
						filtroDebitoTipo.adicionarParametro(new ParametroSimples(FiltroDebitoTipo.FINANCIAMENTO_TIPO_ID,
										idTipoFinanciamento[i]));
					}else{
						if(i == 0){
							filtroDebitoTipo.adicionarParametro(new ParametroSimples(FiltroDebitoTipo.FINANCIAMENTO_TIPO_ID,
											idTipoFinanciamento[i], ParametroSimples.CONECTOR_OR, idTipoFinanciamento.length));
						}else{
							if(i == (idTipoFinanciamento.length - 1)){
								...
								filtroDebitoTipo.adicionarParametro(new ParametroSimples(FiltroDebitoTipo.FINANCIAMENTO_TIPO_ID,
												idTipoFinanciamento[i], ParametroSimples.CONECTOR_OR));
							}
						}
					}
				}
		...
		// Caso o usuário indicar os tipos de financiamentos para pesquisar os tipos de débito
		if(idLancamentoItemContabil != null && !idLancamentoItemContabil[0].equals("-1") && idLancamentoItemContabil.length > 0){
			// Indica que o usuário informou um parâmetro para pesquisar
			peloMenosUmParametroInformado = true;
			...
			for(int i = 0; i < idLancamentoItemContabil.length; i++){
				if(!(new Integer(idLancamentoItemContabil[i]).equals(new Integer(ConstantesSistema.NUMERO_NAO_INFORMADO)))){
					...
					if(idLancamentoItemContabil.length == 1){
						filtroDebitoTipo.adicionarParametro(new ParametroSimples(FiltroDebitoTipo.LANCAMENTO_ITEM_CONTABIL_ID,
										idLancamentoItemContabil[i]));
					}else{
						if(i == 0){
							filtroDebitoTipo.adicionarParametro(new ParametroSimples(FiltroDebitoTipo.LANCAMENTO_ITEM_CONTABIL_ID,
											idLancamentoItemContabil[i], ParametroSimples.CONECTOR_OR, idLancamentoItemContabil.length));
						}else{
							if(i == (idLancamentoItemContabil.length - 1)){
								...
								filtroDebitoTipo.adicionarParametro(new ParametroSimples(FiltroDebitoTipo.LANCAMENTO_ITEM_CONTABIL_ID,
												idLancamentoItemContabil[i], ParametroSimples.CONECTOR_OR));
							}
						}
					}
				}
		...
		if(intervaloValorLimiteInicial != null && !intervaloValorLimiteInicial.trim().equalsIgnoreCase("")){
			...
			if(intervaloValorLimiteFinal == null || intervaloValorLimiteFinal.trim().equalsIgnoreCase("")){
				...
				if((Util.formatarMoedaRealparaBigDecimal(intervaloValorLimiteInicial)).doubleValue() > ((Util
								.formatarMoedaRealparaBigDecimal(intervaloValorLimiteFinal))).doubleValue()){
		...
		if(intervaloValorLimiteFinal != null && !intervaloValorLimiteFinal.trim().equalsIgnoreCase("")){
			...
			filtroDebitoTipo.adicionarParametro(new MaiorQue(FiltroDebitoTipo.VALOR_LIMITE, Util
							.formatarMoedaRealparaBigDecimal(intervaloValorLimiteInicial), ParametroSimples.CONECTOR_AND));
			filtroDebitoTipo.adicionarParametro(new MenorQue(FiltroDebitoTipo.VALOR_LIMITE, Util
							.formatarMoedaRealparaBigDecimal(intervaloValorLimiteFinal)));
		}
		...
		// Collection colecaoTipoDebitos =
		// fachada.pesquisar(filtroDebitoTipo,DebitoTipo.class.getName());

		Map resultado = controlarPaginacao(httpServletRequest, retorno, filtroDebitoTipo, DebitoTipo.class.getName());
		...
		if(colecaoTipoDebitos == null || colecaoTipoDebitos.isEmpty()){
			...
			// se a pesquisa retornou uma quantidade de registros maior que a permitida para a
			// pesquisa
