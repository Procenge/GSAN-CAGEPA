 ...
 * @date 14/03/2007
 */
public class ExibirAtualizarTipoDebitoAction
				extends GcomAction {
	...
	public ActionForward execute(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest httpServletRequest,
		...
		ActionForward retorno = actionMapping.findForward("atualizarTipoDebito");
		...
		filtroLancamentoItemContabil.adicionarParametro(new ParametroSimples(FiltroLancamentoItemContabil.INDICADOR_USO,
						ConstantesSistema.INDICADOR_USO_ATIVO));

		// Verifica se os dados foram informados da tabela existem e joga numa
		// colecao

		Collection<LancamentoItemContabil> colecaoLancamentoItemContabil = fachada.pesquisar(filtroLancamentoItemContabil,
						LancamentoItemContabil.class.getName());

		if(colecaoLancamentoItemContabil == null || colecaoLancamentoItemContabil.isEmpty()){
			throw new ActionServletException("atencao.entidade_sem_dados_para_selecao", null, "Tabela Lancamento Item Contabil");
		}
		...
		Collection<FinanciamentoTipo> colecaoFinanciamentoTipo = fachada.pesquisar(filtroFinanciamentoTipo, FinanciamentoTipo.class
						.getName());

		if(colecaoFinanciamentoTipo == null || colecaoFinanciamentoTipo.isEmpty()){
			throw new ActionServletException("atencao.entidade_sem_dados_para_selecao", null, "Tabela Financiamento Tipo");
		}
		...
		if(sessao.getAttribute("colecaoTipoDebito") == null){
			...
			if(sessao.getAttribute("objetoDebitoTipo") != null){
				...
				DebitoTipo debitoTipo = (DebitoTipo) sessao.getAttribute("objetoDebitoTipo");
				sessao.setAttribute("idDebitoTipo", debitoTipo.getId().toString());
				...
				form.setFinanciamentoTipo(debitoTipo.getFinanciamentoTipo().getId().toString());
				form.setLancamentoItemContabil(debitoTipo.getLancamentoItemContabil().getId().toString());
				form.setIndicadorGeracaoDebitoAutomatica("" + debitoTipo.getIndicadorGeracaoAutomatica());
				form.setIndicadorGeracaoDebitoConta("" + debitoTipo.getIndicadorGeracaoConta());
				form.setIndicadorUso("" + debitoTipo.getIndicadorUso());
				String valorAux = Util.formatarMoedaReal(debitoTipo.getValorLimite());
				...
				// Alterações NILA
				form.setIndicadorValorCalcular(debitoTipo.getIndicadorValorCalcular().toString());
				...
				if(httpServletRequest.getParameter("idDebitoTipo") == null || httpServletRequest.getParameter("idDebitoTipo").equals("")){
					idTipoDebito = (String) sessao.getAttribute("idTipoDebito");
				}else{
					idTipoDebito = (String) httpServletRequest.getParameter("idDebitoTipo");
		...
		if("addDebitoTipoValorLocalidade".equalsIgnoreCase(form.getMethod())){
			form.addDebitoTipoValorLocalidade();
			form.setMethod(null);
		}
		...
		if(atualizarIdLocalidade != null && !atualizarIdLocalidade.equals("") && debitoTipoValorLocalidade != null
			...
			for(Iterator iter = form.getDebitoTipoValorLocalidade().iterator(); iter.hasNext();){
				DebitoTipoValorLocalidade dtvl = (DebitoTipoValorLocalidade) iter.next();

				if(atualizarIdLocalidade.equals(dtvl.getLocalidade().getId().toString())){
					dtvl.setValorDebitoTipo(Util.formatarMoedaRealparaBigDecimal(debitoTipoValorLocalidade));
				}
				colecaoAtualizada.add(dtvl);
			}
			form.setDebitoTipoValorLocalidade(colecaoAtualizada);
		}

		sessao.setAttribute("colecaoServicoTipoValorLocalidade", form.getDebitoTipoValorLocalidade());
		...
		if(httpServletRequest.getParameter("desfazer") != null && httpServletRequest.getParameter("desfazer").equalsIgnoreCase("S")){
			...
			if(httpServletRequest.getParameter("idDebitoTipo") == null || httpServletRequest.getParameter("idDebitoTipo").equals("")){
				...
				idDebitoTipo = (String) httpServletRequest.getParameter("idDebitoTipo");
		...
		// -------------- bt DESFAZER ---------------
		httpServletRequest.setAttribute("colecaoDebitoTipoTela", sessao.getAttribute("colecaoDebitoTipoTela"));
