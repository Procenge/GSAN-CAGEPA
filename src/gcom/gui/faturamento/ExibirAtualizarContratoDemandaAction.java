 ...
 * @since 31/03/2006
 */
public class ExibirAtualizarContratoDemandaAction
				extends GcomAction {
	...
	public ActionForward execute(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest httpServletRequest,
		...
		ActionForward retorno = actionMapping.findForward("exibirAtualizarContratoDemanda");
		...
		Collection colecaoMotivoCancelamento = fachada.pesquisar(filtroContratoMotivoCancelamento, ContratoMotivoCancelamento.class
						.getName());
		...
		if(idImovel != null && !idImovel.trim().equals("") && (inscricaoImovel == null || inscricaoImovel.trim().equals(""))){

			Imovel imovel = fachada.pesquisarImovelDigitado(new Integer(idImovel));

			if(imovel != null){

				atualizarContratoDemandaActionForm.setInscricaoImovel(imovel.getInscricaoFormatada());
				httpServletRequest.setAttribute("nomeCampo", "dataInicioContrato");

			}else{

				atualizarContratoDemandaActionForm.setInscricaoImovel("IMÓVEL INEXISTENTE");
		...
		}else if(inscricaoImovel != null && !inscricaoImovel.trim().equals("") && (idImovel == null || idImovel.trim().equals(""))){
		...
		if(sessao.getAttribute("contratoDemandaAtualizar") == null || httpServletRequest.getParameter("desfazer") != null){

			ContratoDemanda contratoDemanda = null;

			if(httpServletRequest.getParameter("desfazer") != null){

				String idContratoDemanda = ((ContratoDemanda) sessao.getAttribute("contratoDemandaAtualizar")).getId().toString();
				...
				Collection colecaoContratoDemanda = fachada.pesquisar(filtroContratoDemanda, ContratoDemanda.class.getName());

				if(colecaoContratoDemanda == null || colecaoContratoDemanda.isEmpty()){
					throw new ActionServletException("atencao.atualizacao.timestamp");
				}

				contratoDemanda = (ContratoDemanda) Util.retonarObjetoDeColecao(colecaoContratoDemanda);
				...
				if(sessao.getAttribute("contratoDemanda") != null){
					...
					contratoDemanda = (ContratoDemanda) sessao.getAttribute("contratoDemanda");
			...
			atualizarContratoDemandaActionForm.setIdImovel(contratoDemanda.getImovel().getId().toString());
			atualizarContratoDemandaActionForm.setInscricaoImovel(contratoDemanda.getImovel().getInscricaoFormatada());
			...
			if(contratoDemanda.getContratoMotivoCancelamento() != null){
				atualizarContratoDemandaActionForm.setIdMotivoCancelamento(contratoDemanda.getContratoMotivoCancelamento().getId()
								.toString());
			}else{
				atualizarContratoDemandaActionForm.setIdMotivoCancelamento("" + ConstantesSistema.NUMERO_NAO_INFORMADO);
			}
