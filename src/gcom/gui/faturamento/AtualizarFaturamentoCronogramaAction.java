 ...
 */
public class AtualizarFaturamentoCronogramaAction
				extends GcomAction {
	...
	public ActionForward execute(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest httpServletRequest,
		...
		Collection colecaoFaturamentoAtividadeCronograma = (Collection) sessao.getAttribute("colecaoFaturamentoAtividadeCronograma");

		Collection faturamentoAtividadeCronogramas = new ArrayList();

		// Colecao para guadar todas as atividades inclusive as que não vão ser inseridas ou
		// alteradas
		Collection colecaoTodasAtividades = new ArrayList();

		FaturamentoAtividadeCronograma faturamentoAtividadeCronograma = null;

		FaturamentoGrupoCronogramaMensal faturamentoGrupoCronogramaMensal = (FaturamentoGrupoCronogramaMensal) sessao
						.getAttribute("faturamentoGrupoCronogramaMensalSessao");
		...
		while(iteratorFaturamentoAtividadeCronogramas.hasNext()){
			...
			try{
				...
				if(comandar != null && !comandar.trim().equals("") && dataPrevista != null && !dataPrevista.trim().equals("")){
					int diaVencimento = faturamentoGrupo.getDiaVencimento().intValue();
					String mesVencimento = String.valueOf(faturamentoGrupo.getAnoMesReferencia().intValue()).substring(4, 6);
					String anoVencimento = String.valueOf(faturamentoGrupo.getAnoMesReferencia().intValue()).substring(0, 4);

					/*
					 * Colocado por Raphael Rossiter em 05/05/2007
					 * Obtendo a data de vencimento do grupo
					 */
					Date dataVencimento = fachada.obterDataVencimentoFaturamentoGrupo(diaVencimento, new Integer(mesVencimento),
									new Integer(anoVencimento));
				...
				throw new ActionServletException("errors.date", null, "data prevista");
			}
