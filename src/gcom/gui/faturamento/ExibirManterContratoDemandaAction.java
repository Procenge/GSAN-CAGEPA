 ...
 * @since 27/06/2007
 */
public class ExibirManterContratoDemandaAction
				extends GcomAction {
	...
	public ActionForward execute(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest httpServletRequest,
		...
		FiltroContratoDemanda filtroContratoDemanda = (FiltroContratoDemanda) sessao.getAttribute("filtroContratoDemanda");
		...
		if(filtroContratoDemanda != null && !filtroContratoDemanda.equals("")){
			Map resultado = controlarPaginacao(httpServletRequest, retorno, filtroContratoDemanda, ContratoDemanda.class.getName());
			colecaoContratoDemanda = (Collection) resultado.get("colecaoRetorno");
		...
		if(colecaoContratoDemanda != null && !colecaoContratoDemanda.isEmpty()){
			...
			if(colecaoContratoDemanda.size() == 1
				...
				if(sessao.getAttribute("atualizar") != null){
					retorno = actionMapping.findForward("exibirAtualizarContratoDemanda");
					ContratoDemanda contratoDemanda = (ContratoDemanda) colecaoContratoDemanda.iterator().next();
					sessao.setAttribute("contratoDemanda", contratoDemanda);
				}else{
					httpServletRequest.setAttribute("colecaoContratoDemanda", colecaoContratoDemanda);
				}
			}else{
				httpServletRequest.setAttribute("colecaoContratoDemanda", colecaoContratoDemanda);
			}
