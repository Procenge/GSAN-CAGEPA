 ...
 */
public class ExibirInserirCreditoARealizarAction
				extends GcomAction {
	...
	public ActionForward execute(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest httpServletRequest,
		...
		httpServletRequest.setAttribute("collectionCreditoTipo", collectionCreditoTipo);

		// Coleção de Origem do Crédito
		Collection<CreditoOrigem> collectionCreditoOrigem = new ArrayList<CreditoOrigem>();

		if(StringUtils.isNotBlank(inserirCreditoARealizarActionForm.getTipoCredito())
						&& StringUtils.isNumeric(inserirCreditoARealizarActionForm.getTipoCredito())){
			...
			Short indicadorUsoLivre = ConstantesSistema.SIM;
			if(temPermissaoEspecial == ConstantesSistema.SENHA_ESPECIAL){
				indicadorUsoLivre = null; // Se tiver permissão, poderá pesquisar com todos os tipos
											// de indicadores de uso.
			}
			
			collectionCreditoOrigem = fachada.listarCreditoOrigem(Integer.valueOf(inserirCreditoARealizarActionForm.getTipoCredito()),
							indicadorUsoLivre);

		}

		httpServletRequest.setAttribute("collectionCreditoOrigem", collectionCreditoOrigem);
		...
		// .getMatriculaImovel();
		String clearGrid = httpServletRequest.getParameter("clearGrid");
		String idImovel = null;

		if(clearGrid != null && clearGrid.equals("y")){
			inserirCreditoARealizarActionForm.setOrigemCredito(null);
			inserirCreditoARealizarActionForm.setTipoCredito(null);
			inserirCreditoARealizarActionForm.setValorCredito(null);
			inserirCreditoARealizarActionForm.setNumeroPrestacoes(null);
			inserirCreditoARealizarActionForm.setReferencia(null);
		...
		if(httpServletRequest.getParameter("objetoConsulta") != null
			...
			if(idRegistroAtendimento != null && !idRegistroAtendimento.trim().equals("")){
				...
				if(colecaoRegistroAtendimento != null && !colecaoRegistroAtendimento.isEmpty()){
					...
					// Especificação do Tipo de Solicitação do Registro de Atendimento não permite
					// geração de credito
					if(registroAtendimento.getSolicitacaoTipoEspecificacao().getIndicadorGeracaoCredito() == 2){
						// FS0005 - Validar Registro de Atendimento
						throw new ActionServletException("atencao.registro_atendimento.nao.permite.geracao.credito");
					}
					...
					inserirCreditoARealizarActionForm.setNomeRegistroAtendimento(registroAtendimento.getSolicitacaoTipoEspecificacao()
									.getDescricao());
					...
					sessao.setAttribute("travarTipoCredito", "nao");
			...
			if(idOrdemSerico != null && !idOrdemSerico.trim().equals("")){
				...
				if(colecaoOrdemServico != null && !colecaoOrdemServico.isEmpty()){
					...
					if(ordemServico.getRegistroAtendimento() == null){
						// FS0005 - Validar Registro de Atendimento
						throw new ActionServletException("atencao.ordem_servico.nao.esta.associado.registro_atendimento");
					}
					...
					inserirCreditoARealizarActionForm.setNomeRegistroAtendimento(ordemServico.getRegistroAtendimento()
			...
			sessao.setAttribute("travarRegistroAtendimento", "valor");
			sessao.setAttribute("travarOrdemServico", "valor");
			sessao.setAttribute("travarTipoCredito", "valor");
		}
		...
		if(idImovel != null && !idImovel.trim().equals("")){
			...
			if(imovelPesquisado != null && imovelPesquisado.isEmpty()){
				...
				inserirCreditoARealizarActionForm.setInscricaoImovel(ConstantesSistema.CODIGO_IMOVEL_INEXISTENTE);
			...
			if(imovelPesquisado != null && !imovelPesquisado.isEmpty()){
				...
				if(clienteImovelPesquisado != null && !clienteImovelPesquisado.isEmpty()){
					...
					if(clienteImovel.getCliente() != null){
						inserirCreditoARealizarActionForm.setNomeCliente(clienteImovel.getCliente().getNome());
					}
				}
				if(imovel.getLigacaoAguaSituacao() != null){
					inserirCreditoARealizarActionForm.setSituacaoAgua(imovel.getLigacaoAguaSituacao().getDescricao());
				}

				if(imovel.getLigacaoEsgotoSituacao() != null){
					inserirCreditoARealizarActionForm.setSituacaoEsgoto(imovel.getLigacaoEsgotoSituacao().getDescricao());
				}
				inserirCreditoARealizarActionForm.setMatriculaImovel(idImovel);

				inserirCreditoARealizarActionForm.setInscricaoImovel(imovel.getInscricaoFormatada());
			}
		}

		if(limparForm != null && !limparForm.trim().equalsIgnoreCase("")){

			inserirCreditoARealizarActionForm.reset(actionMapping, httpServletRequest);
