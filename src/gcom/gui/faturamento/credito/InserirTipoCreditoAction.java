...
public class InserirTipoCreditoAction
				extends GcomAction {
	...
	public ActionForward execute(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest httpServletRequest,
		...
		if(Util.validarNumeroMaiorQueZERO(inserirTipoCreditoActionForm.getTipoLancamentoContabil())){
			...
			filtroLancamentoItemContabil.adicionarParametro(new ParametroSimples(FiltroCreditoTipo.LANCAMENTO_ITEM_CONTABIL,
		...
		if(indicadorgeracaoAutomaica == null || indicadorgeracaoAutomaica.equalsIgnoreCase("")){
			throw new ActionServletException("atencao.required", null, "indicador de Geração Automática do Crédito");
		}
		...
		if(tipoLancamentoContabil != null
						&& !tipoLancamentoContabil.equalsIgnoreCase(String.valueOf(ConstantesSistema.NUMERO_NAO_INFORMADO))){

			FiltroLancamentoItemContabil filtroLancamentoItemContabil = new FiltroLancamentoItemContabil();

			filtroLancamentoItemContabil.adicionarParametro(new ParametroSimples(FiltroLancamentoItemContabil.ID, tipoLancamentoContabil));
			...
			colecaoPesquisa = fachada.pesquisar(filtroLancamentoItemContabil, LancamentoItemContabil.class.getName());

			if(colecaoPesquisa == null || colecaoPesquisa.isEmpty()){

				throw new ActionServletException("atencao.pesquisa_elo_nao_inexistente");
			}else{
				lancamentoItemContabil = (LancamentoItemContabil) Util.retonarObjetoDeColecao(colecaoPesquisa);
				if(lancamentoItemContabil.getId().intValue() != lancamentoItemContabil.getId().intValue()){
		...
		filtroCreditoTipo.adicionarParametro(new ParametroSimples(FiltroCreditoTipo.DESCRICAO, descricao));
		...
		if(colCreditoTipo != null && !colCreditoTipo.isEmpty()){
			throw new ActionServletException("atencao.credito_tipo.descricao_ja_existente", null, descricao);
		}
		// *****************************************************

		// Pesquisa se a descrição abreviada informada já foi cadastrada
		filtroCreditoTipo = new FiltroCreditoTipo();
		filtroCreditoTipo.adicionarParametro(new ParametroSimples(FiltroCreditoTipo.DESCRICAO_ABREVIADA, abreviatura));
		colCreditoTipo = fachada.pesquisar(filtroCreditoTipo, CreditoTipo.class.getName());
		if(colCreditoTipo != null && !colCreditoTipo.isEmpty()){
			throw new ActionServletException("atencao.credito_tipo.abreviatura_ja_existente", null, abreviatura);
		}
		// *****************************************************
		...
		montarPaginaSucesso(httpServletRequest, "Tipo de Crédito de código  " + tipoCreditoInserir.getId() + " inserido com sucesso.",
						"Inserir outro Tipo de Crédito", "exibirInserirTipoCreditoAction.do?menu=sim",
						"exibirAtualizarTipoCreditoAction.do?idRegistroAtualizacao=" + idTipoCredito, "Atualizar Tipo de Crédito Inserido");
