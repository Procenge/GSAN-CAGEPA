 ...
 * @date 05/08/2006
 */
public class FiltrarTipoCreditoAction
				extends GcomAction {
	...
	public ActionForward execute(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest httpServletRequest,
		...
		ActionForward retorno = actionMapping.findForward("exibirManterTipoCredito");
		...
		String indicadorgeracaoAutomaica = filtrarTipoCreditoActionForm.getIndicadorgeracaoAutomaica();
		...
		if(descricao != null && !descricao.equalsIgnoreCase("")){
			...
			if(tipoPesquisa != null && tipoPesquisa.equals(ConstantesSistema.TIPO_PESQUISA_COMPLETA.toString())){
				...
				filtroCreditoTipo.adicionarParametro(new ComparacaoTextoCompleto(FiltroCreditoTipo.DESCRICAO, descricao));
			}else{

				filtroCreditoTipo.adicionarParametro(new ComparacaoTexto(FiltroSistemaEsgoto.DESCRICAO, descricao));
			}
		}
		...
		if(abreviatura != null && !abreviatura.trim().equalsIgnoreCase("")){
			...
			filtroCreditoTipo.adicionarParametro(new ComparacaoTexto(FiltroCreditoTipo.DESCRICAO_ABREVIADA, abreviatura));

		}

		// Verifica se o campo LancamentoItemContabil foi informado

		if(tipoLancamentoContabil != null && !tipoLancamentoContabil.trim().equals("" + ConstantesSistema.NUMERO_NAO_INFORMADO)){

			peloMenosUmParametroInformado = true;

			filtroCreditoTipo.adicionarParametro(new ParametroSimples(FiltroCreditoTipo.LANCAMENTO_ITEM_CONTABIL, tipoLancamentoContabil));

		}

		// Verifica se o campo LancamentoItemContabil foi informado
		if(indicadorgeracaoAutomaica != null && !indicadorgeracaoAutomaica.trim().equals("3")
						&& !indicadorgeracaoAutomaica.trim().equals("")){

			peloMenosUmParametroInformado = true;

			filtroCreditoTipo.adicionarParametro(new ParametroSimples(FiltroCreditoTipo.INDICADOR_GERACAO_AUTOMATICO,
							indicadorgeracaoAutomaica));

		}

		// se o usuário informar o intervalo inicial do valor de limite
		if(valorLimiteCreditoInicial != null && !valorLimiteCreditoInicial.trim().equalsIgnoreCase("")){

			// se o usuário não informar o intervalo final do valor de
			// limite
			if(valorLimiteCreditoFinal == null || valorLimiteCreditoFinal.trim().equalsIgnoreCase("")){
				...
				// inicial
				if((Util.formatarMoedaRealparaBigDecimal(valorLimiteCreditoInicial)).doubleValue() > ((Util
								.formatarMoedaRealparaBigDecimal(valorLimiteCreditoFinal))).doubleValue()){
		...
		if(valorLimiteCreditoFinal != null && !valorLimiteCreditoFinal.trim().equalsIgnoreCase("")){
			...
			// valores informados
			filtroCreditoTipo.adicionarParametro(new MaiorQue(FiltroCreditoTipo.VALOR_LIMITE_CREDITO, Util
							.formatarMoedaRealparaBigDecimal(valorLimiteCreditoInicial), ParametroSimples.CONECTOR_AND));
			filtroCreditoTipo.adicionarParametro(new MenorQue(FiltroCreditoTipo.VALOR_LIMITE_CREDITO, Util
		...
		if(indicadorUso != null && !indicadorUso.trim().equals("3") && !indicadorUso.trim().equals("")){
			...
			filtroCreditoTipo.adicionarParametro(new ParametroSimples(FiltroCreditoTipo.INDICADOR_USO, indicadorUso));
		...
		sessao.setAttribute("filtroTipoCredito", filtroCreditoTipo);
