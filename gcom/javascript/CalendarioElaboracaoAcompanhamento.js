function CalendarElaboracaoAcompanhamento(Month, Year, situacao) {
    var output = '';
	
	var vetorSituacao = situacao.split(",");
	
    output += '<form name="ExibirCalendarioElaboracaoAcompanhamentoRoteiroActionForm"><body onLoad="window.focus();" background="../../imagens/background.gif">';
    ...
    output += '&nbsp;&nbsp;&nbsp;<select name="Month" onChange="changeMonth(ExibirCalendarioElaboracaoAcompanhamentoRoteiroActionForm.mes.value,ExibirCalendarioElaboracaoAcompanhamentoRoteiroActionForm.ano.value);"><font size=-1 color="#000000" face="ARIAL">';
...
function changeMonth(Month,Year) {
  ...
  var mes = document.ExibirCalendarioElaboracaoAcompanhamentoRoteiroActionForm.Month.options[document.ExibirCalendarioElaboracaoAcompanhamentoRoteiroActionForm.Month.selectedIndex].value + '';
  var ano = document.ExibirCalendarioElaboracaoAcompanhamentoRoteiroActionForm.Year.options[document.ExibirCalendarioElaboracaoAcompanhamentoRoteiroActionForm.Year.selectedIndex].value + '';
  var tipoCalendario = document.forms[0].tipoCalendario.value;
  
  // Verifica se o mês informado no combo é menor que o mês atual
  var hoje = new Date();
  var diaHoje   = hoje.getDate();
  var mesHoje = hoje.getMonth();
  var anoHoje  = y2k(hoje.getYear());
  
  var anoMesCombo = (ano * 100) + Number(mes);
  var anoMesAtual = (y2k(hoje.getYear()) * 100) + mesHoje;
  
  if (anoMesCombo < anoMesAtual && tipoCalendario == "elaboracao") {
	  alert("Mês/Ano de Referência inválida.");
	  location.href = '/gsan/exibirCalendarioElaboracaoAcompanhamentoRoteiroAction.do?mes='+mesHoje+'&ano='+anoHoje+'&'+tipoCalendario+'=true';
  } else {  
	  location.href = '/gsan/exibirCalendarioElaboracaoAcompanhamentoRoteiroAction.do?mes='+mes+'&ano='+ano+'&'+tipoCalendario+'=true';
  }
}
