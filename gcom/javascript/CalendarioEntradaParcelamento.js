function CalendarEntradaParcelamento(Month, Year, situacao) {
    var output = '';
	
    var data; 
	var vetorSituacao = situacao.split(",");
	
    output += '<form name="ExibirCalendarioEntradaParcelamentoActionForm"><body onLoad="window.focus();" background="../../imagens/background.gif">';
    ...
    output += '&nbsp;&nbsp;&nbsp;<select name="Month" onChange="javascript:changeMonth();"><font size=-1 color="#000000" face="ARIAL">';
    ...
    for (i=1; i<=days[Month]; i++, column++) {
    	if( column == 0 ){
	        ...
	        if (vetorSituacao[i - 1] == "4"){	 	        	
	        	output += '<td width="10%" height="25" bgcolor="#ffffff">' + '<a href="javascript:changeDayMonthYear(' + i + ',' + Month + ',' + Year + ')"><font SIZE=-1 face="ARIAL" color="#FF0000">' + i + '<\/font><\/a>' +'<\/td>';
	        }
	        else if (vetorSituacao[i - 1] == "5"){
	    	     output += '<td width="10%" height="25" bgcolor="#ffffff">' + '<a href="javascript:changeDayMonthYear(' + i + ',' + Month + ',' + Year + ')"><font SIZE=-1 face="ARIAL" color="#0000BB">' + i + '<\/font><\/a> <span class="elaboracaoAcompanhamento">R</span>' +'<\/td>';
	        }
	        ...
	        if (vetorSituacao[i - 1] == "4"){
	        	output += '<td width="10%" height="25" bgcolor="#cbe5fe">' + '<a href="javascript:changeDayMonthYear(' + i + ',' + Month + ',' + Year + ')"><font SIZE=-1 face="ARIAL" color="#FF0000">' + i + '<\/font><\/a>' +'<\/td>';
	        }
	        else if (vetorSituacao[i - 1] == "5"){
	           output += '<td width="10%" height="25" bgcolor="#cbe5fe">' + '<a href="javascript:changeDayMonthYear(' + i + ',' + Month + ',' + Year + ')"><font SIZE=-1 face="ARIAL" color="#0000BB">' + i + '<\/font><\/a> <span class="elaboracaoAcompanhamento">R</span>' +'<\/td>';
	        }
	        else if (vetorSituacao[i - 1] == "6"){
...
function restart() {	
	nomeCampo = '' + padout(day) + '/' + padout(month - 0 + 1) + '/' + year;   
    location.href ='/gsan/efetuarParcelamentoDebitosWizardAction.do?destino=3&action=processarProcesso2Action&idCampoEnviarDados='+nomeCampo;	
}

function abrirCalendario(formName, fieldName) {
    nomeForm = formName;
    nomeCampo = fieldName;    
	centerpopup('./jsp/util/calendario_entrada_parcelamento.jsp','calendario',225,268);
}
...
function changeDayMonthYear(day,month,year) {
    opener.day = day + '';   
    opener.month = month + '';   
    opener.year = year + '';   
    opener.restart();  
...
function changeMonth() { 
  var mes = document.ExibirCalendarioEntradaParcelamentoActionForm.Month.options[document.ExibirCalendarioEntradaParcelamentoActionForm.Month.selectedIndex].value + '';
  var ano = document.ExibirCalendarioEntradaParcelamentoActionForm.Year.options[document.ExibirCalendarioEntradaParcelamentoActionForm.Year.selectedIndex].value + '';
  location.href ='/gsan/exibirCalendarioEntradaParcelamentoAction.do?mes='+mes+'&ano='+ano;  
}
...
function changeYear(Year) {
  ...
  var tipoCalendario = document.forms[0].tipoCalendario.value;  
   location.href = '/gsan/exibirCalendarioEntradaParcelamentoAction.do?mes='+mes+'&ano='+ano+'&'+tipoCalendario+'=true';
}
...
function abrirCalendarioReplicando(formName, fieldName, fieldNameReplicar) {
	...
	opener.window.document.forms[0].action = '/gsan/exibirCalendarioEntradaParcelamentoAction.do'; 
