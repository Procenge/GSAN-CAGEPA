...
function adicionarInterrupcao() {
	...
	if (form.horaInicioInterrupcao.value.length < 5) {
		alert("Hora Início de Intervalo inválida.");
			form.horaInicioInterrupcao.focus();
	} else if (form.horaFimInterrupcao.value.length < 5) {
		alert("Hora Fim de Intervalo inválida.");
			form.horaFimInterrupcao.focus();
	} else if (form.horaInicioInterrupcao.value == ""
		|| form.horaFimInterrupcao.value == ""
		|| form.idMotivoInterrupcao.value == "-1") {
		alert("Informe intervalo e motivo da interrupção."); 
	} else {
		form.action = "/gsan/exibirManterHorasExecucaoEquipeOSAction.do?adicionarInterrupcao=OK";
		form.submit();
	}
}
...
function removerInterrupcao(posicaoInterrupcao) {
 	...
 	form.posicaoInterrupcao.value = posicaoInterrupcao;
	form.action = "/gsan/exibirManterHorasExecucaoEquipeOSAction.do";
...
<body leftmargin="5" topmargin="5" onload="window.focus();resizePageSemLink(680, 600);setarFoco('${requestScope.nomeCampo}');">
...
<input type="hidden" name="posicaoInterrupcao" value=""/>
      ...
      <table width="100%" border="0">
        ...
        <tr>
        	...
        	<td width="75%" colspan="3">
        		...
        		<html:text property="idEquipeNaoProgramada" size="4" maxlength="5" onkeypress="limparDescricao();validaEnterComMensagem(event, 'exibirManterHorasExecucaoEquipeOSAction.do', 'idEquipeNaoProgramada', 'Equipe não Programada');"/>
        ...
        <tr>
          ...
          <td colspan="3" align="right"><input type="button" class="bottonRightCol" value="Adicionar"
			style="width: 80px" onclick="adicionar();"></td>
        </tr>
        <tr>
      		<td colspan="4">
      			...
      			<logic:present name="colecaoManterDadosAtividadesOrdemServicoHelper">
      				...
      				<logic:present name="dadosAtividadesOrdemServico" property="ordemServicoAtividade">
      	...
      	<tr><td colspan="5"><strong>Dados das Interrupções:</strong></td></tr>
	    <tr>
          <td width="25%"><strong>Intervalo:<span style="color:#FF0000">*</span></strong></td>
          <td width="75%" colspan="3">
          	<html:text property="horaInicioInterrupcao" size="6" maxlength="5" onkeyup="mascaraHora(this, event)"/><strong>&nbsp;&nbsp;(hh:mm) às&nbsp;&nbsp;</strong> 
          	<html:text property="horaFimInterrupcao" size="6" maxlength="5" onkeyup="mascaraHora(this, event)"/><strong>&nbsp;&nbsp;(hh:mm)</strong>
          </td>
          <tr>
        ...
        <tr>
       	  <td colspan="3"><strong>Horários Interrupção:</strong></td>
		  <td align="right">
			<input type="button" class="bottonRightCol" value="Adicionar" style="width: 80px" onclick="adicionarInterrupcao();">
		  </td>
        </tr>
        <tr>
        <td colspan="6">
        	<c:set var="i" value="0" />
        	<display:table class="dataTable" name="colecaoInterrupcaoExecucao" sort="list" id="interrupcao"  pagesize="15" excludedParams="" > 
