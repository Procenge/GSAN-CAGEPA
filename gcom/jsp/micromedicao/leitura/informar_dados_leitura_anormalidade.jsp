...
function fazerPaginacao(page, pageAtual){
	...
	function validaDataCamposDigitacao(data, event, anoMesMovimento, dataLeituraAnterior){
		if(verificaDataMensagemPersonalizada(data,'Data do Pagamento Inválida') && data.value != ""){
			if(compararDataComDataAtual(data.value) == false ){
				alert("Data de Leitura não pode ser maior que a data atual.");fazerPaginacao
				data.value ="";
				return;
			}else{
	...
	function abrirPopupPesquisaRota(){
		...
		if(msg != ''){
			...
			abrirPopup('exibirPesquisarRotaLeituristaAction.do?chamarPopup=true&empresa='+form.idEmpresa.value+'&leiturista='+form.codigoLeiturista
.value, 250, 470);
		}
...
<body leftmargin="5" topmargin="5" onload="javascript:setarFoco('${requestScope.nomeCampo}');marcarRadio(<logic:present scope="session" name="pesquisaImovelRota"> <bean:write scope="session" name="pesquisaImovelRota" /></logic:present><logic:notPresent scope="session" name="pesquisaImovelRota">''</logic:notPresent>);verificarFormaEntradaDados();">
	...
	<table width="770" border="0" cellspacing="4" cellpadding="0">
		...
		<tr>
			...
			<table width="100%" border="0">
				<tr>
					<td height="10" colspan="2" width="100%">Para informar dados de
							leitura e anormalidade, informe os dados abaixo:</td>
				</tr>
				</table>
				<table>
					<tr>
						...
						<logic:equal name="indicadoPermitirInformarDadosLeituraLote" value="<%=ConstantesSistema.INATIVO %>">
							<html:radio
							property="indicadorEntradaDados" value="2" onclick="javascript:validarFormaEntradaDados(2);" /> Manual
						</logic:equal>
							 <html:radio
							property="indicadorEntradaDados" value="1" onclick="javascript:validarFormaEntradaDados(1);"  /> Via Arquivo 
						<logic:equal name="indicadoPermitirInformarDadosLeituraLote" value="<%=ConstantesSistema.ATIVO %>">
							<html:radio
							property="indicadorEntradaDados" value="3" onclick="javascript:validarFormaEntradaDados(3);"  /> Manual
						</logic:equal>
						 </strong></td>
				</table>
				<table id="arquivo">
						<tr>
					<td><strong>Grupo de Faturamento:</strong> <span class="style2"> <strong>
					<font color="#FF0000">*</font> </strong> </span></td>

					<td><strong> <html:select property="idGrupoFaturamento"
						style="width: 230px;">
						<html:option
						...
						<logic:present name="colecaoFaturamentoGrupo" scope="session">
							...
							<tr bgcolor="#cbe5fe">
								<td height="10">
									<strong>
										Arquivo de leituras e anormalidades (.txt, .dat):<font color="#FF0000">*</font>
									</strong>
								</td>
								<td>
				...
				<tr>
					...
					<td><strong> <html:select property="codigoLeiturista"
						style="width: 230px;" onchange="javascript:pesquisaRota();">
						<html:option
							value="<%=""+ConstantesSistema.NUMERO_NAO_INFORMADO%>">&nbsp;
							</html:option>
						...
						<logic:present name="colecaoLeiturista" scope="session">
							<html:options collection="colecaoLeiturista"
								labelProperty="nomeLeiturista" property="id" />
						</logic:present>
					</html:select> </strong></td>
		        ...
		        <tr>
					<td colspan="2">
						...
						onClick="javascript:buscaImoveis();"
						style="width: 80px" />
					</td>
				</tr>
				</table>
							...
							<table id="dadosLeituraImovelCampos">
								...
								<tr>
									<td>
										...
										<logic:notPresent scope="session" name="verificaFaturamentoRota">
											<td>
												<strong> <html:select property="idOcorrencia" style="width: 230px;">
												<html:option value="<%=""+ConstantesSistema.NUMERO_NAO_INFORMADO%>">&nbsp;</html:option>
						
												<logic:present name="colecaoOcorrencia" scope="session">
													<html:options collection="colecaoOcorrencia"
														labelProperty="descricao" property="id" />
												</logic:present>
											    </html:select> </strong>
										   </td>
									   ...
									   <logic:present scope="session" name="verificaFaturamentoRota">
											<td>
												<strong> <html:select property="idOcorrencia" style="width: 230px;" disabled="true">
												<html:option value="<%=""+ConstantesSistema.NUMERO_NAO_INFORMADO%>">&nbsp;</html:option>
															...
															<%if(MovimentoRoteiroEmpresa.FASE_PROCESSADO.equals(movimentoRoteiroEmpresa.getIndicadorFase()) ||
																...
																<td width="26%">
																		<input style="background-color: #E5E5E5" name="dataLeitura${movimentoRoteiroEmpresa.id}" id="registro<%=cont%>" type="text" size="10" maxlength="10"
																								value="${movimentoRoteiroEmpresa.dataLeituraFormatada}" readonly="true" onkeyup="mascaraData(this, event);"
																								onfocus="javascript:mudaFoco(<%=cont +1%>);"> 
																</td>
																<td width="26%"> <input style="background-color: #E5E5E5" name="leitura${movimentoRoteiroEmpresa.id}" type="text" 
																							value="${movimentoRoteiroEmpresa.numeroLeitura}" readonly="true" onfocus="javascript:mudaFoco(<%=cont +1%>);"> </td>
																<td width="22%"> <input style="background-color: #E5E5E5" name="ocorrencia${movimentoRoteiroEmpresa.id}" type="text" size="8" maxlength="4"
																							value="${movimentoRoteiroEmpresa.leituraAnormalidade.id}" readonly="true" onfocus="javascript:mudaFoco(<%=cont +1%>);"> </td>
															<%}else{ %>
																...
																<td width="26%">
																		<input name="dataLeitura${movimentoRoteiroEmpresa.id}" id="registro<%=cont%>" type="text" size="10" maxlength="10"
																								...
																								onchange="javascript:validaDataCamposDigitacao(this, event, ${movimentoRoteiroEmpresa.anoMesMovimento}, 
																								<%=movimentoRoteiroEmpresa.getDataLeituraAnteriorFormatada().toString()%>);"> 
																</td>
																...
																<%} else if(movimentoRoteiroEmpresa.getMedicaoTipo() != null
																	...
																	<%tamanhoMaximo = movimentoRoteiroEmpresa.getImovel().getHidrometroInstalacaoHistorico().getHidrometro()
																			.getNumeroDigitosLeitura().intValue(); %>
																<%} %>
																<td width="26%"> <input name="leitura${movimentoRoteiroEmpresa.id}" type="text" maxlength="<%=tamanhoMaximo%>"
																							value="${movimentoRoteiroEmpresa.numeroLeitura}" onkeypress="return isCampoNumerico(event);"
																							onchange="javascript:validaDataCamposDigitacao(document.forms[0].dataLeitura${movimentoRoteiroEmpresa.id}, event, ${movimentoRoteiroEmpresa.anoMesMovimento}, 
																								<%=movimentoRoteiroEmpresa.getDataLeituraAnteriorFormatada().toString()%>);"> </td>
																<td width="22%"> 
