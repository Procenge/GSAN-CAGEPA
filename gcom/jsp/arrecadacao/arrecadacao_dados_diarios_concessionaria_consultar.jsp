...
<html:form action="/consultarDadosDiariosArrecadacaoWizardAction" 
	...
	method="post">
	<%String nuPagina = (String) session.getAttribute("nuPaginaConcessionaria"); %>
	<jsp:include
                ...
                <table width="100%" bgcolor="#99CCFF" dwcopytype="CopyTableRow">
	                ...
	                Map<Integer,Map<BigDecimal,Collection>> map = 
	                	(Map<Integer,Map<BigDecimal,Collection>>)
			                	session.getAttribute("dadosArrecadacaoConcessionaria");
	                
	                BigDecimal valorTotal = (BigDecimal)session.getAttribute("valordadosArrecadacaoConcessionaria");
							...
							while(iteratorColecaoArrecadacaoDadosDiarios.hasNext()){
								...
								if (arrecadacaoDadosDiarios.getGerenciaRegional() != null )
								{
				        	        ...
				        	        <tr bordercolor="#FFFFFF" bgcolor="#90c7fc""> 
					                	<td  width="38%"> <div align="center" class="style9"><strong>Concessionária</strong></div></td>
									...
									<tr bgcolor="#cbe5fe">
									<%}%>
					                  <td height="17"><a href="javascript:abrirPopup('exibirConsultarDadosDiariosConcessionariaPorArrecadadorAction.do?referencia=<%=arrecadacaoDadosDiarios.getAnoMesReferenciaArrecadacao()%>&idConcessionaria=<%=arrecadacaoDadosDiarios.getConcessionaria().getId()%>', 475, 600);"><%=arrecadacaoDadosDiarios.getConcessionaria().getNome()%></a></td>
					                  <td><div align="right"><a href="javascript:abrirPopup('exibirConsultarDadosDiariosValoresDiariosAction.do?referencia=<%=arrecadacaoDadosDiarios.getAnoMesReferenciaArrecadacao()%>&idConcessionaria=<%=arrecadacaoDadosDiarios.getConcessionaria().getId()%>', 475, 600);"><%=Util.formatarMoedaReal(arrecadacaoDadosDiarios.getValorPagamentos())%></a></div></td>
