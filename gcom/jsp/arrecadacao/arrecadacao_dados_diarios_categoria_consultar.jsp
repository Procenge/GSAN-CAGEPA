...
<html:form action="/consultarDadosDiariosArrecadacaoWizardAction" 
	...
	method="post">
	<%String nuPagina = (String) session.getAttribute("nuPaginaCategoria"); %>
	<jsp:include
                ...
                <table width="100%" bgcolor="#99CCFF" dwcopytype="CopyTableRow">
	                ...
	                Map<Integer,Map<BigDecimal,Collection>> map = 
	                	(Map<Integer,Map<BigDecimal,Collection>>)
			                	session.getAttribute("dadosArrecadacaoCategoria");
	                
	                BigDecimal valorTotal = (BigDecimal)session.getAttribute("valordadosArrecadacaoCategoria");
					...
					while (iteratorColecaoDadosDiarios.hasNext()) {
						...
						while (iteratorValorAcumulado.hasNext()) {
							...
							while(iteratorColecaoArrecadacaoDadosDiarios.hasNext()){
								...
								if (arrecadacaoDadosDiarios.getCategoria() != null )
								{
				        	        ...
				        	        <tr bordercolor="#FFFFFF" bgcolor="#90c7fc""> 
					                	<td  width="38%"> <div align="center" class="style9"><strong>Categoria</strong></div></td>
									...
									<%}%>
					                  <td height="17">
					                  <%= arrecadacaoDadosDiarios.getCategoria().getDescricao()%>
					                  </td>
					                   <td>
					                  	<div align="right">
					                  	 <a href="javascript:abrirPopup('exibirConsultarDadosDiariosValoresDiariosAction.do?referencia=<%=arrecadacaoDadosDiarios.getAnoMesReferenciaArrecadacao()%>&idCategoria=<%=arrecadacaoDadosDiarios.getCategoria().getId() %>&nomeCategoria=<%=arrecadacaoDadosDiarios.getCategoria().getDescricao()%>', 475, 600);">
					                  		<%=Util.formatarMoedaReal(arrecadacaoDadosDiarios.getValorPagamentos())%>
				                  		 </a>
					                  	</div>
					                  </td>
