...
<html:form action="/consultarDadosDiariosArrecadacaoWizardAction" 
	...
	method="post">
	<%String nuPagina = (String) session.getAttribute("nuPaginaDocumento"); %>
	<jsp:include
                ...
                <table width="100%" bgcolor="#99CCFF" dwcopytype="CopyTableRow">
	                ...
	                Map<Integer,Map<BigDecimal,Collection>> map = 
	                	(Map<Integer,Map<BigDecimal,Collection>>)
			                	session.getAttribute("dadosArrecadacaoDocumento");
	                
	                BigDecimal valorTotal = (BigDecimal)session.getAttribute("valordadosArrecadacaoDocumento");
					...
					while (iteratorColecaoDadosDiarios.hasNext()) {
						...
						while (iteratorValorAcumulado.hasNext()) {
							...
							while(iteratorColecaoArrecadacaoDadosDiarios.hasNext()){
								...
								if (arrecadacaoDadosDiarios.getDocumentoTipo() != null )
								{
									...
									%>
				        	        <tr>
					                  <td colspan="6" bgcolor="#90c7fc"><div align="center"><strong>M&ecirc;s/Ano: <%=Util.formatarAnoMesParaMesAno(arrecadacaoDadosDiarios.getAnoMesReferenciaArrecadacao())%> - <%=(arrecadacaoDadosDiarios.getAnoMesReferenciaArrecadacao() < anoMesArrecadacao?"DEFINITIVO":"PARCIAL") %></strong></div></td>
				    	            </tr>
				        	        <tr bgcolor="#cbe5fe"> 
				        	        ...
				        	        <tr bordercolor="#FFFFFF" bgcolor="#90c7fc""> 
					                	<td  width="35%"> <div align="center" class="style9"><strong>Tipo</strong></div></td>
					                	<td width="11%"> <div align="center" class="style9"><strong>QTD.</strong></div></td>
					                  	<td width="18%"> <div align="center" class="style9"><strong>Valor</strong></div></td>
					                  	<td width="17%"><div align="center"><strong>Percentual M&ecirc;s</strong></div></td>
						                <td width="19%"> <div align="center" class="style9"><strong>Percentual Per&iacute;odo</strong></div></td>
			            		    </tr>
			            		    <%
			            		    	primeiraVez = false;
			            		    }
									...
									<tr bgcolor="#cbe5fe">
									<%}%>
					                  <td height="17"><%=arrecadacaoDadosDiarios.getDocumentoTipo().getDescricaoDocumentoTipo()%></td>
					                  <td><div align="right"><%=arrecadacaoDadosDiarios.getQuantidadePagamentos()%></div></td>
					                  <td><div align="right"><a href="javascript:abrirPopup('exibirConsultarDadosDiariosValoresDiariosAction.do?referencia=<%=arrecadacaoDadosDiarios.getAnoMesReferenciaArrecadacao()%>&idDocumento=<%=arrecadacaoDadosDiarios.getDocumentoTipo().getId() %>&nomeDocumento=<%=arrecadacaoDadosDiarios.getDocumentoTipo().getDescricaoDocumentoTipo()%>', 475, 600);"><%=Util.formatarMoedaReal(arrecadacaoDadosDiarios.getValorPagamentos())%></a></div></td>
