...
function adicionaGrupo(){
	...
	if(indiceSelecionado>0){				
			...
			redirecionarSubmit('exibirAtualizarResolucaoDiretoriaAction.do?adicionaGrupoAcesso='+grupoAAdicionar);			
	}else{
...
function adicionaCondicaoPagamentoAVista(){
	...
	redirecionarSubmit('exibirAtualizarResolucaoDiretoriaAction.do?adicionaCondicaoPagamentoAVista='+1);				
...
function adicionaCondicaoValorEntrada(){
	...
	redirecionarSubmit('exibirAtualizarResolucaoDiretoriaAction.do?adicionaCondicaoValorEntrada='+1);				
...
function removeCondicaoPagtoAVista(pCondicaoPagtoAVista){
	...
	document.forms[0].target='';
	redirecionarSubmit('exibirAtualizarResolucaoDiretoriaAction.do?removeCondicaoPagtoAVista='+pCondicaoPagtoAVista);
}
...
function removeGrupo(pGrupo){
	...
	if(confirm('Confirma remoção Grupo código '+pGrupo+' ?')){
		redirecionarSubmit('exibirAtualizarResolucaoDiretoriaAction.do?removeGrupoAcesso='+pGrupo);
	}		
}
...
function desfazer(){
	...
	var desfazer = document.forms[0].numero.value;
	
	document.forms[0].target='';
	if(confirm('Deseja realmente desfazer as alterações ?')){
		redirecionarSubmit('exibirAtualizarResolucaoDiretoriaAction.do?&desfazer='+desfazer);
	}
}
...
function bloquearGruposAcesso(){
	document.forms[0].target='';
	limparLocalidade();
	redirecionarSubmit('exibirAtualizarResolucaoDiretoriaAction.do?limparGrupos='+1);	
}
...
function adicionaRestricaoDebito(){
	...
	if(form.idLocalidade.value != "" && form.anoMesReferenciaDebitoInicio.value != "" && form.anoMesReferenciaDebitoFim.value != ""){		
			redirecionarSubmit('exibirAtualizarResolucaoDiretoriaAction.do?adicionaRestricaoDebito='+1);				
	}else{
...
function removeRestricaoDebito(pParcelamentoSituacaoEspecial){
	...
	document.forms[0].target='';
	redirecionarSubmit('exibirAtualizarResolucaoDiretoriaAction.do?removeRestricaoDebito='+pParcelamentoSituacaoEspecial);
}
...
<body leftmargin="5" topmargin="5"
	onload="javascript:setarFoco('${requestScope.nomeCampo}');inicializarCampos();">
...
<html:form action="/atualizarResolucaoDiretoriaAction"
				...
				<tr>	
					...
					<td colspan="2" width="74%">
						...
						<html:text maxlength="3" 
							...
							size="3"
							onkeypress="javascript:validaEnterComMensagem(event, 'exibirAtualizarResolucaoDiretoriaAction.do?objetoConsulta=1','idLocalidade','Localidade');"/>
		...
		<!-- Condições para Valor de Entrada -->
				...
				<tr>
					...
					<div align="left"><html:text property="dataNegociacaoInicio" size="10"
						...
						src="<bean:message key="caminho.imagens"/>calendario.gif"
						width="20" border="0" align="absmiddle" alt="Exibir Calendário"
						onclick="javascript:abrirCalendario('AtualizarResolucaoDiretoriaActionForm', 'dataNegociacaoInicio')" />
					dd/mm/aaaa</div>
					</td>
				</tr>
				<tr>
					...
					<div align="left"><html:text property="dataNegociacaoFinal" size="10"
						...
						src="<bean:message key="caminho.imagens"/>calendario.gif"
						width="20" border="0" align="absmiddle" alt="Exibir Calendário"
						onclick="javascript:abrirCalendario('AtualizarResolucaoDiretoriaActionForm', 'dataNegociacaoFinal')" />
					dd/mm/aaaa</div>
					</td>
				</tr>
				<tr>
	...
	<logic:notEmpty name="colecaoCondicaoValorEntradaSelecionados">
				...
				<tr>
					...
					<td align="right">
					  <gcom:controleAcessoBotao name="Button" value="Atualizar" onclick="javascript:validarForm(document.forms[0]);" url="atualizarResolucaoDiretoriaAction.do"/>
