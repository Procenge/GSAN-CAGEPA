...
function validarForm(form, inserir){
	...
	if (validateInserirContaActionForm(form)){
		...
		var DATA_MAX_VENC = document.getElementById("DATA_MAX_VENC").value;
		var ANO_LIMITE = document.getElementById("ANO_LIMITE").value;
		var PARAM_QUANTIDADE_MAX_DIAS = document.getElementById("PARAM_QUANTIDADE_MAX_DIAS").value;

 		var campo = "";
 		
 		var listBoxAgua = form.situacaoAguaConta;
		var listBoxEsgoto = form.situacaoEsgotoConta;
		var listBoxPerfilLigEsgoto = form.ligacaoEsgotoPerfilId;
		var indicadorEsgotoFaturavel = form.indicadorEsgotoFaturavel;
		var indicadorAguaFaturavel = form.indicadorAguaFaturavel;
		var mesAnoDataVencimento = form.vencimentoConta.value.substring(3, 10);
		...
		if ((form.mesAnoConta.value.substring(3, 7) * 1) < (ANO_LIMITE * 1)){
			...
			form.vencimentoConta.focus();
		} else if (comparaMesAno(mesAnoDataVencimento, "<", form.mesAnoConta.value)){
			alert("Mês/Ano do vencimento menor que a refêrencia da Conta.");
			form.mesAnoConta.focus();
		}
		else if (comparaData(form.vencimentoConta.value, ">", DATA_MAX_VENC )){
			alert("Data de vencimento posterior à data corrente mais " + PARAM_QUANTIDADE_MAX_DIAS + " dias permitidos para emissão do vencimento.");
			form.vencimentoConta.focus();
		}
		...
		else if (listBoxAgua.options[listBoxAgua.selectedIndex].id == 1 && form.consumoAgua.value.length < 1){
		...
		else if (listBoxEsgoto.options[listBoxEsgoto.selectedIndex].value > 0 && 
				listBoxPerfilLigEsgoto.options[listBoxPerfilLigEsgoto.selectedIndex].value == 0 &&
				indicadorEsgotoFaturavel.value == 1 && form.percentualEsgoto.value.length < 1){
			alert("Informe o perfil da ligação de esgoto.");
			form.ligacaoEsgotoPerfilId.focus();
		}
		...
		else if (indicadorEsgotoFaturavel.value == 1 && form.percentualEsgoto.value.length < 1){
...
<body leftmargin="5" topmargin="5"
	onload="setarFoco('${requestScope.nomeCampo}');habilitacaoCamposEsgoto();">

<html:form action="/exibirInserirContaAction" method="post">

	<INPUT TYPE="hidden" ID="DATA_ATUAL" value="${requestScope.dataAtual}" />
	<INPUT TYPE="hidden" ID="DATA_MAX_VENC"
		value="${requestScope.dataMaximaVencimento}" />
	<INPUT TYPE="hidden" ID="ANO_LIMITE" value="${requestScope.anoLimite}" />
	<INPUT TYPE="hidden" ID="PARAM_QUANTIDADE_MAX_DIAS" value="${requestScope.parametroQuantMaxDiasVenc}" />
		...
		<tr>
			...
			<table width="100%" border="0">	
							...
							<table width="100%" border="0">
								...
								<tr>
									...
									<td><html:text property="vencimentoConta" size="11"
										...
										href="javascript:abrirCalendario('InserirContaActionForm', 'vencimentoConta')">
									<img border="0"
								...
								<tr>
									<td height="10"><strong>Tarifa:<font
										color="#FF0000">*</font></strong></td>
									<td><html:select property="tarifaID"
										style="width: 340px;" tabindex="3">
										<logic:present name="colecaoTarifa">
							...
							<table width="100%" border="0">
								<tr>
									...
									<td><html:select property="situacaoAguaConta" style="width: 190px;" tabindex="5"										
										onchange="verificarIndicadorFaturavelLigacaoAguaSituacao(this.value, document.forms[0].consumoAgua);">
								...
								<tr>
									...
									<td><html:text property="consumoAgua" size="10" maxlength="6"
										tabindex="6" style="text-align: right;" disabled="true" /></td>
							...
							<table width="100%" border="0">
								<tr>
									...
									<td><html:select property="situacaoEsgotoConta" style="width: 190px;" tabindex="7"
										onchange="verificarIndicadorFaturavel(this.value, document.forms[0].consumoEsgoto, document.forms[0].percentualEsgoto);">
								...
								<tr>
									...
									<td><html:text property="consumoEsgoto" size="10" maxlength="6"
										tabindex="8" style="text-align: right;" /></td>
				...
				<tr>
					...
					<td align="right">
						<input type="button"
						class="bottonRightCol" value="Inserir" style="width: 80px"
