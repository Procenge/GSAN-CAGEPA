...
<script>
	...
	function replicaDados(obj){
		var valorAuxiliar = "";
		valorAuxiliar = obj.value;

		if(obj.value.length > 6 && obj.value.length == 8){
			valorAuxiliar = valorAuxiliar.substring(2,8);
		}else if(obj.value.length > 6 && obj.value.length == 7){
			valorAuxiliar = valorAuxiliar.substring(1,7);
		}

		for (i=0; i < document.forms[0].elements.length; i++) {
			if (document.forms[0].elements[i].id == "primeira" || document.forms[0].elements[i].id == "clone") {
				document.forms[0].elements[i].value = valorAuxiliar;
			}
		}
	}
	
	function validarTamanhoCampo(obj){
		var valorAuxiliar = "";
		valorAuxiliar = obj.value;
				...
				cor = "#cbe5fe";%>
									<tr bgcolor="#cbe5fe">
										...
										<INPUT type="text" style="text-align: right;" onkeyup="javaScript:formataValorMonetario(this, 6);replicaDados(this);"
											maxlength="6" size="6" id="primeira"
											name="<bean:write name="categoria"
												property="id" />"
											value="0">
										<%} else { %>
											<INPUT type="text" style="text-align: right;" onkeyup="javaScript:formataValorMonetario(this, 6);validarTamanhoCampo(this);"
